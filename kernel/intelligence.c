...
static int state_proc_show(struct seq_file *m, void *v)
{
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		...
		for_each_leaf_cfs_rq(cpu_rq(cpu), cfs_rq) {
			...
			runnable_load_avg_cpu[cpu] += cfs_rq->avg.runnable_load_avg;
			removed_load_avg_cpu[cpu] += cfs_rq->removed.load_avg;
			removed_util_avg_cpu[cpu] += cfs_rq->removed.util_avg;
		}
		rcu_read_unlock();
	}
