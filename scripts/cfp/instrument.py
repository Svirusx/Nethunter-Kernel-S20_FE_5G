...
class Objdump(object):
    ...
    def __init__(self, vmlinux, config_file=None,
        ...
        #load config flags
        self._load_config()
        self.ROPP = self.is_conf_set('CONFIG_CFP_ROPP')
        self.JOPP = self.is_conf_set('CONFIG_CFP_JOPP')
        if self.JOPP:
            self.JOPP_MAGIC = int(self.get_conf('CONFIG_CFP_JOPP_MAGIC'), 16)
    ...
    def _copy_to_tmp(self, from_path, to_basename):
        ...
        """
        vfile = os.path.join(my_dirname(self.vmlinux), 'scripts/cfp/tmp', to_basename)
