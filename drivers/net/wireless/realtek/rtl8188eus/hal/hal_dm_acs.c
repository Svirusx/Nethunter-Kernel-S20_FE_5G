...
static void _rtw_bss_nums_count(_adapter *adapter, u8 *pbss_nums)
{
	...
	_rtw_memsetx(pbss_nums, 0, MAX_CHANNEL_NUM);
	...
	while (1) {
		...
		if (!pnetwork)
			break;
		chan_idx = rtw_chset_search_chx(adapter_to_chset(adapter), pnetwork->network.Configuration.DSConfig);
...
u8 rtw_phydm_clm_ratio(_adapter *adapter)
{
	...
	return phydm_cmn_info_queryx(phydm, (enum phydm_info_query) PHYDM_INFO_CLM_RATIO);
}
u8 rtw_phydm_nhm_ratio(_adapter *adapter)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);

	return phydm_cmn_info_queryx(phydm, (enum phydm_info_query) PHYDM_INFO_NHM_RATIO);
}
void rtw_acs_reset(_adapter *adapter)
{
	...
	_rtw_memsetx(pacs, 0, sizeof(struct auto_chan_sel));
...
void rtw_acs_trigger(_adapter *adapter, u16 scan_time_ms, u8 scan_chan, enum NHM_PID pid)
{
	...
	hal_data->acs.trig_rst = phydm_env_mntr_triggerx(phydm, &nhm_para, &clm_para, &trig_rpt);
...
void rtw_acs_get_rst(_adapter *adapter)
{
	...
	chan_idx = rtw_chset_search_chx(adapter_to_chset(adapter), cur_chan);
	if ((chan_idx == -1) || (chan_idx >= MAX_CHANNEL_NUM)) {
...

	{
		...
		rst = phydm_env_mntr_resultx(phydm, &rpt);
		if ((rst == (NHM_SUCCESS | CLM_SUCCESS)) &&
			...
			_rtw_memcpyx(&hal_data->acs.nhm[chan_idx][0], rpt.nhm_result, NHM_RPT_NUM);
...
u8 rtw_acs_get_clm_ratio_by_ch_num(_adapter *adapter, u8 chan)
{
	...
	chan_idx = rtw_chset_search_chx(adapter_to_chset(adapter), chan);
	if ((chan_idx == -1) || (chan_idx >= MAX_CHANNEL_NUM)) {
...
void rtw_noise_measure(_adapter *adapter, u8 chan, u8 is_pause_dig, u8 igi_value, u32 max_time)
{
	...
	noise = odm_inband_noise_monitorx(phydm, is_pause_dig, igi_value, max_time); /*dBm*/
...
s16 rtw_noise_measure_curchan(_adapter *padapter)
{
	...
	u8 cur_chan = rtw_get_oper_chx(padapter);
