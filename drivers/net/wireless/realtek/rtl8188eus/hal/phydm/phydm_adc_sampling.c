...
void phydm_la_pre_run(void *dm_void)
{
	...
	/*pre run */
	/*force to bb trigger*/
	odm_set_mac_regx(dm, R_0x7c0, BIT(3), 0);
	/*dma_trig_and(AND1) output 1*/
	odm_set_bb_regx(dm, R_0x1ce4, 0xf0000000, 0x0);
	/*r_dma_trigger_AND1_inv = 1*/
	odm_set_bb_regx(dm, R_0x1ce8, BIT5, 1); /*@AND 1 val*/
	/* polling bit for BB ADC mode */
	odm_set_mac_regx(dm, 0x7c0, BIT(1), 1);

	pr_debug("buf[end:start]=(0x%x~0x%x)\n", buf->end_pos, buf->start_pos);

	do {
		tmp = odm_read_1bytex(dm, R_0x7c0);
	...
	odm_set_bb_regx(dm, R_0x1ce8, BIT5, 0); /*@AND 1 val*/

	if (smp->la_trig_mode == PHYDM_ADC_MAC_TRIG)
		odm_set_mac_regx(dm, R_0x7c0, BIT(3), 1);
}
#endif
...
phydm_la_clk_en(void *dm_void, boolean enable)
{
	...
	odm_set_bb_regx(dm, R_0x95c, BIT(23), val);
}
#endif
...
phydm_la_stop_dma_8197f(void *dm_void, enum phydm_backup_type opt)
{
	...
	if (opt == PHYDM_BACKUP) {
		/*Stop DMA*/
		smp->backup_dma = odm_get_mac_regx(dm, R_0x300, 0xffff);
		odm_set_mac_regx(dm, R_0x300, 0x7fff, 0x7fff);
	} else { /*restore*/
		/*Resume DMA*/
		odm_set_mac_regx(dm, R_0x300, 0x7fff, smp->backup_dma);
	}
}
#endif
...
phydm_la_mv_data_2_tx_buffer(void *dm_void)
{
	...
	odm_set_bb_regx(dm, R_0x7c0, BIT(0), 0x0); /*Disable LA mode HW block*/
...
#if 1
	odm_memory_setx(dm, adv, 0, sizeof(struct la_adv_trig));
#else
...
void phydm_la_bb_adv_trig_setting_jgr3(void *dm_void)
{
	...
	if (!adv_bb_trig_en) { /*normal LA mode & back to default*/
		/*@AND0*/
		odm_set_bb_regx(dm, R_0x1ce4, BIT(27), 0);

		/*@AND1*/
		odm_set_bb_regx(dm, R_0x1ce4, MASKH4BITS, 0);
		odm_set_bb_regx(dm, R_0x1ce8, BIT(5), 0); /*@AND 1 inv*/
		/*@AND2*/
		odm_set_bb_regx(dm, R_0x1ce8, 0x3c0, 0);
		odm_set_bb_regx(dm, R_0x1ce8, BIT(15), 0); /*@AND 2 inv*/
		/*@AND3*/
		odm_set_bb_regx(dm, R_0x1ce8, 0xf0000, 0);
		odm_set_bb_regx(dm, R_0x1ce8, BIT(25), 0); /*@AND 3 inv*/
		/*@AND4*/
		odm_set_bb_regx(dm, R_0x1cf0, MASKDWORD, 0); /*@AND 4 mask en*/
		odm_set_bb_regx(dm, R_0x1ce8, BIT(26), 0); /*@AND 4 inv*/
	} else {
		/* @AND0 */
		/*path 1 default: enable ori. BB trigger*/
		odm_set_bb_regx(dm, R_0x1ce4, BIT(27),
			       (adv->la_ori_bb_dis ? 1 : 0));

		/* @AND1 */
		odm_set_bb_regx(dm, R_0x1ce8, BIT(5), adv->la_and1_inv);
		odm_set_bb_regx(dm, R_0x1ce4, MASKH4BITS, adv->la_and1_sel);
		odm_set_bb_regx(dm, R_0x1ce8, 0x1f, adv->la_and1_val);

		/* @AND2 */
		odm_set_bb_regx(dm, R_0x1ce8, BIT(15), adv->la_and2_inv);
		odm_set_bb_regx(dm, R_0x1ce8, 0x3c0, adv->la_and2_sel);
		odm_set_bb_regx(dm, R_0x1ce8, 0x7c00, adv->la_and2_val);

		/* @AND3 */
		odm_set_bb_regx(dm, R_0x1ce8, BIT(25), adv->la_and3_inv);
		odm_set_bb_regx(dm, R_0x1ce8, 0xf0000, adv->la_and3_sel);
		odm_set_bb_regx(dm, R_0x1ce8, 0x1f00000, adv->la_and3_val);

		/* @AND4 */
		odm_set_bb_regx(dm, R_0x1ce8, BIT(26), adv->la_and4_inv);
		odm_set_bb_regx(dm, R_0x1cf0, MASKDWORD, adv->la_and4_mask);
		odm_set_bb_regx(dm, R_0x1cec, MASKDWORD, adv->la_and4_bitmap);
	}
}
...
phydm_la_buffer_release(void *dm_void)
{
	...
	if (buf->length != 0x0) {
		odm_free_memoryx(dm, buf->octet, buf->length);
		buf->length = 0x0;
	}
}
...
phydm_la_buffer_allocate(void *dm_void)
{
	...
	if (buf->length == 0) {
		...
		if (PlatformAllocateMemoryWithZero(adapter, (void **)&
			...
			ret = false;
	#else
		odm_allocate_memoryx(dm, (void **)&buf->octet, buf->buffer_size);
...
void phydm_la_access_tx_pkt_buf(void *dm_void, u32 addr, u32 buff_idx)
{
	...
	if (dm->support_ic_type & ODM_RTL8192F) {
		...
		indirect_access_sdram_8192f(dm->adapter, TX_PACKET_BUFFER,
					    ...
					    &data_h, &data_l);
		#else
		odm_write_1bytex(dm, R_0x0106, 0x69);
		odm_set_bb_regx(dm, R_0x0140, MASKDWORD, addr >> 3);
		data_l = odm_get_bb_regx(dm, R_0x0144, MASKDWORD);
		data_h = odm_get_bb_regx(dm, R_0x0148, MASKDWORD);
		odm_write_1bytex(dm, R_0x0106, 0x0);
		#endif
	} else
	#endif
	{
		...
		if (page != smp->txff_page) {
			smp->txff_page = page;
			odm_set_bb_regx(dm, R_0x0140, MASKLWORD, 0x780 + page);
		}
		data_l = odm_read_4bytex(dm, 0x8000 + (addr & 0xfff));
		data_h = odm_read_4bytex(dm, 0x8000 + (addr & 0xfff) + 4);
	}
...
void phydm_la_get_tx_pkt_buf(void *dm_void)
{
	...
	odm_memory_setx(dm, buf->octet, 0, buf->length);
	pr_debug("GetTxPktBuf\n");

	/*@==== [Get LA Report] ==============================================*/
	if (dm->support_ic_type & ODM_RTL8192F) {
		value32 = odm_read_4bytex(dm, R_0x7f0);
		is_round_up = (boolean)((value32 & BIT(31)) >> 31);
		finish_addr = (value32 & 0x7FFF8000) >> 15; /*@16 bit (unit: 8Byte)*/
	} else {
		odm_write_1bytex(dm, R_0x0106, 0x69);
		value32 = odm_read_4bytex(dm, R_0x7c0);
		is_round_up = (boolean)((value32 & BIT(31)) >> 31);
...
void phydm_la_set_trig_src(void *dm_void, u8 la_trig_mode)
{
	...
	if (la_trig_mode == PHYDM_ADC_MAC_TRIG)
		odm_set_mac_regx(dm, reg, BIT(3), 1);
	else
		odm_set_mac_regx(dm, reg, BIT(3), 0);
}
...
void phydm_la_set_mac_iq_dump(void *dm_void, boolean impossible_trig_condi)
{
	...
	odm_write_1bytex(dm, reg1, 0); /*@clear all reg1*/
	/*@Enable LA mode HW block*/
	odm_set_mac_regx(dm, reg1, BIT(0), 1);

	if (smp->la_trig_mode == PHYDM_MAC_TRIG) {
		smp->la_dump_mode = LA_MAC_DBG_DUMP;
		/*polling bit for MAC mode*/
		odm_set_mac_regx(dm, reg1, BIT(2), 1);
		/*trigger mode for MAC*/
		odm_set_mac_regx(dm, reg1, 0x18,	smp->la_trigger_edge);
		pr_debug("[MAC_trig] ref_mask=(0x%x), ref_value=(0x%x), dbg_port =(0x%x)\n",
			 smp->la_mac_mask_or_hdr_sel, smp->la_trig_sig_sel,
			 smp->la_dbg_port);
		/*@[Set MAC Debug Port]*/
		odm_set_mac_regx(dm, R_0xf4, BIT(16), 1);
		odm_set_mac_regx(dm, R_0x38, 0xff0000, smp->la_dbg_port);
		odm_set_mac_regx(dm, reg2, MASKDWORD,
				smp->la_mac_mask_or_hdr_sel);
		odm_set_mac_regx(dm, reg3, MASKDWORD, smp->la_trig_sig_sel);
	} else {
		...
		if (smp->la_trig_mode == PHYDM_ADC_MAC_TRIG) {
			...
			odm_set_mac_regx(dm, reg1, 0xc0,	smp->la_trig_sig_sel);

			if (smp->la_trig_sig_sel == ADCSMP_TRIG_REG) {
				/* @manual trigger reg1[5] = 0->1*/
				odm_set_mac_regx(dm, reg1, BIT(5), 1);
			}
		}
		/*polling bit for BB ADC mode*/
		odm_set_mac_regx(dm, reg1, BIT(1), 1);
	}

	reg_value = odm_get_bb_regx(dm, reg1, 0xff);
...
void phydm_la_set_bb_dbg_port(void *dm_void, boolean impossible_trig_condi)
{
	...
	if (phydm_set_bb_dbg_portx(dm, DBGPORT_PRI_3, dbg_port)) {
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_regx(dm, R_0x95c, 0x1f, trig_sel);
	#ifdef PHYDM_IC_JGR3_SERIES_SUPPORT
	} else if (dm->support_ic_type & ODM_IC_JGR3_SERIES) {
		odm_set_bb_regx(dm, R_0x1ce4, 0x3e000, trig_sel);
	#endif
	} else {
		odm_set_bb_regx(dm, R_0x9a0, 0x1f, trig_sel);
	}
...
void phydm_la_set_bb(void *dm_void)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		phydm_bb_dbg_port_header_selx(dm, dbg_port_hdr_sel);

		odm_set_bb_regx(dm, R_0x8b4, BIT(7), 1);/*@update rpt every pkt*/
		odm_set_bb_regx(dm, R_0x95c, 0xf00, dma_type);
		/*@0: posedge, 1: negedge*/
		odm_set_bb_regx(dm, R_0x95c, BIT(31), edge);
		odm_set_bb_regx(dm, R_0x95c, 0xe0, smp_rate);
		/*	@(0:) '80MHz'
		 *	(1:) '40MHz'
		 *	(2:) '20MHz'
		 *	(3:) '10MHz'
		...
		odm_set_bb_regx(dm, R_0x1eb4, BIT(23), 0x1);/*@update rpt every pkt*/
		/*@MAC-PHY timing*/
		odm_set_bb_regx(dm, R_0x1ce4, BIT(7) | BIT(6), 0);
		odm_set_bb_regx(dm, R_0x1cf4, BIT(23), 1); /*@LA mode on*/
		odm_set_bb_regx(dm, R_0x1ce4, 0x3f, dma_type);
		/*@0: posedge, 1: negedge ??*/
		odm_set_bb_regx(dm, R_0x1ce4, BIT(26), edge);
		odm_set_bb_regx(dm, R_0x1ce4, 0x700, smp_rate);

		phydm_la_bb_adv_trig_setting_jgr3(dm);
	#endif
	} else {
		if (dm->support_ic_type & (ODM_RTL8197F | ODM_RTL8192F))
			odm_set_bb_regx(dm, R_0xd00, BIT(26), 0x1); /*@update rpt every pkt*/

		#if (RTL8192F_SUPPORT)
		if ((dm->support_ic_type & ODM_RTL8192F))
			/*@LA reset HW block enable for true-mac asic*/
			odm_set_bb_regx(dm, R_0x9a0, BIT(15), 1);
		#endif

		odm_set_bb_regx(dm, R_0x9a0, 0xf00, dma_type);
		/*@0: posedge, 1: negedge*/
		odm_set_bb_regx(dm, R_0x9a0, BIT(31), edge);
		odm_set_bb_regx(dm, R_0x9a0, 0xe0, smp_rate);
		/*	@(0:) '80MHz'
		 *	(1:) '40MHz'
		 *	(2:) '20MHz'
		 *	(3:) '10MHz'
...
void phydm_la_set_mac_trigger_time(void *dm_void, u32 trigger_time_mu_sec)
{
	...
	if (dm->support_ic_type & ODM_RTL8192F) {
		odm_set_mac_regx(dm, R_0x7fc, BIT(2) | BIT(1) | BIT(0), unit);
		odm_set_mac_regx(dm, R_0x7f0, 0x7f00, (time_unit_num & 0x7f));
	#ifdef PHYDM_IC_JGR3_SERIES_SUPPORT
	} else if (dm->support_ic_type & ODM_IC_JGR3_SERIES) {
		odm_set_mac_regx(dm, R_0x7cc, BIT(18) | BIT(17) | BIT(16), unit);
		odm_set_mac_regx(dm, R_0x7c0, 0x7f00, (time_unit_num & 0x7f));
	#endif
	} else {
		odm_set_mac_regx(dm, R_0x7cc, BIT(20) | BIT(19) | BIT(18), unit);
		odm_set_mac_regx(dm, R_0x7c0, 0x7f00, (time_unit_num & 0x7f));
	}
}
...
void phydm_la_set_buff_mode(void *dm_void, enum la_buff_mode mode)
{
	...
	if (dm->support_ic_type & FULL_BUFF_MODE_SUPPORT) {
		if (mode == ADCSMP_BUFF_HALF) {
			odm_set_mac_regx(dm, R_0x7cc, BIT(30), 0);
		} else {
			buf->buffer_size = buf->buffer_size << 1;
			odm_set_mac_regx(dm, R_0x7cc, BIT(30), 1);
		}
	}
...
void phydm_la_adc_smp_start(void *dm_void)
{
	...
	do { /*Polling time always use 100ms, when it exceed 2s, break loop*/
		if (dm->support_ic_type & ODM_RTL8192F)
			tmp_u1b = odm_read_1bytex(dm, R_0x7f0);
		else
			tmp_u1b = odm_read_1bytex(dm, R_0x7c0);
	      ...
	      PULONG bytes_written)
{
	...
	if (info_buf_length != buf->buffer_size) {
		...
		*bytes_written = 0;
		...
		odm_move_memoryx(dm, info_buf, buf->octet, buf->buffer_size);
