...
const u32 db_invert_tablex[12][8] = {
...
/*Y = 10*log(X)*/
s32 odm_pwdb_conversionx(s32 X, u32 total_bit, u32 decimal_bit)
{
...
s32 odm_sign_conversionx(s32 value, u32 total_bit)
{
...
u16 phydm_find_intrvlx(void *dm_void, u16 val, u16 *threshold, u16 th_len)
{
...
void phydm_seq_sortingx(void *dm_void, u32 *value, u32 *rank_idx, u32 *idx_out,
		       u8 seq_length)
{
	u8 i = 0, j = 0;
	u32 tmp_a, tmp_b;
...
u32 odm_convert_to_dbx(u64 value)
{
	u8 i;
	u8 j;
	u32 dB;

	if (value >= db_invert_tablex[11][7])
		return 96; /* @maximum 96 dB */

	for (i = 0; i < 12; i++) {
		if (i <= 2 && (value << FRAC_BITS) <= db_invert_tablex[i][7])
			break;
		else if (i > 2 && value <= db_invert_tablex[i][7])
			break;
	}

	for (j = 0; j < 8; j++) {
		if (i <= 2 && (value << FRAC_BITS) <= db_invert_tablex[i][j])
			break;
		else if (i > 2 && i < 12 && value <= db_invert_tablex[i][j])
			break;
	}
	...
	if (j == 0) {
		if (i != 3) {
			if (db_invert_tablex[i][0] - value >
			    value - db_invert_tablex[i - 1][7]) {
				i = i - 1;
				j = 7;
			}
		} else {
			if (db_invert_tablex[3][0] - value >
			    value - db_invert_tablex[2][7]) {
				i = 2;
				j = 7;
			}
		}
	} else {
		if (db_invert_tablex[i][j] - value >
		    value - db_invert_tablex[i][j - 1]) {
			/*i = i;*/
			j = j - 1;
		}
	}
...
u64 phydm_db_2_linearx(u32 value)
{
	u8 i = 0;
	u8 j = 0;
	u64 linear = 0;
...
u16 phydm_show_fraction_numx(u32 frac_val, u8 bit_num)
{
	u8 i = 0;
	u16 val = 0;
	u16 base = 5000;
...
u64 phydm_gen_bitmaskx(u8 mask_num)
{
	u8 i = 0;
	u64 bitmask = 0;
...
s32 phydm_cnvrt_2_signx(u32 val, u8 bit_num)
{
	if (bit_num >= 32)
		return (s32)val;
...
s64 phydm_cnvrt_2_signx_64(u64 val, u8 bit_num)
{
	u64 one = 1;
