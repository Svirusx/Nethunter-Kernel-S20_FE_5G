...
static u8 _is_fw_read_cmd_down(_adapter *padapter, u8 msgbox_num)
{
	...
	/* RTW_INFO(" _is_fw_read_cmd_down ,reg_1cc(%x),msg_box(%d)...\n",rtw_read8x(padapter,REG_HMETFR),msgbox_num); */

	do {
		valid = rtw_read8x(padapter, REG_HMETFR) & BIT(msgbox_num);
		if (0 == valid)
			read_down = _TRUE;
		else
			rtw_msleep_osx(1);
...
s32 FillH2CCmd_88E(PADAPTER padapter, u8 ElementID, u32 CmdLen, u8 *pCmdBuffer)
{
	...
	do {
		...
		*(u8 *)(&h2c_cmd) = ElementID;
		...
		if (CmdLen <= 3)
			_rtw_memcpyx((u8 *)(&h2c_cmd) + 1, pCmdBuffer, CmdLen);
		else {
			_rtw_memcpyx((u8 *)(&h2c_cmd) + 1, pCmdBuffer, 3);
			_rtw_memcpyx((u8 *)(&h2c_cmd_ex), pCmdBuffer + 3, CmdLen - 3);
		}
		/* Write Ext command */
		msgbox_ex_addr = REG_HMEBOX_EXT_0 + (h2c_box_num * RTL88E_EX_MESSAGE_BOX_SIZE);
#ifdef CONFIG_H2C_EF
		for (cmd_idx = 0; cmd_idx < RTL88E_EX_MESSAGE_BOX_SIZE; cmd_idx++)
			rtw_write8x(padapter, msgbox_ex_addr + cmd_idx, *((u8 *)(&h2c_cmd_ex) + cmd_idx));
#else
		h2c_cmd_ex = le32_to_cpu(h2c_cmd_ex);
		rtw_write32x(padapter, msgbox_ex_addr, h2c_cmd_ex);
#endif
		...
		for (cmd_idx = 0; cmd_idx < RTL88E_MESSAGE_BOX_SIZE; cmd_idx++)
			rtw_write8x(padapter, msgbox_addr + cmd_idx, *((u8 *)(&h2c_cmd) + cmd_idx));
#else
		h2c_cmd = le32_to_cpu(h2c_cmd);
		rtw_write32x(padapter, msgbox_addr, h2c_cmd);
#endif
...
u8 rtl8192c_h2c_msg_hdlx(_adapter *padapter, unsigned char *pbuf)
{
	u8 ElementID, CmdLen;
	u8 *pCmdBuffer;
...
GetTxBufferRsvdPageNum8188E(_adapter *padapter, bool wowlan)
{
	...
	rtw_hal_get_def_varx(padapter, HAL_DEF_TX_PAGE_BOUNDARY, (u8 *)&TxPageBndy);
...
void rtl8188e_set_FwJoinBssReport_cmd(PADAPTER padapter, u8 mstatus)
{
	...
	if (mstatus == 1) {
		u8 bcn_ctrl = rtw_read8x(padapter, REG_BCN_CTRL);

		/* We should set AID, correct TSF, HW seq enable before set JoinBssReport to Fw in 88/92C. */
		/* Suggested by filen. Added by tynli. */
		rtw_write16x(padapter, REG_BCN_PSR_RPT, (0xC000 | pmlmeinfo->aid));
		/* Hw sequende enable by dedault. 2010.06.23. by tynli. */
		/* rtw_write16x(padapter, REG_NQOS_SEQ, ((pmlmeext->mgnt_seq+100)&0xFFF)); */
		/* rtw_write8x(padapter, REG_HWSEQ_CTRL, 0xFF); */

		/* Set REG_CR bit 8. DMA beacon by SW. */
		rtw_write8x(padapter,  REG_CR + 1,
			rtw_read8x(padapter,  REG_CR + 1) | BIT0);

		/* Disable Hw protection for a time which revserd for Hw sending beacon. */
		/* Fix download reserved page packet fail that access collision with the protection time. */
		/* 2010.05.11. Added by tynli. */
		rtw_write8x(padapter, REG_BCN_CTRL, (bcn_ctrl & (~EN_BCN_FUNCTION)) | DIS_TSF_UDT);

		RegFwHwTxQCtrl = rtw_read8x(padapter, REG_FWHW_TXQ_CTRL + 2);
		...
		RegFwHwTxQCtrl &= (~BIT6);
		rtw_write8x(padapter, REG_FWHW_TXQ_CTRL + 2, RegFwHwTxQCtrl);
		...
		do {
			/* download rsvd page.*/
			rtw_hal_set_fw_rsvd_pagex(padapter, _FALSE);
			DLBcnCount++;
			do {
				rtw_yield_osx();
				/* rtw_mdelay_osx(10); */
				/* check rsvd page download OK. */
				rtw_hal_get_hwregx(padapter, HW_VAR_BCN_VALID, (u8 *)(&bcn_valid));
				poll++;
		...
		if (RTW_CANNOT_RUN(padapter))
			...
			rtw_hal_set_fw_rsvd_pagex(padapter, _TRUE);
			RTW_INFO(ADPT_FMT": 1 DL RSVD page success! DLBcnCount:%u, poll:%u\n",
				 ADPT_ARG(padapter), DLBcnCount, poll);
		}

		/* restore bcn_ctrl */
		rtw_write8x(padapter, REG_BCN_CTRL, bcn_ctrl);
...
void rtl8188e_set_p2p_ps_offload_cmd(_adapter *padapter, u8 p2p_ps_state)
{
	...
	case P2P_PS_DISABLE:
		...
		_rtw_memsetx(p2p_ps_offload, 0 , 1);
		break;
	case P2P_PS_ENABLE:
		...
		if (pwdinfo->ctwindow > 0) {
			...
			rtw_write8x(padapter, REG_P2P_CTWIN, pwdinfo->ctwindow);
		}

		/* hw only support 2 set of NoA */
		for (i = 0 ; i < pwdinfo->noa_num ; i++) {
			/* To control the register setting for which NOA */
			rtw_write8x(padapter, REG_NOA_DESC_SEL, (i << 4));
			if (i == 0)
			...
			rtw_write32x(padapter, REG_NOA_DESC_DURATION, pwdinfo->noa_duration[i]);

			/* RTW_INFO("%s(): noa_interval = %x\n",__FUNCTION__,pwdinfo->noa_interval[i]); */
			rtw_write32x(padapter, REG_NOA_DESC_INTERVAL, pwdinfo->noa_interval[i]);

			/* RTW_INFO("%s(): start_time = %x\n",__FUNCTION__,pwdinfo->noa_start_time[i]); */
			rtw_write32x(padapter, REG_NOA_DESC_START, pwdinfo->noa_start_time[i]);

			/* RTW_INFO("%s(): noa_count = %x\n",__FUNCTION__,pwdinfo->noa_count[i]); */
			rtw_write8x(padapter, REG_NOA_DESC_COUNT, pwdinfo->noa_count[i]);
		}

		if ((pwdinfo->opp_ps == 1) || (pwdinfo->noa_num > 0)) {
			/* rst p2p circuit */
			rtw_write8x(padapter, REG_DUAL_TSF_RST, BIT(4));
