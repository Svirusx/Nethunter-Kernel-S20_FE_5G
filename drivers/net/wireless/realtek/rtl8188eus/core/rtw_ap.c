...
extern unsigned char	WMM_OUIx[];
extern unsigned char	WPS_OUIx[];
extern unsigned char	P2P_OUIx[];
extern unsigned char	WFD_OUIx[];

void init_mlme_ap_infox(_adapter *padapter)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);

	_rtw_spinlockx_init(&pmlmepriv->bcn_update_lock);
	/* pmlmeext->bstart_bss = _FALSE; */
}

void free_mlme_ap_infox(_adapter *padapter)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);

	stop_ap_modex(padapter);
	_rtw_spinlockx_free(&pmlmepriv->bcn_update_lock);
...
*/
u8 rtw_set_tim_iex(u8 dtim_cnt, u8 dtim_period
	, const u8 *tim_bmp, u8 tim_bmp_len, u8 *tim_ie)
{
	...
	if (rtw_bmp_not_emptyx(tim_bmp, tim_bmp_len)) {
	...
	*p++ = dtim_cnt;
	*p++ = dtim_period;
	*p++ = (rtw_bmp_is_setx(tim_bmp, tim_bmp_len, 0) ? BIT0 : 0) | n1;
	_rtw_memcpyx(p, tim_bmp + n1, bmp_len);
...
static void update_BCNTIM(_adapter *padapter)
{
	...
	/* update TIM IE */
	/* if(rtw_tim_map_anyone_be_setx(padapter, pstapriv->tim_bitmap)) */
#endif
	if (_TRUE) {
		u8 *p, *dst_ie, *premainder_ie = NULL, *pbackup_remainder_ie = NULL;
		uint offset, tmp_len, tim_ielen, tim_ie_offset, remainder_ielen;

		p = rtw_get_iex(pie + _FIXED_IE_LENGTH_, _TIM_IE_, &tim_ielen, pnetwork_mlmeext->IELength - _FIXED_IE_LENGTH_);
		if (p != NULL && tim_ielen > 0) {
			...
			/* get ssid_ie len */
			p = rtw_get_iex(pie + _BEACON_IE_OFFSET_, _SSID_IE_, &tmp_len, (pnetwork_mlmeext->IELength - _BEACON_IE_OFFSET_));
			if (p != NULL)
				offset += tmp_len + 2;

			/*get supported rates len*/
			p = rtw_get_iex(pie + _BEACON_IE_OFFSET_, _SUPPORTEDRATES_IE_, &tmp_len, (pnetwork_mlmeext->IELength - _BEACON_IE_OFFSET_));
			if (p !=  NULL)
				offset += tmp_len + 2;
		...
		if (remainder_ielen > 0) {
			pbackup_remainder_ie = rtw_mallocx(remainder_ielen);
			if (pbackup_remainder_ie && premainder_ie)
				_rtw_memcpyx(pbackup_remainder_ie, premainder_ie, remainder_ielen);
		}

		/* append TIM IE */
		dst_ie += rtw_set_tim_iex(0, 1, pstapriv->tim_bitmap, pstapriv->aid_bmp_len, dst_ie);

		/*copy remainder IE*/
		if (pbackup_remainder_ie) {
			_rtw_memcpyx(dst_ie, pbackup_remainder_ie, remainder_ielen);

			rtw_mfreex(pbackup_remainder_ie, remainder_ielen);
		}
...
void rtw_add_bcn_iex(_adapter *padapter, WLAN_BSSID_EX *pnetwork, u8 index, u8 *data, u8 len)
{
...
void rtw_remove_bcn_iex(_adapter *padapter, WLAN_BSSID_EX *pnetwork, u8 index)
{
	u8 *p, *dst_ie = NULL, *premainder_ie = NULL, *pbackup_remainder_ie = NULL;
	uint offset, ielen, ie_offset, remainder_ielen = 0;
	u8	*pie = pnetwork->IEs;

	p = rtw_get_iex(pie + _FIXED_IE_LENGTH_, index, &ielen, pnetwork->IELength - _FIXED_IE_LENGTH_);
	if (p != NULL && ielen > 0) {
		ielen += 2;
...
u8 chk_sta_is_alivex(struct sta_info *psta);
u8 chk_sta_is_alivex(struct sta_info *psta)
{
	u8 ret = _FALSE;
	...
	if (MLME_IS_MESH(psta->padapter)) {
		...
		/* The reference for expire_timeout_chkx */
...
static int issue_aka_chk_frame(_adapter *adapter, struct sta_info *psta)
{
	...
	if (MLME_IS_AP(adapter)) {
		...
		if (psta->state & WIFI_SLEEP_STATE)
			ret = issue_nulldatax(adapter, target_addr, 0, 1, 50);
		else
			ret = issue_nulldatax(adapter, target_addr, 0, 3, 50);
	}
...
static void rtw_check_restore_rf18(_adapter *padapter)
{
	...
	reg = rtw_hal_read_rfregxx(padapter, 0, 0x18, 0x3FF);
	if ((reg & 0xFF) == 0)
			setchbw = _TRUE;
	reg = rtw_hal_read_rfregxx(padapter, 1, 0x18, 0x3FF);
	if ((reg & 0xFF) == 0)
			setchbw = _TRUE;
...
void	expire_timeout_chkx(_adapter *padapter)
{
	_irqL irqL;
	_list	*phead, *plist;
	u8 updated = _FALSE;
	...
	if (MLME_IS_MESH(padapter)
		&& check_fwstatex(&padapter->mlmepriv, WIFI_ASOC_STATE)
	) {
	...
	/* check auth_queue */
#ifdef DBG_EXPIRATION_CHK
	if (rtw_end_of_queue_searchx(phead, plist) == _FALSE) {
	...
	while ((rtw_end_of_queue_searchx(phead, plist)) == _FALSE) {
		...
		if (_rtw_memcmpx((void *)(pstapriv->atmel_rc_pattern), (void *)(psta->cmn.mac_addr), ETH_ALEN) == _TRUE)
			continue;
	...
	while ((rtw_end_of_queue_searchx(phead, plist)) == _FALSE) {
		...
		if (psta->isrc)
			continue;
#endif
		if (chk_sta_is_alivex(psta) || !psta->expire_to) {
		...
		if ((psta->flags & WLAN_STA_HT) && (psta->htpriv.agg_enable_bitmap || psta->under_exist_checking)) {
			...
			if (psta->expire_to <= (pstapriv->expire_to - 50)) {
				...
				/* tear down TX AMPDU */
				send_delbax(padapter, 1, psta->cmn.mac_addr);/*  */ /* originator */
...
#define KEEP_ALIVE_TRYCNT (3)
			...
			if ((psta->htpriv.ht_option == _TRUE) && (psta->htpriv.ampdu_enable == _TRUE)) {
				...
				if (0 == issued) {
					if (!(psta->state & WIFI_STA_ALIVE_CHK_STATE)) {
						...
						/* add_ba_hdlx(padapter, (u8*)paddbareq_parm); */

						RTW_INFO("issue addba_req to check if sta alive, keep_alive_trycnt=%d\n", psta->keep_alive_trycnt);

						issue_addba_reqx(padapter, psta->cmn.mac_addr, (u8)priority);
			...
			if (psta->state & WIFI_SLEEP_STATE) {
				if (!(psta->state & WIFI_STA_ALIVE_CHK_STATE)) {
					...
					rtw_tim_map_setx(padapter, pstapriv->tim_bitmap, psta->cmn.aid);
					update_beaconx(padapter, _TIM_IE_, NULL, _TRUE);
...

			{
				...
				stainfo_offset = rtw_stainfo_offsetx(pstapriv, psta);
			...
			if (psta->sleepq_len > (NR_XMITFRAME / pstapriv->asoc_list_cnt)
				...
				wakeup_sta_to_xmitx(padapter, psta);
			}
		}
	}
	...
	if (chk_alive_num) {
		...
		_rtw_memsetx(del_asoc_list, NUM_STA, NUM_STA);
		...
		if (pmlmeext->active_keep_alive_check) {
			...
			if (switch_channel_by_drv == _TRUE && rtw_get_oper_chx(padapter) != pmlmeext->cur_channel) {
		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			psta = rtw_get_stainfox_by_offset(pstapriv, chk_alive_list[i]);

			#ifdef CONFIG_ATMEL_RC_PATCH
			if (_rtw_memcmpx(pstapriv->atmel_rc_pattern, psta->cmn.mac_addr, ETH_ALEN) == _TRUE)
				continue;
			if (psta->flag_atmel_rc)
				continue;
			#endif
		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			psta = rtw_get_stainfox_by_offset(pstapriv, del_asoc_list[i]);
			_rtw_memcpyx(sta_addr, psta->cmn.mac_addr, ETH_ALEN);

			RTW_INFO(FUNC_ADPT_FMT" asoc expire "MAC_FMT", state=0x%x\n"
				, FUNC_ADPT_ARG(padapter), MAC_ARG(psta->cmn.mac_addr), psta->state);
			updated |= ap_free_stax(padapter, psta, _FALSE, WLAN_REASON_DEAUTH_LEAVING, _FALSE);
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(padapter))
...
void rtw_ap_update_sta_ra_infox(_adapter *padapter, struct sta_info *psta)
{
...
void rtw_update_bmc_stax_tx_rate(_adapter *adapter)
{
	struct sta_info *psta = NULL;
	u8 tx_rate;
	...
	psta->init_rate = hw_rate_to_m_ratex(tx_rate);
...
void rtw_init_bmc_sta_tx_ratex(_adapter *padapter, struct sta_info *psta)
{
#ifdef CONFIG_BMC_TX_LOW_RATE
	struct mlme_ext_priv *pmlmeext = &(padapter->mlmeextpriv);
#endif
	...
	if (padapter->bmc_tx_rate != MGN_UNKNOWN)
		...
		if (IsEnableHWOFDM(pmlmeext->cur_wireless_mode) && (psta->cmn.ra_info.ramask && 0xFF0))
			rate_idx = get_lowest_rate_idx_exx(psta->cmn.ra_info.ramask, 4); /*from basic rate*/
		else
			rate_idx = get_lowest_rate_idx(psta->cmn.ra_info.ramask); /*from basic rate*/
		#else
		rate_idx = get_highest_rate_idxx(psta->cmn.ra_info.ramask); /*from basic rate*/
		#endif
		if (rate_idx < 12)
...
void update_bmc_stax(_adapter *padapter)
{
	_irqL	irqL;
	...
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	WLAN_BSSID_EX *pcur_network = (WLAN_BSSID_EX *)&pmlmepriv->cur_network.network;
	struct sta_info *psta = rtw_get_bcmc_stainfox(padapter);
	...
	if (psta) {
		...
		supportRateNum = rtw_get_rateset_lenx((u8 *)&pcur_network->SupportedRates);
		network_type = rtw_check_network_typex((u8 *)&pcur_network->SupportedRates, supportRateNum, pcur_network->Configuration.DSConfig);
		...
		update_sta_basic_ratex(psta, network_type);
		...
		rtw_init_bmc_sta_tx_ratex(padapter, psta);
...
void update_sta_infox_apmode_ht_bf_cap(_adapter *padapter, struct sta_info *psta)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
void update_sta_infox_apmode(_adapter *padapter, struct sta_info *psta)
{
	_irqL	irqL;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	/*alloc macid when call rtw_alloc_stainfox(),release macid when call rtw_free_stainfox()*/
	...
	if (phtpriv_sta->ht_option) {
		...
		update_sta_infox_apmode_ht_bf_cap(padapter, psta);
		#endif
	} else {
	...
	/* Rx AMPDU */
	send_delbax(padapter, 0, psta->cmn.mac_addr);/* recipient */
	...
	/* add ratid */
	/* add_RATid(padapter, psta); */ /* move to ap_sta_info_defer_updatex() */
...
static void update_ap_info(_adapter *padapter, struct sta_info *psta)
{
	...
	psta->bssratelen = rtw_get_rateset_lenx(pnetwork->SupportedRates);
	_rtw_memcpyx(psta->bssrateset, pnetwork->SupportedRates, psta->bssratelen);
	...
	_rtw_memcpyx(&psta->htpriv, &pmlmepriv->htpriv, sizeof(struct ht_priv));
...
static void rtw_ap_check_scan(_adapter *padapter)
{
	...
	if (rtw_end_of_queue_searchx(phead, get_next(phead)) == _TRUE)
	...
	if (_TRUE == do_scan) {
		...
		rtw_scan_wait_completedx(padapter);
	}
	...
	while (1) {
		...
		if (rtw_chset_search_chx(adapter_to_chset(padapter), pnetwork->network.Configuration.DSConfig) >= 0
		    && rtw_mlme_band_checkx(padapter, pnetwork->network.Configuration.DSConfig) == _TRUE
			...
			if (delta_time < lifetime) {
				...
				pbuf = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_, &ie_len, (pbss->IELength - _BEACON_IE_OFFSET_));
				if (pbuf == NULL) {
					...
					if (_FALSE == ATOMIC_READx(&pmlmepriv->olbc_ht))
...
void rtw_start_bss_hdl_after_chbw_decidedx(_adapter *adapter)
{
	WLAN_BSSID_EX *pnetwork = &(adapter->mlmepriv.cur_network.network);
	struct sta_info *sta = NULL;

	/* update cur_wireless_mode */
	update_wireless_modex(adapter);

	/* update RRSR and RTS_INIT_RATE register after set channel and bandwidth */
	UpdateBrateTblx(adapter, pnetwork->SupportedRates);
	rtw_hal_set_hwregx(adapter, HW_VAR_BASIC_RATE, pnetwork->SupportedRates);

	/* update capability after cur_wireless_mode updated */
	update_capinfox(adapter, rtw_get_capabilityx(pnetwork));

	/* update bc/mc sta_info */
	update_bmc_stax(adapter);

	/* update AP's sta info */
	sta = rtw_get_stainfox(&adapter->stapriv, pnetwork->MacAddress);
	if (!sta) {
...
void start_bss_networkx(_adapter *padapter, struct createbss_parm *parm)
{
#define DUMP_ADAPTERS_STATUS 0
	...
	if (parm->req_ch != 0) {
		...
		if (MLME_IS_AP(padapter))
			...
			rtw_warn_on(1);
		req_ch = 0;
		_rtw_memcpyx(pnetwork_mlmeext, pnetwork, pnetwork->Length);
	}
	...
	if (NULL == rtw_get_wps_iex(pnetwork->IEs + _FIXED_IE_LENGTH_, pnetwork->IELength - _FIXED_IE_LENGTH_, NULL, NULL))
	...
	/* set MSR to AP_Mode		 */
	Set_MSRx(padapter, _HW_STATE_AP_);

	/* Set BSSID REG */
	rtw_hal_set_hwregx(padapter, HW_VAR_BSSID, pnetwork->MacAddress);
	...
	rtw_hal_set_hwregx(padapter, HW_VAR_BEACON_INTERVAL, (u8 *)(&bcn_interval));

	rtw_hal_rcr_set_chk_bssidx(padapter, mlme_act);

chbw_decision:
	ifbmp_ch_changed = rtw_ap_chbw_decisionx(padapter, parm->ifbmp, parm->excl_ifbmp
	...
	for (i = 0; i < pdvobj->iface_nums; i++) {
		...
		_rtw_memcpyx(&(pdvobj->padapters[i]->mlmepriv.cur_network.network)
			, &(pdvobj->padapters[i]->mlmeextpriv.mlmext_info.network)
			, pdvobj->padapters[i]->mlmeextpriv.mlmext_info.network.Length);

		rtw_start_bss_hdl_after_chbw_decidedx(pdvobj->padapters[i]);

		/* Set EDCA param reg after update cur_wireless_mode & update_capinfox */
		if (pregpriv->wifi_spec == 1)
	...
	if (MCC_EN(padapter)) {
		/* 
		* due to check under rtw_ap_chbw_decisionx
	...
	if (ch_to_set != 0) {
		set_channel_bwmodex(padapter, ch_to_set, offset_to_set, bw_to_set);
		rtw_mi_update_union_chan_infx(padapter, ch_to_set, offset_to_set, bw_to_set);
	}
...
#ifdef CONFIG_MCC_MODE
	/* after set_channel_bwmodex for backup IQK */
	rtw_hal_set_mcc_setting_start_bss_networkx(padapter, chbw_allow);
#endif
	...
	for (i = 0; i < pdvobj->iface_nums; i++) {
		...
		if (MLME_IS_GO(pdvobj->padapters[i])
			...
			rtw_cfg80211_ch_switch_notifyx(pdvobj->padapters[i]
	...
	if (DUMP_ADAPTERS_STATUS) {
		...
		dump_adapters_statusx(RTW_DBGDUMP , adapter_to_dvobj(padapter));
	}
...
update_beaconx:
	...
	for (i = 0; i < pdvobj->iface_nums; i++) {
		...
		#ifdef CONFIG_80211N_HT
		if ((ATOMIC_READx(&mlme->olbc) == _TRUE) || (ATOMIC_READx(&mlme->olbc_ht) == _TRUE)) {
			...
			update_beaconx(pdvobj->padapters[i], _HT_ADD_INFO_IE_, NULL, _FALSE);
		}
		#endif

		update_beaconx(pdvobj->padapters[i], _TIM_IE_, NULL, _FALSE);
	}
	...
	if (mlme_act != MLME_OPCH_SWITCH
		...
		if (rtw_is_list_emptyx(&padapter->list)) {
			#ifdef CONFIG_FW_HANDLE_TXBCN
			padapter->vap_id = rtw_ap_allocate_vapid(pdvobj);
			#endif
			rtw_list_insert_tailx(&padapter->list, get_list_head(&pdvobj->ap_if_q));
			pdvobj->nr_ap_if++;
		...
		rtw_hal_set_hwregx(padapter, HW_VAR_BEACON_INTERVAL, (u8 *)(&pdvobj->inter_bcn_space));
		#endif /*CONFIG_SWTIMER_BASED_TXBCN*/

#endif /*CONFIG_SUPPORT_MULTI_BCN*/

		#ifdef CONFIG_HW_P0_TSF_SYNC
		correct_TSFx(padapter, mlme_act);
		#endif
	}
	...
	ResumeTxBeaconx(padapter);
	{
		...
		for (i = 0; i < pdvobj->iface_nums; i++) {
			...
			if (send_beaconx(pdvobj->padapters[i]) == _FAIL)
				RTW_INFO(ADPT_FMT" issue_beaconx, fail!\n", ADPT_ARG(pdvobj->padapters[i]));
		}
#endif
#endif
...
int rtw_check_beacon_datax(_adapter *padapter, u8 *pbuf,  int len)
{
	int ret = _SUCCESS;
	u8 *p;
	u8 *pHT_caps_ie = NULL;
	...
	_rtw_memsetx(ie, 0, MAX_IE_SZ);

	_rtw_memcpyx(ie, pbuf, pbss_network->IELength);
	...
	_rtw_memcpyx(pbss_network->MacAddress, adapter_mac_addr(padapter), ETH_ALEN);

	/* beacon interval */
	p = rtw_get_beacon_intervalx_from_ie(ie);/* ie + 8;	 */ /* 8: TimeStamp, 2: Beacon Interval 2:Capability */
	/* pbss_network->Configuration.BeaconPeriod = le16_to_cpu(*(unsigned short*)p); */
	pbss_network->Configuration.BeaconPeriod = RTW_GET_LE16(p);

	/* capability */
	/* cap = *(unsigned short *)rtw_get_capabilityx_from_ie(ie); */
	/* cap = le16_to_cpu(cap); */
	cap = RTW_GET_LE16(ie);

	/* SSID */
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _SSID_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0) {
		_rtw_memsetx(&pbss_network->Ssid, 0, sizeof(NDIS_802_11_SSID));
		_rtw_memcpyx(pbss_network->Ssid.Ssid, (p + 2), ie_len);
		pbss_network->Ssid.SsidLength = ie_len;
#ifdef CONFIG_P2P
		_rtw_memcpyx(padapter->wdinfo.p2p_group_ssid, pbss_network->Ssid.Ssid, pbss_network->Ssid.SsidLength);
	...
	/* Mesh ID */
	if (MLME_IS_MESH(padapter)) {
		p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, WLAN_EID_MESH_ID, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len > 0) {
			_rtw_memsetx(&pbss_network->mesh_id, 0, sizeof(NDIS_802_11_SSID));
			_rtw_memcpyx(pbss_network->mesh_id.Ssid, (p + 2), ie_len);
	...
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _DSSET_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0)
		channel = *(p + 2);
	...
	/* parsing ERP_IE */
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _ERPINFO_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0)  {
		if(padapter->registrypriv.wireless_mode == WIRELESS_11B ) {

			pbss_network->IELength = pbss_network->IELength - *(p+1) - 2;
			ret_rm = rtw_ies_remove_iex(ie , &len, _BEACON_IE_OFFSET_, _ERPINFO_IE_,NULL,0);
			RTW_DBG("%s, remove_ie of ERP_IE=%d\n", __FUNCTION__, ret_rm);
		} else 
			ERP_IE_handlerx(padapter, (PNDIS_802_11_VARIABLE_IEs)p);
	...
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _RSN_IE_2_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0) {
		if (rtw_parse_wpa2_iex(p, ie_len + 2, &group_cipher, &pairwise_cipher, &akm, &mfp_opt) == _SUCCESS) {
	...
	for (p = ie + _BEACON_IE_OFFSET_; ; p += (ie_len + 2)) {
		p = rtw_get_iex(p, _SSN_IE_1_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_ - (ie_len + 2)));
		if ((p) && (_rtw_memcmpx(p + 2, OUI1, 4))) {
			if (rtw_parse_wpa_iex(p, ie_len + 2, &group_cipher, &pairwise_cipher, NULL) == _SUCCESS) {
				psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
	...
	if (pregistrypriv->wmm_enable) {
		for (p = ie + _BEACON_IE_OFFSET_; ; p += (ie_len + 2)) {
			p = rtw_get_iex(p, _VENDOR_SPECIFIC_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_ - (ie_len + 2)));
			if ((p) && _rtw_memcmpx(p + 2, WMM_PARA_IE, 6)) {
				pmlmepriv->qospriv.qos_option = 1;
				...
				*(p + 22) &= ~BIT(4); /* VO */
				...
				WMM_param_handlerx(padapter, (PNDIS_802_11_VARIABLE_IEs)p);
	...
	if(padapter->registrypriv.ht_enable &&
		...
		/* parsing HT_CAP_IE */
		p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len > 0) {
			u8 rf_type = 0;
			...
			if (0) {
				...
				dump_ht_cap_iex_content(RTW_DBGDUMP, p + 2, ie_len);
			}
			...
			_rtw_memcpyx(&(pmlmeinfo->HT_caps), pht_cap, sizeof(struct HT_caps_element));
			...
			/* Update Supported MCS Set field */
			{
				...
				switch (rx_nss) {
				case 1:
					set_mcs_rate_by_maskx(HT_CAP_ELE_RX_MCS_MAP(pht_cap), MCS_RATE_1R);
					break;
				case 2:
					set_mcs_rate_by_maskx(HT_CAP_ELE_RX_MCS_MAP(pht_cap), MCS_RATE_2R);
					break;
				case 3:
					set_mcs_rate_by_maskx(HT_CAP_ELE_RX_MCS_MAP(pht_cap), MCS_RATE_3R);
					break;
				case 4:
					set_mcs_rate_by_maskx(HT_CAP_ELE_RX_MCS_MAP(pht_cap), MCS_RATE_4R);
					break;
				default:
			...
			_rtw_memcpyx(&pmlmepriv->htpriv.ht_cap, p + 2, ie_len);
		...
		/* parsing HT_INFO_IE */
		p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _HT_ADD_INFO_IE_, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len > 0) {
			pHT_info_ie = p;
			if (channel == 0)
	...
	if (padapter->registrypriv.ht_enable &&
		...
		HT_caps_handlerx(padapter, (PNDIS_802_11_VARIABLE_IEs)pHT_caps_ie);

		HT_info_handlerx(padapter, (PNDIS_802_11_VARIABLE_IEs)pHT_info_ie);
	}
#endif
	...
	if (pmlmepriv->htpriv.ht_option == _TRUE
		...
		/* Parsing VHT CAP IE */
		p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, EID_VHTCapability, &ie_len, (pbss_network->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len > 0)
			vht_cap = _TRUE;
	...
	if(padapter->registrypriv.ht_enable &&
		...
		rtw_set_iex(pbss_network->IEs + pbss_network->IELength, EID_EXTCapability, 8, pmlmepriv->ext_capab_ie_data, &len);
	...
	rtw_ies_get_chbwx(pbss_network->IEs + _BEACON_IE_OFFSET_, pbss_network->IELength - _BEACON_IE_OFFSET_
...

	{
		...
		sta = rtw_get_stainfox(&padapter->stapriv, pbss_network->MacAddress);
		if (!sta) {
			sta = rtw_alloc_stainfox(&padapter->stapriv, pbss_network->MacAddress);
			if (sta == NULL)
				return _FAIL;
		}
	}

	rtw_startbss_cmdx(padapter, RTW_CMDF_WAIT_ACK);
	{
		int sk_band = RTW_GET_SCAN_BAND_SKIPx(padapter);

		if (sk_band)
			RTW_CLR_SCAN_BAND_SKIPx(padapter, sk_band);
	}
...
void rtw_macaddr_acl_initx(_adapter *adapter, u8 period)
{
	...
	_rtw_spinlockx_init(&(acl_node_q->lock));

	_enter_critical_bh(&(acl_node_q->lock), &irqL);
	_rtw_init_listheadx(&(acl_node_q->queue));
	acl->num = 0;
	acl->mode = RTW_ACL_MODE_DISABLED;
	for (i = 0; i < NUM_ACL; i++) {
		_rtw_init_listheadx(&acl->aclnode[i].list);
		acl->aclnode[i].valid = _FALSE;
	}
	_exit_critical_bh(&(acl_node_q->lock), &irqL);
}

static void _rtw_macaddr_acl_deinitx(_adapter *adapter, u8 period, bool clear_only)
{
	struct sta_priv *stapriv = &adapter->stapriv;
	struct wlan_acl_pool *acl;
	_queue *acl_node_q;
	...
	list = get_next(head);
	while (rtw_end_of_queue_searchx(head, list) == _FALSE) {
	...
	if (!clear_only)
		_rtw_spinlockx_free(&(acl_node_q->lock));

	rtw_warn_on(acl->num);
	acl->mode = RTW_ACL_MODE_DISABLED;
}

void rtw_macaddr_acl_deinitx(_adapter *adapter, u8 period)
{
	_rtw_macaddr_acl_deinitx(adapter, period, 0);
}

void rtw_macaddr_acl_clearx(_adapter *adapter, u8 period)
{
	_rtw_macaddr_acl_deinitx(adapter, period, 1);
}

void rtw_set_macaddr_aclx(_adapter *adapter, u8 period, int mode)
{
	struct sta_priv *stapriv = &adapter->stapriv;
	struct wlan_acl_pool *acl;
...
int rtw_acl_add_stax(_adapter *adapter, u8 period, const u8 *addr)
{
	_irqL irqL;
	_list *list, *head;
	u8 existed = 0;
	...
	while (rtw_end_of_queue_searchx(head, list) == _FALSE) {
		...
		if (_rtw_memcmpx(acl_node->addr, addr, ETH_ALEN)) {
			if (acl_node->valid == _TRUE) {
				existed = 1;
				break;
			}
	...
	for (i = 0; i < NUM_ACL; i++) {
		...
		if (acl_node->valid == _FALSE) {
			...
			_rtw_init_listheadx(&acl_node->list);
			_rtw_memcpyx(acl_node->addr, addr, ETH_ALEN);
			acl_node->valid = _TRUE;

			rtw_list_insert_tailx(&acl_node->list, get_list_head(acl_node_q));
			acl->num++;
			break;
		}
	}
...
int rtw_acl_remove_stax(_adapter *adapter, u8 period, const u8 *addr)
{
	_irqL irqL;
	_list *list, *head;
	int ret = 0;
	...
	while (rtw_end_of_queue_searchx(head, list) == _FALSE) {
		...
		if (is_baddr || _rtw_memcmpx(acl_node->addr, addr, ETH_ALEN)) {
			if (acl_node->valid == _TRUE) {
				acl_node->valid = _FALSE;
				rtw_list_delete(&acl_node->list);
				acl->num--;
...
u8 rtw_ap_set_sta_keyx(_adapter *adapter, const u8 *addr, u8 alg, const u8 *key, u8 keyid, u8 gk)
{
	...
	param = (struct set_stakey_parm *)rtw_zmallocx(sizeof(struct set_stakey_parm));
	if (param == NULL) {
		rtw_mfreex((u8 *) cmd, sizeof(struct cmd_obj));
		res = _FAIL;
		goto exit;
	}
	...
	_rtw_memcpyx(param->addr, addr, ETH_ALEN);
	param->algorithm = alg;
	param->keyid = keyid;
	_rtw_memcpyx(param->key, key, 16);
	param->gk = gk;

	res = rtw_enqueue_cmdx(cmdpriv, cmd);

exit:
	return res;
}

u8 rtw_ap_set_pairwise_keyx(_adapter *padapter, struct sta_info *psta)
{
	return rtw_ap_set_sta_keyx(padapter
		, psta->cmn.mac_addr
...
static int rtw_ap_set_key(_adapter *padapter, u8 *key, u8 alg, int keyid, u8 set_tx)
{
	...
	_rtw_memcpyx(&(psetkeyparm->key[0]), key, keylen);
...
int rtw_ap_set_group_keyx(_adapter *padapter, u8 *key, u8 alg, int keyid)
{
	RTW_INFO("%s\n", __FUNCTION__);
...
int rtw_ap_set_wep_keyx(_adapter *padapter, u8 *key, u8 keylen, int keyid, u8 set_tx)
{
	u8 alg;
...
u8 rtw_ap_bmc_frames_hdlx(_adapter *padapter)
{
#define HIQ_XMIT_COUNTS (6)
	...
	if ((rtw_tim_map_is_setx(padapter, pstapriv->tim_bitmap, 0)) && (psta_bmc->sleepq_len > 0)) {
		...
		_update_beaconx(padapter, _TIM_IE_, NULL, _FALSE, "update TIM with TIB=1");
	...
	while (_FALSE == empty && rtw_get_passing_time_msx(start) < 3000) {
		rtw_msleep_osx(100);
		rtw_hal_get_hwregx(padapter, HW_VAR_CHK_HI_QUEUE_EMPTY, &empty);
	}
...
static void update_bcn_ext_capab_ie(_adapter *padapter)
{
	...
	if (pbuf && ie_len > 0)
		rtw_remove_bcn_iex(padapter, pnetwork, _EXT_CAP_IE_);

	if ((pmlmepriv->ext_capab_ie_len > 0) &&
	    (_rtw_memcmpx(pmlmepriv->ext_capab_ie_data, null_extcap_data, sizeof(null_extcap_data)) == _FALSE))
		rtw_add_bcn_iex(padapter, pnetwork, _EXT_CAP_IE_, pmlmepriv->ext_capab_ie_data, pmlmepriv->ext_capab_ie_len);
...
static void update_bcn_erpinfo_ie(_adapter *padapter)
{
	...
	/* parsing ERP_IE */
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _ERPINFO_IE_, &len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
	if (p && len > 0) {
...
static void update_bcn_htinfo_ie(_adapter *padapter)
{
	...
	u32 sta_info_updatex_type = STA_INFO_UPDATE_NONE;
	*/
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	RTW_INFO("num_sta_40mhz_intolerant(%d), 20mhz_width_req(%d), intolerant_ch_rpt(%d), olbc(%d)\n",
		pmlmepriv->num_sta_40mhz_intolerant, pmlmepriv->ht_20mhz_width_req, pmlmepriv->ht_intolerant_ch_reported, ATOMIC_READx(&pmlmepriv->olbc));

	/*parsing HT_INFO_IE, currently only update ht_op_mode - pht_info->infos[1] & pht_info->infos[2] for wifi logo test*/
	p = rtw_get_iex(ie + _BEACON_IE_OFFSET_, _HT_ADD_INFO_IE_, &len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
	if (p && len > 0) {
		...
		if ((pmlmepriv->sw_to_20mhz == 0) && (pmlmeext->cur_channel <= 14)) {
			if ((pmlmepriv->num_sta_40mhz_intolerant > 0) || (pmlmepriv->ht_20mhz_width_req == _TRUE)
			    || (pmlmepriv->ht_intolerant_ch_reported == _TRUE) || (ATOMIC_READx(&pmlmepriv->olbc) == _TRUE)) {
				SET_HT_OP_ELE_2ND_CHL_OFFSET(pht_info, 0);
				SET_HT_OP_ELE_STA_CHL_WIDTH(pht_info, 0);

				pmlmepriv->sw_to_20mhz = 1;
				/*
				sta_info_updatex_type |= STA_INFO_UPDATE_BW;
			...
			if ((pmlmepriv->num_sta_40mhz_intolerant == 0) && (pmlmepriv->ht_20mhz_width_req == _FALSE)
			    && (pmlmepriv->ht_intolerant_ch_reported == _FALSE) && (ATOMIC_READx(&pmlmepriv->olbc) == _FALSE)) {
	...
	/*associated_clients_updatex(padapter, beacon_updated, sta_info_updatex_type);*/
#endif /* CONFIG_80211N_HT */
}
...
static void update_bcn_wps_ie(_adapter *padapter)
{
	...
	pwps_ie = rtw_get_wps_iex(ie + _FIXED_IE_LENGTH_, ielen - _FIXED_IE_LENGTH_, NULL, &wps_ielen);
	...
	if ((wps_offset + wps_ielen + 2 + remainder_ielen) <= MAX_IE_SZ) {
		_rtw_memcpyx(pwps_ie, pwps_ie_src, wps_ielen + 2);
		pwps_ie += (wps_ielen + 2);

		if (pbackup_remainder_ie)
			_rtw_memcpyx(pwps_ie, pbackup_remainder_ie, remainder_ielen);
	...
	/* deal with the case without set_tx_beacon_cmdx() in update_beaconx() */
#if defined(CONFIG_INTERRUPT_BASED_TXBCN) || defined(CONFIG_PCI_HCI)
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
		u8 sr = 0;
		rtw_get_wps_attrx_content(pwps_ie_src,  wps_ielen, WPS_ATTR_SELECTED_REGISTRAR, (u8 *)(&sr), NULL);
...
static void update_bcn_vendor_spec_ie(_adapter *padapter, u8 *oui)
{
	...
	if (_rtw_memcmpx(RTW_WPA_OUIx, oui, 4))
		update_bcn_wpa_ie(padapter);
	else if (_rtw_memcmpx(WMM_OUIx, oui, 4))
		update_bcn_wmm_ie(padapter);
	else if (_rtw_memcmpx(WPS_OUIx, oui, 4))
		update_bcn_wps_ie(padapter);
	else if (_rtw_memcmpx(P2P_OUIx, oui, 4))
...
void _update_beaconx(_adapter *padapter, u8 ie_id, u8 *oui, u8 tx, const char *tag)
{
	_irqL irqL;
	struct mlme_priv *pmlmepriv;
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI) || defined(CONFIG_PCI_BCN_POLLING)
	if (tx && updated) {
		/* send_beaconx(padapter); */ /* send_beaconx must execute on TSR level */
		if (0)
			RTW_INFO(FUNC_ADPT_FMT" ie_id:%u - %s\n", FUNC_ADPT_ARG(padapter), ie_id, tag);
		set_tx_beacon_cmdx(padapter);
	}
#else
	{
...
void rtw_process_public_act_bsscoexx(_adapter *padapter, u8 *pframe, uint frame_len)
{
...
void rtw_process_ht_action_smpsx(_adapter *padapter, u8 *ta, u8 ctrl_field)
{
	u8 e_field, m_field;
	struct sta_info *psta;
	struct sta_priv *pstapriv = &padapter->stapriv;

	psta = rtw_get_stainfox(pstapriv, ta);
	if (psta == NULL)
		return;
	...
	if (psta->htpriv.smps_cap != WLAN_HT_CAP_SM_PS_DYNAMIC)
		rtw_ssmps_wk_cmdx(padapter, psta, e_field, 1);
}
...
*/
int rtw_ht_operation_updatex(_adapter *padapter)
{
	...
	if (!(pmlmepriv->ht_op_mode & HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT) &&
	    (pmlmepriv->num_sta_no_ht || ATOMIC_READx(&pmlmepriv->olbc_ht))) {
		pmlmepriv->ht_op_mode |= HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT;
		op_mode_changes++;
	} else if ((pmlmepriv->ht_op_mode &
		    HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT) &&
		   (pmlmepriv->num_sta_no_ht == 0 && !ATOMIC_READx(&pmlmepriv->olbc_ht))) {
		pmlmepriv->ht_op_mode &=
			~HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT;
		op_mode_changes++;
	}
	...
	else if (ATOMIC_READx(&pmlmepriv->olbc_ht))
...
void associated_clients_updatex(_adapter *padapter, u8 updated, u32 sta_info_type)
{
...
void bss_cap_update_on_sta_joinx(_adapter *padapter, struct sta_info *psta)
{
	u8 beacon_updated = _FALSE;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv *pmlmeext = &(padapter->mlmeextpriv);
	...
	if (psta->flags & WLAN_STA_NONERP) {
		if (!psta->nonerp_set) {
			...
			if (pmlmepriv->num_sta_non_erp == 1) {
				beacon_updated = _TRUE;
				update_beaconx(padapter, _ERPINFO_IE_, NULL, _FALSE);
			}
		}
	...
	if(padapter->registrypriv.ht_enable &&
		...
		if (rtw_ht_operation_updatex(padapter) > 0) {
			...
			update_beaconx(padapter, _HT_ADD_INFO_IE_, NULL, _FALSE);
			beacon_updated = _TRUE;
		}
	}
#endif /* CONFIG_80211N_HT */
	...
	if (MLME_IS_MESH(padapter)) {
		...
		update_beaconx(padapter, WLAN_EID_MESH_CONFIG, NULL, _FALSE);
	...
	if (beacon_updated)
		update_beaconx(padapter, 0xFF, NULL, _TRUE);

	/* update associcated stations cap. */
	associated_clients_updatex(padapter,  beacon_updated, STA_INFO_UPDATE_ALL);

	RTW_INFO("%s, updated=%d\n", __func__, beacon_updated);

}

u8 bss_cap_update_on_sta_leavex(_adapter *padapter, struct sta_info *psta)
{
	u8 beacon_updated = _FALSE;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	if (rtw_tim_map_is_setx(padapter, pstapriv->tim_bitmap, psta->cmn.aid)) {
		rtw_tim_map_clearx(padapter, pstapriv->tim_bitmap, psta->cmn.aid);
		beacon_updated = _TRUE;
		update_beaconx(padapter, _TIM_IE_, NULL, _FALSE);
	}
	...
	associated_clients_updatex(padapter,  beacon_updated, STA_INFO_UPDATE_ALL); /* move it to avoid deadlock */
#endif
...
u8 ap_free_stax(_adapter *padapter, struct sta_info *psta, bool active, u16 reason, bool enqueue)
{
	_irqL irqL;
	u8 beacon_updated = _FALSE;
	...
	if (active == _TRUE) {
		...
		if (!MLME_IS_MESH(padapter))
			issue_deauthx(padapter, psta->cmn.mac_addr, reason);
	}
	...
	/* clear cam entry / key */
	rtw_clearstakey_cmdx(padapter, psta, enqueue);
	...
	if (!MLME_IS_MESH(padapter)) {
		...
		rtw_cfg80211_indicate_sta_disassocx(padapter, psta->cmn.mac_addr, reason);
		#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) && !defined(CONFIG_CFG80211_FORCE_COMPATIBLE_2_6_37_UNDER)
		rtw_cfg80211_indicate_sta_disassocx(padapter, psta->cmn.mac_addr, reason);
		#else /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) && !defined(CONFIG_CFG80211_FORCE_COMPATIBLE_2_6_37_UNDER) */
		/* will call rtw_cfg80211_indicate_sta_disassocx() in cmd_thread for old API context */
		#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) && !defined(CONFIG_CFG80211_FORCE_COMPATIBLE_2_6_37_UNDER) */
#else
		rtw_indicate_sta_disassoc_eventx(padapter, psta);
#endif
	}

	beacon_updated = bss_cap_update_on_sta_leavex(padapter, psta);

	report_del_sta_eventx(padapter, psta->cmn.mac_addr, reason, enqueue, _FALSE);

	return beacon_updated;

}

int rtw_ap_inform_ch_switchx(_adapter *padapter, u8 new_ch, u8 ch_offset)
{
	_irqL irqL;
	_list	*phead, *plist;
	int ret = 0;
	...
	while ((rtw_end_of_queue_searchx(phead, plist)) == _FALSE) {
		...
		issue_action_spct_ch_switchx(padapter, psta->cmn.mac_addr, new_ch, ch_offset);
		psta->expire_to = ((pstapriv->expire_to * 2) > 5) ? 5 : (pstapriv->expire_to * 2);
	}
	_exit_critical_bh(&pstapriv->asoc_list_lock, &irqL);

	issue_action_spct_ch_switchx(padapter, bc_addr, new_ch, ch_offset);

	return ret;
}

int rtw_sta_flushx(_adapter *padapter, bool enqueue)
{
	_irqL irqL;
	_list	*phead, *plist;
	int ret = 0;
	...
	/* call ap_free_stax() for each sta picked */
	for (i = 0; i < flush_num; i++) {
		u8 sta_addr[ETH_ALEN];

		psta = rtw_get_stainfox_by_offset(pstapriv, flush_list[i]);
		_rtw_memcpyx(sta_addr, psta->cmn.mac_addr, ETH_ALEN);

		ap_free_stax(padapter, psta, _TRUE, WLAN_REASON_DEAUTH_LEAVING, enqueue);
		#ifdef CONFIG_RTW_MESH
		if (MLME_IS_MESH(padapter))
			rtw_mesh_expire_peer(padapter, sta_addr);
		#endif
	...
	if (!MLME_IS_MESH(padapter))
		issue_deauthx(padapter, bc_addr, WLAN_REASON_DEAUTH_LEAVING);

	associated_clients_updatex(padapter, _TRUE, STA_INFO_UPDATE_ALL);

	return ret;
}

/* called > TSR LEVEL for USB or SDIO Interface*/
void sta_info_updatex(_adapter *padapter, struct sta_info *psta)
{
...
/* called >= TSR LEVEL for USB or SDIO Interface*/
void ap_sta_info_defer_updatex(_adapter *padapter, struct sta_info *psta)
{
	if (psta->state & _FW_LINKED)
		rtw_hal_update_ra_maskx(psta); /* DM_RATR_STA_INIT */
}
/* restore hw setting from sw data structures */
void rtw_ap_restore_networkx(_adapter *padapter)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *psta;
	...
	rtw_startbss_cmdx(padapter, RTW_CMDF_DIRECTLY);

	if ((padapter->securitypriv.dot11PrivacyAlgrthm == _TKIP_) ||
	    (padapter->securitypriv.dot11PrivacyAlgrthm == _AES_)) {
		/* restore group key, WEP keys is restored in ips_leave() */
		rtw_set_keyx(padapter, psecuritypriv, psecuritypriv->dot118021XGrpKeyid, 0, _FALSE);
	}
	...
	for (i = 0; i < chk_alive_num; i++) {
		...
		if (psta == NULL)
			...
			if ((padapter->securitypriv.dot11PrivacyAlgrthm == _TKIP_) ||
			    (padapter->securitypriv.dot11PrivacyAlgrthm == _AES_))
				rtw_setstakey_cmdx(padapter, psta, UNICAST_KEY, _FALSE);
		}
	}

}

void start_ap_modex(_adapter *padapter)
{
	int i;
	struct sta_info *psta = NULL;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	_rtw_memsetx(pmlmepriv->ext_capab_ie_data, 0, sizeof(pmlmepriv->ext_capab_ie_data));
	...
	psta = rtw_get_bcmc_stainfox(padapter);
	/*_enter_critical_bh(&(pstapriv->sta_hash_lock), &irqL);*/
	if (psta)
		rtw_free_stainfox(padapter, psta);
	/*_exit_critical_bh(&(pstapriv->sta_hash_lock), &irqL);*/
	...
	if (rtw_mi_get_ap_num(padapter))
		RTW_SET_SCAN_BAND_SKIPx(padapter, BAND_5G);

}

void rtw_ap_bcmc_sta_flushx(_adapter *padapter)
{
#ifdef CONFIG_CONCURRENT_MODE
	int cam_id = -1;
...
void stop_ap_modex(_adapter *padapter)
{
	...
	/* reset and init security priv , this can refine with rtw_reset_securityprivx */
...
#if CONFIG_RTW_MACADDR_ACL
	rtw_macaddr_acl_clearx(padapter, RTW_ACL_PERIOD_BSS);
#endif

	rtw_sta_flushx(padapter, _TRUE);
	rtw_ap_bcmc_sta_flushx(padapter);

	/* free_assoc_sta_resources	 */
	rtw_free_all_stainfox(padapter);

	psta = rtw_get_bcmc_stainfox(padapter);
	if (psta) {
		rtw_sta_mstatus_disc_rptx(padapter, psta->cmn.mac_id);
		/* _enter_critical_bh(&(pstapriv->sta_hash_lock), &irqL);		 */
		rtw_free_stainfox(padapter, psta);
		/*_exit_critical_bh(&(pstapriv->sta_hash_lock), &irqL);*/
	}
...
void rtw_ap_update_bss_chbwx(_adapter *adapter, WLAN_BSSID_EX *bss, u8 ch, u8 bw, u8 offset)
{
...
#define UPDATE_HT_CAP 1
...
#endif
	{
		...
		p = rtw_get_iex((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)), _DSSET_IE_, &ie_len, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		if (p && ie_len > 0)
			*(p + 2) = ch;
		...
		if (change_band == _TRUE)
			change_band_update_iex(adapter, bss, ch);
	}
	...
	if (vhtpriv->vht_option == _TRUE) {
		...
		vht_cap_ie = rtw_get_iex((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)), EID_VHTCapability, &vht_cap_ielen, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		vht_op_ie = rtw_get_iex((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)), EID_VHTOperation, &vht_op_ielen, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		center_freq = rtw_get_center_chx(ch, bw, offset);
...
#ifdef CONFIG_80211N_HT
	{
		...
		ht_cap_ie = rtw_get_iex((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)), EID_HTCapability, &ht_cap_ielen, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		ht_op_ie = rtw_get_iex((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)), EID_HTInfo, &ht_op_ielen, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (MLME_IS_ASOC(iface)
			...
			if (rtw_linked_checkx(iface) == _TRUE) {
				...
				if (1)
					rtw_ap_inform_ch_switchx(iface, dec_ch[i], dec_offset[i]);
				else
				#endif
		...
		rtw_ap_update_bss_chbwx(iface, network, dec_ch[i], dec_bw[i], dec_offset[i]);
	}
...
static u8 rtw_ap_ch_specific_chk(_adapter *adapter, u8 ch, u8 *bw, u8 *offset, const char *caller)
{
	...
	rtw_adjust_chbwx(adapter, ch, bw, offset);
	...
	while (!rtw_chset_is_chbw_validx(chset, ch, *bw, *offset)
		|| (rtw_odm_dfs_domain_unknownx(dvobj) && rtw_is_dfs_chxbw(ch, *bw, *offset))
	) {
	...
	if (rtw_odm_dfs_domain_unknownx(dvobj) && rtw_is_dfs_chxbw(ch, *bw, *offset)) {
...
#if defined(CONFIG_DFS_MASTER)
	if (!rtw_odm_dfs_domain_unknownx(dvobj)) {
		if (rfctl->radar_detected
			&& rfctl->dbg_dfs_choose_dfs_ch_first
		) {
			ch_avail = rtw_choose_shortest_waiting_chx(rfctl, sel_ch, max_bw
						, ch, bw, offset
...
u8 rtw_ap_chbw_decisionx(_adapter *adapter, u8 ifbmp, u8 excl_ifbmp
	...
	, u8 *ch, u8 *bw, u8 *offset, u8 *chbw_allow)
{
	...
	rtw_mi_statusx_by_ifbmp(dvobj, ifbmp_others, &mstate_others);
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		rtw_ies_get_chbwx(BSS_EX_TLV_IES(network), BSS_EX_TLV_IES_LEN(network)
	...
	if (MSTATE_STA_LD_NUM(&mstate_others) || MSTATE_STA_LG_NUM(&mstate_others)
		...
		rtw_warn_on(!rtw_mi_get_ch_setting_unionx_by_ifbmp(dvobj, ifbmp_others, &u_ch, &u_bw, &u_offset));
	...
	if (MSTATE_STA_LG_NUM(&mstate_others) && !MSTATE_STA_LD_NUM(&mstate_others)) {
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			rtw_adjust_chbwx(iface, dec_ch[i], &dec_bw[i], &dec_offset[i]);
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(iface))
				rtw_mesh_adjust_chbw(dec_ch[i], &dec_bw[i], &dec_offset[i]);
			#endif

			if (rtw_is_chbw_groupedx(u_ch, u_bw, u_offset, dec_ch[i], dec_bw[i], dec_offset[i])) {
				rtw_chset_sync_chbwx(chset
					, &dec_ch[i], &dec_bw[i], &dec_offset[i]
					, &u_ch, &u_bw, &u_offset);
				set_u_ch = _TRUE;
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			rtw_adjust_chbwx(iface, u_ch, &dec_bw[i], &dec_offset[i]);
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(iface))
			...
			if (req_ch == 0 && dec_bw[i] > u_bw
				&& rtw_is_dfs_chxbw(u_ch, u_bw, u_offset)
			) {
		...
		*chbw_allow = _TRUE;
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (u_ch == 0) {
				...
				rtw_adjust_chbwx(adapter, u_ch, &u_bw, &u_offset);
				rtw_get_offset_by_chbwx(u_ch, u_bw, &u_offset);
			} else {
				u8 tmp_ch = dec_ch[i];
				u8 tmp_bw = dec_bw[i];
				u8 tmp_offset = dec_offset[i];
				
				rtw_adjust_chbwx(adapter, tmp_ch, &tmp_bw, &tmp_offset);
				rtw_get_offset_by_chbwx(tmp_ch, tmp_bw, &tmp_offset);

				rtw_warn_on(!rtw_is_chbw_groupedx(u_ch, u_bw, u_offset, tmp_ch, tmp_bw, tmp_offset));
				rtw_sync_chbwx(&tmp_ch, &tmp_bw, &tmp_offset, &u_ch, &u_bw, &u_offset);
			}
		}
...
u8 rtw_ap_sta_states_checkx(_adapter *adapter)
{
	struct sta_info *psta;
	struct sta_priv *pstapriv = &adapter->stapriv;
	_list *plist, *phead;
...
#define BEACON_EARLY_TIME		20	/* unit:TU*/
	...
	_enter_critical_bh(&pdvobj->ap_if_q.lock, &irqL);
	if (rtw_is_list_emptyx(&pdvobj->ap_if_q.queue) || (pdvobj->nr_ap_if == 0)) {
	...
	/* read TSF */
	timestamp[1] = rtw_read32x(padapter, 0x560 + 4);
	timestamp[0] = rtw_read32x(padapter, 0x560);
	while (timestamp[1]) {
...
void rtw_ap_parse_sta_capabilityx(_adapter *adapter, struct sta_info *sta, u8 *cap)
{
...
u16 rtw_ap_parse_sta_supported_ratesx(_adapter *adapter, struct sta_info *sta, u8 *tlv_ies, u16 tlv_ies_len)
{
	u8 rate_set[12];
	u8 rate_num;
	int i;
	...
	rtw_ies_get_supported_ratex(tlv_ies, tlv_ies_len, rate_set, &rate_num);
	if (rate_num == 0) {
	...
	_rtw_memcpyx(sta->bssrateset, rate_set, rate_num);
	sta->bssratelen = rate_num;

	if (MLME_IS_AP(adapter)) {
		/* this function force only CCK rates to be bassic rate... */
		UpdateBrateTblxForSoftAP(sta->bssrateset, sta->bssratelen);
	}
...
u16 rtw_ap_parse_sta_security_iex(_adapter *adapter, struct sta_info *sta, struct rtw_ieee802_11_elems *elems)
{
	...
	_rtw_memsetx(sta->wpa_ie, 0, sizeof(sta->wpa_ie));

	if ((sec->wpa_psk & BIT(1)) && elems->rsn_ie) {
		wpa_ie = elems->rsn_ie;
		wpa_ie_len = elems->rsn_ie_len;

		if (rtw_parse_wpa2_iex(wpa_ie - 2, wpa_ie_len + 2, &group_cipher, &pairwise_cipher, &akm, &mfp_opt) == _SUCCESS) {
		...
		if (rtw_parse_wpa_iex(wpa_ie - 2, wpa_ie_len + 2, &group_cipher, &pairwise_cipher, NULL) == _SUCCESS) {
			sta->dot8021xalg = 1;/* psk, todo:802.1x */
			sta->wpa_psk |= BIT(0);
	...
	if (MLME_IS_AP(adapter) &&
		...
		if (rtw_cached_pmkidx(adapter, sta->cmn.mac_addr) == -1) {
	...
	if (wpa_ie == NULL) {
		...
		if ((sec->wpa_psk > 0)
			...
			if (mlme->wps_beacon_ie) {
				...
				rtw_get_wps_attrx_content(mlme->wps_beacon_ie, mlme->wps_beacon_ie_len, WPS_ATTR_SELECTED_REGISTRAR, &selected_registrar, NULL);
		...
		if (copy_len > 0)
			_rtw_memcpyx(sta->wpa_ie, wpa_ie - 2, copy_len);
	}
...
void rtw_ap_parse_sta_wmm_iex(_adapter *adapter, struct sta_info *sta, u8 *tlv_ies, u16 tlv_ies_len)
{
	struct mlme_priv *mlme = &adapter->mlmepriv;
	...
	p = rtw_get_iex_ex(tlv_ies, tlv_ies_len, WLAN_EID_VENDOR_SPECIFIC, WMM_IE, 6, NULL, NULL);
	if (!p)
		goto exit;
...
void rtw_ap_parse_sta_ht_iex(_adapter *adapter, struct sta_info *sta, struct rtw_ieee802_11_elems *elems)
{
	struct mlme_priv *mlme = &adapter->mlmepriv;
	...
	_rtw_memsetx(&sta->htpriv.ht_cap, 0, sizeof(struct rtw_ieee80211_ht_cap));
	if (elems->ht_capabilities && elems->ht_capabilities_len >= sizeof(struct rtw_ieee80211_ht_cap)) {
		sta->flags |= WLAN_STA_HT;
		sta->flags |= WLAN_STA_WME;
		_rtw_memcpyx(&sta->htpriv.ht_cap, elems->ht_capabilities, sizeof(struct rtw_ieee80211_ht_cap));

		if (elems->ht_operation && elems->ht_operation_len == HT_OP_IE_LEN) {
			_rtw_memcpyx(sta->htpriv.ht_op, elems->ht_operation, HT_OP_IE_LEN);
...
void rtw_ap_parse_sta_vht_iex(_adapter *adapter, struct sta_info *sta, struct rtw_ieee802_11_elems *elems)
{
	struct mlme_priv *mlme = &adapter->mlmepriv;
	...
	_rtw_memsetx(&sta->vhtpriv, 0, sizeof(struct vht_priv));
	if (elems->vht_capabilities && elems->vht_capabilities_len == VHT_CAP_IE_LEN) {
		sta->flags |= WLAN_STA_VHT;
		_rtw_memcpyx(sta->vhtpriv.vht_cap, elems->vht_capabilities, VHT_CAP_IE_LEN);

		if (elems->vht_operation && elems->vht_operation_len== VHT_OP_IE_LEN) {
			_rtw_memcpyx(sta->vhtpriv.vht_op, elems->vht_operation, VHT_OP_IE_LEN);
			sta->vhtpriv.op_present = 1;
		}

		if (elems->vht_op_mode_notify && elems->vht_op_mode_notify_len == 1) {
			_rtw_memcpyx(&sta->vhtpriv.vht_op_mode_notify, elems->vht_op_mode_notify, 1);
