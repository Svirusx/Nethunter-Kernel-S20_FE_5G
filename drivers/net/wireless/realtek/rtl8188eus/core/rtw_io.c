...
Only sync read/rtw_write_memx operations are provided.
...
u8 _rtw_read8x(_adapter *adapter, u32 addr)
{
	u8 r_val;
...
u16 _rtw_read16x(_adapter *adapter, u32 addr)
{
	u16 r_val;
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
...
u32 _rtw_read32x(_adapter *adapter, u32 addr)
{
	u32 r_val;
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
...
int _rtw_write8x(_adapter *adapter, u32 addr, u8 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	...
	return RTW_STATUS_CODEx(ret);
}
int _rtw_write16x(_adapter *adapter, u32 addr, u16 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
...
int _rtw_write32x(_adapter *adapter, u32 addr, u32 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
...
int _rtw_writeNx(_adapter *adapter, u32 addr , u32 length , u8 *pdata)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
...
int _rtw_write8x_async(_adapter *adapter, u32 addr, u8 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
...
int _rtw_write16x_async(_adapter *adapter, u32 addr, u16 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
...
int _rtw_write32x_async(_adapter *adapter, u32 addr, u32 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
...
void _rtw_read_memx(_adapter *adapter, u32 addr, u32 cnt, u8 *pmem)
{
...
void _rtw_write_memx(_adapter *adapter, u32 addr, u32 cnt, u8 *pmem)
{
...
void _rtw_read_portx(_adapter *adapter, u32 addr, u32 cnt, u8 *pmem)
{
...
void _rtw_read_portx_cancel(_adapter *adapter)
{
...
u32 _rtw_write_portx(_adapter *adapter, u32 addr, u32 cnt, u8 *pmem)
{
...
u32 _rtw_write_portx_and_wait(_adapter *adapter, u32 addr, u32 cnt, u8 *pmem, int timeout_ms)
{
	...
	ret = _rtw_write_portx(adapter, addr, cnt, pmem);
...
void _rtw_write_portx_cancel(_adapter *adapter)
{
...
int rtw_inc_and_chk_continual_io_errorx(struct dvobj_priv *dvobj)
{
	...
	value = ATOMIC_INCx_RETURN(&dvobj->continual_io_error);
...
{
	ATOMIC_SETx(&dvobj->continual_io_error, 0);
}
...
u8 dbg_rtw_read8x(_adapter *adapter, u32 addr, const char *caller, const int line)
{
	u8 val = _rtw_read8x(adapter, addr);

	if (match_read_sniff(adapter, addr, 1, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_read8x(0x%04x) return 0x%02x\n"
...
u16 dbg_rtw_read16x(_adapter *adapter, u32 addr, const char *caller, const int line)
{
	u16 val = _rtw_read16x(adapter, addr);

	if (match_read_sniff(adapter, addr, 2, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_read16x(0x%04x) return 0x%04x\n"
			, caller, line, addr, val);
	}
...
u32 dbg_rtw_read32x(_adapter *adapter, u32 addr, const char *caller, const int line)
{
	u32 val = _rtw_read32x(adapter, addr);

	if (match_read_sniff(adapter, addr, 4, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_read32x(0x%04x) return 0x%08x\n"
			, caller, line, addr, val);
	}
...
int dbg_rtw_write8x(_adapter *adapter, u32 addr, u8 val, const char *caller, const int line)
{
	if (match_write_sniff(adapter, addr, 1, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_write8x(0x%04x, 0x%02x)\n"
			, caller, line, addr, val);
	}

	return _rtw_write8x(adapter, addr, val);
}
int dbg_rtw_write16x(_adapter *adapter, u32 addr, u16 val, const char *caller, const int line)
{
	if (match_write_sniff(adapter, addr, 2, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_write16x(0x%04x, 0x%04x)\n"
			, caller, line, addr, val);
	}

	return _rtw_write16x(adapter, addr, val);
}
int dbg_rtw_write32x(_adapter *adapter, u32 addr, u32 val, const char *caller, const int line)
{
	if (match_write_sniff(adapter, addr, 4, val)) {
		RTW_INFO("DBG_IO %s:%d rtw_write32x(0x%04x, 0x%08x)\n"
			, caller, line, addr, val);
	}

	return _rtw_write32x(adapter, addr, val);
}
int dbg_rtw_writeNx(_adapter *adapter, u32 addr , u32 length , u8 *data, const char *caller, const int line)
{
	if (match_write_sniff(adapter, addr, length, 0)) {
		RTW_INFO("DBG_IO %s:%d rtw_writeNx(0x%04x, %u)\n"
			, caller, line, addr, length);
	}

	return _rtw_writeNx(adapter, addr, length, data);
}
