...
#define rtw_phydm_func_disable_all(adapter)	\
		rtw_phydm_ability_opsx(adapter, HAL_PHYDM_DIS_ALL_FUNC, 0)

#ifdef CONFIG_RTW_ACS
#define rtw_phydm_func_for_offchannel(adapter) \
		do { \
			rtw_phydm_ability_opsx(adapter, HAL_PHYDM_DIS_ALL_FUNC, 0); \
			if (rtw_odm_adaptivity_neededx(adapter)) \
				rtw_phydm_ability_opsx(adapter, HAL_PHYDM_FUNC_SET, ODM_BB_ADAPTIVITY); \
			if (IS_ACS_ENABLE(adapter))\
				rtw_phydm_ability_opsx(adapter, HAL_PHYDM_FUNC_SET, ODM_BB_ENV_MONITOR); \
		} while (0)
#else
#define rtw_phydm_func_for_offchannel(adapter) \
		do { \
...
#define rtw_phydm_func_clr(adapter, ability)	\
		rtw_phydm_ability_opsx(adapter, HAL_PHYDM_FUNC_CLR, ability)

#define rtw_phydm_ability_backup(adapter)	\
		rtw_phydm_ability_opsx(adapter, HAL_PHYDM_ABILITY_BK, 0)

#define rtw_phydm_ability_restore(adapter)	\
		rtw_phydm_ability_opsx(adapter, HAL_PHYDM_ABILITY_RESTORE, 0)


static inline u32 rtw_phydm_ability_get(_adapter *adapter)
{
	return rtw_phydm_ability_opsx(adapter, HAL_PHYDM_ABILITY_GET, 0);
}
...
u8 rtw_odm_get_dfs_domainx(struct dvobj_priv *dvobj);
u8 rtw_odm_dfs_domain_unknownx(struct dvobj_priv *dvobj);
