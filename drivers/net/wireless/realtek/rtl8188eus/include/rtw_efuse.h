...
typedef struct _EFUSE_HAL {
	u8	fakeEfuseBankx;
	u32	fakeEfuseUsedBytesx;
	u8	fakeEfuseContentx[EFUSE_MAX_HW_SIZE];
	u8	fakeEfuseInitMapx[EFUSE_MAX_MAP_LEN];
	u8	fakeEfuseModifiedMapx[EFUSE_MAX_MAP_LEN];
	u32	EfuseUsedBytes;
	u8	EfuseUsedPercentage;

	u16	BTEfuseUsedBytesx;
	u8	BTEfuseUsedPercentage;
	u8	BTEfuseContentx[EFUSE_MAX_BT_BANK][EFUSE_MAX_HW_SIZE];
	u8	BTEfuseInitMapx[EFUSE_BT_MAX_MAP_LEN];
	u8	BTEfuseModifiedMapx[EFUSE_BT_MAX_MAP_LEN];

	u16	fakeBTEfuseUsedBytesxx;
	u8	fakeBTEfuseContentxx[EFUSE_MAX_BT_BANK][EFUSE_MAX_HW_SIZE];
	u8	fakeBTEfuseInitMapxx[EFUSE_BT_MAX_MAP_LEN];
	u8	fakeBTEfuseModifiedMapxx[EFUSE_BT_MAX_MAP_LEN];
...
extern u8 maskfileBufferx[64];

/*------------------------Export global variable----------------------------*/
extern u8 fakeEfuseBankx;
extern u32 fakeEfuseUsedBytesx;
extern u8 fakeEfuseContentx[];
extern u8 fakeEfuseInitMapx[];
extern u8 fakeEfuseModifiedMapx[];

extern u32 BTEfuseUsedBytesx;
extern u8 BTEfuseContentx[EFUSE_MAX_BT_BANK][EFUSE_MAX_HW_SIZE];
extern u8 BTEfuseInitMapx[];
extern u8 BTEfuseModifiedMapx[];

extern u32 fakeBTEfuseUsedBytesxx;
extern u8 fakeBTEfuseContentxx[EFUSE_MAX_BT_BANK][EFUSE_MAX_HW_SIZE];
extern u8 fakeBTEfuseInitMapxx[];
extern u8 fakeBTEfuseModifiedMapxx[];
/*------------------------Export global variable----------------------------*/
...
u8	rtw_efuse_accessx(PADAPTER padapter, u8 bRead, u16 start_addr, u16 cnts, u8 *data);
...
u16	Efuse_GetCurrentSizex(PADAPTER pAdapter, u8 efuseType, BOOLEAN bPseudoTest);
u8	Efuse_CalculateWordCntsx(u8 word_en);
void	ReadEFuseBytex(PADAPTER Adapter, u16 _offset, u8 *pbuf, BOOLEAN bPseudoTest) ;
void	EFUSE_GetEfuseDefinitionx(PADAPTER pAdapter, u8 efuseType, u8 type, void *pOut, BOOLEAN bPseudoTest);
u8	efuse_OneByteReadx(PADAPTER pAdapter, u16 addr, u8 *data, BOOLEAN	 bPseudoTest);
#define efuse_onebyte_read(adapter, addr, data, pseudo_test) efuse_OneByteReadx((adapter), (addr), (data), (pseudo_test))

u8	efuse_OneByteWritex(PADAPTER pAdapter, u16 addr, u8 data, BOOLEAN	 bPseudoTest);

void	BTEfuse_PowerSwitchxx(PADAPTER pAdapter, u8	bWrite, u8	 PwrState);
void	Efuse_PowerSwitchx(PADAPTER pAdapter, u8	bWrite, u8	 PwrState);
int	Efuse_PgPacketReadx(PADAPTER pAdapter, u8 offset, u8 *data, BOOLEAN bPseudoTest);
int	Efuse_PgPacketWritex(PADAPTER pAdapter, u8 offset, u8 word_en, u8 *data, BOOLEAN bPseudoTest);
void	efuse_WordEnableDataReadx(u8 word_en, u8 *sourdata, u8 *targetdata);
u8	Efuse_WordEnableDataWritex(PADAPTER pAdapter, u16 efuse_addr, u8 word_en, u8 *data, BOOLEAN bPseudoTest);
void	EFUSE_ShadowMapUpdatex(PADAPTER pAdapter, u8 efuseType, BOOLEAN bPseudoTest);
void	EFUSE_ShadowReadx(PADAPTER pAdapter, u8 Type, u16 Offset, u32 *Value);
#define efuse_logical_map_read(adapter, type, offset, value) EFUSE_ShadowReadx((adapter), (type), (offset), (value))
...
extern const u8 _mac_hidden_max_bw_to_hal_bw_capx[];
#define mac_hidden_max_bw_to_hal_bw_capx(max_bw) (((max_bw) >= MAC_HIDDEN_MAX_BW_NUM) ? 0 : _mac_hidden_max_bw_to_hal_bw_capx[(max_bw)])

#define MAC_HIDDEN_PROTOCOL_NUM 4
extern const u8 _mac_hidden_proto_to_hal_proto_capx[];
#define mac_hidden_proto_to_hal_proto_capx(proto) (((proto) >= MAC_HIDDEN_PROTOCOL_NUM) ? 0 : _mac_hidden_proto_to_hal_proto_capx[(proto)])
