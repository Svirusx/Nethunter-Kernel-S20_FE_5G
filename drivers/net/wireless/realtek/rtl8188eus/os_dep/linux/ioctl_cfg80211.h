...
#define rtw_wdev_invit_info_init(invit_info) \
	do { \
		...
		_rtw_memsetx((invit_info)->peer_mac, 0, ETH_ALEN); \
		_rtw_memsetx((invit_info)->group_bssid, 0, ETH_ALEN); \
...
#define rtw_wdev_nego_info_init(nego_info) \
	do { \
		...
		_rtw_memsetx((nego_info)->iface_addr, 0, ETH_ALEN); \
		_rtw_memsetx((nego_info)->peer_mac, 0, ETH_ALEN); \
		_rtw_memsetx((nego_info)->peer_iface_addr, 0, ETH_ALEN); \
...
struct rtw_wdev_priv {
	...
	char ifnamex_mon[IFNAMSIZ + 1]; /* interface name for monitor interface */
...
#define rtw_wdev_freex_connect_req(rtw_wdev_data) \
	do { \
		if ((rtw_wdev_data)->connect_req) { \
			rtw_mfreex((u8 *)(rtw_wdev_data)->connect_req, sizeof(*(rtw_wdev_data)->connect_req)); \
...
void rtw_cfg80211_surveydone_event_callbackx(_adapter *padapter);
...
#ifdef CONFIG_P2P
void rtw_cfg80211_set_is_rochx(_adapter *adapter, bool val);
bool rtw_cfg80211_get_is_rochx(_adapter *adapter);
bool rtw_cfg80211_is_ro_ch_oncex(_adapter *adapter);
void rtw_cfg80211_set_last_ro_ch_timex(_adapter *adapter);
s32 rtw_cfg80211_get_last_ro_ch_passing_msx(_adapter *adapter);

int rtw_cfg80211_iface_has_p2p_group_capx(_adapter *adapter);
int rtw_cfg80211_is_p2p_scanx(_adapter *adapter);
#if defined(RTW_DEDICATED_P2P_DEVICE)
...
void rtw_cfg80211_set_is_mgmt_txx(_adapter *adapter, u8 val);
u8 rtw_cfg80211_get_is_mgmt_txx(_adapter *adapter);
...
void rtw_cfg80211_rx_p2p_action_publicx(_adapter *padapter, union recv_frame *rframe);
void rtw_cfg80211_rx_actionx_p2p(_adapter *padapter, union recv_frame *rframe);
...
void rtw_cfg80211_rx_probe_requestx(_adapter *padapter, union recv_frame *rframe);
...
int rtw_cfg80211_set_mgnt_wpsp2piex(struct net_device *net, char *buf, int len, int type);

bool rtw_cfg80211_pwr_mgmtx(_adapter *adapter);
