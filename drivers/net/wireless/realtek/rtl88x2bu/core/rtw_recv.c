...
u8 signal_stat_calc_profilebu[SIGNAL_STAT_CALC_PROFILE_MAX][3] = {
...
u8 rtw_bridge_tunnel_headerbu[] = { 0xaa, 0xaa, 0x03, 0x00, 0x00, 0xf8 };
u8 rtw_rfc1042_headerbu[] = { 0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00 };
...
void _rtw_init_sta_recv_privbu(struct sta_recv_priv *psta_recvpriv)
{



	_rtw_memsetbu((u8 *)psta_recvpriv, 0, sizeof(struct sta_recv_priv));

	_rtw_spinlockbu_initbu(&psta_recvpriv->lock);

	/* for(i=0; i<MAX_RX_NUMBLKS; i++) */
	/*	_rtw_init_queuebu(&psta_recvpriv->blk_strms[i]); */

	_rtw_init_queuebu(&psta_recvpriv->defrag_q);


}

sint _rtw_init_recv_privbu(struct recv_priv *precvpriv, _adapter *padapter)
{
	sint i;
	...
	/* _rtw_memsetbu((unsigned char *)precvpriv, 0, sizeof (struct  recv_priv)); */

	_rtw_spinlockbu_initbu(&precvpriv->lock);

#ifdef CONFIG_RECV_THREAD_MODE
	_rtw_init_semabu(&precvpriv->recv_sema, 0);

#endif

	_rtw_init_queuebu(&precvpriv->free_recv_queue);
	_rtw_init_queuebu(&precvpriv->recv_pending_queue);
	_rtw_init_queuebu(&precvpriv->uc_swdec_pending_queue);
	...
	rtw_os_recv_resource_initbu(precvpriv, padapter);
	...
	/* _rtw_memsetbu(precvpriv->pallocated_frame_buf, 0, NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ); */
	...
	for (i = 0; i < NR_RECVFRAME ; i++) {
		_rtw_init_listheadbu(&(precvframe->u.list));

		rtw_list_insert_tailbu(&(precvframe->u.list), &(precvpriv->free_recv_queue.queue));

		rtw_os_recv_resource_allocbu(padapter, precvframe);
	...
	ATOMIC_SETbu(&(precvpriv->rx_pending_cnt), 1);

	_rtw_init_semabu(&precvpriv->allrxreturnevt, 0);

#endif

	res = rtw_hal_initbu_recv_priv(padapter);

#ifdef CONFIG_NEW_SIGNAL_STAT_PROCESS
	rtw_init_timerbu(&precvpriv->signal_stat_timer, padapter, rtw_signal_stat_timer_hdl, padapter);
...
void rtw_mfree_recv_priv_lockbu(struct recv_priv *precvpriv);
void rtw_mfree_recv_priv_lockbu(struct recv_priv *precvpriv)
{
	_rtw_spinlockbu_freebu(&precvpriv->lock);
#ifdef CONFIG_RECV_THREAD_MODE
	_rtw_free_semabu(&precvpriv->recv_sema);
#endif

	_rtw_spinlockbu_freebu(&precvpriv->free_recv_queue.lock);
	_rtw_spinlockbu_freebu(&precvpriv->recv_pending_queue.lock);

	_rtw_spinlockbu_freebu(&precvpriv->free_recv_buf_queue.lock);

#ifdef CONFIG_USE_USB_BUFFER_ALLOC_RX
	_rtw_spinlockbu_freebu(&precvpriv->recv_buf_pending_queue.lock);
#endif /* CONFIG_USE_USB_BUFFER_ALLOC_RX */
}

void _rtw_free_recv_privbu(struct recv_priv *precvpriv)
{
	...
	rtw_mfree_recv_priv_lockbu(precvpriv);

	rtw_os_recv_resource_freebu(precvpriv);

	if (precvpriv->pallocated_frame_buf)
		rtw_vmfree(precvpriv->pallocated_frame_buf, NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ);

	rtw_hal_free_recv_privbu(padapter);


}

bool rtw_rframe_del_wfd_iebu(union recv_frame *rframe, u8 ies_offset)
{
#define DBG_RFRAME_DEL_WFD_IE 0
	...
	ies_len = rtw_del_wfd_iebu(ies, ies_len_ori, DBG_RFRAME_DEL_WFD_IE ? __func__ : NULL);
	rframe->u.hdr.len -= ies_len_ori - ies_len;

	return ies_len_ori != ies_len;
}

union recv_frame *_rtw_alloc_recvframebubu(_queue *pfree_recv_queue)
{
	...
	if (_rtw_queue_emptybu(pfree_recv_queue) == _TRUE)
		precvframe = NULL;
	else {
...
union recv_frame *rtw_alloc_recvframebu(_queue *pfree_recv_queue)
{
	_irqL irqL;
	union recv_frame  *precvframe;
	...
	precvframe = _rtw_alloc_recvframebubu(pfree_recv_queue);
...
void rtw_init_recvframebu(union recv_frame *precvframe, struct recv_priv *precvpriv)
{
...
int rtw_free_recvframebu(union recv_frame *precvframe, _queue *pfree_recv_queue)
{
	_irqL irqL;
	...
	rtw_os_free_recvframebu(precvframe);
	...
	rtw_list_insert_tailbu(&(precvframe->u.hdr.list), get_list_head(pfree_recv_queue));
...
sint _rtw_enqueue_recvframebubu(union recv_frame *precvframe, _queue *queue)
{
	...
	rtw_list_insert_tailbu(&(precvframe->u.hdr.list), get_list_head(queue));
...
sint rtw_enqueue_recvframebu(union recv_frame *precvframe, _queue *queue)
{
	sint ret;
	_irqL irqL;
	...
	ret = _rtw_enqueue_recvframebubu(precvframe, queue);
	/* _rtw_spinunlockbu(&pfree_recv_queue->lock); */
...
sint	rtw_enqueue_recvframebu(union recv_frame *precvframe, _queue *queue)
{
	return rtw_free_recvframebu(precvframe, queue);
}
*/
...
/*
caller : defrag ; recvframe_chk_defragbu in recv_thread  (passive)
...
void rtw_free_recvframebu_queue(_queue *pframequeue,  _queue *pfree_recv_queue)
{
	union	recv_frame	*precvframe;
	_list	*plist, *phead;

	_rtw_spinlockbu(&pframequeue->lock);
	...
	while (rtw_end_of_queue_searchbu(phead, plist) == _FALSE) {
		...
		/* rtw_list_delete(&precvframe->u.hdr.list); */ /* will do this in rtw_free_recvframebu() */

		rtw_free_recvframebu(precvframe, pfree_recv_queue);
	}

	_rtw_spinunlockbu(&pframequeue->lock);


}

u32 rtw_free_uc_swdec_pending_queuebu(_adapter *adapter)
{
	u32 cnt = 0;
	union recv_frame *pending_frame;
	while ((pending_frame = rtw_alloc_recvframebu(&adapter->recvpriv.uc_swdec_pending_queue))) {
		rtw_free_recvframebu(pending_frame, &adapter->recvpriv.free_recv_queue);
		cnt++;
	}
...
sint rtw_enqueue_recvbufbu_to_headbu(struct recv_buf *precvbuf, _queue *queue)
{
	_irqL irqL;
	...
	rtw_list_insert_headbu(&precvbuf->list, get_list_head(queue));
...
sint rtw_enqueue_recvbufbu(struct recv_buf *precvbuf, _queue *queue)
{
	_irqL irqL;
	...
	rtw_list_insert_tailbu(&precvbuf->list, get_list_head(queue));
...
struct recv_buf *rtw_dequeue_recvbufbu(_queue *queue)
{
	_irqL irqL;
...
sint recvframe_chkmicbu(_adapter *adapter,  union recv_frame *precvframe);
sint recvframe_chkmicbu(_adapter *adapter,  union recv_frame *precvframe)
{
	...
	stainfo = rtw_get_stainfobu(&adapter->stapriv , &prxattrib->ta[0]);
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		if (stainfo != NULL) {
			if (IS_MCAST(prxattrib->ra)) {
				...
				/* RTW_INFO("\n recvframe_chkmicbu: bcmc key psecuritypriv->dot118021XGrpKeyid(%d),pmlmeinfo->key_index(%d) ,recv key_id(%d)\n", */
				/*								psecuritypriv->dot118021XGrpKeyid,pmlmeinfo->key_index,rxdata_key_idx); */

				if (psecuritypriv->binstallGrpkey == _FALSE) {
					res = _FAIL;
					RTW_INFO("\n recvframe_chkmicbu:didn't install group key!!!!!!!!!!\n");
					goto exit;
				}
			} else {
			...
			/* rtw_seccalctkipmicbu(&stainfo->dot11tkiprxmickey.skey[0],pframe,payload, datalen ,&miccode[0],(unsigned char)prxattrib->priority); */ /* care the length of the data */

			rtw_seccalctkipmicbu(mickey, pframe, payload, datalen , &miccode[0], (unsigned char)prxattrib->priority); /* care the length of the data */
			...
			if (bmic_err == _TRUE) {
				...
				if ((prxattrib->bdecrypted == _TRUE) && (brpt_micerror == _TRUE)) {
					rtw_handle_tkip_mic_errbu(adapter, stainfo, (u8)IS_MCAST(prxattrib->ra));
...
union recv_frame *decryptorbu(_adapter *padapter, union recv_frame *precv_frame);
union recv_frame *decryptorbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (prxattrib->encrypt && !prxattrib->bdecrypted) {
		...
		RTW_INFO(ADPT_FMT" - sec_type:%s DO SW decryption\n",
			ADPT_ARG(padapter), security_type_strbu(prxattrib->encrypt));
#endif
		...
		case _TKIP_:
			...
			res = rtw_tkip_decryptbu(padapter, (u8 *)precv_frame);
			break;
		case _AES_:
		case _CCMP_256_:
			DBG_COUNTER(padapter->rx_logs.core_rx_post_decrypt_aes);
			res = rtw_aes_decryptbu(padapter, (u8 *)precv_frame);
			break;
		case _GCMP_:
		case _GCMP_256_:
		...
		if ((prxstat->icv == 1) && (prxattrib->encrypt != _AES_)) {
			...
			rtw_free_recvframebu(precv_frame, &padapter->recvpriv.free_recv_queue);
	...
	if (res == _FAIL) {
		rtw_free_recvframebu(return_packet, &padapter->recvpriv.free_recv_queue);
		return_packet = NULL;
	} else
		prxattrib->bdecrypted = _TRUE;
	/* recvframe_chkmicbu(adapter, precv_frame);   */ /* move to recvframme_defrag function */
...
union recv_frame *portctrlbu(_adapter *adapter, union recv_frame *precv_frame);
union recv_frame *portctrlbu(_adapter *adapter, union recv_frame *precv_frame)
{
	u8 *psta_addr = NULL;
	u8 *ptr;
	...
	psta = rtw_get_stainfobu(pstapriv, psta_addr);
	...
	if (auth_alg == dot11AuthAlgrthm_8021X) {
		if ((psta != NULL) && (psta->ieee8021x_blocked)) {
			...
			_rtw_memcpybu(&ether_type, ptr, 2);
			...
			if (ether_type == eapol_type)
				...
				/* free this frame */
				rtw_free_recvframebu(precv_frame, &adapter->recvpriv.free_recv_queue);
				prtnframe = NULL;
			}
		} else {
			/* allowed */
...
sint recv_ucast_pn_decachebu(union recv_frame *precv_frame);
sint recv_ucast_pn_decachebu(union recv_frame *precv_frame)
{
...
sint recv_bcast_pn_decachebu(union recv_frame *precv_frame);
sint recv_bcast_pn_decachebu(union recv_frame *precv_frame)
{
...
sint recv_decachebu(union recv_frame *precv_frame)
{
	...
	if (seq_ctrl == *prxseq) {
		...
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_decachebu _FAIL for sta="MAC_FMT"\n"
...
void process_pwrbit_databu(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	if (pwrbit) {
		if (!(psta->state & WIFI_SLEEP_STATE)) {
			...
			/* rtw_tim_map_setbu(padapter, pstapriv->sta_dz_bitmap, BIT(psta->cmn.aid)); */

			stop_sta_xmitbu(padapter, psta);
			/* RTW_INFO_DUMP("to sleep, sta_dz_bitmap=", pstapriv->sta_dz_bitmap, pstapriv->aid_bmp_len); */
		}
	} else {
		if (psta->state & WIFI_SLEEP_STATE) {
			/* psta->state ^= WIFI_SLEEP_STATE; */
			/* rtw_tim_map_clearbu(padapter, pstapriv->sta_dz_bitmap, BIT(psta->cmn.aid)); */
...
void process_wmmps_databu(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
#ifdef CONFIG_AP_MODE
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	...
	if (psta->state & WIFI_SLEEP_STATE) {
		...
		if (wmmps_ac) {
			if (psta->sleepq_ac_len > 0) {
				...
				xmit_delivery_enabled_framesbu(padapter, psta);
			} else {
				/* issue one qos null frame with More data bit = 0 and the EOSP bit set (=1) */
				issue_qos_nulldatabu(padapter, psta->cmn.mac_addr, (u16)pattrib->priority, 0, 0, 0);
			}
		}
...
sint OnTDLS(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (hal_chk_wl_funcbu(adapter, WL_FUNC_TDLS) == _FALSE) {
	...
	ptdls_sta = rtw_get_stainfobu(pstapriv, get_sa(ptr));
	if (ptdls_sta == NULL) {
		switch (*paction) {
	...
	case 0x50:
		if (_rtw_memcmpbu(WFA_OUI, paction, 3)) {
			/* Probe request frame */
			if (*(paction + 3) == 0x04) {
...
)
{
	...
	*psta = ptdls_sta = rtw_get_stainfobu(pstapriv, pattrib->ta);
	if (ptdls_sta == NULL) {
		ret = _FAIL;
		goto exit;
	} else if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE) {
		/* filter packets that SA is myself or multicast or broadcast */
		if (_rtw_memcmpbu(myhwaddr, pattrib->src, ETH_ALEN)) {
			ret = _FAIL;
			goto exit;
		}
		/* da should be for me */
		if ((!_rtw_memcmpbu(myhwaddr, pattrib->dst, ETH_ALEN)) && (!bmcast)) {
			ret = _FAIL;
			goto exit;
		}
		/* check BSSID */
		if (_rtw_memcmpbu(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		    _rtw_memcmpbu(mybssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		    (!_rtw_memcmpbu(pattrib->bssid, mybssid, ETH_ALEN))) {
			ret = _FAIL;
			goto exit;
		}

#ifdef CONFIG_TDLS_CH_SW
		if (ATOMIC_READbu(&pchsw_info->chsw_on) == _TRUE) {
			if (adapter->mlmeextpriv.cur_channel != rtw_get_oper_chbu(adapter)) {
		...
		process_pwrbit_databu(adapter, precv_frame, ptdls_sta);
		...
		if (_rtw_memcmpbu(psnap_type, SNAP_ETH_TYPE_TDLS, 2)) {
			ret = OnTDLS(adapter, precv_frame);
			goto exit;
		}

		if ((get_frame_sub_type(ptr) & WIFI_QOS_DATA_TYPE) == WIFI_QOS_DATA_TYPE)
			process_wmmps_databu(adapter, precv_frame, ptdls_sta);
...
void count_rx_statsbu(_adapter *padapter, union recv_frame *prframe, struct sta_info *sta)
{
	int	sz;
...
int rtw_sta_rx_data_validate_hdr(_adapter *adapter, union recv_frame *rframe, struct sta_info **sta)
{
	...
	if (rattrib->to_fr_ds == 0) {
		...
		_rtw_memcpybu(rattrib->dst, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->src, get_addr2_ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->bssid, GetAddr3Ptr(whdr), ETH_ALEN);
		...
		#endif
		{
			...
			if (!_rtw_memcmpbu(rattrib->bssid, rattrib->src, ETH_ALEN))
				goto exit;

			*sta = rtw_get_stainfobu(stapriv, get_addr2_ptr(whdr));
			if (*sta)
				ret = _SUCCESS;
		}
		goto exit;
	...
	if (!(MLME_STATE(adapter) & (WIFI_ASOC_STATE | WIFI_UNDER_LINKING))) {
		if (!is_ra_bmc) {
			...
			static systime send_issue_deauthbu_time = 0;

			/* RTW_INFO("After send deauth , %u ms has elapsed.\n", rtw_get_passing_time_ms(send_issue_deauthbu_time)); */
			if (rtw_get_passing_time_ms(send_issue_deauthbu_time) > 10000 || send_issue_deauthbu_time == 0) {
				send_issue_deauthbu_time = rtw_get_current_time();
				RTW_INFO(FUNC_ADPT_FMT" issue_deauthbu to "MAC_FMT" with reason(7), mlme_state:0x%x\n"
					, FUNC_ADPT_ARG(adapter), MAC_ARG(get_addr2_ptr(whdr)), MLME_STATE(adapter));
				issue_deauthbu(adapter, get_addr2_ptr(whdr), WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
			}
		}
		#ifdef DBG_RX_DROP_FRAME
	...
	case 2:
		_rtw_memcpybu(rattrib->dst, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->src, GetAddr3Ptr(whdr), ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpybu(rattrib->bssid, get_addr2_ptr(whdr), ETH_ALEN);
		break;
	case 3:
	...
	if (!rattrib->amsdu && _rtw_memcmpbu(myhwaddr, rattrib->src, ETH_ALEN)) {
		#ifdef DBG_RX_DROP_FRAME
...
)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) ||
		...
		_rtw_memcpybu(pattrib->dst, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->bssid, GetAddr3Ptr(ptr), ETH_ALEN);
	...
	*psta = rtw_get_stainfobu(pstapriv, sta_addr);
	if (*psta == NULL) {
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE) &&
		...
		*psta = rtw_get_stainfobu(pstapriv, pattrib->bssid); /* get sta_info */
		if (*psta == NULL) {
			#ifdef DBG_RX_DROP_FRAME
		...
		if (_rtw_memcmpbu(myhwaddr, pattrib->dst, ETH_ALEN) && (!bmcast)) {
			*psta = rtw_get_stainfobu(pstapriv, pattrib->ta);
			if (*psta == NULL) {
				...
				if (rtw_get_passing_time_ms(send_issue_deauthbu_time) > 10000 || send_issue_deauthbu_time == 0) {
					...
					RTW_INFO("issue_deauthbu to the ap=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->bssid));

					issue_deauthbu(adapter, pattrib->bssid, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
				}
			}
		}
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE) &&
		...
		*psta = rtw_get_stainfobu(pstapriv, pattrib->bssid); /* get sta_info */
		...
		u8 *myhwaddr = adapter_mac_addr(adapter);
		if (!_rtw_memcmpbu(pattrib->ra, myhwaddr, ETH_ALEN)) {
			ret = RTW_RX_HANDLED;
			goto exit;
		}
#ifndef CONFIG_CUSTOMER_ALIBABA_GENERAL
		RTW_INFO("issue_deauthbu to sta=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->src));
		issue_deauthbu(adapter, pattrib->src, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
#endif
		ret = RTW_RX_HANDLED;
		goto exit;
	}
...
sint validate_recv_ctrl_framebu(_adapter *padapter, union recv_frame *precv_frame);
sint validate_recv_ctrl_framebu(_adapter *padapter, union recv_frame *precv_frame)
{
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	...
	/* RTW_INFO("+validate_recv_ctrl_framebu\n"); */
	...
	if (get_frame_sub_type(pframe) == WIFI_PSPOLL) {
		...
		if ((psta->state & WIFI_SLEEP_STATE) && (rtw_tim_map_is_setbu(padapter, pstapriv->sta_dz_bitmap, psta->cmn.aid))) {
			_irqL irqL;
			...
			if ((rtw_end_of_queue_searchbu(xmitframe_phead, xmitframe_plist)) == _FALSE) {
				...
				if (rtw_tim_map_is_setbu(padapter, pstapriv->tim_bitmap, psta->cmn.aid)) {
					if (psta->sleepq_len == 0) {
						...
						issue_nulldatabu(padapter, psta->cmn.mac_addr, 0, 0, 0);
					} else {
		...
		rtw_process_bar_framebu(padapter, precv_frame);
	}
			...
			if (subtype == WIFI_DEAUTH || subtype == WIFI_DISASSOC) {
				if (!MLME_IS_MESH(adapter)) {
					...
					if (reason == 6 || reason == 7) {
						...
						issue_action_SA_Querybu(adapter, psta->cmn.mac_addr, 0, 0, IEEE80211W_RIGHT_KEY);
					}
				}
				goto fail;
			}
...
bip_verify:
	...
	_rtw_memcpybu(pattrib->ra, GetAddr1Ptr(ptr), ETH_ALEN);
	_rtw_memcpybu(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
	...
	precv_frame = decryptorbu(adapter, precv_frame);
	/* save actual management data frame body */
	_rtw_memcpybu(mgmt_DATA, ptr + pattrib->hdrlen + pattrib->iv_len, data_len);
	/* overwrite the iv field */
	_rtw_memcpybu(ptr + pattrib->hdrlen, mgmt_DATA, data_len);
...
union recv_frame *recvframe_chk_defragbu(PADAPTER padapter, union recv_frame *precv_frame);

sint validate_recv_mgnt_framebu(PADAPTER padapter, union recv_frame *precv_frame)
{
	struct sta_info *psta = precv_frame->u.hdr.psta
		= rtw_get_stainfobu(&padapter->stapriv, get_addr2_ptr(precv_frame->u.hdr.rx_data));
	...
	precv_frame = recvframe_chk_defragbu(padapter, precv_frame);
	if (precv_frame == NULL)
		return _SUCCESS;
	...
	if (psta) {
		...
		if (get_frame_sub_type(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
			...
			if (_rtw_memcmpbu(adapter_mac_addr(padapter), GetAddr1Ptr(precv_frame->u.hdr.rx_data), ETH_ALEN) == _TRUE)
	...
	mgt_dispatcherbu(padapter, precv_frame);
...
sint validate_recv_data_framebu(_adapter *adapter, union recv_frame *precv_frame)
{
	u8 bretry, a4_shift;
	struct sta_info *psta = NULL;
	u8 *ptr = precv_frame->u.hdr.rx_data;
	...
	case 0:
		...
		_rtw_memcpybu(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->bssid, GetAddr3Ptr(ptr), ETH_ALEN);
		ret = sta2sta_data_framebu(adapter, precv_frame, &psta);
		break;

	case 1:
		_rtw_memcpybu(pattrib->ra, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->dst, GetAddr3Ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->bssid, GetAddr1Ptr(ptr), ETH_ALEN);
		ret = sta2ap_data_framebu(adapter, precv_frame, &psta);
		break;
	...
	case 2:
		...
		_rtw_memcpybu(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->dst, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->src, GetAddr3Ptr(ptr), ETH_ALEN);
		_rtw_memcpybu(pattrib->bssid, get_addr2_ptr(ptr), ETH_ALEN);
		ret = ap2sta_data_framebu(adapter, precv_frame, &psta);
		break;
...
pre_validate_status_chk:
	...
	ret = recv_decachebu(precv_frame);
	if (ret  == _FAIL)
		goto exit;
	...
	if (!IS_MCAST(pattrib->ra)) {
		...
		if (recv_ucast_pn_decachebu(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_ucast_pn_decachebu return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		...
		if (recv_bcast_pn_decachebu(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_bcast_pn_decachebu return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
	...
	if (psecuritypriv->dot11PrivacyAlgrthm != _NO_PRIVACY_ ) {
		if (IS_MCAST(pattrib->ra)) {
			...
			_rtw_memcpybu(&ether_type, ether_ptr, 2);
			ether_type = ntohs((unsigned short)ether_type);
			...
			if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
				...
				if ((ether_type == eapol_type)
					...
					&& (!_rtw_memcmpbu( adapter_mac_addr(adapter), pattrib->dst, ETH_ALEN))) {
					#ifdef DBG_RX_DROP_FRAME
...
sint validate_recv_framebu(_adapter *adapter, union recv_frame *precv_frame);
sint validate_recv_framebu(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_PROCESS) {
		int ch_set_idx = rtw_chset_search_chbu(rfctl->channel_set, rtw_get_oper_chbu(adapter));
		if (ch_set_idx >= 0)
...
#if 1 /* Dump rx packets */
	{
		...
		rtw_hal_get_def_varbu(adapter, HAL_DEF_DBG_DUMP_RXPKT, &(bDumpRxPkt));
	...
	case WIFI_MGT_TYPE: /* mgnt */
		...
		retval = validate_recv_mgnt_framebu(adapter, precv_frame);
		if (retval == _FAIL) {
	...
	case WIFI_CTRL_TYPE: /* ctrl */
		...
		retval = validate_recv_ctrl_framebu(adapter, precv_frame);
		if (retval == _FAIL) {
	...
	case WIFI_DATA_TYPE: /* data */
		...
		retval = validate_recv_data_framebu(adapter, precv_frame);
		if (retval == _FAIL) {
			...
			if (!pattrib->encrypt || pattrib->bdecrypted) {
				...
				rtw_hal_get_def_varbu(adapter, HAL_DEF_DBG_DUMP_RXPKT, &(bDumpRxPkt));
				/* get ether_type */
				_rtw_memcpybu(&eth_type, ptr + pattrib->hdrlen + pattrib->iv_len + RATTRIB_GET_MCTRL_LEN(pattrib) + LLC_HEADER_SIZE, 2);
...
enum rtw_rx_llc_hdl rtw_recv_llc_parse(u8 *msdu, u16 msdu_len)
{
	...
	if ((_rtw_memcmpbu(msdu, rtw_rfc1042_headerbu, SNAP_SIZE)
			&& eth_type != ETH_P_AARP && eth_type != ETH_P_IPX)
		|| _rtw_memcmpbu(msdu, rtw_bridge_tunnel_headerbu, SNAP_SIZE)) {
...
sint wlanhdr_to_ethhdrbu(union recv_frame *precvframe, enum rtw_rx_llc_hdl llc_hdl)
{
	...
	_rtw_memcpybu(ptr, pattrib->dst, ETH_ALEN);
	_rtw_memcpybu(ptr + ETH_ALEN, pattrib->src, ETH_ALEN);

	if (!llc_hdl) {
		len = htons(len);
		_rtw_memcpybu(ptr + 12, &len, 2);
	}

	rtw_rframe_set_os_pktbu(precvframe);
	...
	union recv_frame *prframe)
{
	...
	*	alloc_sz = _RND(1536 + RXDESC_SIZE + pfhdr->attrib.drvinfosize + shift_sz + 8, 128); */
	...
	if (ptr)
		_rtw_memcpybu(ptr, pfhdr->rx_data, pfhdr->len);
...
/* perform defrag */
union recv_frame *recvframe_defragbu(_adapter *adapter, _queue *defrag_q);
union recv_frame *recvframe_defragbu(_adapter *adapter, _queue *defrag_q)
{
	_list	*plist, *phead;
	...
	if (curfragnum != pfhdr->attrib.frag_num) {
		...
		rtw_free_recvframebu(prframe, pfree_recv_queue);
		rtw_free_recvframebu_queue(defrag_q, pfree_recv_queue);
	...
	while (rtw_end_of_queue_searchbu(phead, plist) == _FALSE) {
		...
		/* memcpy */
		_rtw_memcpybu(pfhdr->rx_tail, pnfhdr->rx_data, pnfhdr->len);
...
union recv_frame *recvframe_chk_defragbu(PADAPTER padapter, union recv_frame *precv_frame)
{
	...
	if ((ismfrag == 0) && (fragnum == 0)) {
		...
		if (IS_MCAST(pfhdr->attrib.ra)) {
			...
			rtw_free_recvframebu(precv_frame, pfree_recv_queue);
			return NULL;
		}
		/* CVE-2020-24587 */
		if ((psta) && (pdefrag_q))
			precv_frame->u.hdr.keytrack = ATOMIC_READbu(&psta->keytrack);
	}
	...
	if (ismfrag == 1) {
		...
		if (pdefrag_q != NULL) {
			if (fragnum == 0) {
				/* the first fragment */
				if (_rtw_queue_emptybu(pdefrag_q) == _FALSE) {
					/* free current defrag_q */
					rtw_free_recvframebu_queue(pdefrag_q, pfree_recv_queue);
				}
			}
			...
			/* _rtw_spinlockbu(&pdefrag_q->lock); */
			phead = get_list_head(pdefrag_q);
			rtw_list_insert_tailbu(&pfhdr->list, phead);
			/* _rtw_spinunlockbu(&pdefrag_q->lock); */
	...
	if ((ismfrag == 0) && (fragnum != 0)) {
		...
		if (pdefrag_q != NULL) {
			...
			/* call recvframe_defragbu to defrag */
			precv_frame = recvframe_defragbu(padapter, pdefrag_q);
			prtnframe = precv_frame;
	...
	if ((prtnframe != NULL) && (prtnframe->u.hdr.attrib.privacy)) {
		...
		if (recvframe_chkmicbu(padapter,  prtnframe) == _FAIL) {
			rtw_free_recvframebu(prtnframe, pfree_recv_queue);
			prtnframe = NULL;
		}
	}
...
static int rtw_recv_indicatepktbu_check(union recv_frame *rframe, u8 *ehdr_pos, u32 pkt_len)
{
	_adapter *adapter = rframe->u.hdr.adapter;
	...
	if (rframe->u.hdr.psta)
		rtw_st_ctl_rxbu(rframe->u.hdr.psta, ehdr_pos);

	if (ntohs(ehdr->h_proto) == 0x888e) {
		eapol_type = parsing_eapol_packetbu(adapter, ehdr_pos + ETH_HLEN, rframe->u.hdr.psta, 0);
		...
		dump_arp_pktbu(RTW_DBGDUMP, ehdr->h_dest, ehdr->h_source, ehdr_pos + ETH_HLEN, 0);
#endif

	if (recvpriv->sink_udpport > 0)
		rtw_sink_rtp_seq_dbgbu(adapter, ehdr_pos);
...
static void recv_free_fwd_resource(_adapter *adapter, struct xmit_frame *fwd_frame, _list *b2u_list)
{
	...
	if (fwd_frame)
		rtw_free_xmitframebu(xmitpriv, fwd_frame);
	...
	if (!rtw_is_list_emptybu(b2u_list)) {
		...
		while (rtw_end_of_queue_searchbu(b2u_list, list) == _FALSE) {
			...
			rtw_free_xmitframebu(xmitpriv, b2uframe);
		}
	}
#endif
}
	...
	, u8 act, struct xmit_frame *fwd_frame, _list *b2u_list)
{
	...
	if (act & RTW_RX_MSDU_ACT_INDICATE) {
		fwd_pkt = rtw_os_pkt_copybu(pkt);
		if (!fwd_pkt) {
			#ifdef DBG_TX_DROP_FRAME
			RTW_INFO("DBG_TX_DROP_FRAME %s rtw_os_pkt_copybu fail\n", __func__);
			#endif
	...
	if (!rtw_is_list_emptybu(b2u_list)) {
		...
		while (rtw_end_of_queue_searchbu(b2u_list, list) == _FALSE) {
			...
			if (!fwd_frame && rtw_is_list_emptybu(b2u_list)) /* the last fwd_pkt */
				b2uframe->pkt = fwd_pkt;
			else
				b2uframe->pkt = rtw_os_pkt_copybu(fwd_pkt);
			if (!b2uframe->pkt) {
				rtw_free_xmitframebu(xmitpriv, b2uframe);
				continue;
			}

			rtw_xmitbu_posthandlebu(adapter, b2uframe, b2uframe->pkt);
		}
	}
#endif
	...
	if (fwd_frame) {
		...
		if (rtw_xmitbu_posthandlebu(adapter, fwd_frame, fwd_pkt) < 0) {
			#ifdef DBG_TX_DROP_FRAME
			RTW_INFO("DBG_TX_DROP_FRAME %s rtw_xmitbu_posthandlebu fail\n", __func__);
			#endif
			xmitpriv->tx_drop++;
		}
	}
	...
	const u8 *da, const u8 *sa)
{
	...
	if (padapter->registrypriv.amsdu_mode == RTW_AMSDU_MODE_NON_SPP &&
		...
		if (pattrib->to_fr_ds == 2) {
			if (_rtw_memcmpbu(da, pattrib->ra, ETH_ALEN) == _FALSE)
			ret = _FAIL;
		}

		/* 2.check To DS */
		if (pattrib->to_fr_ds == 1) {
			if (_rtw_memcmpbu(sa, pattrib->ta, ETH_ALEN) == _FALSE)
			ret = _FAIL;
		}

		/* 3.Check whether DA is AA:AA:03:00:00:00 */
		if (_rtw_memcmpbu(da, rtw_rfc1042_headerbu, ETH_ALEN) == _TRUE)
			ret = _FAIL;
...
int amsdu_to_msdubu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		rtw_led_rx_controlbu(padapter, da);

		sub_pkt = rtw_os_alloc_msdu_pktbu(prframe, da, sa
			, pdata + ETH_HLEN + mctrl_len, nSubframe_Length - mctrl_len, llc_hdl);
		if (sub_pkt == NULL) {
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME %s rtw_os_alloc_msdu_pktbu fail\n", __func__);
				#endif
			}
			#if defined(CONFIG_AP_MODE) || defined(CONFIG_RTW_MESH)
			if (act & RTW_RX_MSDU_ACT_FORWARD) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_INFO("DBG_TX_DROP_FRAME %s rtw_os_alloc_msdu_pktbu fail\n", __func__);
				#endif
		...
		if (rtw_recv_indicatepktbu_check(prframe, rtw_os_pkt_databu(sub_pkt), rtw_os_pkt_lenbu(sub_pkt)) == _SUCCESS)
			subframes[nr_subframes++] = sub_pkt;
		else
			rtw_os_pkt_freebu(sub_pkt);
...
move_to_next:
	...
	for (i = 0; i < nr_subframes; i++) {
		...
		if (sub_pkt)
			rtw_os_recv_indicate_pktbu(padapter, sub_pkt, prframe);
	}

	prframe->u.hdr.len = 0;
	rtw_free_recvframebu(prframe, pfree_recv_queue);/* free this recv_frame */
...
static int recv_process_mpdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (pattrib->amsdu) {
		ret = amsdu_to_msdubu(padapter, prframe);
		if (ret != _SUCCESS) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" amsdu_to_msdubu fail\n"
		...
		rtw_led_rx_controlbu(padapter, pattrib->dst);

		ret = wlanhdr_to_ethhdrbu(prframe, llc_hdl);
		if (ret != _SUCCESS) {
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" wlanhdr_to_ethhdrbu: drop pkt\n"
					, FUNC_ADPT_ARG(padapter));
				#endif
			}
			#if defined(CONFIG_AP_MODE) || defined(CONFIG_RTW_MESH)
			if (act & RTW_RX_MSDU_ACT_FORWARD) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_INFO("DBG_TX_DROP_FRAME %s wlanhdr_to_ethhdrbu fail\n", __func__);
				#endif
				recv_free_fwd_resource(padapter, fwd_frame, &b2u_list);
			}
			#endif
		...
		if (!RTW_CANNOT_RUN(padapter)) {
			ret = rtw_recv_indicatepktbu_check(prframe
			...
			ret = rtw_recv_indicatepktbu(padapter, prframe);
			if (ret != _SUCCESS) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" rtw_recv_indicatepktbu fail!\n"
					, FUNC_ADPT_ARG(padapter));
				#endif
				goto exit;
			}
...
static int enqueue_reorder_recvframe(struct recv_reorder_ctrl *preorder_ctrl, union recv_frame *prframe)
{
	...
	/* _rtw_spinlockbu_ex(&ppending_recvframe_queue->lock); */
	...
	rtw_list_insert_tailbu(&(prframe->u.hdr.list), plist);

	/* _rtw_spinunlockbu_ex(&ppending_recvframe_queue->lock); */
	/* _exit_critical_ex(&ppending_recvframe_queue->lock, &irql); */
...
static int recv_indicatepkts_in_order(_adapter *padapter, struct recv_reorder_ctrl *preorder_ctrl, int bforced)
{
	...
	while (!rtw_is_list_emptybu(phead)) {
...
static int recv_indicatepkt_reorder(_adapter *padapter, union recv_frame *prframe)
{
	...
	if(rtw_test_and_clear_bitbu(RTW_RECV_ACK_OR_TIMEOUT, &preorder_ctrl->rec_abba_rsp_ack))
	...
	if (!check_indicate_seq(preorder_ctrl, pattrib->seq_num)) {
		...
		#endif
#if 0
		rtw_recv_indicatepktbu(padapter, prframe);
...
void rtw_reordering_ctrl_timeout_handlerbu(void *pcontext)
{
	_irqL irql;
	...
	/* RTW_INFO("+rtw_reordering_ctrl_timeout_handlerbu()=>\n"); */
...
int validate_mp_recv_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (pmppriv->mplink_brx == _FALSE) {
		...
		for (i = 0; i < precv_frame->u.hdr.len; i++) {
			...
			if (_rtw_memcmpbu(prx_data + i, (void *)&rtk_prefix, 11) == _FALSE) {
				bDumpRxPkt = 0;
		...
		if (bDumpRxPkt == 1) { /* dump all rx packets */
			...
			for (i = 0; i < precv_frame->u.hdr.len; i = i + 8)
				RTW_INFO("%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:\n", *(ptr + i),
					*(ptr + i + 1), *(ptr + i + 2) , *(ptr + i + 3) , *(ptr + i + 4), *(ptr + i + 5), *(ptr + i + 6), *(ptr + i + 7));
				RTW_INFO("#############################\n");
				_rtw_memsetbu(pmppriv->mplink_buf, '\0' , sizeof(pmppriv->mplink_buf));
				_rtw_memcpybu(pmppriv->mplink_buf, ptr, precv_frame->u.hdr.len);
				pmppriv->mplink_rx_len = precv_frame->u.hdr.len;
				pmppriv->mplink_brx =_TRUE;
		}
	}
	if (pmppriv->bloopback) {
		if (_rtw_memcmpbu(ptr + 24, pmptx->buf + 24, precv_frame->u.hdr.len - 24) == _FALSE) {
 	...
 	if (pmppriv->bSetRxBssid == _TRUE) {
		...
		if (_rtw_memcmpbu(bs, adapter->mppriv.network_macaddr, ETH_ALEN) == _FALSE)
			ret = _FAIL;
	...
	if (_rtw_memcmpbu(get_addr2_ptr(ptr), adapter->mppriv.mac_filter, ETH_ALEN) == _FALSE)
		ret = _FAIL;

	return ret;
}

static sint MPwlanhdr_to_ethhdrbu(union recv_frame *precvframe)
{
	sint	rmv_len;
	u16 len;
	...
	if (adapter->mppriv.bRTWSmbCfg == _TRUE) {
		/* if(_rtw_memcmpbu(mcastheadermac, pattrib->dst, 3) == _TRUE) */ /* SimpleConfig Dest. */
		/*			_rtw_memcpybu(ptr+ETH_ALEN, pattrib->bssid, ETH_ALEN); */

		if (_rtw_memcmpbu(mcastheadermac, pattrib->bssid, 3) == _TRUE) /* SimpleConfig Dest. */
			_rtw_memcpybu(ptr, pattrib->bssid, ETH_ALEN);
...
int mp_recv_frame(_adapter *padapter, union recv_frame *rframe)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE)) { /* &&(padapter->mppriv.check_mp_pkt == 0)) */
		...
		if (pmppriv->rx_bindicatePkt == _FALSE) {
			ret = _FAIL;
			rtw_free_recvframebu(rframe, pfree_recv_queue);/* free this recv_frame */
			goto exit;
		} else {
			type =	GetFrameType(ptr);
			pattrib->to_fr_ds = get_tofr_ds(ptr);
			...
			if (type == WIFI_DATA_TYPE) {
				...
				_rtw_memcpybu(pattrib->dst, pda, ETH_ALEN);
				_rtw_memcpybu(pattrib->src, psa, ETH_ALEN);
				_rtw_memcpybu(pattrib->bssid, pbssid, ETH_ALEN);

				switch (pattrib->to_fr_ds) {
				case 0:
					_rtw_memcpybu(pattrib->ra, pda, ETH_ALEN);
					_rtw_memcpybu(pattrib->ta, psa, ETH_ALEN);
					ret = sta2sta_data_framebu(padapter, rframe, &psta);
					break;
				case 1:
					_rtw_memcpybu(pattrib->ra, pbssid, ETH_ALEN);
					_rtw_memcpybu(pattrib->ta, psa, ETH_ALEN);
					ret = sta2ap_data_framebu(padapter, rframe, &psta);
					break;
				case 2:
					_rtw_memcpybu(pattrib->ra, pda, ETH_ALEN);
					_rtw_memcpybu(pattrib->ta, pbssid, ETH_ALEN);
					ret = ap2sta_data_framebu(padapter, rframe, &psta);
					break;
				case 3:
					_rtw_memcpybu(pattrib->ra, GetAddr1Ptr(ptr), ETH_ALEN);
					_rtw_memcpybu(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
				...
				ret = MPwlanhdr_to_ethhdrbu(rframe);
				...
				if (!RTW_CANNOT_RUN(padapter)) {
					/* indicate this recv_frame */
					ret = rtw_recv_indicatepktbu(padapter, rframe);
					if (ret != _SUCCESS) {
						#ifdef DBG_RX_DROP_FRAME
						RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" rtw_recv_indicatepktbu fail!\n"
							, FUNC_ADPT_ARG(padapter));
...
int recv_frame_monitorbu(_adapter *padapter, union recv_frame *rframe)
{
	int ret = _SUCCESS;
	_queue *pfree_recv_queue = &padapter->recvpriv.free_recv_queue;
	...
	if (!RTW_CANNOT_RUN(padapter)) {
		/* indicate this recv_frame */
		ret = rtw_recv_monitorbu(padapter, rframe);
	} else 
		ret = _FAIL;
...
exit:
	...
	if (rframe) /* free this recv_frame */
		rtw_free_recvframebu(rframe, pfree_recv_queue);

	return ret;
}
#endif
int recv_funcbu_prehandlebu(_adapter *padapter, union recv_frame *rframe)
{
	int ret = _SUCCESS;
...
#endif
	{
		...
		ret = validate_recv_framebu(padapter, rframe);
		if (ret != _SUCCESS) {
			rtw_free_recvframebu(rframe, pfree_recv_queue);/* free this recv_frame */
			goto exit;
		}
...
int recv_funcbu_posthandlebu(_adapter *padapter, union recv_frame *prframe)
{
	int ret = _SUCCESS;
	...
	prframe = decryptorbu(padapter, prframe);
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" decryptorbu: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
	...
	if ((_rtw_memcmpbu(psnap_type, SNAP_ETH_TYPE_TDLS, ETH_TYPE_LEN)) &&
	    ((*pcategory == RTW_WLAN_CATEGORY_TDLS) || (*pcategory == RTW_WLAN_CATEGORY_P2P))) {
		ret = OnTDLS(padapter, prframe);
		if (ret == _FAIL)
			goto _exit_recv_funcbu;
	}
#endif /* CONFIG_TDLS */

	prframe = recvframe_chk_defragbu(padapter, prframe);
	if (prframe == NULL)	{
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recvframe_chk_defragbu: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
	...
	prframe = portctrlbu(padapter, prframe);
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" portctrlbu: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
		DBG_COUNTER(padapter->rx_logs.core_rx_post_portctrlbu_err);
		goto _recv_data_drop;
	}

	count_rx_statsbu(padapter, prframe, NULL);
	...
	if (ret == _FAIL) {
		rtw_free_recvframebu(orig_prframe, pfree_recv_queue);
		goto _recv_data_drop;
...
int recv_funcbu(_adapter *padapter, union recv_frame *rframe)
{
	int ret;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 24))
		recv_frame_monitorbu(padapter, rframe);
#endif
		ret = _SUCCESS;
		goto exit;
	}
	...
	if ((type == WIFI_DATA_TYPE)&& check_fwstate(mlmepriv, WIFI_STATION_STATE)) {
		...
		if ( _rtw_memcmpbu(get_addr2_ptr(ptr), cur_network->network.MacAddress, ETH_ALEN)==0) {
			recv_frame_monitorbu(padapter, rframe);
			ret = _SUCCESS;
			goto exit;
		}
		...
		if (check_fwstate(mlmepriv, WIFI_STATION_STATE) && psecuritypriv->busetkipkey) {
			...
			while ((pending_frame = rtw_alloc_recvframebu(&padapter->recvpriv.uc_swdec_pending_queue))) {
				cnt++;
				DBG_COUNTER(padapter->rx_logs.core_rx_dequeue);
				recv_funcbu_posthandlebu(padapter, pending_frame);
			}
	...
	ret = recv_funcbu_prehandlebu(padapter, rframe);
	...
	if (ret == _SUCCESS) {
		...
		if (check_fwstate(mlmepriv, WIFI_STATION_STATE) &&
			...
			rtw_enqueue_recvframebu(rframe, &padapter->recvpriv.uc_swdec_pending_queue);
			/* RTW_INFO("%s: no key, enqueue uc_swdec_pending_queue\n", __func__); */

			if (recvpriv->free_recvframe_cnt < NR_RECVFRAME / 4) {
				/* to prevent from recvframe starvation, get recvframe from uc_swdec_pending_queue to free_recvframe_cnt */
				rframe = rtw_alloc_recvframebu(&padapter->recvpriv.uc_swdec_pending_queue);
				if (rframe)
					goto do_posthandle;
			}
			goto exit;
...
do_posthandle:
		ret = recv_funcbu_posthandlebu(padapter, rframe);
	}
...
s32 rtw_recv_entrybu(union recv_frame *precvframe)
{
	_adapter *padapter;
	struct recv_priv *precvpriv;
	s32 ret = _SUCCESS;
	...
	ret = recv_funcbu(padapter, precvframe);
	if (ret == _FAIL) {
...
static void rtw_signal_stat_timer_hdl(void *ctx)
{
	...
	if (adapter->recvpriv.is_signal_dbg) {
		...
		if (num_signal_strength == 0) {
			if (rtw_get_on_cur_ch_timebu(adapter) == 0
			    || rtw_get_passing_time_ms(rtw_get_on_cur_ch_timebu(adapter)) < 2 * adapter->mlmeextpriv.mlmext_info.bcn_interval
			   )
				goto set_timer;
		}
...
#ifdef CONFIG_CONCURRENT_MODE
		if (rtw_mi_buddy_check_fwstatebu(adapter, WIFI_UNDER_SURVEY) == _TRUE)
			goto set_timer;
#endif
		if (adapter->registrypriv.mp_mode == 1)
		...
		ratio_pre_stat = signal_stat_calc_profilebu[ratio_profile][0];
		ratio_curr_stat = signal_stat_calc_profilebu[ratio_profile][1];
		...
		RTW_INFO(FUNC_ADPT_FMT" signal_strength:%3u, rssi:%3d, signal_qual:%3u"
			...
			, rtw_get_on_cur_ch_timebu(adapter) ? rtw_get_passing_time_ms(rtw_get_on_cur_ch_timebu(adapter)) : 0
			);
#endif
	}
...
void rx_process_phy_infobu(_adapter *padapter, union recv_frame *rframe)
{
	/* Check RSSI */
	...
	rtw_store_phy_infobu(padapter, rframe);
}

void rx_query_phy_statusbu(
	union recv_frame	*precvframe,
	u8 *pphy_status)
{
	...
	if (_rtw_memcmpbu(adapter_mac_addr(padapter), ta, ETH_ALEN) == _TRUE) {
	...
	pkt_info.is_packet_match_bssid = (!IsFrameTypeCtrl(wlanhdr))
		...
		&& ((!MLME_IS_MESH(padapter) && _rtw_memcmpbu(get_hdr_bssid(wlanhdr), get_bssid(&padapter->mlmepriv), ETH_ALEN))
			|| (MLME_IS_MESH(padapter) && psta));

	pkt_info.is_to_self = (!pattrib->icv_err) && (!pattrib->crc_err)
		&& _rtw_memcmpbu(ra, adapter_mac_addr(padapter), ETH_ALEN);
	...
	odm_phy_status_querybu(&pHalData->odmpriv, p_phy_info, pphy_status, &pkt_info);
		...
		if (padapter->registrypriv.mp_mode != 1) {
			if ((!MLME_IS_MESH(padapter) && pkt_info.is_packet_match_bssid)
				...
				if (psta) {
					...
					rx_process_phy_infobu(padapter, precvframe);
				}
	...
	rtw_odm_parse_rx_phy_status_chinfobu(precvframe, pphy_status);
}
/*
...
* @return _FALSE:
*/
int rtw_inc_and_chk_continual_no_rx_packetbu(struct sta_info *sta, int tid_index)
{

	int ret = _FALSE;
	int value = ATOMIC_INCbu_RETURN(&sta->continual_no_rx_packet[tid_index]);
...
*/
void rtw_reset_continual_no_rx_packetbu(struct sta_info *sta, int tid_index)
{
	ATOMIC_SETbu(&sta->continual_no_rx_packet[tid_index], 0);
}

u8 adapter_allow_bmc_data_rxbu(_adapter *adapter)
{
	...
	if (rtw_linked_checkbu(adapter) == _FALSE)
		return 0;

	return 1;
}

s32 pre_recv_entrybu(union recv_frame *precvframe, u8 *pphy_status)
{
	s32 ret = _SUCCESS;
...
#ifdef CONFIG_MP_INCLUDED
	if (rtw_mp_modebu_check(primary_padapter))
	...
	if (ra_is_bmc == _FALSE) {
		/* UC frame */
		iface = rtw_get_iface_by_macddrbu(primary_padapter , ra);
		if (!iface) {
			#if defined(CONFIG_RTW_CFGVENDOR_RANDOM_MAC_OUI) || defined(CONFIG_RTW_SCAN_RAND)
			if (_rtw_memcmpbu(ra, adapter_pno_mac_addr(primary_padapter), ETH_ALEN))
				goto query_phy_status;
			#endif
			...
			if (rtw_unassoc_sta_src_chk(primary_padapter, UNASOC_STA_SRC_RX_NMY_UC)) {
				if (pphy_status) {
					rx_query_phy_statusbu(precvframe, pphy_status);
			...
			rtw_free_recvframebu(precvframe, &precvframe->u.hdr.adapter->recvpriv.free_recv_queue);
			goto exit;
		}
		#ifdef CONFIG_CONCURRENT_MODE
		else
		...
		/* BMC frame */
		#ifdef CONFIG_CONCURRENT_MODE
		rtw_mi_buddy_clone_bcmc_packetbu(primary_padapter, precvframe, pphy_status);
		#endif
		...
		if (GetFrameType(pbuf) == WIFI_DATA_TYPE
			&& !adapter_allow_bmc_data_rxbu(precvframe->u.hdr.adapter)
		) {
			rtw_free_recvframebu(precvframe, &precvframe->u.hdr.adapter->recvpriv.free_recv_queue);
			goto exit;
		}
...
query_phy_status:
	...
	ret = rtw_recv_entrybu(precvframe);
...
thread_return rtw_recv_thread(thread_context context)
{
	...
	do {
		err = _rtw_down_semabu(&recvpriv->recv_sema);
		if (_FAIL == err) {
		...
		if (err == RTW_RFRAME_UNAVAIL
			...
			rtw_msleep_osbu(1);
			_rtw_up_semabu(&recvpriv->recv_sema);
		}
