...
static struct rtw_mesh_table *rtw_mesh_table_alloc(void)
{
	...
	rtw_hlist_head_initbu(&newtbl->known_gates);
	ATOMIC_SETbu(&newtbl->entries,  0);
	_rtw_spinlockbu_initbu(&newtbl->gates_lock);
...
void rtw_mesh_path_assign_nexthop(struct rtw_mesh_path *mpath, struct sta_info *sta)
{
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		list = get_next(list);
		_rtw_memcpybu(xframe->attrib.ra, sta->cmn.mac_addr, ETH_ALEN);
	}
			     ...
			     struct rtw_mesh_path *gate_mpath)
{
	...
	_rtw_memcpybu(attrib->ra, next_hop, ETH_ALEN);
	rtw_rcu_read_unlock();
	_rtw_memcpybu(attrib->mda, dst_addr, ETH_ALEN);
}
				    ...
				    bool copy)
{
	...
	_rtw_init_listheadbu(&failq);

	_enter_critical_bh(&from_mpath->frame_queue.lock, &flags);
	rtw_list_splicebu_init(&from_mpath->frame_queue.queue, &failq);
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		rtw_list_insert_tailbu(&fskb->list, get_list_head(&gate_mpath->frame_queue));
	...
	_enter_critical_bh(&from_mpath->frame_queue.lock, &flags);
	rtw_list_splicebu(&failq, &from_mpath->frame_queue.queue);
...
void dump_mpath(void *sel, _adapter *adapter)
{
	...
	do {
		...
		if (mpath) {
			...
			_rtw_memcpybu(next_hop, mpath->next_hop->cmn.mac_addr, ETH_ALEN);
			sn = mpath->sn;
			metric = mpath->metric;
...
int rtw_mesh_path_add_gate(struct rtw_mesh_path *mpath)
{
	...
	mpath->is_gate = true;
	_rtw_spinlockbu(&tbl->gates_lock);
	ori_num_gates = minfo->num_gates;
	minfo->num_gates++;
	rtw_hlist_add_headbu_rcu(&mpath->gate_list, &tbl->known_gates);

	if (ori_num_gates == 0
		|| rtw_macaddr_is_largerbu(mpath->dst, minfo->max_addr_gate->dst)
	) {
		minfo->max_addr_gate = mpath;
		minfo->max_addr_gate_is_larger_than_self =
			rtw_macaddr_is_largerbu(mpath->dst, adapter_mac_addr(mpath->adapter));
	}

	_rtw_spinunlockbu(&tbl->gates_lock);
...
void rtw_mesh_gate_del(struct rtw_mesh_table *tbl, struct rtw_mesh_path *mpath)
{
	...
	rtw_hlist_delbu_rcu(&mpath->gate_list);
	ori_num_gates = minfo->num_gates;
	minfo->num_gates--;
	...
	if (ori_num_gates == 1) {
		...
		rtw_hlist_for_each_entry_rcu(gate, node, &tbl->known_gates, gate_list) {
			if (!max_addr_gate || rtw_macaddr_is_largerbu(gate->dst, max_addr_gate->dst))
				max_addr_gate = gate;
		}
		minfo->max_addr_gate = max_addr_gate;
		minfo->max_addr_gate_is_larger_than_self =
			rtw_macaddr_is_largerbu(max_addr_gate->dst, adapter_mac_addr(mpath->adapter));
	}
...
bool rtw_mesh_gate_search(struct rtw_mesh_table *tbl, const u8 *addr)
{
	...
	rtw_rcu_read_lock();
	rtw_hlist_for_each_entry_rcu(gate, node, &tbl->known_gates, gate_list) {
		if (_rtw_memcmpbu(gate->dst, addr, ETH_ALEN) == _TRUE) {
			exist = 1;
			break;
		}
	}
				...
				const u8 *dst)
{
	...
	_rtw_memcpybu(new_mpath->dst, dst, ETH_ALEN);
	_rtw_memsetbu(new_mpath->rann_snd_addr, 0xFF, ETH_ALEN);
	new_mpath->is_root = false;
	new_mpath->adapter = adapter;
	new_mpath->flags = 0;
	new_mpath->gate_asked = false;
	_rtw_init_queuebu(&new_mpath->frame_queue);
	new_mpath->frame_queue_len = 0;
	new_mpath->exp_time = rtw_get_current_time();
	_rtw_spinlockbu_initbu(&new_mpath->state_lock);
	rtw_init_timerbu(&new_mpath->timer, adapter, rtw_mesh_path_timer, new_mpath);
				...
				const u8 *dst)
{
	...
	if (ATOMIC_INCbu_UNLESS(&adapter->mesh_info.mpaths, RTW_MESH_MAX_MPATHS) == 0)
		 ...
		 const u8 *dst, const u8 *mpp)
{
	...
	_rtw_memcpybu(new_mpath->mpp, mpp, ETH_ALEN);
	ret = rtw_rhashtable_lookup_insert_fast(&tbl->rhead,
					    &new_mpath->rhash,
					    rtw_mesh_rht_params);
			       ...
			       struct rtw_mesh_path *mpath)
{
	...
	ATOMIC_DECbu(&adapter->mesh_info.mpaths);
	ATOMIC_DECbu(&tbl->entries);
	_rtw_spinlockbu_freebu(&mpath->state_lock);
			       ...
			       const u8 *proxy)
{
	...
	while ((mpath = rtw_rhashtable_walk_next(&iter))) {
		...
		if (_rtw_memcmpbu(mpath->mpp, proxy, ETH_ALEN) == _TRUE)
			__rtw_mesh_path_del(tbl, mpath);
	}
out:
	rtw_rhashtable_walk_stop(&iter);
...
void rtw_mesh_path_tx_pending(struct rtw_mesh_path *mpath)
{
	if (mpath->flags & RTW_MESH_PATH_ACTIVE) {
		...
		_rtw_init_listheadbu(&q);

		/* move to local queue */
		enter_critical_bh(&mpath->frame_queue.lock);
		if (mpath->frame_queue_len) {
			rtw_list_splicebu_init(&mpath->frame_queue.queue, &q);
		...
		if (q_len) {
			...
			rtw_list_splicebu_tail(&q, &minfo->mpath_tx_queue.queue);
...
void rtw_mesh_path_flush_pending(struct rtw_mesh_path *mpath)
{
	...
	enter_critical_bh(&mpath->frame_queue.lock);
	rtw_list_splicebu_init(&mpath->frame_queue.queue, &tmp);
	mpath->frame_queue_len = 0;
	exit_critical_bh(&mpath->frame_queue.lock);
