...
inline u8 *rtw_set_iebu_mesh_id(u8 *buf, u32 *buf_len, const char *mesh_id, u8 id_len)
{
	return rtw_set_iebu(buf, WLAN_EID_MESH_ID, id_len, mesh_id, buf_len);
}

inline u8 *rtw_set_iebu_mesh_config(u8 *buf, u32 *buf_len
	...
	, bool mbca_en, bool tbtt_adj, bool ps_level)
{
	...
	return rtw_set_iebu(buf, WLAN_EID_MESH_CONFIG, 7, conf, buf_len);
}

inline u8 *rtw_set_iebu_mpm(u8 *buf, u32 *buf_len
	, u8 proto_id, u16 llid, u16 *plid, u16 *reason, u8 *chosen_pmk)
{
	...
	if (chosen_pmk) {
		_rtw_memcpybu(pos, chosen_pmk, 16);
		pos += 16;
	}

	return rtw_set_iebu(buf, WLAN_EID_MPM, pos - data, data, buf_len);
}
...
bool rtw_bss_is_forwarding(WLAN_BSSID_EX *bss)
{
	...
	ie = rtw_get_iebu(BSS_EX_TLV_IES(bss), WLAN_EID_MESH_CONFIG, &ie_len,
...
int _rtw_bss_is_same_mbss(WLAN_BSSID_EX *a, WLAN_BSSID_EX *b, u8 **a_mconf_ie_r, u8 **b_mconf_ie_r)
{
	...
	if (a->InfrastructureMode != Ndis802_11_mesh)
		goto exit;
	a_mconf_ie = rtw_get_iebu(BSS_EX_TLV_IES(a), WLAN_EID_MESH_CONFIG, &a_mconf_ie_len, BSS_EX_TLV_IES_LEN(a));
	...
	if (b->InfrastructureMode != Ndis802_11_mesh)
		goto exit;
	b_mconf_ie = rtw_get_iebu(BSS_EX_TLV_IES(b), WLAN_EID_MESH_CONFIG, &b_mconf_ie_len, BSS_EX_TLV_IES_LEN(b));
	...
	if (a->mesh_id.SsidLength != b->mesh_id.SsidLength
		|| _rtw_memcmpbu(a->mesh_id.Ssid, b->mesh_id.Ssid, a->mesh_id.SsidLength) == _FALSE)
		goto exit;

	if (_rtw_memcmpbu(a_mconf_ie + 2, b_mconf_ie + 2, 5) == _FALSE)
		goto exit;
...
int rtw_bss_is_candidate_mesh_peer(_adapter *adapter, WLAN_BSSID_EX *target, u8 ch, u8 add_peer)
{
		...
		if (target->SupportedRates[i] & 0x80) {
			...
			if (!ch) {
				...
				if (target->Configuration.DSConfig > 14)
					match = rtw_is_basic_rate_ofdmbu(target->SupportedRates[i]);
				else
					match = rtw_is_basic_rate_mixbu(target->SupportedRates[i]);
			} else { 
	...
	if (auth_pid && auth_pid <= 2) {
		...
		/* RSN */
		rsn_ie = rtw_get_wpa2_iebu(BSS_EX_TLV_IES(target), &rsn_ie_len, BSS_EX_TLV_IES_LEN(target));
		if (!rsn_ie || rsn_ie_len == 0)
			goto exit;
		if (rtw_parse_wpa2_iebu(rsn_ie, rsn_ie_len + 2, &group_cipher, &pairwise_cipher, &gmcs, NULL, &mfp_opt, NULL) != _SUCCESS)
			goto exit;
...
static bool rtw_mesh_acnode_candidate_exist(_adapter *adapter)
{
	...
	list = get_next(head);
	while (!rtw_end_of_queue_searchbu(head, list)) {
		...
		if (rtw_get_passing_time_ms(scanned->last_scanned) < mcfg->peer_sel_policy.scanr_exp_ms
			...
			&& rtw_access_ctrlbu(adapter, scanned->network.MacAddress) == _TRUE
			#endif
...
static void rtw_mesh_acnode_set_notify_etime(_adapter *adapter, u8 *rframe_whdr)
{
	if (adapter->mesh_info.plink_ctl.acnode_rsvd) {
		struct wlan_network *scanned = rtw_find_networkbu(&adapter->mlmepriv.scanned_queue, get_addr2_ptr(rframe_whdr));
...
static void rtw_mesh_cto_mgate_blacklist_chk(_adapter *adapter)
{
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		scanned = rtw_find_networkbu(&adapter->mlmepriv.scanned_queue, ent->addr);
		if (!scanned)
			continue;
...
void rtw_chk_candidate_peer_notify(_adapter *adapter, struct wlan_network *scanned)
{
	...
	if (rtw_get_passing_time_ms(scanned->last_scanned) >= mcfg->peer_sel_policy.scanr_exp_ms
		...
		|| !rtw_bss_is_candidate_mesh_peer(adapter, &scanned->network, 1, 1)
		#if CONFIG_RTW_MACADDR_ACL
		|| rtw_access_ctrlbu(adapter, scanned->network.MacAddress) == _FALSE
		#endif
...
void rtw_mesh_peer_status_chk(_adapter *adapter)
{
	...
	if (rtw_mesh_cto_mgate_required(adapter)) {
		...
		issue_probereqbu_ex(adapter, &adapter->mlmepriv.cur_network.network.mesh_id, NULL, 0, 0, 0, 0);
	}
#endif
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		if (!rtw_bss_is_candidate_mesh_peer(adapter, &plink->scanned->network, 1, 0)
			#if CONFIG_RTW_MACADDR_ACL
			|| rtw_access_ctrlbu(adapter, plink->addr) == _FALSE
			#endif
		) {
			flush = 1;
			goto flush_add;
...
flush_add:
	...
	if (flush_num) {
		...
		for (i = 0; i < flush_num; i++) {
			sta = rtw_get_stainfobu_by_offsetbu(stapriv, flush_list[i]);
			_rtw_memcpybu(sta_addr, sta->cmn.mac_addr, ETH_ALEN);

			updated |= ap_free_stabu(adapter, sta, _TRUE, WLAN_REASON_DEAUTH_LEAVING, _FALSE);
...
static u8 rtw_mesh_offch_cto_mgate_required(_adapter *adapter)
{
	...
	head = get_list_head(queue);
	pos = get_next(head);
	while (!rtw_end_of_queue_searchbu(head, pos)) {
	...
	if (rtw_end_of_queue_searchbu(head, pos))
		ret = 1;
...
u8 rtw_mesh_offch_candidate_accepted(_adapter *adapter)
{
	...
	if (ret) {
		...
		rtw_mi_statusbu_no_self(adapter, &mstate_no_self);
...
u8 rtw_mesh_select_operating_ch(_adapter *adapter)
{
	...
	while (!rtw_end_of_queue_searchbu(head, pos)) {
		...
		if (rtw_get_passing_time_ms(scanned->last_scanned) < mcfg->peer_sel_policy.scanr_exp_ms
			...
			int ch_set_idx = rtw_chset_search_chbu(rfctl->channel_set, scanned->network.Configuration.DSConfig);
...
#define NSTATE_VALUE_ARG_ACN
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		mesh_conf_ie = rtw_get_iebu(BSS_EX_TLV_IES(&network->network), WLAN_EID_MESH_CONFIG
	...
	if (MLME_IS_MESH(adapter) && MLME_IS_ASOC(adapter)) {
		...
		if (mesh_conf_ie && mesh_conf_ie_len == 7) {
			...
			if (auth_pid && auth_pid <= 2) {
				rsn_ie = rtw_get_wpa2_iebu(BSS_EX_TLV_IES(&network->network)
							, &rsn_ie_len, BSS_EX_TLV_IES_LEN(&network->network));
				if (rsn_ie && rsn_ie_len)
					rtw_parse_wpa2_iebu(rsn_ie, rsn_ie_len + 2, &gcs, &pcs, &gmcs, NULL, &mfp_opt, NULL);
			}
	...
	for (i = 0; i < mesh_network_cnt; i++) {
		...
		auth_pid = GET_MESH_CONF_ELE_AUTH_PROTO_ID(mesh_conf_ie + 2);
		if (auth_pid && auth_pid <= 2) {
			rsn_ie = rtw_get_wpa2_iebu(BSS_EX_TLV_IES(&network->network), &rsn_ie_len, BSS_EX_TLV_IES_LEN(&network->network));
			if (rsn_ie && rsn_ie_len)
				rtw_parse_wpa2_iebu(rsn_ie, rsn_ie_len + 2, &gcs, &pcs, &gmcs, NULL, &mfp_opt, NULL);
		}
...
{	
	...
	_rtw_memcpybu(iv_crypt, mic_ie + 2, iv_crypt_len);
	...
	, u8* fbody, u8 *mic_ie, u8 *ampe_buf, bool inverse)
{
	...
	_rtw_memcpybu(ampe_ie, ampe_buf, ampe_ie_len);
	...
	, u8 **nbuf, size_t *nlen)
{
	...
	if (mic_ie) {
		...
		if (*(ampe_buf + 1) >= 68) {
			_rtw_memcpybu(plink->sel_pcs, ampe_buf + 2, 4);
			_rtw_memcpybu(plink->l_nonce, ampe_buf + 6, 32);
			_rtw_memcpybu(plink->p_nonce, ampe_buf + 38, 32);
		}
	}
#endif
	...
	/* build new frame  */
	_rtw_memcpybu(new_buf, fhead, sizeof(struct rtw_ieee80211_hdr_3addr) + tlv_ies_offset);
	new_fhead = new_buf;
	new_flen = new_len;
	...
	while (left >= 2) {
		...
		case WLAN_EID_TIM:
			break;
		default:
			fpos = rtw_set_iebu(fpos, id, elen, pos, NULL);
			if (id == WLAN_EID_MESH_CONFIG)
				fpos = rtw_set_iebu(fpos, WLAN_EID_MPM, mpm_ielen, mpm_ie + 2, NULL);
		}
...
static int rtw_mpm_check_frames(_adapter *adapter, u8 action, const u8 **buf, size_t *len, u8 tx)
{
	...
	_rtw_memsetbu(&mpm_info, 0, sizeof(struct mpm_frame_info));
	...
	mpm_ie = rtw_get_iebu(fhead + sizeof(struct rtw_ieee80211_hdr_3addr) + tlv_ies_offset
	...
	if (mpm_info.pid_v == 1) {
		mic_ie = rtw_get_iebu(fhead + sizeof(struct rtw_ieee80211_hdr_3addr) + tlv_ies_offset
...
bypass_sync_bss:
		...
		if (tx && mpm_info.reason && mpm_info.reason_v == WLAN_REASON_MESH_MAX_PEERS) {
			if (rtw_mesh_scanned_is_acnode_confirmed(adapter, plink->scanned)
				...
				if (sac) {
					...
					if (!rtw_is_list_emptybu(&sac->asoc_list)) {
					...
					_rtw_memcpybu(sta_addr, sac->cmn.mac_addr, ETH_ALEN);
					updated = ap_free_stabu(adapter, sac, 0, 0, 1);
	...
	if (action == RTW_ACT_SELF_PROTECTED_MESH_CONF) {
		...
		if (tx == _FALSE) {
			...
			if (mpm_info.pid_v == 1)
				_rtw_memcpybu(plink->chosen_pmk, mpm_info.chosen_pmk, 16);
		}
		...
		if (ies) {
			_rtw_memcpybu(ies, fhead + sizeof(struct rtw_ieee80211_hdr_3addr) + 2, ies_len);
			if (tx == _FALSE) {
...
int rtw_mesh_on_auth(_adapter *adapter, union recv_frame *rframe)
{
...
#if CONFIG_RTW_MACADDR_ACL
	if (rtw_access_ctrlbu(adapter, get_addr2_ptr(whdr)) == _FALSE)
		return _SUCCESS;
#endif
	...
	if (!rtw_mesh_plink_get(adapter, get_addr2_ptr(whdr))) {
		...
		if (adapter_to_rfctl(adapter)->offch_state == OFFCHS_NONE)
			issue_probereqbu(adapter, &adapter->mlmepriv.cur_network.network.mesh_id, get_addr2_ptr(whdr));

		/* only peer being added (checked by notify conditions) is allowed */
		return _SUCCESS;
	}

	rtw_cfg80211_rx_mframebu(adapter, rframe, NULL);
	return _SUCCESS;
}
...
unsigned int on_action_self_protected(_adapter *adapter, union recv_frame *rframe)
{
	...
	if (!_rtw_memcmpbu(adapter_mac_addr(adapter), GetAddr1Ptr(pframe), ETH_ALEN))
		goto exit;
...
#ifdef CONFIG_IOCTL_CFG80211
		#if CONFIG_RTW_MACADDR_ACL
		if (rtw_access_ctrlbu(adapter, get_addr2_ptr(pframe)) == _FALSE)
			goto exit;
		#endif
		#if CONFIG_RTW_MESH_CTO_MGATE_BLACKLIST
		if (rtw_mesh_cto_mgate_required(adapter)
			...
			goto exit;
		#endif
		rtw_cfg80211_rx_actionbu(adapter, rframe, NULL);
		ret = _SUCCESS;
...
bool rtw_mesh_update_bss_peering_status(_adapter *adapter, WLAN_BSSID_EX *bss)
{
	...
	ie = rtw_get_iebu(BSS_EX_TLV_IES(bss), WLAN_EID_MESH_CONFIG, &ie_len, BSS_EX_TLV_IES_LEN(bss));
	if (!ie || ie_len != 7) {
		rtw_warn_on(1);
		goto exit;
	}
...
struct mesh_plink_ent *_rtw_mesh_plink_get(_adapter *adapter, const u8 *hwaddr)
{
	...
	for (i = 0; i < RTW_MESH_MAX_PEER_CANDIDATES; i++) {
		if (plink_ctl->ent[i].valid == _TRUE
			&& _rtw_memcmpbu(plink_ctl->ent[i].addr, hwaddr, ETH_ALEN) == _TRUE
		) {
			ent = &plink_ctl->ent[i];
			break;
		}
...
int _rtw_mesh_plink_add(_adapter *adapter, const u8 *hwaddr)
{
	...
	if (exist == _FALSE && ent) {
		_rtw_memcpybu(ent->addr, hwaddr, ETH_ALEN);
		ent->valid = _TRUE;
		#ifdef CONFIG_RTW_MESH_AEK
		ent->aek_valid = 0;
		#endif
		ent->llid = 0;
		ent->plid = 0;
		_rtw_memsetbu(ent->chosen_pmk, 0, 16);
		#ifdef CONFIG_RTW_MESH_AEK
		_rtw_memsetbu(ent->sel_pcs, 0, 4);
		_rtw_memsetbu(ent->l_nonce, 0, 32);
		_rtw_memsetbu(ent->p_nonce, 0, 32);
		#endif
...
void rtw_mesh_plink_ctl_init(_adapter *adapter)
{
	...
	_rtw_spinlockbu_initbu(&plink_ctl->lock);
	plink_ctl->num = 0;
	for (i = 0; i < RTW_MESH_MAX_PEER_CANDIDATES; i++)
		plink_ctl->ent[i].valid = _FALSE;

#if CONFIG_RTW_MESH_PEER_BLACKLIST
	_rtw_init_queuebu(&plink_ctl->peer_blacklist);
#endif
#if CONFIG_RTW_MESH_CTO_MGATE_BLACKLIST
	_rtw_init_queuebu(&plink_ctl->cto_mgate_blacklist);
#endif
}
...
void rtw_mesh_plink_ctl_deinit(_adapter *adapter)
{
	...
	_rtw_spinlockbu_freebu(&plink_ctl->lock);

#if CONFIG_RTW_MESH_PEER_BLACKLIST
	rtw_mesh_peer_blacklist_flush(adapter);
	_rtw_deinit_queuebu(&plink_ctl->peer_blacklist);
#endif
#if CONFIG_RTW_MESH_CTO_MGATE_BLACKLIST
	rtw_mesh_cto_mgate_blacklist_flush(adapter);
	_rtw_deinit_queuebu(&plink_ctl->cto_mgate_blacklist);
#endif
}
...
#define DBG_RTW_MESH_PEER_ESTABLISH 0
	...
	if (DBG_RTW_MESH_PEER_ESTABLISH)
		dump_iesbu(RTW_DBGDUMP, tlv_ies, tlv_ieslen);

	if (rtw_ieee802_11_parse_elemsbu(tlv_ies, tlv_ieslen, &elems, 1) == ParseFailed) {
	...
	rtw_ap_parse_sta_capabilitybu(adapter, sta, plink->rx_conf_ies);

	status = rtw_ap_parse_sta_supported_ratesbu(adapter, sta, tlv_ies, tlv_ieslen);
	if (status != _STATS_SUCCESSFUL_)
		goto exit;

	status = rtw_ap_parse_sta_security_iebu(adapter, sta, &elems);
	if (status != _STATS_SUCCESSFUL_) {
		RTW_INFO(FUNC_ADPT_FMT" security check fail, status=%u\n", FUNC_ADPT_ARG(adapter), status);
		goto exit;
	}

	rtw_ap_parse_sta_wmm_iebu(adapter, sta, tlv_ies, tlv_ieslen);
#ifdef CONFIG_RTS_FULL_BW
	/*check vendor IE*/
	rtw_parse_sta_vendor_ie_8812bu(adapter, sta, tlv_ies, tlv_ieslen);
	...
	rtw_ap_parse_sta_ht_iebu(adapter, sta, &elems);
	rtw_ap_parse_sta_vht_iebu(adapter, sta, &elems);
	...
	bss_cap_update_on_sta_joinbu(adapter, sta);
	sta_info_updatebu(adapter, sta);
	report_add_sta_eventbu(adapter, sta->cmn.mac_addr);
...
int rtw_mesh_set_plink_state(_adapter *adapter, const u8 *mac, u8 plink_state)
{
	...
	if (plink_state == RTW_MESH_PLINK_OPN_SNT) {
		if (rtw_mesh_scanned_is_acnode_confirmed(adapter, plink->scanned)
			...
			if (sac) {
				del_sta = sac;
				_enter_critical_bh(&stapriv->asoc_list_lock, &irqL);
				if (!rtw_is_list_emptybu(&del_sta->asoc_list)) {
	...
	if (plink_state == RTW_MESH_PLINK_OPN_RCVD
		...
		sta = rtw_get_stainfobu(stapriv, mac);
		if (!sta) {
			sta = rtw_alloc_stainfobu(stapriv, mac);
			if (!sta)
				goto release_plink_ctl;
		}
		...
		del_sta = rtw_get_stainfobu(stapriv, mac);
		if (!del_sta)
			goto release_plink_ctl;
...
release_plink_ctl:
	...
	if (del_sta) {
		...
		_rtw_memcpybu(sta_addr, del_sta->cmn.mac_addr, ETH_ALEN);
		updated = ap_free_stabu(adapter, del_sta, 0, 0, 1);
		rtw_mesh_expire_peer(stapriv->padapter, sta_addr);
...
static u8 *rtw_mesh_construct_peer_mesh_close(_adapter *adapter, struct mesh_plink_ent *plink, u16 reason, u32 *len)
{
	...
	_rtw_memcpybu(whdr->addr1, adapter_mac_addr(adapter), ETH_ALEN);
	_rtw_memcpybu(whdr->addr2, plink->addr, ETH_ALEN);
	_rtw_memcpybu(whdr->addr3, adapter_mac_addr(adapter), ETH_ALEN);
	...
	*(pos++) = RTW_ACT_SELF_PROTECTED_MESH_CLOSE;
	...
	pos = rtw_set_iebu_mesh_id(pos, NULL, minfo->mesh_id, minfo->mesh_id_len);

	pos = rtw_set_iebu_mpm(pos, NULL
	...
	if (minfo->mesh_auth_id) {
		...
		*(pos + 1) = 16 /* AES_BLOCK_SIZE */;
		...
		ampe_buf[1] = 68;
		_rtw_memcpybu(ampe_buf + 2, plink->sel_pcs, 4);
		_rtw_memcpybu(ampe_buf + 6, plink->p_nonce, 32);
		_rtw_memcpybu(ampe_buf + 38, plink->l_nonce, 32);
...
void _rtw_mesh_expire_peer_ent(_adapter *adapter, struct mesh_plink_ent *plink)
{
	...
	rtw_cfg80211_indicate_sta_disassocbu(adapter, plink->addr, 0);
#else
	u8 *frame = NULL;
	u32 flen;
	...
	if (frame) {
		...
		s32 freq = rtw_ch2freqbu(mlmeext->cur_channel);
...
u8 rtw_mesh_ps_annc(_adapter *adapter, u8 ps)
{
	...
	for (i = 0; i < sta_alive_num; i++) {
		sta = rtw_get_stainfobu_by_offsetbu(stapriv, sta_alive_list[i]);
		if (!sta)
			continue;

		issue_qos_nulldatabu(adapter, sta->cmn.mac_addr, 7, ps, 3, 500);
		annc_cnt++;
	}
...
static void mpath_tx_tasklet_hdl(void *priv)
{
	...
	while (1) {
		...
		if (minfo->mpath_tx_queue_len) {
			rtw_list_splicebu_init(&minfo->mpath_tx_queue.queue, &tmp);
		...
		while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
			...
			res = rtw_xmitbu_posthandlebu(adapter, xframe, xframe->pkt);
			if (res < 0) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_INFO("DBG_TX_DROP_FRAME %s rtw_xmitbu fail\n", __FUNCTION__);
				#endif
...
static void rtw_mpath_tx_queue_flush(_adapter *adapter)
{
	...
	while (rtw_end_of_queue_searchbu(head, list) == _FALSE) {
		...
		list = get_next(list);
		rtw_list_delete(&xframe->list);
		rtw_free_xmitframebu(&adapter->xmitpriv, xframe);
	}
}
...
static int rtw_mrc_init(_adapter *adapter)
{
	...
	for (i = 0; i < RTW_MRC_BUCKETS; i++)
		rtw_hlist_head_initbu(&minfo->mrc->bucket[i]);
...
static int rtw_mrc_check(_adapter *adapter, const u8 *msa, u32 seq)
{
	...
	rtw_hlist_for_each_entry_safe(p, np, n, &mrc->bucket[idx], list) {
		...
		if (timeout || entries == RTW_MRC_QUEUE_MAX_LEN) {
			...
			rtw_mcache_free(mrc->cache, p);
			--entries;
		} else if ((seq == p->seqnum) && _rtw_memcmpbu(msa, p->msa, ETH_ALEN) == _TRUE)
			return -1;
	}
	...
	_rtw_memcpybu(p->msa, msa, ETH_ALEN);
	rtw_hlist_add_headbu(&p->list, &mrc->bucket[idx]);
	return 0;
}
...
void rtw_mesh_init_mesh_info(_adapter *adapter)
{
	...
	_rtw_memsetbu(minfo, 0, sizeof(struct rtw_mesh_info));
	...
	ATOMIC_SETbu(&minfo->mpaths, 0);
	rtw_mesh_pathtbl_init(adapter);

	_rtw_init_queuebu(&minfo->mpath_tx_queue);
	...
	_rtw_init_listheadbu(&minfo->preq_queue.list);
	_rtw_spinlockbu_initbu(&minfo->mesh_preq_queue_lock);
	
	rtw_init_timerbu(&adapter->mesh_path_timer, adapter, rtw_ieee80211_mesh_path_timer, adapter);
	rtw_init_timerbu(&adapter->mesh_path_root_timer, adapter, rtw_ieee80211_mesh_path_root_timer, adapter);
	rtw_init_timerbu(&adapter->mesh_atlm_param_req_timer, adapter, rtw_mesh_atlm_param_req_timer, adapter);
...
void rtw_mesh_deinit_mesh_info(_adapter *adapter)
{
	...
	_rtw_deinit_queuebu(&adapter->mesh_info.mpath_tx_queue);
			...
			struct xmit_frame *xframe)
{
	...
	rtw_list_insert_tailbu(&xframe->list, get_list_head(&mpath->frame_queue));
	mpath->frame_queue_len++;
	...
	const u8 *mda, const u8 *msa, u8 *ra)
{
	...
	if (_rtw_memcmpbu(adapter_mac_addr(adapter), msa, ETH_ALEN) == _TRUE &&
	...
	/* Avoid keeping trying unicast PREQ toward root,
	   when next_hop leaves */
		   ...
		   _rtw_memcmpbu(adapter_mac_addr(adapter), msa, ETH_ALEN) == _TRUE &&
		   (mpath->flags & RTW_MESH_PATH_RESOLVING) &&
		   !(mpath->flags & RTW_MESH_PATH_FIXED) &&
	...
	, u16 os_qid, _list *b2u_list, u8 *b2u_num, u32 *b2u_mseq)
{
	...
	for (i = 0; i < b2u_sta_num; i++) {
		...
		if (!(sta->state & WIFI_ASOC_STATE)
			|| _rtw_memcmpbu(sta->cmn.mac_addr, msa, ETH_ALEN) == _TRUE
		...
		_rtw_memcpybu(attrib->mda, mda, ETH_ALEN);
		_rtw_memcpybu(attrib->msa, msa, ETH_ALEN);
...
int rtw_mesh_addr_resolve(_adapter *adapter, u16 os_qid, struct xmit_frame *xframe, _pkt *pkt, _list *b2u_list)
{
	...
	if (!is_da_mcast) {
		...
		ae_need = _rtw_memcmpbu(adapter_mac_addr(adapter), etherhdr.h_source, ETH_ALEN) == _FALSE
			|| (mppath && _rtw_memcmpbu(mppath->mpp, etherhdr.h_dest, ETH_ALEN) == _FALSE);
	} else {
		ae_need = _rtw_memcmpbu(adapter_mac_addr(adapter), etherhdr.h_source, ETH_ALEN) == _FALSE;
	...
	if (is_da_mcast) {
		...
		_rtw_memcpybu(attrib->msa, adapter_mac_addr(adapter), ETH_ALEN);
	} else {
		attrib->mesh_frame_mode = ae_need ? MESH_UCAST_PX_DATA : MESH_UCAST_DATA;
		_rtw_memcpybu(attrib->mda, (mppath && ae_need) ? mppath->mpp : attrib->dst, ETH_ALEN);
		_rtw_memcpybu(attrib->msa, adapter_mac_addr(adapter), ETH_ALEN);
...
void rtw_mesh_tx_build_mctrl(_adapter *adapter, struct pkt_attrib *attrib, u8 *buf)
{
	...
	_rtw_memsetbu(mctrl, 0, XATTRIB_GET_MCTRL_LEN(attrib));
	...
	case MESH_UCAST_PX_DATA:
		...
		_rtw_memcpybu(mctrl->eaddr1, attrib->dst, ETH_ALEN);
		_rtw_memcpybu(mctrl->eaddr2, attrib->src, ETH_ALEN);
		break;
	case MESH_BMCAST_PX_DATA:
		mctrl->flags |= MESH_FLAGS_AE_A4;
		_rtw_memcpybu(mctrl->eaddr1, attrib->src, ETH_ALEN);
		break;
	...
	, u16 *fctrl, struct rtw_ieee80211_hdr *whdr)
{
	...
	case MESH_UCAST_PX_DATA:	/* 1, 1, RA, TA, mDA,		mSA,		[DA, SA] */
		SetToDs(fctrl);
		SetFrDs(fctrl);
		_rtw_memcpybu(whdr->addr1, attrib->ra, ETH_ALEN);
		_rtw_memcpybu(whdr->addr2, attrib->ta, ETH_ALEN);
		_rtw_memcpybu(whdr->addr3, attrib->mda, ETH_ALEN);
		_rtw_memcpybu(whdr->addr4, attrib->msa, ETH_ALEN);
		break;
	...
	case MESH_BMCAST_PX_DATA:	/* 0, 1, RA(DA), TA, mSA,		[SA] */
		...
		_rtw_memcpybu(whdr->addr1, attrib->ra, ETH_ALEN);
		_rtw_memcpybu(whdr->addr2, attrib->ta, ETH_ALEN);
		_rtw_memcpybu(whdr->addr3, attrib->msa, ETH_ALEN);
		break;
	case MESH_MHOP_UCAST_ACT:
		/* TBD */
...
int rtw_mesh_rx_data_validate_hdr(_adapter *adapter, union recv_frame *rframe, struct sta_info **sta)
{
	...
	case 2:
		...
		*sta = rtw_get_stainfobu(stapriv, get_addr2_ptr(whdr));
		...
		_rtw_memcpybu(rattrib->mda, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->msa, GetAddr3Ptr(whdr), ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpybu(rattrib->dst, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->src, GetAddr3Ptr(whdr), ETH_ALEN); /* may change after checking mesh ctrl field */
	...
	case 3:
		...
		*sta = rtw_get_stainfobu(stapriv, get_addr2_ptr(whdr));
		...
		_rtw_memcpybu(rattrib->ra, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->ta, get_addr2_ptr(whdr), ETH_ALEN);
		_rtw_memcpybu(rattrib->mda, GetAddr3Ptr(whdr), ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpybu(rattrib->msa, GetAddr4Ptr(whdr), ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpybu(rattrib->dst, GetAddr3Ptr(whdr), ETH_ALEN); /* may change after checking mesh ctrl field */
		_rtw_memcpybu(rattrib->src, GetAddr4Ptr(whdr), ETH_ALEN); /* may change after checking mesh ctrl field */
		_rtw_memcpybu(rattrib->bssid, get_addr2_ptr(whdr), ETH_ALEN);
		a4_shift = ETH_ALEN;
		break;
	default:
	...
	if (ps) {
		if (!((*sta)->state & WIFI_SLEEP_STATE))
			stop_sta_xmitbu(adapter, *sta);
	} else {
		if ((*sta)->state & WIFI_SLEEP_STATE)
			wakeup_sta_to_xmitbu(adapter, *sta);
	}
...
inline int rtw_mesh_rx_validate_mctrl_non_amsdu(_adapter *adapter, union recv_frame *rframe)
{
	...
	if (ret == _SUCCESS) {
		_rtw_memcpybu(rattrib->dst, da, ETH_ALEN);
		_rtw_memcpybu(rattrib->src, sa, ETH_ALEN);
	}
	...
	, struct xmit_frame **fwd_frame, _list *b2u_list)
{
	...
	u8 is_mda_self = !is_mda_bmc && _rtw_memcmpbu(mda, adapter_mac_addr(adapter), ETH_ALEN);
	u16 os_qid;
	struct xmit_frame *xframe;
	...
	if ((mctrl->flags & MESH_FLAGS_AE)
		&& sa != msa && _rtw_memcmpbu(sa, msa, ETH_ALEN) == _FALSE
	) {
		...
		if (!mppath)
			...
			if (_rtw_memcmpbu(mppath->mpp, mpp_addr, ETH_ALEN) == _FALSE)
				_rtw_memcpybu(mppath->mpp, mpp_addr, ETH_ALEN);
			mppath->exp_time = rtw_get_current_time();
	...
	if (_rtw_memcmpbu(msa, adapter_mac_addr(adapter), ETH_ALEN) == _TRUE)
		goto exit;
	...
	if (is_mda_bmc) {
		...
		if (da == mda
			|| _rtw_memcmpbu(da, adapter_mac_addr(adapter), ETH_ALEN)
		) {
			/* DA is self, indicate */
			act |= RTW_RX_MSDU_ACT_INDICATE;
			goto exit;
		...
		if (rtw_get_iface_by_macddrbu(adapter, da)) {
			/* DA is buddy, indicate */
			act |= RTW_RX_MSDU_ACT_INDICATE;
			#if DBG_RTW_MESH_FORWARD_MDA_SELF_COND
			RTW_INFO(FUNC_ADPT_FMT" DA("MAC_FMT") is buddy("ADPT_FMT")\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(da), ADPT_ARG(rtw_get_iface_by_macddrbu(adapter, da)));
			#endif
			goto exit;
		}
		...
		if (mppath) {
			if (_rtw_memcmpbu(mppath->mpp, adapter_mac_addr(adapter), ETH_ALEN) == _FALSE) {
				...
				_rtw_memcpybu(fwd_mpp, mppath->mpp, ETH_ALEN);
				mda = fwd_mpp;
...
fwd_chk:
	...
	ae_need = _rtw_memcmpbu(da , mda, ETH_ALEN) == _FALSE
		|| _rtw_memcmpbu(sa , msa, ETH_ALEN) == _FALSE;
...
#endif
	{
		...
		_rtw_memcpybu(xattrib->mda, mda, ETH_ALEN);
		_rtw_memcpybu(xattrib->msa, msa, ETH_ALEN);
		...
		if (is_mda_bmc) {
			...
			_rtw_memcpybu(xattrib->ra, mda, ETH_ALEN);
		} else {
