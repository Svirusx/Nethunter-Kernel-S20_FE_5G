...
u32 rtw_phydm_abilitybu_opsbu(_adapter *adapter, HAL_PHYDM_OPS ops, u32 ability)
{
	...
	case HAL_PHYDM_DIS_ALL_FUNC:
		...
		halrf_cmn_info_setbu(podmpriv, HALRF_CMNINFO_ABILITY, DYNAMIC_FUNC_DISABLE);
		break;
	...
	case HAL_PHYDM_ABILITY_BK:
		...
		pHalData->bk_rf_ability = halrf_cmn_info_getbu(podmpriv, HALRF_CMNINFO_ABILITY);
		break;
	case HAL_PHYDM_ABILITY_RESTORE:
		/* restore dm flag */
		podmpriv->support_ability = podmpriv->bk_support_ability;
		halrf_cmn_info_setbu(podmpriv, HALRF_CMNINFO_ABILITY, pHalData->bk_rf_ability);
		break;
...
void rtw_odm_init_ic_typebu(_adapter *adapter)
{
	...
	odm_cmn_info_initbu(odm, ODM_CMNINFO_IC_TYPE, ic_type);
}

void rtw_odm_adaptivity_ver_msgbu(void *sel, _adapter *adapter)
{
...
void rtw_odm_adaptivity_en_msgbu(void *sel, _adapter *adapter)
{
...
void rtw_odm_adaptivity_mode_msgbu(void *sel, _adapter *adapter)
{
	struct registry_priv *regsty = &adapter->registrypriv;
...
{
	rtw_odm_adaptivity_ver_msgbu(sel, adapter);
	rtw_odm_adaptivity_en_msgbu(sel, adapter);
	rtw_odm_adaptivity_mode_msgbu(sel, adapter);
}
...
void rtw_odm_adaptivity_parm_msgbu(void *sel, _adapter *adapter)
{
	...
	rtw_odm_adaptivity_config_msgbu(sel, adapter);
...
{
	odm_cmn_info_initbu(dvobj_to_phydm(dvobj), ODM_CMNINFO_DFS_REGION_DOMAIN, rtw_dfs_regd_to_phydm(rtw_rfctl_get_dfs_domain(dvobj_to_rfctl(dvobj))));
}
...
)
{
	...
	temp1 = odm_get_bb_regbu(dm, 0x1860, bMaskDWord);
	temp2 = odm_get_bb_regbu(dm, 0x4160, bMaskDWord);
	temp3 = odm_get_bb_regbu(dm, 0x9b4, bMaskDWord);

	odm_set_bb_regbu(dm, 0x9b4, bMaskDWord, 0xdb66db00);

	//pathA
	odm_set_bb_regbu(dm, 0x1830, BIT(30), 0x0);
	odm_set_bb_regbu(dm, 0x1860, 0xfc000000, 0x3c);

	RTW_INFO("path A i\n");
	//i
	for (i = 0; i < 0xf; i++) {
		odm_set_bb_regbu(dm, 0x18b0, 0xf0000000, i);
		RTW_INFO("[0][0][%d] = 0x%08x\n", i, (u16)odm_get_bb_regbu(dm,0x2810,0x7fc0000));
		//pIQK_info->msbk_d[0][0][i] = (u16)odm_get_bb_regbu(dm,0x2810,0x7fc0000);
	}
	RTW_INFO("path A q\n");
	//q
	for (i = 0; i < 0xf; i++) {
		odm_set_bb_regbu(dm, 0x18cc, 0xf0000000, i);
		RTW_INFO("[0][1][%d] = 0x%08x\n", i, (u16)odm_get_bb_regbu(dm,0x283c,0x7fc0000));
		//pIQK_info->msbk_d[0][1][i] = (u16)odm_get_bb_regbu(dm,0x283c,0x7fc0000);
	}
	//pathB
	odm_set_bb_regbu(dm, 0x4130, BIT(30), 0x0);
	odm_set_bb_regbu(dm, 0x4160, 0xfc000000, 0x3c);

	RTW_INFO("\npath B i\n");
	//i
	for (i = 0; i < 0xf; i++) {
		odm_set_bb_regbu(dm, 0x41b0, 0xf0000000, i);
		RTW_INFO("[1][0][%d] = 0x%08x\n", i, (u16)odm_get_bb_regbu(dm,0x4510,0x7fc0000));
		//pIQK_info->msbk_d[1][0][i] = (u16)odm_get_bb_regbu(dm,0x2810,0x7fc0000);
	}
	RTW_INFO("path B q\n");
	//q
	for (i = 0; i < 0xf; i++) {
		odm_set_bb_regbu(dm, 0x41cc, 0xf0000000, i);
		RTW_INFO("[1][1][%d] = 0x%08x\n", i, (u16)odm_get_bb_regbu(dm,0x453c,0x7fc0000));
		//pIQK_info->msbk_d[1][1][i] = (u16)odm_get_bb_regbu(dm,0x283c,0x7fc0000);
	}

	//restore to normal
	odm_set_bb_regbu(dm, 0x1830, BIT(30), 0x1);
	odm_set_bb_regbu(dm, 0x4130, BIT(30), 0x1);
	odm_set_bb_regbu(dm, 0x1860, bMaskDWord, temp1);
	odm_set_bb_regbu(dm, 0x4160, bMaskDWord, temp2);
	odm_set_bb_regbu(dm, 0x9b4, bMaskDWord, temp3);
...
)
{
	...
	odm_set_bb_regbu(dm, 0x1b00, MASKDWORD, 0x8 | path << 1);

	if (idx == TX_IQK) {//TXCFIR
		odm_set_bb_regbu(dm, R_0x1b20, BIT(31) | BIT(30), 0x3);
	} else {//RXCFIR
		odm_set_bb_regbu(dm, R_0x1b20, BIT(31) | BIT(30), 0x1);		
	}
	odm_set_bb_regbu(dm, R_0x1bd4, BIT(21), 0x1);
	odm_set_bb_regbu(dm, R_0x1bd4, bit_mask_20_16, 0x10);
	for (i = 0; i <= 16; i++) {
		odm_set_bb_regbu(dm, R_0x1bd8, MASKDWORD, 0xe0000001 | i << 2);
	...
	odm_set_bb_regbu(dm, R_0x1b20, BIT(31) | BIT(30), 0x0);
	...
	struct dm_struct *dm)
{
	...
	if (odm_get_bb_regbu(dm, R_0x1e7c, BIT(30)))
		dpk_info->is_tssi_mode = true;
	else
		dpk_info->is_tssi_mode = false;

	reg_rf18 = odm_get_rf_regbu(dm, RF_PATH_A, RF_0x18, RFREG_MASK);
...
{	
	...
	for (path = 0; path < DPK_RF_PATH_NUM_8822C; path++) {
		...
		odm_set_bb_regbu(dm, R_0x1b00, MASKDWORD, 0x8 | (path << 1));

		 /*txagc bnd*/
		if (dpk_info->dpk_band == 0x0)
			u32tmp = odm_get_bb_regbu(dm, R_0x1b60, MASKDWORD);
		else
			u32tmp = odm_get_bb_regbu(dm, R_0x1b60, MASKDWORD);

 		RTW_INFO("[DPK] txagc bnd = 0x%08x\n", u32tmp);

		u32tmp = odm_get_bb_regbu(dm, R_0x1b64, MASKBYTE3);
		...
		odm_set_bb_regbu(dm, R_0x1b00, 0x0000000f, 0xc);

		if (path == RF_PATH_A)
			u32tmp = odm_get_bb_regbu(dm, R_0x1b04, 0x0fffffff);
		else 
			u32tmp = odm_get_bb_regbu(dm, R_0x1b5c, 0x0fffffff);
