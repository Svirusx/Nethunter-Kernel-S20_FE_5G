...
struct mlme_handler mlme_sta_tblbu[] = {
	{WIFI_ASSOCREQ,		"OnAssocReqbu",	&OnAssocReqbu},
	{WIFI_ASSOCRSP,		"OnAssocRspbu",	&OnAssocRspbu},
	{WIFI_REASSOCREQ,	"OnReAssocReq",	&OnAssocReqbu},
	{WIFI_REASSOCRSP,	"OnReAssocRsp",	&OnAssocRspbu},
	{WIFI_PROBEREQ,		"OnProbeReqbu",	&OnProbeReqbu},
	{WIFI_PROBERSP,		"OnProbeRspbu",		&OnProbeRspbu},

	/*----------------------------------------------------------
					below 2 are reserved
	-----------------------------------------------------------*/
	{0,					"DoReservedbu",		&DoReservedbu},
	{0,					"DoReservedbu",		&DoReservedbu},
	{WIFI_BEACON,		"OnBeaconbu",		&OnBeaconbu},
	{WIFI_ATIM,			"OnATIM",		&OnAtimbu},
	{WIFI_DISASSOC,		"OnDisassocbu",		&OnDisassocbu},
	{WIFI_AUTH,			"OnAuthbu",		&OnAuthbuClientbu},
	{WIFI_DEAUTH,		"OnDeAuthbu",		&OnDeAuthbu},
	{WIFI_ACTION,		"OnActionbu",		&OnActionbu},
	{WIFI_ACTION_NOACK, "OnActionbuNoAck",	&OnActionbu},
};
...
struct action_handler OnActionbu_tbl[] = {
	{RTW_WLAN_CATEGORY_SPECTRUM_MGMT,	 "ACTION_SPECTRUM_MGMT", on_action_spctbu},
	{RTW_WLAN_CATEGORY_QOS, "ACTION_QOS", &OnActionbu_qos},
	{RTW_WLAN_CATEGORY_DLS, "ACTION_DLS", &OnActionbu_dls},
	{RTW_WLAN_CATEGORY_BACK, "ACTION_BACK", &OnActionbu_back},
	{RTW_WLAN_CATEGORY_PUBLIC, "ACTION_PUBLIC", on_action_publicbu},
	{RTW_WLAN_CATEGORY_RADIO_MEAS, "ACTION_RADIO_MEAS", &on_action_rmbu},
	{RTW_WLAN_CATEGORY_FT, "ACTION_FT",	&OnActionbu_ft},
	{RTW_WLAN_CATEGORY_HT,	"ACTION_HT",	&OnActionbu_ht},
#ifdef CONFIG_IEEE80211W
	{RTW_WLAN_CATEGORY_SA_QUERY, "ACTION_SA_QUERY", &OnActionbu_sa_query},
#else
	{RTW_WLAN_CATEGORY_SA_QUERY, "ACTION_SA_QUERY", &DoReservedbu},
#endif /* CONFIG_IEEE80211W */
#ifdef CONFIG_RTW_WNM
	{RTW_WLAN_CATEGORY_WNM, "ACTION_WNM", &on_action_wnm},
#endif
	{RTW_WLAN_CATEGORY_UNPROTECTED_WNM, "ACTION_UNPROTECTED_WNM", &DoReservedbu},
...
#endif
	{RTW_WLAN_CATEGORY_WMM, "ACTION_WMM", &OnActionbu_wmm},
	{RTW_WLAN_CATEGORY_VHT, "ACTION_VHT", &OnActionbu_vht},
	{RTW_WLAN_CATEGORY_P2P, "ACTION_P2P", &OnActionbu_p2p},
#ifdef CONFIG_RTW_TOKEN_BASED_XMIT
	{RTW_WLAN_CATEGORY_TBTX, "ACTION_TBTX_TOKEN", &OnActionbu_tbtx_token}
#endif
};


u8	null_addrbu[ETH_ALEN] = {0, 0, 0, 0, 0, 0};

/**************************************************
OUI definitions for the vendor specific IE
***************************************************/
unsigned char	RTW_WPA_OUIbu[] = {0x00, 0x50, 0xf2, 0x01};
unsigned char WMM_OUIbu[] = {0x00, 0x50, 0xf2, 0x02};
unsigned char	WPS_OUIbu[] = {0x00, 0x50, 0xf2, 0x04};
unsigned char	P2P_OUIbu[] = {0x50, 0x6F, 0x9A, 0x09};
unsigned char	WFD_OUIbu[] = {0x50, 0x6F, 0x9A, 0x0A};
unsigned char	DPP_OUIbu[] = {0x50, 0x6F, 0x9A, 0x1A};
unsigned char	MULTI_AP_OUI[] = {0x50, 0x6F, 0x9A, 0x1B};

unsigned char	WMM_INFO_OUIbu[] = {0x00, 0x50, 0xf2, 0x02, 0x00, 0x01};
unsigned char	WMM_PARA_OUIbu[] = {0x00, 0x50, 0xf2, 0x02, 0x01, 0x01};

unsigned char WPA_TKIP_CIPHERbu[4] = {0x00, 0x50, 0xf2, 0x02};
unsigned char RSN_TKIP_CIPHERbu[4] = {0x00, 0x0f, 0xac, 0x02};

#ifdef CONFIG_RTW_TOKEN_BASED_XMIT
unsigned char REALTEK_TBTX_IE[] = {0x00, 0xe0, 0x4c, 0x01, 0x00, 0x00, 0x00, 0x00};
#endif
extern unsigned char REALTEK_96B_IEbu[];
	...
	, struct p2p_channels *channel_list)
{
	...
	for (op = 0; op_class[op].op_class; op++) {
		...
		for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
			if (rtw_chset_search_chbu(channel_set, ch) == -1)
				continue;
...
void rtw_txpwr_init_regdbu(struct rf_ctl_t *rfctl)
{
	...
	exc = _rtw_regd_exc_searchbubu(rfctl
	...
	if (exc) {
		...
		if (strcmp(exc->regd_name, regd_str(TXPWR_LMT_NONE)) == 0)
			...
			ent = _rtw_txpwr_lmt_get_by_namebubu(rfctl, exc->regd_name);
			if (ent)
	...
	if (rfctl->regd_src == REGD_SRC_OS) {
		if (IS_ALPHA2_WORLDWIDE(rfctl->country_ent->alpha2))
			...
			ent = _rtw_txpwr_lmt_get_by_namebubu(rfctl, alpha2);
			if (ent)
				rfctl->regd_name = ent->regd_name;
		}
	...
	regd = rtw_chplan_get_default_regdbu(rfctl->ChannelPlan);
	if (regd == TXPWR_LMT_NONE)
		rfctl->regd_name = regd_str(TXPWR_LMT_NONE);
	else if (regd == TXPWR_LMT_WW)
		rfctl->regd_name = regd_str(TXPWR_LMT_WW);
	else {
		ent = _rtw_txpwr_lmt_get_by_namebubu(rfctl, regd_str(regd));
		if (ent)
			rfctl->regd_name = ent->regd_name;
	}
...
int rtw_rfctl_initbu(_adapter *adapter)
{
	...
	_rtw_mutex_initbu(&rfctl->offch_mutex);

#if CONFIG_TXPWR_LIMIT
	_rtw_mutex_initbu(&rfctl->txpwr_lmt_mutex);
	_rtw_init_listheadbu(&rfctl->reg_exc_list);
	_rtw_init_listheadbu(&rfctl->txpwr_lmt_list);
#endif
	...
	rtw_init_timerbu(&(rfctl->radar_detect_timer), adapter, rtw_dfs_rd_timer_hdl, rfctl);
#endif
...
void rtw_rfctl_deinitbu(_adapter *adapter)
{
	...
	_rtw_mutex_freebu(&rfctl->offch_mutex);

#if CONFIG_TXPWR_LIMIT
	rtw_regd_exc_list_freebu(rfctl);
	rtw_txpwr_lmt_list_freebu(rfctl);
	_rtw_mutex_freebu(&rfctl->txpwr_lmt_mutex);
#endif
...
void rtw_rfctl_chplan_init(_adapter *adapter)
{
	...
	rfctl->max_chan_nums = init_channel_setbu(adapter);
...
void rtw_rfctl_update_op_mode(struct rf_ctl_t *rfctl, u8 ifbmp_mod, u8 if_op)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (!u_ch) {
			...
			rtw_warn_on(!rtw_is_chbw_groupedbu(u_ch, u_bw, u_offset, ch, bw, offset));
			rtw_sync_chbwbu(&ch, &bw, &offset, &u_ch, &u_bw, &u_offset);
		}
	}
	...
	if (op_class != rfctl->op_class
		...
		|| _rtw_memcmpbu(if_op_class, rfctl->if_op_class, sizeof(u8) * CONFIG_IFACE_NUMBER) == _FALSE
		|| _rtw_memcmpbu(if_op_ch, rfctl->if_op_ch, sizeof(u8) * CONFIG_IFACE_NUMBER) == _FALSE)
		notify = 1;

	rfctl->op_class = op_class;
	rfctl->op_ch = op_ch;
	rfctl->op_txpwr_max = op_txpwr_max;
	_rtw_memcpybu(rfctl->if_op_class, if_op_class, sizeof(u8) * CONFIG_IFACE_NUMBER);
	_rtw_memcpybu(rfctl->if_op_ch, if_op_ch, sizeof(u8) * CONFIG_IFACE_NUMBER);
...
bool rtw_is_cac_reset_needed(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset)
{
	...
	if (rtw_chbw_to_freq_rangebu(rfctl->radar_detect_ch, rfctl->radar_detect_bw, rfctl->radar_detect_offset, &cur_hi, &cur_lo) == _FALSE) {
...
bool _rtw_rfctl_overlap_radar_detect_ch(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset)
{
	...
	if (rtw_chbw_to_freq_rangebu(rfctl->radar_detect_ch
...
bool rtw_chset_is_chbw_non_ocp(RT_CHANNEL_INFO *ch_set, u8 ch, u8 bw, u8 offset)
{
	...
	for (i = 0; i < MAX_CHANNEL_NUM && ch_set[i].ChannelNum != 0; i++) {
		if (!rtw_ch2freqbu(ch_set[i].ChannelNum)) {
			rtw_warn_on(1);
			continue;
		}
		...
		if (lo <= rtw_ch2freqbu(ch_set[i].ChannelNum)
			&& rtw_ch2freqbu(ch_set[i].ChannelNum) <= hi
		) {
			ret = _TRUE;
			break;
		}
...
static bool rtw_chset_chk_non_ocp_finish_for_chbw(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset)
{
	...
	for (i = 0; i < op_ch_num; i++) {
		...
		ch_idx = rtw_chset_search_chbu(ch_set, *(op_chs + i));
		if (ch_idx == -1)
			break;
...
u32 rtw_get_ch_waiting_ms(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset, u32 *r_non_ocp_ms, u32 *r_cac_ms)
{
	...
	if (!rtw_chset_is_dfs_chbw(rfctl->channel_set, ch, bw, offset))
		...
		if (IS_CH_WAITING(rfctl))
			...
			cac_ms = 0;
	} else if (rtw_is_long_cac_chbu(ch, bw, offset, rtw_rfctl_get_dfs_domain(rfctl)))
...
#define DBG_CHOOSE_SHORTEST_WAITING_CH 0
	...
	for (bw = CHANNEL_WIDTH_20; bw <= max_bw; bw++) {
		if (!hal_is_bw_supportbu(dvobj_get_primary_adapter(dvobj), bw))
			continue;
		...
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			...
			if (!rtw_chset_is_chbw_validbu(rfctl->channel_set, ch, bw, offset, 0, 0))
				continue;
			...
			if ((e_flags & RTW_CHF_LONG_CAC) || (d_flags & RTW_CHF_LONG_CAC)) {
				long_cac = rtw_is_long_cac_chbu(ch, bw, offset, rtw_rfctl_get_dfs_domain(rfctl));
...
void dump_chsetbu(void *sel, RT_CHANNEL_INFO *ch_set, u8 chset_num)
{
	...
	for (i = 0; i < MAX_CHANNEL_NUM && i < chset_num && ch_set[i].ChannelNum != 0; i++) {
		...
		RTW_PRINT_SEL(sel, "%3u %4u %4s"RTW_CHF_FMT"\n"
			, ch_set[i].ChannelNum, rtw_ch2freqbu(ch_set[i].ChannelNum), buf
...
void dump_cur_chsetbu(void *sel, struct rf_ctl_t *rfctl)
{
	...
	if (chplan->has_country)
		dump_country_chplanbu(sel, &chplan->country_ent);
	else
...
#ifdef CONFIG_DFS_MASTER
	RTW_PRINT_SEL(sel, "dfs_domain:%s(%u)\n", rtw_dfs_regd_strbu(chplan->dfs_domain), chplan->dfs_domain);
#endif
	...
	dump_chsetbu(sel, chplan->chset, chplan->chset_num);
...
/**
 * rtw_chset_sync_chbwbu - obey g_ch, adjust g_bw, g_offset, bw, offset to fit in channel plan
	...
	, u8 *g_ch, u8 *g_bw, u8 *g_offset, bool allow_primary_passive, bool allow_passive)
{
	...
	while (1) {
		...
		rtw_sync_chbwbu(&r_ch, &r_bw, &r_offset, &u_ch, &u_bw, &u_offset);

		if (rtw_chset_is_chbw_validbu(ch_set, r_ch, r_bw, r_offset, allow_primary_passive, allow_passive))
			break;
...
inline void RTW_SET_SCAN_BAND_SKIPbu(_adapter *padapter, int skip_band)
{
	int bs = ATOMIC_READbu(&padapter->bandskip);

	bs |= skip_band;
	ATOMIC_SETbu(&padapter->bandskip, bs);
}

inline void RTW_CLR_SCAN_BAND_SKIPbu(_adapter *padapter, int skip_band)
{
	int bs = ATOMIC_READbu(&padapter->bandskip);
...
inline int RTW_GET_SCAN_BAND_SKIPbu(_adapter *padapter)
{
	return ATOMIC_READbu(&padapter->bandskip);
}

#define RTW_IS_SCAN_BAND_SKIP(padapter, skip_band) (ATOMIC_READbu(&padapter->bandskip) & (skip_band))
...
int init_hw_mlme_extbu(_adapter *padapter)
{
	...
	rtw_hal_set_hwregbu(padapter, HW_VAR_ENABLE_RX_BAR, &rx_bar_enble);
	set_channel_bwmodebu(padapter, pmlmeext->cur_channel, pmlmeext->cur_ch_offset, pmlmeext->cur_bwmode);
...
void init_mlme_default_rate_setbu(_adapter *padapter)
{
	...
	if (IsSupportedTxCCK(padapter->registrypriv.wireless_mode)) {
		...
		_rtw_memcpybu(pmlmeext->datarate, datarate_b, B_MODE_RATE_NUM);
		_rtw_memcpybu(pmlmeext->basicrate, datarate_b, B_MODE_RATE_NUM);
	...
	if(IsSupportedTxOFDM(padapter->registrypriv.wireless_mode)) {
		...
		_rtw_memcpybu(pmlmeext->datarate + offset_datarate, datarate_g, G_MODE_RATE_NUM);
		_rtw_memcpybu(pmlmeext->basicrate + offset_basicrate,basicrate_g, G_MODE_BASIC_RATE_NUM);
		offset_datarate += G_MODE_RATE_NUM;
		offset_basicrate += G_MODE_BASIC_RATE_NUM;
		RTW_INFO("%s: support OFDM\n", __func__);

	}
	_rtw_memcpybu(pmlmeext->datarate + offset_datarate, end_set, 1);
	_rtw_memcpybu(pmlmeext->basicrate + offset_basicrate, end_set, 1);

#ifdef CONFIG_80211N_HT
	if( padapter->registrypriv.ht_enable && is_supported_ht(padapter->registrypriv.wireless_mode))
		_rtw_memcpybu(pmlmeext->default_supported_mcs_set, supported_mcs_set, sizeof(pmlmeext->default_supported_mcs_set));
#endif
}

static void init_mlme_ext_privbu_value(_adapter *padapter)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);

	ATOMIC_SETbu(&pmlmeext->event_seq, 0);
	...
	init_mlme_default_rate_setbu(padapter);
	...
	_rtw_memsetbu(pmlmeinfo->chg_txt, 0, 128);
...
/*infra mode, used to store AP's info*/
	_rtw_memsetbu(pmlmeinfo->SupportedRates_infra_ap, 0, NDIS_802_11_LENGTH_RATES_EX);
...
void init_mlme_ext_timerbu(_adapter *padapter)
{
	struct	mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;

	rtw_init_timerbu(&pmlmeext->survey_timer, padapter, survey_timer_hdlbu, padapter);
	rtw_init_timerbu(&pmlmeext->link_timer, padapter, link_timer_hdlbu, padapter);
#ifdef CONFIG_RTW_80211R
	rtw_init_timerbu(&pmlmeext->ft_link_timer, padapter, rtw_ft_link_timer_hdlbu, padapter);
	rtw_init_timerbu(&pmlmeext->ft_roam_timer, padapter, rtw_ft_roam_timer_hdl, padapter);
#endif

#ifdef CONFIG_RTW_REPEATER_SON
	rtw_init_timerbu(&pmlmeext->rson_scan_timer, padapter, rson_timer_hdl, padapter);
#endif
#ifdef CONFIG_RTW_TOKEN_BASED_XMIT
	rtw_init_timerbu(&pmlmeext->tbtx_xmit_timer, padapter, rtw_tbtx_xmit_timer_hdl, padapter);
	rtw_init_timerbu(&pmlmeext->tbtx_token_dispatch_timer, padapter, rtw_tbtx_token_dispatch_timer_hdl, padapter);
#endif
#ifdef CONFIG_DFS
	rtw_init_timerbu(&pmlmeext->csa_timer, padapter->pnetdev, csa_timer_hdl, padapter);
#endif
}
...
int	init_mlme_ext_privbu(_adapter *padapter)
{
	...
	/* _rtw_memsetbu((u8 *)pmlmeext, 0, sizeof(struct mlme_ext_priv)); */

	pmlmeext->padapter = padapter;

	/* fill_fwpriv(padapter, &(pmlmeext->fwpriv)); */

	init_mlme_ext_privbu_value(padapter);
	pmlmeinfo->bAcceptAddbaReq = pregistrypriv->bAcceptAddbaReq;

	init_mlme_ext_timerbu(padapter);

#ifdef CONFIG_AP_MODE
	init_mlme_ap_infobu(padapter);
#endif
...
static u8 cmp_pkt_chnl_diff(_adapter *padapter, u8 *pframe, uint packet_len)
{
	...
	p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, _DSSET_IE_, &len, packet_len - _BEACON_IE_OFFSET_);
	if (p) {
		channel = *(p + 2);
...
static void _mgt_dispatcherbu(_adapter *padapter, struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	...
	if (ptable->func) {
		...
		if (!_rtw_memcmpbu(GetAddr1Ptr(pframe), adapter_mac_addr(padapter), ETH_ALEN) &&
		    !_rtw_memcmpbu(GetAddr1Ptr(pframe), bc_addr, ETH_ALEN))
#if defined(CONFIG_RTW_CFGVENDOR_RANDOM_MAC_OUI) || defined(CONFIG_RTW_SCAN_RAND)
		{
		    ...
		    if (!_rtw_memcmpbu(GetAddr1Ptr(pframe), adapter_pno_mac_addr(padapter), ETH_ALEN))
				return;
		}
#else
			return;
...
void mgt_dispatcherbu(_adapter *padapter, union recv_frame *precv_frame)
{
	int index;
	struct mlme_handler *ptable;
	u8 bc_addr[ETH_ALEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	u8 *pframe = precv_frame->u.hdr.rx_data;
	struct sta_info *psta = rtw_get_stainfobu(&padapter->stapriv, get_addr2_ptr(pframe));
	...
	if (index >= (sizeof(mlme_sta_tblbu) / sizeof(struct mlme_handler))) {
		return;
	}
	ptable += index;
	...
	case WIFI_AUTH:
		if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter))
			ptable->func = &OnAuthbu;
		else
			ptable->func = &OnAuthbuClientbu;
	/* fall through */
	case WIFI_ASSOCREQ:
	case WIFI_REASSOCREQ:
		_mgt_dispatcherbu(padapter, ptable, precv_frame);
...
#ifdef CONFIG_P2P
u32 p2p_listen_state_processbu(_adapter *padapter, unsigned char *da)
{
	bool response = _TRUE;

#ifdef CONFIG_IOCTL_CFG80211
	if (padapter->wdinfo.driver_interface == DRIVER_CFG80211) {
		if (rtw_cfg80211_get_is_rochbu(padapter) == _FALSE
			|| rtw_get_oper_chbu(padapter) != padapter->wdinfo.listen_channel
			|| adapter_wdev_data(padapter)->p2p_enabled == _FALSE
			|| padapter->mlmepriv.wps_probe_resp_ie == NULL
			|| padapter->mlmepriv.p2p_probe_resp_ie == NULL
		) {
#ifdef CONFIG_DEBUG_CFG80211
			RTW_INFO(ADPT_FMT" DON'T issue_proberspbu_p2p: p2p_enabled:%d, wps_probe_resp_ie:%p, p2p_probe_resp_ie:%p\n"
				, ADPT_ARG(padapter)
				, adapter_wdev_data(padapter)->p2p_enabled
				, padapter->mlmepriv.wps_probe_resp_ie
				, padapter->mlmepriv.p2p_probe_resp_ie);
			RTW_INFO(ADPT_FMT" DON'T issue_proberspbu_p2p: is_ro_ch:%d, op_ch:%d, p2p_listen_channel:%d\n"
				, ADPT_ARG(padapter)
				, rtw_cfg80211_get_is_rochbu(padapter)
				, rtw_get_oper_chbu(padapter)
	...
	if (response == _TRUE)
		issue_proberspbu_p2p(padapter, da);
...
unsigned int OnProbeReqbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if ((pwdinfo->driver_interface == DRIVER_CFG80211)
		...
		rtw_cfg80211_rx_probe_requestbu(padapter, precv_frame);
		return _SUCCESS;
	}
#endif /* CONFIG_IOCTL_CFG80211 */
	...
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) &&
		...
		if (wifi_test_chk_rate == 1) {
			is_valid_p2p_probereq = process_probe_req_p2p_iebu(pwdinfo, pframe, len);
			if (is_valid_p2p_probereq == _TRUE) {
				if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE)) {
					/* FIXME */
					if (padapter->wdinfo.driver_interface == DRIVER_WEXT)
						report_survey_eventbu(padapter, precv_frame);

					p2p_listen_state_processbu(padapter,  get_sa(pframe));
...
_continue:
	...
	/* RTW_INFO("+OnProbeReqbu\n"); */


#ifdef CONFIG_ATMEL_RC_PATCH
	wps_ie = rtw_get_wps_iebu(
			      pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
			      len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_,
			      NULL, &wps_ielen);
	if (wps_ie)
		target_ie = rtw_get_wps_attrbu_content(wps_ie, wps_ielen, WPS_ATTR_MANUFACTURER, NULL, &target_ielen);
	if ((target_ie && (target_ielen == 4)) && (_TRUE == _rtw_memcmpbu((void *)target_ie, "Ozmo", 4))) {
		...
		_rtw_memcpybu(pstapriv->atmel_rc_pattern, get_sa(pframe), ETH_ALEN);
	}
#endif
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, (int *)&ielen,
			       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);

		if (!p || ielen != 14)
			goto _non_rc_device;

		if (!_rtw_memcmpbu(p + 2, RC_OUI, sizeof(RC_OUI)))
			goto _non_rc_device;

		if (!_rtw_memcmpbu(p + 6, get_sa(pframe), ETH_ALEN)) {
		...
		/* new a station */
		psta = rtw_get_stainfobu(pstapriv, get_sa(pframe));
		if (psta == NULL) {
			/* allocate a new one */
			RTW_INFO("going to alloc stainfo for rc="MAC_FMT"\n",  MAC_ARG(get_sa(pframe)));
			psta = rtw_alloc_stainfobu(pstapriv, get_sa(pframe));
			if (psta == NULL) {
				/* TODO: */
			...
			if (rtw_is_list_emptybu(&psta->asoc_list)) {
				psta->expire_to = pstapriv->expire_to;
				rtw_list_insert_tailbu(&psta->asoc_list, &pstapriv->asoc_list);
			...
			if (psta->cmn.aid > 0)
				...
				if (!rtw_aid_allocbu(padapter, psta)) {
			...
			rtw_hal_set_odm_varbu(padapter, HAL_ODM_STA_INFO, psta, _TRUE);

			_rtw_memsetbu((void *)&psta->sta_stats, 0, sizeof(struct stainfo_stats));

			_enter_critical_bh(&psta->lock, &irqL);
			psta->state |= WIFI_ASOC_STATE;
			_exit_critical_bh(&psta->lock, &irqL);

			report_add_sta_eventbu(padapter, psta->cmn.mac_addr);

		}

		issue_proberspbu(padapter, get_sa(pframe), _FALSE);
...
_non_rc_device:
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) &&
	    rtw_mi_buddy_check_fwstatebu(padapter, WIFI_UNDER_LINKING | WIFI_UNDER_SURVEY)) {
		/* don't process probe req */
		return _SUCCESS;
	}
#endif

	p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SSID_IE_, (int *)&ielen,
		       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
	...
	if (p != NULL) {
		if (is_valid_p2p_probereq == _TRUE)
			goto _issue_proberspbu;

		if ((ielen != 0 && _FALSE == _rtw_memcmpbu((void *)(p + 2), (void *)cur->Ssid.Ssid, cur->Ssid.SsidLength))
			|| (ielen == 0 && pmlmeinfo->hidden_ssid_mode))
			goto exit;

		#ifdef CONFIG_RTW_MESH
		if (MLME_IS_MESH(padapter)) {
			p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, WLAN_EID_MESH_ID, (int *)&ielen,
					len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);

			if (!p)
				goto exit;
			if (ielen != 0 && _rtw_memcmpbu((void *)(p + 2), (void *)cur->mesh_id.Ssid, cur->mesh_id.SsidLength) == _FALSE)
				goto exit;
		}
		#endif
...
_issue_proberspbu:
		if (((check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
			...
			/* RTW_INFO("+issue_proberspbu during ap mode\n"); */
			issue_proberspbu(padapter, get_sa(pframe), is_valid_p2p_probereq);
		}
...
unsigned int OnProbeRspbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ)) {
		if (_TRUE == pwdinfo->tx_prov_disc_info.benable) {
			if (_rtw_memcmpbu(pwdinfo->tx_prov_disc_info.peerIFAddr, get_addr2_ptr(pframe), ETH_ALEN)) {
				if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT)) {
					pwdinfo->tx_prov_disc_info.benable = _FALSE;
					issue_p2p_provision_requestbu(padapter,
		...
		if (_TRUE == pwdinfo->nego_req_info.benable) {
			...
			if (_rtw_memcmpbu(pwdinfo->nego_req_info.peerDevAddr, get_addr2_ptr(pframe), ETH_ALEN)) {
				pwdinfo->nego_req_info.benable = _FALSE;
				issue_p2p_GO_requestbu(padapter, pwdinfo->nego_req_info.peerDevAddr);
			}
		}
	} else if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_INVITE_REQ)) {
		if (_TRUE == pwdinfo->invitereq_info.benable) {
			RTW_INFO("[%s] P2P_STATE_TX_INVITE_REQ!\n", __FUNCTION__);
			if (_rtw_memcmpbu(pwdinfo->invitereq_info.peer_macaddr, get_addr2_ptr(pframe), ETH_ALEN)) {
				pwdinfo->invitereq_info.benable = _FALSE;
				issue_p2p_invitation_requestbu(padapter, pwdinfo->invitereq_info.peer_macaddr);
			}
		}
	}
#endif
	...
	if ((mlmeext_chk_scan_state(pmlmeext, SCAN_PROCESS))
		...
		if (_rtw_memcmpbu(GetAddr3Ptr(pframe), get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN)
			&& (pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)
		) {
			if (!rtw_check_bcn_infobu(padapter, pframe, precv_frame->u.hdr.len)) {
				RTW_PRINT(FUNC_ADPT_FMT" ap has changed, disconnect now\n", FUNC_ADPT_ARG(padapter));
				receive_disconnectbu(padapter, pmlmeinfo->network.MacAddress , 0, _FALSE);
			}
		}

		rtw_mi_report_survey_eventbubu(padapter, precv_frame);
		return _SUCCESS;
	}

#if 0 /* move to validate_recv_mgnt_framebu */
	if (_rtw_memcmpbu(GetAddr3Ptr(pframe), get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN)) {
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
			psta = rtw_get_stainfobu(pstapriv, get_addr2_ptr(pframe));
			if (psta != NULL)
...
static void rtw_check_legacy_ap(_adapter *padapter, u8 *pframe, u32 len)
{
	...
	if (pmlmeext->bstart_bss == _TRUE) {
		...
		if (rtw_ieee802_11_parse_elemsbu(pos, left, &elems, 1) == ParseFailed) {
		...
		if (elems.ht_capabilities == NULL && elems.ht_capabilities_len == 0) {
			...
			ATOMIC_SETbu(&pmlmepriv->olbc, _TRUE);
			ATOMIC_SETbu(&pmlmepriv->olbc_ht, _TRUE);
		}
	}
}
...
unsigned int OnBeaconbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (validate_beacon_lenbu(pframe, len) == _FALSE)
		return _SUCCESS;
	...
	if (mlmeext_chk_scan_state(pmlmeext, SCAN_PROCESS)
		...
		if (_rtw_memcmpbu(GetAddr3Ptr(pframe), get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN)
			...
			if (!rtw_check_bcn_infobu(padapter, pframe, len)) {
				RTW_PRINT(FUNC_ADPT_FMT" ap has changed, disconnect now\n", FUNC_ADPT_ARG(padapter));
				receive_disconnectbu(padapter, pmlmeinfo->network.MacAddress , 0, _FALSE);
			}
		}
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE)
		&& (_rtw_memcmpbu(GetAddr3Ptr(pframe), get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN))) {
		struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	...
	if (_rtw_memcmpbu(GetAddr3Ptr(pframe), get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN)) {
		if ((pmlmeinfo->state & WIFI_FW_AUTH_NULL)
			&& (rtw_sta_linking_test_wait_donebu() || pmlmeext->join_abort)
		) {
			if (rtw_sta_linking_test_force_failbu() || pmlmeext->join_abort) {
			...
			if (pbss) {
				if (collect_bss_infobu(padapter, precv_frame, pbss) == _SUCCESS) {
					struct beacon_keys recv_beacon;

					update_networkbu(&(pmlmepriv->cur_network.network), pbss, padapter, _TRUE);

					/* update bcn keys */
					if (rtw_get_bcn_keysbu(padapter, pframe, len, &recv_beacon) == _TRUE) {
						RTW_INFO("%s: beacon keys ready\n", __func__);
						_rtw_memcpybu(&pmlmepriv->cur_beacon_keys,
							&recv_beacon, sizeof(recv_beacon));
						if (is_hidden_ssid(recv_beacon.ssid, recv_beacon.ssid_len)) {
							_rtw_memcpybu(pmlmepriv->cur_beacon_keys.ssid, pmlmeinfo->network.Ssid.Ssid, IW_ESSID_MAX_SIZE);
							pmlmepriv->cur_beacon_keys.ssid_len = pmlmeinfo->network.Ssid.SsidLength;
						}
					} else {
						RTW_ERR("%s: get beacon keys failed\n", __func__);
						_rtw_memsetbu(&pmlmepriv->cur_beacon_keys, 0, sizeof(recv_beacon));
					}
			...
			pmlmeinfo->assoc_AP_vendor = check_assoc_APbu(pframe + sizeof(struct rtw_ieee80211_hdr_3addr), len - sizeof(struct rtw_ieee80211_hdr_3addr));

			/* update TSF Value */
			update_TSFbu(pmlmeext, pframe, len);
			pmlmeext->bcn_cnt = 0;
			...
			/* process_p2p_ps_iebu(padapter, (pframe + WLAN_HDR_A3_LEN), (len - WLAN_HDR_A3_LEN)); */
#endif /* CONFIG_P2P_PS */

#if defined(CONFIG_P2P) && defined(CONFIG_CONCURRENT_MODE)
			if (padapter->registrypriv.wifi_spec) {
				if (process_p2p_cross_connect_iebu(padapter, (pframe + WLAN_HDR_A3_LEN), (len - WLAN_HDR_A3_LEN)) == _FALSE) {
					if (rtw_mi_buddy_check_mlmeinfo_statebu(padapter, WIFI_FW_AP_STATE)) {
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				if (cmp_pkt_chnl_diff(padapter, pframe, len) != 0) {
					...
					issue_deauthbu(padapter, (&(pmlmeinfo->network))->MacAddress, WLAN_REASON_DEAUTH_LEAVING);

					report_del_sta_eventbu(padapter, (&(pmlmeinfo->network))->MacAddress, WLAN_REASON_JOIN_WRONG_CHANNEL, _TRUE, _FALSE);
...
#endif
				ret = rtw_check_bcn_infobu(padapter, pframe, len);
				if (!ret) {
					RTW_PRINT(FUNC_ADPT_FMT" ap has changed, disconnect now\n", FUNC_ADPT_ARG(padapter));
					receive_disconnectbu(padapter, pmlmeinfo->network.MacAddress , 0, _FALSE);
					return _SUCCESS;
				...
				if ((sta_rx_pkts(psta) & 0xf) == 0) {
					...
					update_beacon_infobu(padapter, pframe, len, psta);
				}
				...
				if (rtw_tdls_is_chsw_allowed(padapter) == _TRUE) {
					...
					if ((padapter->tdlsinfo.chsw_info.ch_sw_state & TDLS_CH_SW_INITIATOR_STATE) && (ATOMIC_READbu(&pchsw_info->chsw_on) == _TRUE)
					    && (pmlmeext->cur_channel == rtw_get_oper_chbu(padapter))) {
						ptdls_sta = rtw_get_stainfobu(&padapter->stapriv, padapter->tdlsinfo.chsw_info.addr);
						if (ptdls_sta != NULL) {
				...
				if (pmlmeext->tsf_update_required && pmlmeext->en_hw_update_tsf)
					rtw_enable_hw_update_tsf_cmdbu(padapter);
			...
			if (psta != NULL) {
				...
				*/
				...
				rtw_ies_get_supported_ratebu(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, len - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_, rate_set, &rate_num);
				if (rate_num == 0) {
					RTW_INFO(FUNC_ADPT_FMT" RX beacon with no supported rate\n", FUNC_ADPT_ARG(padapter));
					goto _END_ONBEACON_;
				}

				psta = rtw_alloc_stainfobu(pstapriv, get_addr2_ptr(pframe));
				if (psta == NULL) {
				...
				_rtw_memcpybu(psta->bssrateset, rate_set, rate_num);
				...
				report_add_sta_eventbu(padapter, get_addr2_ptr(pframe));
			}
		}
	}
...
static u32 rtw_get_sta_num_by_state(_adapter *padapter, u32 state)
{
	...
	for (index = 0; index < NUM_STA; index++) {		
		...
		while ((rtw_end_of_queue_searchbu(phead, plist)) == _FALSE) {
...
unsigned int OnAuthbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (GetPrivacy(pframe)) {
		...
		rtw_wep_decryptbu(padapter, (u8 *)precv_frame);
	...
	if (rtw_check_invalid_mac_addressbu(sa, _FALSE)){
	...
	if(rtw_defs_attack_chk(padapter))  {
		...
		_psta = rtw_get_stainfobu(pstapriv, sa);
	...
	if (rtw_ap_linking_test_force_auth_failbu()) {
		status = rtw_ap_linking_test_force_auth_failbu();
	...
	if (rtw_access_ctrlbu(padapter, sa) == _FALSE) {
		status = _STATS_UNABLE_HANDLE_STA_;
		goto auth_fail;
	}
#endif

	pstat = rtw_get_stainfobu(pstapriv, sa);
	if (pstat == NULL) {

		/* allocate a new one */
		RTW_INFO("going to alloc stainfo for sa="MAC_FMT"\n",  MAC_ARG(sa));
		pstat = rtw_alloc_stainfobu(pstapriv, sa);
		if (pstat == NULL) {
			RTW_INFO(" Exceed the upper limit of supported clients...\n");
			status = _STATS_UNABLE_HANDLE_STA_;
			goto auth_fail;
...
#endif /* CONFIG_IEEE80211W */
		{
			...
			_enter_critical_bh(&pstapriv->asoc_list_lock, &irqL);
			if (rtw_is_list_emptybu(&pstat->asoc_list) == _FALSE) {
...
#endif /* CONFIG_IEEE80211W */
	{
		...
		if (rtw_is_list_emptybu(&pstat->auth_list)) {

			rtw_list_insert_tailbu(&pstat->auth_list, &pstapriv->auth_list);
	...
	if (GET_CFG80211_REPORT_MGMT(adapter_wdev_data(padapter), IEEE80211_STYPE_AUTH) == _TRUE) {
		if ((algorithm == WLAN_AUTH_SAE) &&
			...
			rtw_cfg80211_rx_mframebu(padapter, precv_frame, NULL);
			return _SUCCESS;
		}
	}
#endif /* CONFIG_IOCTL_CFG80211 */
	...
	if (algorithm == 0 && (auth_mode == 0 || auth_mode == 2 || auth_mode == 3)) {
		...
		if (seq == 1) {
			...
			_rtw_memsetbu((void *)pstat->chg_txt, 78, 128);
#ifdef CONFIG_IEEE80211W
			if ((pstat->bpairwise_key_installed != _TRUE && (pstat->flags & WLAN_STA_MFP)) 
				|| !(pstat->flags & WLAN_STA_MFP))
#endif /* CONFIG_IEEE80211W */
			...
			p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + 4 + _AUTH_IE_OFFSET_ , _CHLGETXT_IE_, (int *)&ie_len,
			...
			if (_rtw_memcmpbu((void *)(p + 2), pstat->chg_txt, 128)) {
#ifdef CONFIG_IEEE80211W
				if ((pstat->bpairwise_key_installed != _TRUE && (pstat->flags & WLAN_STA_MFP)) 
					|| !(pstat->flags & WLAN_STA_MFP))
#endif /* CONFIG_IEEE80211W */
	...
	/* Now, we are going to issue_authbu... */
	pstat->auth_seq = seq + 1;

#ifdef CONFIG_NATIVEAP_MLME
	issue_authbu(padapter, pstat, (unsigned short)(_STATS_SUCCESSFUL_));
#endif
...
auth_fail:
	...
	if (pstat)
		rtw_free_stainfobu(padapter , pstat);

	pstat = &stat;
	_rtw_memsetbu((char *)pstat, '\0', sizeof(stat));
	pstat->auth_seq = 2;
	_rtw_memcpybu(pstat->cmn.mac_addr, sa, ETH_ALEN);

#ifdef CONFIG_NATIVEAP_MLME
	issue_authbu(padapter, pstat, (unsigned short)status);
#endif
...
unsigned int OnAuthbuClientbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (GET_CFG80211_REPORT_MGMT(adapter_wdev_data(padapter), IEEE80211_STYPE_AUTH) == _TRUE) {
		if (rtw_sec_chk_auth_type(padapter, MLME_AUTHTYPE_SAE)) {
			if (rtw_cached_pmkidbu(padapter, get_my_bssidbu(&pmlmeinfo->network)) != -1) {
...
normal:
	...
	if (!_rtw_memcmpbu(adapter_mac_addr(padapter), get_da(pframe), ETH_ALEN))
		return _SUCCESS;
	...
	if (seq == 2) {
		if (pmlmeinfo->auth_algo == dot11AuthAlgrthm_Shared) {
			...
			p = rtw_get_iebu(pframe + WLAN_HDR_A3_LEN + _AUTH_IE_OFFSET_, _CHLGETXT_IE_, (int *)&len,
			...
			_rtw_memcpybu((void *)(pmlmeinfo->chg_txt), (void *)(p + 2), len);
			pmlmeinfo->auth_seq = 3;
			issue_authbu(padapter, NULL, 0);
...
unsigned int OnAssocReqbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_check_invalid_mac_addressbu(get_addr2_ptr(pframe), _FALSE)) {
	...
	pstat = rtw_get_stainfobu(pstapriv, get_addr2_ptr(pframe));
	...
	if (rtw_ap_linking_test_force_asoc_failbu()) {
		status = rtw_ap_linking_test_force_asoc_failbu();
		RTW_INFO(FUNC_ADPT_FMT" force asoc fail with status:%u\n"
			, FUNC_ADPT_ARG(padapter), status);
		goto OnAssocReqbuFail;
	}
	...
	rtw_ap_parse_sta_capabilitybu(padapter, pstat, pframe + WLAN_HDR_A3_LEN);
	...
	if (elems.ssid == NULL
		...
		|| _rtw_memcmpbu(elems.ssid, cur->Ssid.Ssid, cur->Ssid.SsidLength) == _FALSE
	) {
		status = _STATS_FAILURE_;
		goto OnAssocReqbuFail;
	}
	...
	status = rtw_ap_parse_sta_supported_ratesbu(padapter, pstat
		, pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset);
	if (status != _STATS_SUCCESSFUL_)
		goto OnAssocReqbuFail;

	/* check RSN/WPA/WPS */
	status = rtw_ap_parse_sta_security_iebu(padapter, pstat, &elems);
	if (status != _STATS_SUCCESSFUL_)
		goto OnAssocReqbuFail;

	/* check if there is WMM IE & support WWM-PS */
	rtw_ap_parse_sta_wmm_iebu(padapter, pstat
		, pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset);

#ifdef CONFIG_RTS_FULL_BW
	/*check vendor IE*/
	rtw_parse_sta_vendor_ie_8812bu(padapter, pstat
		, pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset);
	...
	rtw_ap_parse_sta_ht_iebu(padapter, pstat, &elems);
	rtw_ap_parse_sta_vht_iebu(padapter, pstat, &elems);
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
		p2pie = rtw_get_p2p_iebu(pframe + WLAN_HDR_A3_LEN + ie_offset , pkt_len - WLAN_HDR_A3_LEN - ie_offset , NULL, &p2pielen);
		if (p2pie) {
			pstat->is_p2p_device = _TRUE;
			p2p_status_code = (u8)process_assoc_req_p2p_iebu(pwdinfo, pframe, pkt_len, pstat);
...
#ifdef CONFIG_WFD
		rtw_process_wfd_iebus(padapter, pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset, __func__);
#endif
	}
	pstat->p2p_status_code = p2p_status_code;
#endif /* CONFIG_P2P */
	...
	if (pstat->cmn.aid > 0)
		...
		if (!rtw_aid_allocbu(padapter, pstat)) {
			RTW_INFO(FUNC_ADPT_FMT" no room for more AIDs\n", FUNC_ADPT_ARG(padapter));
...
#endif /* CONFIG_IEEE80211W */
	{
		...
		_enter_critical_bh(&pstapriv->asoc_list_lock, &irqL);
		if (rtw_is_list_emptybu(&pstat->asoc_list)) {
			pstat->expire_to = pstapriv->expire_to;
			rtw_list_insert_tailbu(&pstat->asoc_list, &pstapriv->asoc_list);
			pstapriv->asoc_list_cnt++;
#ifdef CONFIG_RTW_TOKEN_BASED_XMIT
			if (sta_tbtx_enable) {
	...
	if (pstat && (pstat->state & WIFI_FW_ASSOC_SUCCESS) && (_STATS_SUCCESSFUL_ == status)) {
...
#endif /* CONFIG_IEEE80211W */
		{
			/* .1 bss_cap_update & sta_info_updatebu */
			bss_cap_update_on_sta_joinbu(padapter, pstat);
			sta_info_updatebu(padapter, pstat);
		}
#ifdef CONFIG_IEEE80211W
		if (pstat->bpairwise_key_installed == _TRUE && (pstat->flags & WLAN_STA_MFP))
			status = _STATS_REFUSED_TEMPORARILY_;
		...
		if (frame_type == WIFI_ASSOCREQ)
			issue_asocrspbu(padapter, status, pstat, WIFI_ASSOCRSP);
		else
			issue_asocrspbu(padapter, status, pstat, WIFI_REASSOCRSP);
		...
		if (pstat->passoc_req) {
			_rtw_memcpybu(pstat->passoc_req, pframe, pkt_len);
...
#endif /* CONFIG_IEEE80211W */
		{
			...
			report_add_sta_eventbu(padapter, pstat->cmn.mac_addr);
		}
#ifdef CONFIG_IEEE80211W
		if (pstat->bpairwise_key_installed == _TRUE && (pstat->flags & WLAN_STA_MFP)) {
			RTW_INFO(MAC_FMT"\n", MAC_ARG(pstat->cmn.mac_addr));
			issue_action_SA_Querybu(padapter, pstat->cmn.mac_addr, 0, 0, IEEE80211W_RIGHT_KEY);
		}
#endif /* CONFIG_IEEE80211W */
...
#ifdef CONFIG_NATIVEAP_MLME
	issue_deauthbu(padapter, (void *)get_addr2_ptr(pframe), status);
#endif
...
void rtw_roam_nb_discover(_adapter *padapter, u8 bfroce)
{
	...
	psta = rtw_get_stainfobu(pstapriv, pmlmeinfo->network.MacAddress);
	if (!psta)
		return;
...
unsigned int OnAssocRspbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	for (i = (6 + WLAN_HDR_A3_LEN); i < pkt_len;) {
		...
		case _VENDOR_SPECIFIC_IE_:
			if (_rtw_memcmpbu(pIE->data, WMM_PARA_OUIbu, 6))	/* WMM */
				WMM_param_handlerbu(padapter, pIE);
#if defined(CONFIG_P2P) && defined(CONFIG_WFD)
			else if (_rtw_memcmpbu(pIE->data, WFD_OUIbu, 4))		/* WFD */
				rtw_process_wfd_iebu(padapter, (u8 *)pIE, pIE->Length, __func__);
#endif
			break;
		...
		case _HT_CAPABILITY_IE_:	/* HT caps */
			HT_caps_handlerbu(padapter, pIE);
#ifdef ROKU_PRIVATE
			HT_caps_handlerbu_infra_ap(padapter, pIE);
#endif /* ROKU_PRIVATE */
			break;

		case _HT_EXTRA_INFO_IE_:	/* HT info */
			HT_info_handlerbu(padapter, pIE);
			break;

#ifdef CONFIG_80211AC_VHT
		case EID_VHTCapability:
			VHT_caps_handlerbubu(padapter, pIE);
#ifdef ROKU_PRIVATE
			VHT_caps_handlerbubu_infra_ap(padapter, pIE);
#endif /* ROKU_PRIVATE */
			break;

		case EID_VHTOperation:
			VHT_operation_handlerbu(padapter, pIE);
			break;
#endif

		case _ERPINFO_IE_:
			ERP_IE_handlerbu(padapter, pIE);
			break;
#ifdef CONFIG_TDLS
		case _EXT_CAP_IE_:
	...
	UpdateBrateTblbu(padapter, pmlmeinfo->network.SupportedRates);

report_assoc_result:
	if (res > 0)
		rtw_buf_updatebu(&pmlmepriv->assoc_rsp, &pmlmepriv->assoc_rsp_len, pframe, pkt_len);
	else
		rtw_buf_freebu(&pmlmepriv->assoc_rsp, &pmlmepriv->assoc_rsp_len);

	report_join_resbu(padapter, res, status);
...
unsigned int OnDeAuthbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (MLME_IS_AP(padapter)) {
		...
		/* _enter_critical_bh(&(pstapriv->sta_hash_lock), &irqL);		 */
		/* rtw_free_stainfobu(padapter, psta); */
		/* _exit_critical_bh(&(pstapriv->sta_hash_lock), &irqL);		 */
		...
		if (psta) {
			...
			_enter_critical_bh(&pstapriv->asoc_list_lock, &irqL);
			if (rtw_is_list_emptybu(&psta->asoc_list) == _FALSE) {
				...
				if (psta->tbtx_enable)
					pstapriv->tbtx_asoc_list_cnt--;
				#endif
				updated = ap_free_stabu(padapter, psta, _FALSE, reason, _TRUE);

			}
			_exit_critical_bh(&pstapriv->asoc_list_lock, &irqL);

			associated_clients_updatebu(padapter, updated, STA_INFO_UPDATE_ALL);
		}
	...
	if (!MLME_IS_MESH(padapter)) {
		...
		if (0 == ignore_received_deauth)
			receive_disconnectbu(padapter, get_addr2_ptr(pframe), reason, _FALSE);
	}
...
unsigned int on_action_spctbu_ch_switch(_adapter *padapter, struct sta_info *psta, u8 *ies, uint ies_len)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		...
		for_each_ie(ie, ies, ies_len) {
			if (ie->id == WLAN_EID_CHANNEL_SWITCH) {
				...
				ch_offset = secondary_ch_offset_to_hal_ch_offsetbu(ie->data[0]);
		...
		ret = rtw_set_chbw_cmdbu(padapter, ch, bwmode, ch_offset, 0);
	}
...
unsigned int on_action_spctbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_SPCT_CHL_SWITCH:
		...
		ret = on_action_spctbu_ch_switch(padapter, psta
...
unsigned int on_action_wnm(_adapter *adapter, union recv_frame *rframe)
{
	...
	sta = rtw_get_stainfobu(stapriv, get_addr2_ptr(frame));
	if (!sta)
		goto exit;
	...
	case RTW_WLAN_ACTION_WNM_BTM_RSP:
		if ((check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE) &&
			...
			_rtw_memsetbu(&rsp, 0, sizeof(rsp));
	...
	default:
		...
		rtw_cfg80211_rx_actionbu(adapter, rframe, msg);
		#endif
		ret = _SUCCESS;
		break;
	}
...
/**
 * rtw_rx_ampdu_sizebu - Get the target RX AMPDU buffer size for the specific @adapter
...
u8 rtw_rx_ampdu_sizebu(_adapter *adapter)
{
	...
	if (adapter->driver_rx_ampdu_factor != 0xFF)
		...
		rtw_hal_get_def_varbu(adapter, HW_VAR_MAX_RX_AMPDU_FACTOR, &max_rx_ampdu_factor);
...
/**
 * rtw_rx_ampdu_is_acceptbu - Get the permission if RX AMPDU should be set up for the specific @adapter
...
/**
 * rtw_rx_ampdu_set_sizebu - Set the target RX AMPDU buffer size for the specific @adapter and specific @reason
...
/**
 * rtw_rx_ampdu_set_acceptbu - Set the permission if RX AMPDU should be set up for the specific @adapter and specific @reason
...
/**
 * rx_ampdu_apply_stabu_tidbu - Apply RX AMPDU setting to the specific @sta and @tid
...
u8 rx_ampdu_apply_stabu_tidbu(_adapter *adapter, struct sta_info *sta, u8 tid, u8 accept, u8 size)
{
	...
	if (reorder_ctl->enable == _FALSE) {
		if (reorder_ctl->ampdu_size != RX_AMPDU_SIZE_INVALID) {
			send_delbabu_sta_tidbu_wait_ackbu(adapter, 0, sta, tid, 1);
			ret = 3;
		}
		goto exit;
	}
	...
	if (accept == _FALSE) {
		send_delbabu_sta_tidbu_wait_ackbu(adapter, 0, sta, tid, 0);
		ret = 1;
...
u8 rx_ampdu_size_sta_limitbu(_adapter *adapter, struct sta_info *sta)
{
	...
	if (is_supported_vht(sta->wireless_mode)) {
		nss = rtw_min(rtw_vht_mcsmap_to_nssbu(mlme->vhtpriv.vht_mcs_map)
				, rtw_vht_mcsmap_to_nssbu(sta->vhtpriv.vht_mcs_map));
	} else
	#endif
	if (is_supported_ht(sta->wireless_mode)) {
		nss = rtw_min(rtw_ht_mcsset_to_nssbu(mlmeinfo->HT_caps.u.HT_cap_element.MCS_rate)
				, rtw_ht_mcsset_to_nssbu(sta->htpriv.ht_cap.supp_mcs_set));
	}
...
/**
 * rx_ampdu_apply_stabu - Apply RX AMPDU setting to the specific @sta
 * @adapter: the adapter to which @sta belongs
 * @sta: the sta to be checked
 * @accept: the target permission if RX AMPDU should be set up
 * @size: the target RX AMPDU buffer size
...
u8 rx_ampdu_apply_stabu(_adapter *adapter, struct sta_info *sta, u8 accept, u8 size)
{
	...
	for (i = 0; i < TID_NUM; i++) {
		if (rx_ampdu_apply_stabu_tidbu(adapter, sta, i, accept, size) != 0)
			change_cnt++;
	}
...
/**
 * rtw_rx_ampdu_applybu - Apply the current target RX AMPDU setting for the specific @adapter
...
u16 rtw_rx_ampdu_applybu(_adapter *adapter)
{
	u16 adj_cnt = 0;
	struct sta_info *sta;
	u8 accept = rtw_rx_ampdu_is_acceptbu(adapter);
	u8 size;

	if (adapter->fix_rx_ampdu_size != RX_AMPDU_SIZE_INVALID)
		size = adapter->fix_rx_ampdu_size;
	else
		size = rtw_rx_ampdu_sizebu(adapter);

	if (MLME_IS_STA(adapter)) {
		sta = rtw_get_stainfobu(&adapter->stapriv, get_bssid(&adapter->mlmepriv));
		if (sta) {
			u8 sta_size = size;

			if (adapter->fix_rx_ampdu_size == RX_AMPDU_SIZE_INVALID)
				sta_size = rtw_min(size, rx_ampdu_size_sta_limitbu(adapter, sta));
			adj_cnt += rx_ampdu_apply_stabu(adapter, sta, accept, sta_size);
		}
		/* TODO: TDLS peer */
#ifdef CONFIG_AP_MODE
		...
		while ((rtw_end_of_queue_searchbu(phead, plist)) == _FALSE) {
			...
			stainfo_offset = rtw_stainfo_offsetbu(pstapriv, sta);
		...
		for (i = 0; i < peer_num; i++) {
			sta = rtw_get_stainfobu_by_offsetbu(pstapriv, peers[i]);
			if (sta) {
				u8 sta_size = size;
...
unsigned int OnActionbu_back(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (!_rtw_memcmpbu(adapter_mac_addr(padapter), GetAddr1Ptr(pframe), ETH_ALEN))
		return _SUCCESS;
	...
	psta = rtw_get_stainfobu(pstapriv, addr);
	...
	if (category == RTW_WLAN_CATEGORY_BACK) { /* representing Block Ack */
		...
		case RTW_WLAN_ACTION_ADDBA_REQ: /* ADDBA request */
			...
			_rtw_memcpybu(&(pmlmeinfo->ADDBA_req), &(frame_body[2]), sizeof(struct ADDBA_request));
			/* process_addba_reqbu(padapter, (u8*)&(pmlmeinfo->ADDBA_req), GetAddr3Ptr(pframe)); */
			process_addba_reqbu(padapter, (u8 *)&(pmlmeinfo->ADDBA_req), addr);
...
u32 rtw_build_vendor_ie(_adapter *padapter , unsigned char **pframe , u8 mgmt_frame_tyte)
{
	...
	for (vendor_ie_num = 0 ; vendor_ie_num < WLAN_MAX_VENDOR_IE_NUM ; vendor_ie_num++) {
		if (pmlmepriv->vendor_ielen[vendor_ie_num] > 0 && pmlmepriv->vendor_ie_mask[vendor_ie_num] & mgmt_frame_tyte) {
			_rtw_memcpybu(*pframe , pmlmepriv->vendor_ie[vendor_ie_num] , pmlmepriv->vendor_ielen[vendor_ie_num]);
...
void issue_p2p_GO_requestbu(_adapter *padapter, u8 *raddr)
{
	...
	pmgntframe = alloc_mgtxmitframebu(pxmitpriv);
	if (pmgntframe == NULL)
		return;
	...
	_rtw_memsetbu(pmgntframe->buf_addr, 0, WLANHDR_OFFSET + TXDESC_OFFSET);
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, raddr, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, adapter_mac_addr(padapter), ETH_ALEN);
	...
	pframe = rtw_set_fixed_iebu(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 1, &(action), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 4, (unsigned char *) &(p2poui), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 1, &(oui_subtype), &(pattrib->pktlen));
	pwdinfo->negotiation_dialog_token = 1;	/*	Initialize the dialog value */
	pframe = rtw_set_fixed_iebu(pframe, 1, &pwdinfo->negotiation_dialog_token, &(pattrib->pktlen));
	...
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0002);
	...
	pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, wpsielen, (unsigned char *) wpsie, &pattrib->pktlen);
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(ETH_ALEN);
	...
	/*	Value: */
	_rtw_memcpybu(p2pie + p2pielen, adapter_mac_addr(padapter), ETH_ALEN);
	p2pielen += ETH_ALEN;
	...
	len_channellist_attr = 3
		       ...
		       + get_reg_classes_full_countbu(ch_list);

#ifdef CONFIG_CONCURRENT_MODE
	if (rtw_mi_check_statusbu(padapter, MI_LINKED) && padapter->registrypriv.full_ch_in_p2p_handshake == 0)
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_be16(pwdinfo->device_name_len);
	...
	/*	Value: */
	_rtw_memcpybu(p2pie + p2pielen, pwdinfo->device_name , pwdinfo->device_name_len);
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, p2pielen, (unsigned char *) p2pie, &pattrib->pktlen);

#ifdef CONFIG_WFD
	wfdielen = build_nego_req_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	...
	dump_mgntframebu(padapter, pmgntframe);
...
void issue_p2p_GO_responsebu(_adapter *padapter, u8 *raddr, u8 *frame_body, uint len, u8 result)
{
	struct p2p_channels *ch_list = &(adapter_to_rfctl(padapter)->channel_list);
	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	u8			action = P2P_PUB_ACTION_ACTION;
	...
	*(fctrl) = 0;
	...
	rtw_get_wps_iebu(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, wpsie, &wpsielen);
	rtw_get_wps_attrbu_content(wpsie, wpsielen, WPS_ATTR_DEVICE_PWID, (u8 *) &wps_devicepassword_id, &wps_devicepassword_id_len);
	wps_devicepassword_id = be16_to_cpu(wps_devicepassword_id);

	_rtw_memsetbu(wpsie, 0x00, 255);
	wpsielen = 0;
	...
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0002);
	...
	if (_rtw_memcmpbu(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "000", 3)) {
		if (wps_devicepassword_id == WPS_DPID_USER_SPEC)
			_rtw_memcpybu(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "dis", 3);
		else if (wps_devicepassword_id == WPS_DPID_REGISTRAR_SPEC)
			_rtw_memcpybu(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "pad", 3);
		else
			_rtw_memcpybu(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "pbc", 3);
	}
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_be16(pwdinfo->device_name_len);
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
		...
		*(u16 *)(p2pie + p2pielen) = cpu_to_le16(ETH_ALEN + pwdinfo->nego_ssidlen);
		...
		/*	Value: */
		/*	p2P Device Address */
		_rtw_memcpybu(p2pie + p2pielen , pwdinfo->device_addr, ETH_ALEN);
		p2pielen += ETH_ALEN;

		/*	SSID */
		_rtw_memcpybu(p2pie + p2pielen, pwdinfo->nego_ssid, pwdinfo->nego_ssidlen);
...
#ifdef CONFIG_WFD
	wfdielen = build_nego_resp_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
void issue_p2p_GO_confirmbu(_adapter *padapter, u8 *raddr, u8 result)
{
...
#ifdef CONFIG_WFD
	wfdielen = build_nego_confirm_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
void issue_p2p_invitation_requestbu(_adapter *padapter, u8 *raddr)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, raddr, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, raddr,  ETH_ALEN);
	...
	pframe = rtw_set_fixed_iebu(pframe, 4, (unsigned char *) &(p2poui), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 1, &(oui_subtype), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 1, &(dialogToken), &(pattrib->pktlen));
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	if (_rtw_memcmpbu(adapter_mac_addr(padapter), pwdinfo->invitereq_info.go_bssid, ETH_ALEN)) {
		/*	P2P Group BSSID */
		/*	Type: */
		...
		*(u16 *)(p2pie + p2pielen) = cpu_to_le16(ETH_ALEN);
		...
		_rtw_memcpybu(p2pie + p2pielen, pwdinfo->invitereq_info.go_bssid, ETH_ALEN);
		p2pielen += ETH_ALEN;
	}
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(6 + pwdinfo->invitereq_info.ssidlen);
	...
	/*	SSID */
	_rtw_memcpybu(p2pie + p2pielen, pwdinfo->invitereq_info.go_ssid, pwdinfo->invitereq_info.ssidlen);
...
#ifdef CONFIG_WFD
	wfdielen = build_invitation_req_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
void issue_p2p_invitation_responsebu(_adapter *padapter, u8 *raddr, u8 dialogToken, u8 status_code)
{
	struct p2p_channels *ch_list = &(adapter_to_rfctl(padapter)->channel_list);
	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	u8			action = P2P_PUB_ACTION_ACTION;
...
#ifdef CONFIG_WFD
	wfdielen = build_invitation_resp_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
void issue_p2p_provision_requestbu(_adapter *padapter, u8 *pssid, u8 ussidlen, u8 *pdev_raddr)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, pdev_raddr, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, pdev_raddr, ETH_ALEN);
	...
	p2pielen = build_prov_disc_request_p2p_iebu(pwdinfo, pframe, pssid, ussidlen, pdev_raddr);
...
#ifdef CONFIG_WFD
	wfdielen = build_provdisc_req_wfd_iebu(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
u8 is_matched_in_profilelistbu(u8 *peermacaddr, struct profile_info *profileinfo)
{
	u8 i, match_result = 0;
	...
	for (i = 0; i < P2P_MAX_PERSISTENT_GROUP_NUM; i++, profileinfo++) {
		...
		if (_rtw_memcmpbu(peermacaddr, profileinfo->peermac, ETH_ALEN)) {
			match_result = 1;
			RTW_INFO("[%s] Match!\n", __FUNCTION__);
			break;
		}
...
void issue_proberspbu_p2p(_adapter *padapter, unsigned char *da)
{
	...
	if (IS_CCK_RATE(pattrib->rate)) {
		/* force OFDM 6M rate */
		pattrib->rate = MGN_6M;
		pattrib->raid = rtw_get_mgntframe_raidbu(padapter, WIRELESS_11G);
	}
	...
	fctrl = &(pwlanhdr->frame_ctl);
	*(fctrl) = 0;
	_rtw_memcpybu(pwlanhdr->addr1, da, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, mac, ETH_ALEN);

	/*	Use the device address for BSSID field.	 */
	_rtw_memcpybu(pwlanhdr->addr3, mac, ETH_ALEN);
	...
	_rtw_memcpybu(pframe, (unsigned char *) &beacon_interval, 2);
	pframe += 2;
	pattrib->pktlen += 2;
	...
	_rtw_memcpybu(pframe, (unsigned char *) &capInfo, 2);
	pframe += 2;
	pattrib->pktlen += 2;


	/* SSID */
	pframe = rtw_set_iebu(pframe, _SSID_IE_, 7, pwdinfo->p2p_wildcard_ssid, &pattrib->pktlen);

	/* supported rates... */
	/*	Use the OFDM rate in the P2P probe response frame. ( 6(B), 9(B), 12, 18, 24, 36, 48, 54 ) */
	pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_, 8, pwdinfo->support_rate, &pattrib->pktlen);

	/* DS parameter set */
	pframe = rtw_set_iebu(pframe, _DSSET_IE_, 1, (unsigned char *)&pwdinfo->listen_channel, &pattrib->pktlen);

#ifdef CONFIG_IOCTL_CFG80211
	if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
		if (pmlmepriv->wps_probe_resp_ie != NULL && pmlmepriv->p2p_probe_resp_ie != NULL) {
			/* WPS IE */
			_rtw_memcpybu(pframe, pmlmepriv->wps_probe_resp_ie, pmlmepriv->wps_probe_resp_ie_len);
			pattrib->pktlen += pmlmepriv->wps_probe_resp_ie_len;
			pframe += pmlmepriv->wps_probe_resp_ie_len;

			/* P2P IE */
			_rtw_memcpybu(pframe, pmlmepriv->p2p_probe_resp_ie, pmlmepriv->p2p_probe_resp_ie_len);
...
#endif /* CONFIG_IOCTL_CFG80211		 */
	{
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0010);
		...
		if (pwdinfo->external_uuid == 0) {
			_rtw_memsetbu(wpsie + wpsielen, 0x0, 16);
			_rtw_memcpybu(wpsie + wpsielen, mac, ETH_ALEN);
		} else
			_rtw_memcpybu(wpsie + wpsielen, pwdinfo->uuid, 0x10);
		wpsielen += 0x10;
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0007);
		...
		/*	Value: */
		_rtw_memcpybu(wpsie + wpsielen, "Realtek", 7);
		wpsielen += 7;
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0006);
		...
		/*	Value: */
		_rtw_memcpybu(wpsie + wpsielen, "8192CU", 6);
		wpsielen += 6;
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(ETH_ALEN);
		...
		/*	Value: */
		_rtw_memcpybu(wpsie + wpsielen, "123456" , ETH_ALEN);
		wpsielen += ETH_ALEN;
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(pwdinfo->device_name_len);
		...
		/*	Value: */
		_rtw_memcpybu(wpsie + wpsielen, pwdinfo->device_name, pwdinfo->device_name_len);
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(pwdinfo->supported_wps_cm);
		...
		p2pielen = build_probe_resp_p2p_iebu(pwdinfo, pframe);
		pframe += p2pielen;
		pattrib->pktlen += p2pielen;
	}

#ifdef CONFIG_WFD
	wfdielen = rtw_append_probe_resp_wfd_iebu(padapter, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
int _issue_probereqbubu_p2pbubu(_adapter *padapter, u8 *da, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	*(fctrl) = 0;
	...
	if (da) {
		_rtw_memcpybu(pwlanhdr->addr1, da, ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr3, da, ETH_ALEN);
	} else {
		if ((pwdinfo->p2p_info.scan_op_ch_only) || (pwdinfo->rx_invitereq_info.scan_op_ch_only)) {
			/*	This two flags will be set when this is only the P2P client mode. */
			_rtw_memcpybu(pwlanhdr->addr1, pwdinfo->p2p_peer_interface_addr, ETH_ALEN);
			_rtw_memcpybu(pwlanhdr->addr3, pwdinfo->p2p_peer_interface_addr, ETH_ALEN);
		} else {
			/*	broadcast probe request frame */
			_rtw_memcpybu(pwlanhdr->addr1, bc_addr, ETH_ALEN);
			_rtw_memcpybu(pwlanhdr->addr3, bc_addr, ETH_ALEN);
		}
	}
	_rtw_memcpybu(pwlanhdr->addr2, mac, ETH_ALEN);
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ))
		pframe = rtw_set_iebu(pframe, _SSID_IE_, pwdinfo->tx_prov_disc_info.ssid.SsidLength, pwdinfo->tx_prov_disc_info.ssid.Ssid, &(pattrib->pktlen));
	else
		pframe = rtw_set_iebu(pframe, _SSID_IE_, P2P_WILDCARD_SSID_LEN, pwdinfo->p2p_wildcard_ssid, &(pattrib->pktlen));
	...
	if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
		if (pmlmepriv->wps_probe_req_ie != NULL && pmlmepriv->p2p_probe_req_ie != NULL) {
			/* WPS IE */
			_rtw_memcpybu(pframe, pmlmepriv->wps_probe_req_ie, pmlmepriv->wps_probe_req_ie_len);
			pattrib->pktlen += pmlmepriv->wps_probe_req_ie_len;
			pframe += pmlmepriv->wps_probe_req_ie_len;

			/* P2P IE */
			_rtw_memcpybu(pframe, pmlmepriv->p2p_probe_req_ie, pmlmepriv->p2p_probe_req_ie_len);
...
#ifdef CONFIG_WFD
	wfdielen = rtw_append_probe_req_wfd_iebu(padapter, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
...
/* Vendor Specific IE */
	...
	if (wait_ack)
		ret = dump_mgntframebu_and_wait_ack(padapter, pmgntframe);
	else {
		dump_mgntframebu(padapter, pmgntframe);
		ret = _SUCCESS;
	}
...
inline void issue_probereqbu_p2pbu(_adapter *adapter, u8 *da)
{
	_issue_probereqbubu_p2pbubu(adapter, da, _FALSE);
}
...
int issue_probereqbu_p2pbu_ex(_adapter *adapter, u8 *da, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_probereqbubu_p2pbubu(adapter, da, wait_ms > 0 ? _TRUE : _FALSE);
	...
	if (try_cnt && wait_ms) {
		if (da)
			RTW_INFO(FUNC_ADPT_FMT" to "MAC_FMT", ch:%u%s, %d/%d in %u ms\n",
				FUNC_ADPT_ARG(adapter), MAC_ARG(da), rtw_get_oper_chbu(adapter),
				ret == _SUCCESS ? ", acked" : "", i, try_cnt, rtw_get_passing_time_ms(start));
		else
			RTW_INFO(FUNC_ADPT_FMT", ch:%u%s, %d/%d in %u ms\n",
				FUNC_ADPT_ARG(adapter), rtw_get_oper_chbu(adapter),
				ret == _SUCCESS ? ", acked" : "", i, try_cnt, rtw_get_passing_time_ms(start));
	}
exit:
	return ret;
...
s32 rtw_action_public_decachebu(union recv_frame *rframe, u8 token_offset)
{
...
unsigned int on_action_publicbu_p2p(union recv_frame *precv_frame)
{
...
#ifdef CONFIG_IOCTL_CFG80211
	if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211)
		rtw_cfg80211_rx_p2p_action_publicbu(padapter, precv_frame);
	else
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		...
		case P2P_GO_NEGO_REQ: {
			...
			_rtw_memsetbu(&pwdinfo->groupid_info, 0x00, sizeof(struct group_id_info));
...
#ifdef CONFIG_CONCURRENT_MODE
			if (rtw_mi_buddy_check_fwstatebu(padapter, WIFI_ASOC_STATE))
			...
			if (_rtw_memcmpbu(pwdinfo->rx_prov_disc_info.peerDevAddr, empty_addr, ETH_ALEN))
				_rtw_memcpybu(pwdinfo->rx_prov_disc_info.peerDevAddr, get_addr2_ptr(pframe), ETH_ALEN);

			result = process_p2p_group_negotation_reqbu(pwdinfo, frame_body, len);
			issue_p2p_GO_responsebu(padapter, get_addr2_ptr(pframe), frame_body, len, result);
		...
		case P2P_GO_NEGO_RESP: {
			...
			if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_GONEGO_ING)) {
				...
				_cancel_timer_ex(&pwdinfo->restore_p2p_state_timer);
				pwdinfo->nego_req_info.benable = _FALSE;
				result = process_p2p_group_negotation_respbu(pwdinfo, frame_body, len);
				issue_p2p_GO_confirmbu(pwdinfo->padapter, get_addr2_ptr(pframe), result);
		...
		case P2P_GO_NEGO_CONF: {
			...
			result = process_p2p_group_negotation_confirmbu(pwdinfo, frame_body, len);
			if (P2P_STATUS_SUCCESS == result) {
				if (rtw_p2p_role(pwdinfo) == P2P_ROLE_CLIENT) {
					pwdinfo->p2p_info.operation_ch[0] = pwdinfo->peer_operating_ch;
#ifdef CONFIG_P2P_OP_CHK_SOCIAL_CH
		...
		case P2P_INVIT_REQ: {
			...
			p2p_ie = rtw_get_p2p_iebu(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen);
			if (p2p_ie) {
				...
				merged_p2p_ielen = rtw_get_p2p_merged_ies_lenbu(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_);
				...
				_rtw_memsetbu(merged_p2pie, 0x00, merged_p2p_ielen);

				merged_p2p_ielen = rtw_p2p_merge_iesbu(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, merged_p2pie);

				rtw_get_p2p_attrbu_content(merged_p2pie, merged_p2p_ielen, P2P_ATTR_INVITATION_FLAGS, &invitation_flag, &attr_contentlen);
				if (attr_contentlen) {

					rtw_get_p2p_attrbu_content(merged_p2pie, merged_p2p_ielen, P2P_ATTR_GROUP_BSSID, pwdinfo->p2p_peer_interface_addr, &attr_contentlen);
					...
					if (invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT) {
						...
						_rtw_memsetbu(&group_id, 0x00, sizeof(struct group_id_info));
						rtw_get_p2p_attrbu_content(merged_p2pie, merged_p2p_ielen, P2P_ATTR_GROUP_ID, (u8 *) &group_id, &attr_contentlen);
						if (attr_contentlen) {
							if (_rtw_memcmpbu(group_id.go_device_addr, adapter_mac_addr(padapter), ETH_ALEN)) {
								...
								if (is_matched_in_profilelistbu(pwdinfo->p2p_peer_interface_addr, &pwdinfo->profileinfo[0])) {
									u8 operatingch_info[5] = { 0x00 };
									if (rtw_get_p2p_attrbu_content(merged_p2pie, merged_p2p_ielen, P2P_ATTR_OPERATING_CH, operatingch_info,
										&attr_contentlen)) {
										if (rtw_chset_search_chbu(adapter_to_chset(padapter), (u32)operatingch_info[4]) >= 0) {
						...
						if (attr_contentlen) {
							if (_rtw_memcmpbu(group_id.go_device_addr, adapter_mac_addr(padapter), ETH_ALEN)) {
								...
								_rtw_memcpybu(pwdinfo->p2p_peer_device_addr, group_id.go_device_addr , ETH_ALEN);
								rtw_p2p_set_role(pwdinfo, P2P_ROLE_CLIENT);
				...
				issue_p2p_invitation_responsebu(padapter, get_addr2_ptr(pframe), pwdinfo->inviteresp_info.token, status_code);
				_set_timer(&pwdinfo->restore_p2p_state_timer, 3000);
			}
			break;
		}
		case P2P_INVIT_RESP: {
			...
			p2p_ie = rtw_get_p2p_iebu(frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen);
			if (p2p_ie) {
				rtw_get_p2p_attrbu_content(p2p_ie, p2p_ielen, P2P_ATTR_STATUS, &attr_content, &attr_contentlen);
				...
				if (attr_contentlen == 1) {
					...
					if (attr_content == P2P_STATUS_SUCCESS) {
						if (_rtw_memcmpbu(pwdinfo->invitereq_info.go_bssid, adapter_mac_addr(padapter), ETH_ALEN))
							rtw_p2p_set_role(pwdinfo, P2P_ROLE_GO);
						else
							rtw_p2p_set_role(pwdinfo, P2P_ROLE_CLIENT);
		...
		case P2P_DEVDISC_REQ:
			...
			process_p2p_devdisc_reqbu(pwdinfo, pframe, len);

			break;

		case P2P_DEVDISC_RESP:

			process_p2p_devdisc_respbu(pwdinfo, pframe, len);

			break;

		case P2P_PROVISION_DISC_REQ:
			RTW_INFO("[%s] Got Provisioning Discovery Request Frame\n", __FUNCTION__);
			process_p2p_provdisc_reqbu(pwdinfo, pframe, len);
			_rtw_memcpybu(pwdinfo->rx_prov_disc_info.peerDevAddr, get_addr2_ptr(pframe), ETH_ALEN);
		...
		case P2P_PROVISION_DISC_RESP:
			...
			process_p2p_provdisc_respbu(pwdinfo, pframe);
			_set_timer(&pwdinfo->restore_p2p_state_timer, P2P_PROVISION_TIMEOUT);
			break;
...
unsigned int on_action_publicbu_vendor(union recv_frame *precv_frame)
{
	unsigned int ret = _FAIL;
	u8 *pframe = precv_frame->u.hdr.rx_data;
	...
	if (_rtw_memcmpbu(frame_body + 2, P2P_OUIbu, 4) == _TRUE) {
		if (rtw_action_public_decachebu(precv_frame, 7) == _FAIL)
			goto exit;

		if (!hal_chk_wl_funcbu(precv_frame->u.hdr.adapter, WL_FUNC_MIRACAST))
			rtw_rframe_del_wfd_iebu(precv_frame, 8);

		ret = on_action_publicbu_p2p(precv_frame);
	} else if (_rtw_memcmpbu(frame_body + 2, DPP_OUIbu, 4) == _TRUE) {
		u8 dpp_type = frame_body[7];

#ifdef CONFIG_IOCTL_CFG80211
		cnt += sprintf((msg + cnt), "DPP(type:%u)", dpp_type);
		rtw_cfg80211_rx_actionbu(adapter, precv_frame, msg);
#endif
	}
...
unsigned int on_action_publicbu_default(union recv_frame *precv_frame, u8 action)
{
	unsigned int ret = _FAIL;
	u8 *pframe = precv_frame->u.hdr.rx_data;
	u8 *frame_body = pframe + sizeof(struct rtw_ieee80211_hdr_3addr);
	...
	if (rtw_action_public_decachebu(precv_frame, 2) == _FAIL)
		goto exit;

#ifdef CONFIG_IOCTL_CFG80211
	cnt += sprintf((msg + cnt), "%s(token:%u)", action_public_strbu(action), token);
	rtw_cfg80211_rx_actionbu(adapter, precv_frame, msg);
#endif
...
unsigned int on_action_publicbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case ACT_PUBLIC_BSSCOEXIST:
		...
		if (check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
			rtw_process_public_act_bsscoexbu(padapter, pframe, frame_len);
#endif /*CONFIG_AP_MODE*/
#endif /*CONFIG_80211N_HT*/
		break;
	case ACT_PUBLIC_VENDOR:
		ret = on_action_publicbu_vendor(precv_frame);
		break;
	default:
		ret = on_action_publicbu_default(precv_frame, action);
		break;
	}
...
unsigned int OnActionbu_ft(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_FT_RSP:
		...
		if (!_rtw_memcmpbu(adapter_mac_addr(padapter), &pframe_body[2], ETH_ALEN)) {
		...
		_rtw_memcpybu(tgt_addr, &pframe_body[8], ETH_ALEN);
		if (is_zero_mac_addr(tgt_addr) || is_broadcast_mac_addr(tgt_addr)) {
			RTW_ERR("FT: Invalid Target MAC Address "MAC_FMT"\n", MAC_ARG(tgt_addr));
			goto exit;
		}

		pie = rtw_get_iebu(pframe_body, _MDIE_, &ft_ie_len, frame_len);
		if (pie) {
			if (!_rtw_memcmpbu(&pft_roam->mdid, pie+2, 2)) {
		...
		receive_disconnectbu(padapter, pmlmepriv->cur_network.network.MacAddress, WLAN_REASON_ACTIVE_ROAM, _FALSE);

		pft_roam->ft_action_len = frame_len;
		_rtw_memcpybu(pft_roam->ft_action, pframe, rtw_min(frame_len, RTW_FT_MAX_IE_SZ));
		ret = _SUCCESS;
		break;
...
#ifdef CONFIG_AP_MODE
		if (check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
			rtw_process_ht_action_smpsbu(padapter, get_addr2_ptr(pframe), frame_body[2]);
#endif /*CONFIG_AP_MODE*/
#endif /*CONFIG_80211N_HT*/
		break;
...
unsigned int OnActionbu_sa_query(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	RTW_INFO("OnActionbu_sa_query\n");

	switch (pframe[WLAN_HDR_A3_LEN + 1]) {
	case 0: /* SA Query req */
		_rtw_memcpybu(&tid, &pframe[WLAN_HDR_A3_LEN + 2], sizeof(u16));
		RTW_INFO("OnActionbu_sa_query request,action=%d, tid=%04x, pframe=%02x-%02x\n"
			, pframe[WLAN_HDR_A3_LEN + 1], tid, pframe[WLAN_HDR_A3_LEN + 2], pframe[WLAN_HDR_A3_LEN + 3]);
		issue_action_SA_Querybu(padapter, get_addr2_ptr(pframe), 1, tid, IEEE80211W_RIGHT_KEY);
		break;
	...
	case 1: /* SA Query rsp */
		...
		_rtw_memcpybu(&tid, &pframe[WLAN_HDR_A3_LEN + 2], sizeof(u16));
		RTW_INFO("OnActionbu_sa_query response,action=%d, tid=%04x, cancel timer\n", pframe[WLAN_HDR_A3_LEN + 1], tid);
		break;
	default:
		break;
	}
...
unsigned int OnActionbu_vht(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_VHT_OPMODE_NOTIFICATION:
		...
		psta = rtw_get_stainfobu(&padapter->stapriv, whdr->addr2);
		if (psta)
			rtw_process_vht_op_mode_notifybu(padapter, &frame_body[2], psta);
		break;
...
unsigned int OnActionbu_p2p(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (adapter_wdev_data(padapter)->p2p_enabled
		...
		rtw_cfg80211_rx_actionbu_p2pbu(padapter, precv_frame);
		return _SUCCESS;
	} else
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		...
		case P2P_PRESENCE_REQUEST:
			...
			process_p2p_presence_reqbu(pwdinfo, pframe, len);
...
#define TOKEN_REL 0x01
	...
	switch (tbtx_action_code)
	{
		...
		case TOKEN_REL:
			...
			if (!src)
				goto exit;
			psta = rtw_get_stainfobu(&padapter->stapriv, src);
			if (!psta)
				goto exit;

			if (ATOMIC_READbu(&pstapriv->nr_token_keeper) < 1)
				goto exit;
			...
			for (i=0; i< NR_MAXSTA_INSLOT; i++) {
				if (pstapriv->token_holder[i] == psta) {
					...
					ATOMIC_DECbu(&pstapriv->nr_token_keeper);
					break;
				}
			}

			if (ATOMIC_READbu(&pstapriv->nr_token_keeper) == 0)
...
void rtw_issue_action_token_rel(_adapter *padapter)
{
	...
	_rtw_memcpybu(pwlanhdr->addr1, get_my_bssidbu(&(pmlmeinfo->network)), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, get_my_bssidbu(&(pmlmeinfo->network)), ETH_ALEN);
	...
	pframe = rtw_set_fixed_iebu(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtw_set_fixed_iebu(pframe, 1, &(val), &(pattrib->pktlen));
...
unsigned int OnActionbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	for (i = 0; i < sizeof(OnActionbu_tbl) / sizeof(struct action_handler); i++) {
		ptable = &OnActionbu_tbl[i];
...
struct xmit_frame *_alloc_mgtxmitframebubu(struct xmit_priv *pxmitpriv, bool once)
{
	struct xmit_frame *pmgntframe;
	struct xmit_buf *pxmitbuf;

	if (once)
		pmgntframe = rtw_alloc_xmitframebu_once(pxmitpriv);
	else
		pmgntframe = rtw_alloc_xmitframebu_ext(pxmitpriv);
	...
	pxmitbuf = rtw_alloc_xmitbufbu_extbu(pxmitpriv);
	if (pxmitbuf == NULL) {
		RTW_INFO(FUNC_ADPT_FMT" alloc xmitbuf fail\n", FUNC_ADPT_ARG(pxmitpriv->adapter));
		rtw_free_xmitframebu(pxmitpriv, pmgntframe);
		pmgntframe = NULL;
		goto exit;
	}
...
inline struct xmit_frame *alloc_mgtxmitframebu(struct xmit_priv *pxmitpriv)
{
	return _alloc_mgtxmitframebubu(pxmitpriv, _FALSE);
}

inline struct xmit_frame *alloc_mgtxmitframebu_once(struct xmit_priv *pxmitpriv)
{
	return _alloc_mgtxmitframebubu(pxmitpriv, _TRUE);
}
...
void update_monitor_frame_attribbu(_adapter *padapter, struct pkt_attrib *pattrib)
{
	...
	psta = rtw_get_stainfobu(pstapriv, pattrib->ra);
	...
	pattrib->raid = rtw_get_mgntframe_raidbu(padapter, wireless_mode);
#ifdef CONFIG_80211AC_VHT
...
void update_mgntframe_subtype(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	rtw_action_frame_parsebu(pframe, pattrib->pktlen, &category, &action);
...
void update_mgntframe_attribbu(_adapter *padapter, struct pkt_attrib *pattrib)
{
	...
	/* _rtw_memsetbu((u8 *)(pattrib), 0, sizeof(struct pkt_attrib)); */
	...
	if (rtw_mi_buddy_check_fwstatebu(padapter, WIFI_ASOC_STATE))
		...
		if (MLME_IS_GC(padapter)) {
			if (pwdinfo->p2p_ps_mode > P2P_PS_NONE) {
				...
				psta = rtw_get_stainfobu(pstapriv, cur_network->MacAddress);
				if (psta) {
...
void update_mgntframe_attribbu_addr(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	_rtw_memcpybu(pattrib->ra, GetAddr1Ptr(pframe), ETH_ALEN);
	_rtw_memcpybu(pattrib->ta, get_addr2_ptr(pframe), ETH_ALEN);

#if defined(CONFIG_BEAMFORMING) || defined(CONFIG_ANTENNA_DIVERSITY) || defined(CONFIG_RTW_MGMT_QUEUE)
	sta = pattrib->psta;
	if (!sta) {
		sta = rtw_get_stainfobu(&padapter->stapriv, pattrib->ra);
		pattrib->psta = sta;
	}
...
void dump_mgntframebu(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	if (RTW_CANNOT_RUN(padapter)) {
		rtw_free_xmitbufbu(&padapter->xmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframebu(&padapter->xmitpriv, pmgntframe);
		return;
	}

	rtw_hal_mgnt_xmitbu(padapter, pmgntframe);
}
...
s32 dump_mgntframebu_and_wait(_adapter *padapter, struct xmit_frame *pmgntframe, int timeout_ms)
{
	...
	rtw_sctx_initbu(&sctx, timeout_ms);
	pxmitbuf->sctx = &sctx;

	ret = rtw_hal_mgnt_xmitbu(padapter, pmgntframe);
	...
	|| ret == RTW_QUEUE_MGMT
		...
		ret = rtw_sctx_waitbu(&sctx, __func__);
...
s32 dump_mgntframebu_and_wait_ack_timeout(_adapter *padapter, struct xmit_frame *pmgntframe, int timeout_ms)
{
	...
	rtw_sctx_initbu(&(pxmitpriv->ack_tx_ops), timeout_ms);
	...
	|| ret == RTW_QUEUE_MGMT
		...
		ret = rtw_sctx_waitbu(&(pxmitpriv->ack_tx_ops), __func__);
...
s32 dump_mgntframebu_and_wait_ack(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	return dump_mgntframebu_and_wait_ack_timeout(padapter, pmgntframe, 500);
}


int update_hidden_ssidbu(u8 *ies, u32 ies_len, u8 hidden_ssid_mode)
{
	u8 *ssid_ie;
	sint ssid_len_ori;
	int len_diff = 0;

	ssid_ie = rtw_get_iebu(ies,  WLAN_EID_SSID, &ssid_len_ori, ies_len);
	...
	if (ssid_ie && ssid_len_ori > 0) {
		...
		case 1: {
			...
			ssid_ie[1] = 0;
			_rtw_memcpybu(ssid_ie + 2, next_ie, remain_len);
			len_diff -= ssid_len_ori;

			break;
		}
		case 2:
			_rtw_memsetbu(&ssid_ie[2], 0, ssid_len_ori);
			break;
		default:
			break;
		}
...
void issue_beaconbu(_adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, bc_addr, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, get_my_bssidbu(cur_network), ETH_ALEN);
	...
	if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter)) {
		...
		u8 *wpsie = NULL;
		wpsie = rtw_get_wps_iebu(cur_network->IEs + _FIXED_IE_LENGTH_, cur_network->IELength - _FIXED_IE_LENGTH_, NULL, &wpsielen);
		...
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) && wpsie && wpsielen > 0) {
			...
			if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
				if (pmlmepriv->wps_beacon_ie && pmlmepriv->wps_beacon_ie_len > 0) {
					_rtw_memcpybu(pframe, cur_network->IEs, wps_offset);
					pframe += wps_offset;
					pattrib->pktlen += wps_offset;

					_rtw_memcpybu(pframe, pmlmepriv->wps_beacon_ie, pmlmepriv->wps_beacon_ie_len);
					pframe += pmlmepriv->wps_beacon_ie_len;
					pattrib->pktlen += pmlmepriv->wps_beacon_ie_len;

					/* copy remainder_ie to pframe */
					_rtw_memcpybu(pframe, premainder_ie, remainder_ielen);
					pframe += remainder_ielen;
					pattrib->pktlen += remainder_ielen;
				} else {
					_rtw_memcpybu(pframe, cur_network->IEs, cur_network->IELength);
...
#endif /* CONFIG_IOCTL_CFG80211 */
			{
				...
				_rtw_memcpybu(pframe, cur_network->IEs, wps_offset + wpsielen);
				...
				*(u16 *)(pframe + insert_len) = cpu_to_be16(pwdinfo->device_name_len);
				...
				/*	Value: */
				_rtw_memcpybu(pframe + insert_len, pwdinfo->device_name, pwdinfo->device_name_len);
...

		{
			...
			uint wps_ielen;
			u8 sr = 0;
			wps_ie = rtw_get_wps_iebu(pmgntframe->buf_addr + TXDESC_OFFSET + sizeof(struct rtw_ieee80211_hdr_3addr) + _BEACON_IE_OFFSET_,
				pattrib->pktlen - sizeof(struct rtw_ieee80211_hdr_3addr) - _BEACON_IE_OFFSET_, NULL, &wps_ielen);
			if (wps_ie && wps_ielen > 0)
				rtw_get_wps_attrbu_content(wps_ie,  wps_ielen, WPS_ATTR_SELECTED_REGISTRAR, (u8 *)(&sr), NULL);
			if (sr != 0)
		...
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
			...
			if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
				...
				if (pmlmepriv->p2p_beacon_ie && len > 0)
					_rtw_memcpybu(pframe, pmlmepriv->p2p_beacon_ie, len);
			} else
#endif /* CONFIG_IOCTL_CFG80211 */
			{
				len = build_beacon_p2p_iebu(pwdinfo, pframe);
			}
...
#ifdef CONFIG_WFD
			len = rtw_append_beacon_wfd_iebu(padapter, pframe);
			pframe += len;
			pattrib->pktlen += len;
#endif
		}
		...
		if (padapter->tbtx_capability == _TRUE)
			pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, 8, REALTEK_TBTX_IE, &pattrib->pktlen);
#endif
	...
	_rtw_memcpybu(pframe, (unsigned char *)(rtw_get_beacon_intervalbu_from_iebu(cur_network->IEs)), 2);
	...
	_rtw_memcpybu(pframe, (unsigned char *)(rtw_get_capabilitybu_from_iebu(cur_network->IEs)), 2);

	pframe += 2;
	pattrib->pktlen += 2;

	/* SSID */
	pframe = rtw_set_iebu(pframe, _SSID_IE_, cur_network->Ssid.SsidLength, cur_network->Ssid.Ssid, &pattrib->pktlen);

	/* supported rates... */
	rate_len = rtw_get_rateset_lenbu(cur_network->SupportedRates);
	pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_, ((rate_len > 8) ? 8 : rate_len), cur_network->SupportedRates, &pattrib->pktlen);

	/* DS parameter set */
	pframe = rtw_set_iebu(pframe, _DSSET_IE_, 1, (unsigned char *)&(cur_network->Configuration.DSConfig), &pattrib->pktlen);
	...
	/* if( (pmlmeinfo->state&0x03) == WIFI_FW_ADHOC_STATE) */
	{
		...
		ATIMWindow = 0;
		pframe = rtw_set_iebu(pframe, _IBSS_PARA_IE_, 2, (unsigned char *)(&ATIMWindow), &pattrib->pktlen);

		/* ERP IE */
		pframe = rtw_set_iebu(pframe, _ERPINFO_IE_, 1, &erpinfo, &pattrib->pktlen);
	}


	/* EXTERNDED SUPPORTED RATE */
	if (rate_len > 8)
		pframe = rtw_set_iebu(pframe, _EXT_SUPPORTEDRATES_IE_, (rate_len - 8), (cur_network->SupportedRates + 8), &pattrib->pktlen);
...
_issue_bcn:
	...
	if (timeout_ms > 0)
		dump_mgntframebu_and_wait(padapter, pmgntframe, timeout_ms);
	else
		dump_mgntframebu(padapter, pmgntframe);
...
void issue_proberspbu(_adapter *padapter, unsigned char *da, u8 is_valid_p2p_probereq)
{
	...
	*(fctrl) = 0;
	_rtw_memcpybu(pwlanhdr->addr1, da, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr2, mac, ETH_ALEN);
	_rtw_memcpybu(pwlanhdr->addr3, bssid, ETH_ALEN);
...
#if defined(CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
		pwps_ie = rtw_get_wps_iebu(cur_network->IEs + _FIXED_IE_LENGTH_, cur_network->IELength - _FIXED_IE_LENGTH_, NULL, &wps_ielen);
		...
		if ((pmlmepriv->wps_probe_resp_ie != NULL) && pwps_ie && (wps_ielen > 0)) {
			...
			if ((wps_offset + wps_ielen + 2) <= MAX_IE_SZ) {
				_rtw_memcpybu(pframe, pmlmepriv->wps_probe_resp_ie, wps_ielen + 2);
				pframe += wps_ielen + 2;
		...
		/* retrieve SSID IE from cur_network->Ssid */
		{
			...
			ssid_ie = rtw_get_iebu(ies + _FIXED_IE_LENGTH_, _SSID_IE_, &ssid_ielen,
			...
			if (ssid_ie &&  cur_network->Ssid.SsidLength) {
				...
				_rtw_memcpybu(buf, remainder_ie, remainder_ielen);
				_rtw_memcpybu(remainder_ie + ssid_ielen_diff, buf, remainder_ielen);
				*(ssid_ie + 1) = cur_network->Ssid.SsidLength;
				_rtw_memcpybu(ssid_ie + 2, cur_network->Ssid.Ssid, cur_network->Ssid.SsidLength);
...
#ifdef CONFIG_IOCTL_CFG80211
		if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
			/* if pwdinfo->role == P2P_ROLE_DEVICE will call issue_proberspbu_p2p() */
			len = pmlmepriv->p2p_go_probe_resp_ie_len;
			if (pmlmepriv->p2p_go_probe_resp_ie && len > 0)
				_rtw_memcpybu(pframe, pmlmepriv->p2p_go_probe_resp_ie, len);
		} else
#endif /* CONFIG_IOCTL_CFG80211 */
		{
			len = build_probe_resp_p2p_iebu(pwdinfo, pframe);
		}
...
#ifdef CONFIG_WFD
		len = rtw_append_probe_resp_wfd_iebu(padapter, pframe);
		pframe += len;
		pattrib->pktlen += len;
#endif
	}
...
#ifdef CONFIG_AUTO_AP_MODE
	{
		...
		/* check rc station */
		psta = rtw_get_stainfobu(pstapriv, da);
		if (psta && psta->isrc && psta->pid > 0) {
			...
			_rtw_memcpybu(RC_INFO, RC_OUI, sizeof(RC_OUI));
			_rtw_memcpybu(&RC_INFO[4], mac, ETH_ALEN);
			_rtw_memcpybu(&RC_INFO[10], (u8 *)&psta->pid, 2);
			_rtw_memcpybu(&RC_INFO[12], (u8 *)&cu_ch, 2);

			pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, sizeof(RC_INFO), RC_INFO, &pattrib->pktlen);
		}
	}
#endif /* CONFIG_AUTO_AP_MODE */
...
int _issue_probereqbubu(_adapter *padapter, const NDIS_802_11_SSID *pssid, const u8 *da, u8 ch, bool append_wps, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	*(fctrl) = 0;
	...
	if (pssid && !MLME_IS_MESH(padapter))
		pframe = rtw_set_iebu(pframe, _SSID_IE_, pssid->SsidLength, pssid->Ssid, &(pattrib->pktlen));
	else
		pframe = rtw_set_iebu(pframe, _SSID_IE_, 0, NULL, &(pattrib->pktlen));

	get_rate_setbu(padapter, bssrate, &bssrate_len);

	if (bssrate_len > 8) {
		pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_ , 8, bssrate, &(pattrib->pktlen));
		pframe = rtw_set_iebu(pframe, _EXT_SUPPORTEDRATES_IE_ , (bssrate_len - 8), (bssrate + 8), &(pattrib->pktlen));
	} else
		pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_ , bssrate_len , bssrate, &(pattrib->pktlen));

	if (ch)
		pframe = rtw_set_iebu(pframe, _DSSET_IE_, 1, &ch, &pattrib->pktlen);

#ifdef CONFIG_RTW_MESH
	if (MLME_IS_MESH(padapter)) {
		if (pssid)
			pframe = rtw_set_iebu_mesh_id(pframe, &pattrib->pktlen, pssid->Ssid, pssid->SsidLength);
		else
			pframe = rtw_set_iebu_mesh_id(pframe, &pattrib->pktlen, NULL, 0);
	}
#endif
...
inline void issue_probereqbu(_adapter *padapter, const NDIS_802_11_SSID *pssid, const u8 *da)
{
	_issue_probereqbubu(padapter, pssid, da, 0, 1, _FALSE);
}
 ...
 * try_cnt means the maximal TX count to try
 */
int issue_probereqbu_ex(_adapter *padapter, const NDIS_802_11_SSID *pssid, const u8 *da, u8 ch, bool append_wps,
		      int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_probereqbubu(padapter, pssid, da, ch, append_wps, wait_ms > 0 ? _TRUE : _FALSE);
	...
	if (try_cnt && wait_ms) {
		if (da)
			RTW_INFO(FUNC_ADPT_FMT" to "MAC_FMT", ch:%u%s, %d/%d in %u ms\n",
				FUNC_ADPT_ARG(padapter), MAC_ARG(da), rtw_get_oper_chbu(padapter),
				ret == _SUCCESS ? ", acked" : "", i, try_cnt, rtw_get_passing_time_ms(start));
		else
			RTW_INFO(FUNC_ADPT_FMT", ch:%u%s, %d/%d in %u ms\n",
				FUNC_ADPT_ARG(padapter), rtw_get_oper_chbu(padapter),
				ret == _SUCCESS ? ", acked" : "", i, try_cnt, rtw_get_passing_time_ms(start));
	}
exit:
	return ret;
...
void issue_authbu(_adapter *padapter, struct sta_info *psta, unsigned short status)
{
	...
	*(fctrl) = 0;
	...
	if (psta) { /* for AP mode */
		...
		_rtw_memcpybu(pwlanhdr->addr1, psta->cmn.mac_addr, ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr3, adapter_mac_addr(padapter), ETH_ALEN);
		...
		pframe = rtw_set_fixed_iebu(pframe, _AUTH_ALGM_NUM_, (unsigned char *)&val16, &(pattrib->pktlen));

		/* setting auth seq number */
		val16 = (u16)psta->auth_seq;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_iebu(pframe, _AUTH_SEQ_NUM_, (unsigned char *)&val16, &(pattrib->pktlen));

		/* setting status code... */
		val16 = status;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_iebu(pframe, _STATUS_CODE_, (unsigned char *)&val16, &(pattrib->pktlen));

		/* added challenging text... */
		if ((psta->auth_seq == 2) && (psta->state & WIFI_FW_AUTH_STATE) && (use_shared_key == 1))
			pframe = rtw_set_iebu(pframe, _CHLGETXT_IE_, 128, psta->chg_txt, &(pattrib->pktlen));
#endif
	} else {
		_rtw_memcpybu(pwlanhdr->addr1, get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
		...
		if ((pmlmeinfo->auth_seq == 3) && (pmlmeinfo->state & WIFI_FW_AUTH_STATE) && (use_shared_key == 1)) {
			...
			pframe = rtw_set_fixed_iebu(pframe, 4, (unsigned char *)&val32, &(pattrib->pktlen));
		...
		if ((pmlmeinfo->auth_seq == 3) && (pmlmeinfo->state & WIFI_FW_AUTH_STATE) && (use_shared_key == 1)) {
			pframe = rtw_set_iebu(pframe, _CHLGETXT_IE_, 128, pmlmeinfo->chg_txt, &(pattrib->pktlen));
	...
	rtw_wep_encryptbu(padapter, (u8 *)pmgntframe);
	RTW_INFO("%s\n", __FUNCTION__);
	dump_mgntframebu(padapter, pmgntframe);
...
void issue_asocrspbu(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu((void *)GetAddr1Ptr(pwlanhdr), pstat->cmn.mac_addr, ETH_ALEN);
	_rtw_memcpybu((void *)get_addr2_ptr(pwlanhdr), adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpybu((void *)GetAddr3Ptr(pwlanhdr), get_my_bssidbu(&(pmlmeinfo->network)), ETH_ALEN);
	...
	/* capability */
	val = *(unsigned short *)rtw_get_capabilitybu_from_iebu(ie);
#ifdef CONFIG_RTW_80211K
	val |= cap_RM;
#endif
	pframe = rtw_set_fixed_iebu(pframe, _CAPABILITY_ , (unsigned char *)&val, &(pattrib->pktlen));

	ie_status = cpu_to_le16(status);
	pframe = rtw_set_fixed_iebu(pframe , _STATUS_CODE_ , (unsigned char *)&ie_status, &(pattrib->pktlen));

	val = cpu_to_le16(pstat->cmn.aid | BIT(14) | BIT(15));
	pframe = rtw_set_fixed_iebu(pframe, _ASOC_ID_ , (unsigned char *)&val, &(pattrib->pktlen));

	if (pstat->bssratelen <= 8)
		pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_, pstat->bssratelen, pstat->bssrateset, &(pattrib->pktlen));
	else {
		pframe = rtw_set_iebu(pframe, _SUPPORTEDRATES_IE_, 8, pstat->bssrateset, &(pattrib->pktlen));
		pframe = rtw_set_iebu(pframe, _EXT_SUPPORTEDRATES_IE_, (pstat->bssratelen - 8), pstat->bssrateset + 8, &(pattrib->pktlen));
	}
	...
	if (status == _STATS_REFUSED_TEMPORARILY_) {
		...
		_rtw_memcpybu(timeout_itvl + 1, &timeout_interval, 4);
		pframe = rtw_set_iebu(pframe, _TIMEOUT_ITVL_IE_, 5, timeout_itvl, &(pattrib->pktlen));
	}
#endif /* CONFIG_IEEE80211W */
	...
	if ((pstat->flags & WLAN_STA_HT) && (pmlmepriv->htpriv.ht_option)) {
		...
		pbuf = rtw_get_iebu(ie + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpybu(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
		...
		pbuf = rtw_get_iebu(ie + _BEACON_IE_OFFSET_, _HT_ADD_INFO_IE_, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpybu(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
			pattrib->pktlen += (ie_len + 2);
	...
	if (pmlmepriv->ext_capab_ie_len > 0) {
		...
		pbuf = rtw_get_iebu(ie + _BEACON_IE_OFFSET_, _EXT_CAP_IE_, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpybu(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
			pattrib->pktlen += (ie_len + 2);
	...
	if ((pstat->flags & WLAN_STA_VHT) && (pmlmepriv->vhtpriv.vht_option)
		...
		/* FILL VHT CAP IE */
		pbuf = rtw_get_iebu(ie + _BEACON_IE_OFFSET_, EID_VHTCapability, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpybu(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
			pattrib->pktlen += (ie_len + 2);
		...
		pbuf = rtw_get_iebu(ie + _BEACON_IE_OFFSET_, EID_VHTOperation, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpybu(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
			pattrib->pktlen += (ie_len + 2);
	...
	if (pstat->capability & cap_RM)
		pframe = rtw_set_iebu(pframe, _EID_RRM_EN_CAP_IE_, 5,
	...
	if ((pstat->flags & WLAN_STA_WME) && (pmlmepriv->qospriv.qos_option)) {
		...
		for (pbuf = ie + _BEACON_IE_OFFSET_; ; pbuf += (ie_len + 2)) {
			pbuf = rtw_get_iebu(pbuf, _VENDOR_SPECIFIC_IE_, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_ - (ie_len + 2)));
			if (pbuf && _rtw_memcmpbu(pbuf + 2, WMM_PARA_IE, 6)) {
				_rtw_memcpybu(pframe, pbuf, ie_len + 2);
				pframe += (ie_len + 2);
				pattrib->pktlen += (ie_len + 2);
	...
	if (pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_REALTEK)
		pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, 6 , REALTEK_96B_IEbu, &(pattrib->pktlen));

	/* add WPS IE ie for wps 2.0 */
	if (pmlmepriv->wps_assoc_resp_ie && pmlmepriv->wps_assoc_resp_ie_len > 0) {
		_rtw_memcpybu(pframe, pmlmepriv->wps_assoc_resp_ie, pmlmepriv->wps_assoc_resp_ie_len);
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) && (pstat->is_p2p_device == _TRUE)) {
		...
		if (padapter->wdinfo.driver_interface == DRIVER_CFG80211) {
			...
			if (pmlmepriv->p2p_assoc_resp_ie && pmlmepriv->p2p_assoc_resp_ie_len > 0) {
				...
				_rtw_memcpybu(pframe, pmlmepriv->p2p_assoc_resp_ie, len);
			}
		} else
			len = build_assoc_resp_p2p_iebu(pwdinfo, pframe, pstat->p2p_status_code);
		pframe += len;
		pattrib->pktlen += len;
	}
...
#ifdef CONFIG_WFD
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
		wfdielen = rtw_append_assoc_resp_wfd_iebu(padapter, pframe);
		pframe += wfdielen;
		pattrib->pktlen += wfdielen;
	}
#endif
...
static u32 rtw_append_assoc_req_owe_ie(_adapter *adapter, u8 *pbuf)
{
	...
	if (sec->owe_ie_len > 0) {
		len = sec->owe_ie_len;
		_rtw_memcpybu(pbuf, sec->owe_ie, len);
	}
...
void _issue_assocreqbubu(_adapter *padapter, u8 is_reassoc)
{
	int ret = _FAIL;
	struct xmit_frame				*pmgntframe;
	struct pkt_attrib				*pattrib;
	...
	*(fctrl) = 0;
...
#if CONFIG_DFS
	_rtw_memcpybu(&cap, rtw_get_capabilitybu_from_iebu(pmlmeinfo->network.IEs), 2);
	cap |= cap_SpecMgmt;
#ifdef CONFIG_RTW_80211K
	cap |= cap_RM;
#endif
	_rtw_memcpybu(pframe, &cap, 2);
#else
	_rtw_memcpybu(pframe, rtw_get_capabilitybu_from_iebu(pmlmeinfo->network.IEs), 2);
#endif
	...
	val16 = cpu_to_le16(3);
	_rtw_memcpybu(pframe , (unsigned char *)&val16, 2);
	pframe += 2;
	pattrib->pktlen += 2;

	/*Construct Current AP Field for Reassoc-Req only*/
	if (is_reassoc == _TRUE) {
		_rtw_memcpybu(pframe, get_my_bssidbu(&(pmlmeinfo->network)), ETH_ALEN);
		pframe += ETH_ALEN;
		pattrib->pktlen += ETH_ALEN;
	}

	/* SSID */
	pframe = rtw_set_iebu(pframe, _SSID_IE_,  pmlmeinfo->network.Ssid.SsidLength, pmlmeinfo->network.Ssid.Ssid, &(pattrib->pktlen));
	...
	if (pmlmeext->cur_channel > 14) {
		...
		pframe = rtw_set_iebu(pframe, EID_PowerCap, 2, pow_cap_ele, &(pattrib->pktlen));
		...
		pframe = rtw_set_iebu(pframe, EID_SupportedChannels, idx_5g, sup_ch, &(pattrib->pktlen));
	}
	...
	get_rate_setbu(padapter, sta_bssrate, &sta_bssrate_len);
	...
	if ((bssrate_len == 0) && (pmlmeinfo->network.SupportedRates[0] != 0)) {
		rtw_free_xmitbufbu(pxmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframebu(pxmitpriv, pmgntframe);
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
		...
		switch (pIE->ElementID) {
		case _VENDOR_SPECIFIC_IE_:
			if ((_rtw_memcmpbu(pIE->data, RTW_WPA_OUIbu, 4)) ||
			    (_rtw_memcmpbu(pIE->data, WMM_OUIbu, 4)) ||
			    (_rtw_memcmpbu(pIE->data, WPS_OUIbu, 4))) {
				vs_ie_length = pIE->Length;
				if ((!padapter->registrypriv.wifi_spec) && (_rtw_memcmpbu(pIE->data, WPS_OUIbu, 4))) {
				...
				pframe = rtw_set_iebu(pframe, _VENDOR_SPECIFIC_IE_, vs_ie_length, pIE->data, &(pattrib->pktlen));
			}
			break;
		...
		case EID_WPA2:
...
#endif
			{
				...
				if (rtw_sec_chk_auth_alg(padapter, WLAN_AUTH_OPEN) &&
					...
					s32 entry = rtw_cached_pmkidbu(padapter, pmlmepriv->assoc_bssid);

					rtw_rsn_sync_pmkidbu(padapter, (u8 *)pIE, (pIE->Length + 2), entry);
				}
#endif /* CONFIG_IOCTL_CFG80211 */

				pframe = rtw_set_iebu(pframe, EID_WPA2, pIE->Length, pIE->data, &(pattrib->pktlen));
		...
		case EID_HTCapability:
			if (padapter->mlmepriv.htpriv.ht_option == _TRUE) {
				if (!(is_ap_in_tkipbu(padapter))) {
					_rtw_memcpybu(&(pmlmeinfo->HT_caps), pIE->data, sizeof(struct HT_caps_element));

					pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info = cpu_to_le16(pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info);

					pframe = rtw_set_iebu(pframe, EID_HTCapability, pIE->Length , (u8 *)(&(pmlmeinfo->HT_caps)), &(pattrib->pktlen));
				}
			}
			break;
		...
		case EID_EXTCapability:
			if (padapter->mlmepriv.htpriv.ht_option == _TRUE)
				pframe = rtw_set_iebu(pframe, EID_EXTCapability, pIE->Length, pIE->data, &(pattrib->pktlen));
			break;
#endif /* CONFIG_80211N_HT */
#ifdef CONFIG_80211AC_VHT
		case EID_VHTCapability:
			if (padapter->mlmepriv.vhtpriv.vht_option == _TRUE)
				pframe = rtw_set_iebu(pframe, EID_VHTCapability, pIE->Length, pIE->data, &(pattrib->pktlen));
			break;

		case EID_OpModeNotification:
			if (padapter->mlmepriv.vhtpriv.vht_option == _TRUE)
				pframe = rtw_set_iebu(pframe, EID_OpModeNotification, pIE->Length, pIE->data, &(pattrib->pktlen));
			break;
#endif /* CONFIG_80211AC_VHT */
		default:
			break;
	...
	if (adapter_wdev_data(padapter)->p2p_enabled && pwdinfo->driver_interface == DRIVER_CFG80211) {
		if (pmlmepriv->p2p_assoc_req_ie && pmlmepriv->p2p_assoc_req_ie_len > 0) {
			_rtw_memcpybu(pframe, pmlmepriv->p2p_assoc_req_ie, pmlmepriv->p2p_assoc_req_ie_len);
...
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) && !rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE)) {
			...
			*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x000D);
			...
			/*	Value: */
			_rtw_memcpybu(p2pie + p2pielen, pwdinfo->device_addr, ETH_ALEN);	/*	P2P Device Address */
			p2pielen += ETH_ALEN;

			p2pie[p2pielen++] = 1;	/*	P2P Interface Address Count */

			_rtw_memcpybu(p2pie + p2pielen, pwdinfo->device_addr, ETH_ALEN);	/*	P2P Interface Address List */
			p2pielen += ETH_ALEN;
...
#ifdef CONFIG_WFD
	wfdielen = rtw_append_assoc_req_wfd_iebu(padapter, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
#endif
#endif /* CONFIG_P2P */
...
exit:
	if (ret == _SUCCESS) {
		rtw_buf_updatebu(&pmlmepriv->assoc_req, &pmlmepriv->assoc_req_len, (u8 *)pwlanhdr, pattrib->pktlen);
	#ifdef CONFIG_RTW_WNM
		if (is_reassoc == _TRUE)
			rtw_wnm_update_reassoc_req_ie(padapter);
	#endif
	} else
		rtw_buf_freebu(&pmlmepriv->assoc_req, &pmlmepriv->assoc_req_len);
...
{
	_issue_assocreqbubu(padapter, _FALSE);
}

void issue_reassocreqbu(_adapter *padapter)
{
	_issue_assocreqbubu(padapter, _TRUE);
}

/* when wait_ack is ture, this function shoule be called at process context */
static int _issue_nulldatabu(_adapter *padapter, unsigned char *da, unsigned int power_mode, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	...
	*(fctrl) = 0;
	...
	if (get_tofr_ds(fctrl) == 3) {
		...
		_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr3, da, ETH_ALEN);
		_rtw_memcpybu(pwlanhdr->addr4, adapter_mac_addr(padapter), ETH_ALEN);
		a4_shift = ETH_ALEN;
...
int issue_nulldatabu(_adapter *padapter, unsigned char *da, unsigned int power_mode, int try_cnt, int wait_ms)
{
	...
	if (da == NULL)
		da = get_my_bssidbu(&(pmlmeinfo->network));

	do {
		ret = _issue_nulldatabu(padapter, da, power_mode, wait_ms > 0 ? _TRUE : _FALSE);
...
/* when wait_ack is ture, this function shoule be called at process context */
static int _issue_qos_nulldatabu(_adapter *padapter, unsigned char *da, u16 tid, u8 ps, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
int issue_qos_nulldatabu(_adapter *padapter, unsigned char *da, u16 tid, u8 ps, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_qos_nulldatabu(padapter, da, tid, ps, wait_ms > 0 ? _TRUE : _FALSE);
...
static int _issue_deauthbu(_adapter *padapter, unsigned char *da, unsigned short reason, u8 wait_ack, u8 key_type)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_fixed_iebu(pframe, _RSON_CODE_ , (unsigned char *)&reason, &(pattrib->pktlen));
...
int issue_deauthbu(_adapter *padapter, unsigned char *da, unsigned short reason)
{
	...
	return _issue_deauthbu(padapter, da, reason, _FALSE, IEEE80211W_RIGHT_KEY);
}
...
{
	RTW_INFO("%s to "MAC_FMT"\n", __func__, MAC_ARG(da));
	return _issue_deauthbu(padapter, da, reason, _FALSE, key_type);
}
#endif /* CONFIG_IEEE80211W */
 ...
 * try_cnt means the maximal TX count to try
 */
int issue_deauthbu_ex(_adapter *padapter, u8 *da, unsigned short reason, int try_cnt,
		    int wait_ms)
{
	int ret = _FAIL;
	int i = 0;
	...
	do {
		ret = _issue_deauthbu(padapter, da, reason, wait_ms > 0 ? _TRUE : _FALSE, IEEE80211W_RIGHT_KEY);
...
void issue_action_spct_ch_switchbu(_adapter *padapter, u8 *ra, u8 new_ch, u8 ch_offset)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, ra, ETH_ALEN); /* RA */
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN); /* TA */
	_rtw_memcpybu(pwlanhdr->addr3, ra, ETH_ALEN); /* DA = RA */
	...
	pframe = rtw_set_iebu_ch_switch(pframe, &(pattrib->pktlen), 0, new_ch, 0);
	pframe = rtw_set_iebu_secondary_ch_offset(pframe, &(pattrib->pktlen),
			hal_ch_offset_to_secondary_ch_offsetbu(ch_offset));
...
void issue_action_SA_Querybu(_adapter *padapter, unsigned char *raddr, unsigned char action, unsigned short tid, u8 key_type)
{
	...
	pmgntframe = alloc_mgtxmitframebu(pxmitpriv);
	if (pmgntframe == NULL) {
		RTW_INFO("%s: alloc_mgtxmitframebu fail\n", __FUNCTION__);
		return;
	}
	...
	*(fctrl) = 0;
	...
	switch (action) {
	case 0: /* SA Query req */
		pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)&pmlmeext->sa_query_seq, &pattrib->pktlen);
		pmlmeext->sa_query_seq++;
		/* send sa query request to AP, AP should reply sa query response in 1 second */
		if (pattrib->key_type == IEEE80211W_RIGHT_KEY) {
			psta = rtw_get_stainfobu(pstapriv, pwlanhdr->addr1);
			if (psta != NULL) {
	...
	case 1: /* SA Query rsp */
		tid = cpu_to_le16(tid);
		/* RTW_INFO("rtw_set_fixed_iebu, %04x\n", tid); */
		pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)&tid, &pattrib->pktlen);
		break;
	default:
		break;
	}
	...
	*(fctrl) = 0;
	...
	if (category == 3) {
		...
		case RTW_WLAN_ACTION_ADDBA_REQ:
			...
			pframe = rtw_set_fixed_iebu(pframe, 1, &(pmlmeinfo->dialogToken), &(pattrib->pktlen));
			...
			psta = rtw_get_stainfobu(pstapriv, raddr);
			if (psta != NULL) {
				if (psta->flags & WLAN_STA_AMSDU_DISABLE)
					BA_para_set &= ~BIT(0);
			}
			BA_para_set = cpu_to_le16(BA_para_set);
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&(BA_para_set)), &(pattrib->pktlen));

			/* BA_timeout_value = 0xffff; */ /* max: 65535 TUs(~ 65 ms) */
			BA_timeout_value = 5000;/* ~ 5ms */
			BA_timeout_value = cpu_to_le16(BA_timeout_value);
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&(BA_timeout_value)), &(pattrib->pktlen));

			/* if ((psta = rtw_get_stainfobu(pstapriv, pmlmeinfo->network.MacAddress)) != NULL) */
			psta = rtw_get_stainfobu(pstapriv, raddr);
			if (psta != NULL) {
			...
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&(BA_starting_seqctrl)), &(pattrib->pktlen));
			break;

		case RTW_WLAN_ACTION_ADDBA_RESP:
			pframe = rtw_set_fixed_iebu(pframe, 1, &(pmlmeinfo->ADDBA_req.dialog_token), &(pattrib->pktlen));
			status = cpu_to_le16(status);
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&status), &(pattrib->pktlen));
			...
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&(BA_para_set)), &(pattrib->pktlen));
			pframe = rtw_set_fixed_iebu(pframe, 2, (unsigned char *)(&(pmlmeinfo->ADDBA_req.BA_timeout_value)), &(pattrib->pktlen));
			break;
...
/**
 * issue_addba_reqbu - TX ADDBA_REQ
 * @adapter: the adapter to TX
 * @ra: receiver address
 * @tid: tid
 */
inline void issue_addba_reqbu(_adapter *adapter, unsigned char *ra, u8 tid)
{
...
/**
 * issue_addba_rspbu - TX ADDBA_RESP
 * @adapter: the adapter to TX
 * @ra: receiver address
 * @tid: tid
 * @status: status code
 ...
 */
inline void issue_addba_rspbu(_adapter *adapter, unsigned char *ra, u8 tid, u16 status, u8 size)
{
...
/**
 * issue_addba_rspbu_wait_ack - TX ADDBA_RESP and wait ack
 * @adapter: the adapter to TX
 * @ra: receiver address
 * @tid: tid
 * @status: status code
 ...
 */
inline u8 issue_addba_rspbu_wait_ack(_adapter *adapter, unsigned char *ra, u8 tid, u16 status, u8 size, int try_cnt, int wait_ms)
{
	int ret = _FAIL;
	int i = 0;
	systime start = rtw_get_current_time();
 ...
 * @initiator: if we are the initiator of AMPDU association. used by DELBA
 */
inline void issue_del_babu(_adapter *adapter, unsigned char *ra, u8 tid, u16 reason, u8 initiator)
{
...
/**
 * issue_del_babu_ex - TX DELBA with xmit ack options
 * @adapter: the adapter to TX
 * @ra: receiver address
 * @tid: tid
 * @reason: reason code
 ...
 *           > 0 means you want to wait ack through C2H_CCX_TX_RPT, and the value of wait_ms means the interval between each TX
 */
int issue_del_babu_ex(_adapter *adapter, unsigned char *ra, u8 tid, u16 reason, u8 initiator
		    , int try_cnt, int wait_ms)
{
	int ret = _FAIL;
	int i = 0;
...
void issue_action_BSSCoexistPacketbu(_adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	/* if (pmlmepriv->num_FortyMHzIntolerant > 0) */
	{
		...
		pframe = rtw_set_iebu(pframe, EID_BSSCoexistence,  1, &iedata, &(pattrib->pktlen));
	}

	/*  */
	_rtw_memsetbu(ICS, 0, sizeof(ICS));
	if (pmlmepriv->num_sta_no_ht > 0) {
		...
		while (1) {
			...
			p = rtw_get_iebu(pbss_network->IEs + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_, &len, pbss_network->IELength - _FIXED_IE_LENGTH_);
		...
		for (i = 0; i < 8; i++) {
			if (ICS[i][0] == 1) {
				...
				pframe = rtw_set_iebu(pframe, EID_BSSIntolerantChlReport, k, InfoContent, &(pattrib->pktlen));
...
int _issue_action_SM_PSbubu(_adapter *padapter ,  unsigned char *raddr , u8 NewMimoPsMode ,  u8 wait_ack)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpybu(pwlanhdr->addr1, raddr, ETH_ALEN); /* RA */
	_rtw_memcpybu(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN); /* TA */
	_rtw_memcpybu(pwlanhdr->addr3, get_my_bssidbu(&(pmlmeinfo->network)), ETH_ALEN); /* DA = RA */
	...
	pframe = rtw_set_fixed_iebu(pframe, 1, &(sm_power_control), &(pattrib->pktlen));
...
int issue_action_SM_PSbu_wait_ackbu(_adapter *padapter, unsigned char *raddr, u8 NewMimoPsMode, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_action_SM_PSbubu(padapter, raddr, NewMimoPsMode , wait_ms > 0 ? _TRUE : _FALSE);
...
int issue_action_SM_PSbu(_adapter *padapter ,  unsigned char *raddr , u8 NewMimoPsMode)
{
	...
	return _issue_action_SM_PSbubu(padapter, raddr, NewMimoPsMode , _FALSE);
}

/**
 * _send_delbabu_sta_tidbu - Cancel the AMPDU association for the specific @sta, @tid
 * @adapter: the adapter to which @sta belongs
 ...
 */
static unsigned int _send_delbabu_sta_tidbu(_adapter *adapter, u8 initiator, struct sta_info *sta, u8 tid
					, u8 force, int wait_ack)
{
	...
	if (initiator == 0) {
		...
		if (force || sta->recvreorder_ctrl[tid].enable == _TRUE) {
			...
			if (rtw_del_rx_ampdu_test_trigger_no_tx_failbu())
				ret = _FAIL;
			else if (wait_ack)
				ret = issue_del_babu_ex(adapter, sta->cmn.mac_addr, tid, 37, initiator, 3, 1);
			else
				issue_del_babu(adapter, sta->cmn.mac_addr, tid, 37, initiator);
...
inline unsigned int send_delbabu_sta_tidbu(_adapter *adapter, u8 initiator, struct sta_info *sta, u8 tid
				       , u8 force)
{
	return _send_delbabu_sta_tidbu(adapter, initiator, sta, tid, force, 0);
}

inline unsigned int send_delbabu_sta_tidbu_wait_ackbu(_adapter *adapter, u8 initiator, struct sta_info *sta, u8 tid
		, u8 force)
{
	return _send_delbabu_sta_tidbu(adapter, initiator, sta, tid, force, 1);
}
...
unsigned int send_delbabu(_adapter *padapter, u8 initiator, u8 *addr)
{
	...
	for (tid = 0; tid < TID_NUM; tid++)
		send_delbabu_sta_tidbu(padapter, initiator, psta, tid, 0);
...
unsigned int send_beaconbu(_adapter *padapter)
{
	...
	rtw_hal_set_hwregbu(padapter, HW_VAR_BCN_VALID, NULL);

	/* 8192EE Port select for Beacon DL */
	rtw_hal_set_hwregbu(padapter, HW_VAR_DL_BCN_SEL, NULL);
	#ifdef CONFIG_FW_HANDLE_TXBCN
	rtw_hal_set_hwregbu(padapter, HW_VAR_BCN_HEAD_SEL, &vap_id);
	#endif

	issue_beaconbu(padapter, 0);
	...
	#endif
	{
		...
		do {
			...
			do {
				...
				rtw_yield_osbu();
				#endif
				rtw_hal_get_hwregbu(padapter, HW_VAR_BCN_VALID, (u8 *)(&bxmitok));
				poll++;
...
u8 collect_bss_infobu(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	_rtw_memsetbu(bssid, 0, sizeof(WLAN_BSSID_EX));
	...
	bssid->IELength = len;
	_rtw_memcpybu(bssid->IEs, (pframe + sizeof(struct rtw_ieee80211_hdr_3addr)), bssid->IELength);
	...
	rtw_hal_get_odm_varbu(padapter, HAL_ODM_ANTDIV_SELECT, &(bssid->PhyInfo.Optimum_antenna), NULL);
#endif

	/* checking SSID */
	p = rtw_get_iebu(bssid->IEs + ie_offset, _SSID_IE_, &len, bssid->IELength - ie_offset);
	if (p == NULL) {
	...
	if (*(p + 1)) {
		...
		_rtw_memcpybu(bssid->Ssid.Ssid, (p + 2), *(p + 1));
		bssid->Ssid.SsidLength = *(p + 1);
	} else
		bssid->Ssid.SsidLength = 0;

	_rtw_memsetbu(bssid->SupportedRates, 0, NDIS_802_11_LENGTH_RATES_EX);

	/* checking rate info... */
	i = 0;
	p = rtw_get_iebu(bssid->IEs + ie_offset, _SUPPORTEDRATES_IE_, &len, bssid->IELength - ie_offset);
	if (p != NULL) {
		...
		if (rtw_validate_value(_SUPPORTEDRATES_IE_, p+2, len) == _FALSE) {
			rtw_absorb_ssid_ifneedbu(padapter, bssid, pframe);
			RTW_DBG_DUMP("Invalidated Support Rate IE --", p, len+2);
			return _FAIL;
		}
#endif /* #ifdef CONFIG_CHECK_SPECIFIC_IE_CONTENT */
		_rtw_memcpybu(bssid->SupportedRates, (p + 2), len);
		i = len;
	}

	p = rtw_get_iebu(bssid->IEs + ie_offset, _EXT_SUPPORTEDRATES_IE_, &len, bssid->IELength - ie_offset);
	if (p != NULL) {
...
#endif /* #ifdef CONFIG_CHECK_SPECIFIC_IE_CONTENT */
		_rtw_memcpybu(bssid->SupportedRates + i, (p + 2), len);
	}
	...
	if (subtype == WIFI_PROBEREQ) {
		...
		u32	p2p_ielen;
		/* Set Listion Channel */
		p2p_ie = rtw_get_p2p_iebu(bssid->IEs, bssid->IELength, NULL, &p2p_ielen);
		if (p2p_ie) {
			u32	attr_contentlen = 0;
			u8 listen_ch[5] = { 0x00 };

			rtw_get_p2p_attrbu_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH, listen_ch, &attr_contentlen);
		...
		_rtw_memcpybu(bssid->MacAddress, get_addr2_ptr(pframe), ETH_ALEN);
		bssid->Privacy = 1;
		return _SUCCESS;
	}
#endif /* CONFIG_P2P */
	...
	p = rtw_get_iebu(bssid->IEs + ie_offset, _DSSET_IE_, &len, bssid->IELength - ie_offset);
	...
	if (p)
		...
		p = rtw_get_iebu(bssid->IEs + ie_offset, _HT_ADD_INFO_IE_, &len, bssid->IELength - ie_offset);
		if (p) {
			struct HT_info_element *HT_info = (struct HT_info_element *)(p + 2);
			bssid->Configuration.DSConfig = HT_info->primary_channel;
		} else {
			/* use current channel */
			bssid->Configuration.DSConfig = rtw_get_oper_chbu(padapter);
		}
	}

	_rtw_memcpybu(&bssid->Configuration.BeaconPeriod, rtw_get_beacon_intervalbu_from_iebu(bssid->IEs), 2);
	bssid->Configuration.BeaconPeriod = le32_to_cpu(bssid->Configuration.BeaconPeriod);

	val16 = rtw_get_capabilitybu((WLAN_BSSID_EX *)bssid);
	...
	if ((val16 & 0x03) == cap_ESS) {
		...
		_rtw_memcpybu(bssid->MacAddress, GetAddr3Ptr(pframe), ETH_ALEN);
	} else if ((val16 & 0x03) == 0x00){
		u8 *mesh_id_ie, *mesh_conf_ie;
		sint mesh_id_ie_len, mesh_conf_ie_len;

		mesh_id_ie = rtw_get_iebu(bssid->IEs + ie_offset, WLAN_EID_MESH_ID, &mesh_id_ie_len, bssid->IELength - ie_offset);
		mesh_conf_ie = rtw_get_iebu(bssid->IEs + ie_offset, WLAN_EID_MESH_CONFIG, &mesh_conf_ie_len, bssid->IELength - ie_offset);
		if (mesh_id_ie || mesh_conf_ie) {
			...
			if (mesh_id_ie_len) {
				...
				_rtw_memcpybu(bssid->mesh_id.Ssid, (mesh_id_ie + 2), mesh_id_ie_len);
	...
	if ((pregistrypriv->wifi_spec == 1) && (_FALSE == pmlmeinfo->bwmode_updated)) {
		struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
#ifdef CONFIG_80211N_HT
		p = rtw_get_iebu(bssid->IEs + ie_offset, _HT_CAPABILITY_IE_, &len, bssid->IELength - ie_offset);
		if (p && len > 0) {
	...
	if (bssid->Configuration.DSConfig != rtw_get_oper_chbu(padapter))
		bssid->PhyInfo.SignalQuality = 101;

#ifdef CONFIG_RTW_80211K
	p = rtw_get_iebu(bssid->IEs + ie_offset, _EID_RRM_EN_CAP_IE_, &len, bssid->IELength - ie_offset);
	if (p)
		_rtw_memcpybu(bssid->PhyInfo.rm_en_cap, (p + 2), *(p + 1));
...
void start_create_ibssbu(_adapter *padapter)
{
	...
	pmlmeinfo->bcn_interval = get_beacon_intervalbu(pnetwork);

	/* update wireless mode */
	update_wireless_modebu(padapter);

	/* udpate capability */
	caps = rtw_get_capabilitybu((WLAN_BSSID_EX *)pnetwork);
	update_capinfobu(padapter, caps);
	if (caps & cap_IBSS) { /* adhoc master */
		/* set_opmode_cmd(padapter, adhoc); */ /* removed */

		val8 = 0xcf;
		rtw_hal_set_hwregbu(padapter, HW_VAR_SEC_CFG, (u8 *)(&val8));

		doiqk = _TRUE;
		rtw_hal_set_hwregbu(padapter , HW_VAR_DO_IQK , &doiqk);

		/* switch channel */
		set_channel_bwmodebu(padapter, pmlmeext->cur_channel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);

		doiqk = _FALSE;
		rtw_hal_set_hwregbu(padapter , HW_VAR_DO_IQK , &doiqk);

		beacon_timing_controlbu(padapter);

		/* set msr to WIFI_FW_ADHOC_STATE */
		pmlmeinfo->state = WIFI_FW_ADHOC_STATE;
		Set_MSRbu(padapter, (pmlmeinfo->state & 0x3));

		/* issue beacon */
		if (send_beaconbu(padapter) == _FAIL) {

			report_join_resbu(padapter, -1, WLAN_STATUS_UNSPECIFIED_FAILURE);
			pmlmeinfo->state = WIFI_FW_NULL_STATE;
		} else {
			rtw_hal_set_hwregbu(padapter, HW_VAR_BSSID, padapter->registrypriv.dev_network.MacAddress);
			rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_ADHOC_STARTED);
			join_type = 0;
			rtw_hal_set_hwregbu(padapter, HW_VAR_MLME_JOIN, (u8 *)(&join_type));

			rtw_btcoex_connect_notify(padapter, join_type);

			report_join_resbu(padapter, 1, WLAN_STATUS_SUCCESS);
			pmlmeinfo->state |= WIFI_FW_ASSOC_SUCCESS;
			rtw_indicate_connectbu(padapter);
		}
	} else {
		RTW_INFO("start_create_ibssbu, invalid cap:%x\n", caps);
		return;
	}
	/* update bc/mc sta_info */
	update_bmc_stabu(padapter);
...
void start_clnt_joinbu(_adapter *padapter)
{
	...
	if (_rtw_memcmpbu(get_my_bssidbu(&pmlmeinfo->network) , ASIX_ID , 3)) {
		u8 iot_flag = _TRUE;
		rtw_hal_set_hwregbu(padapter, HW_VAR_ASIX_IOT, (u8 *)(&iot_flag));
	}

	if (caps & cap_ESS) {
		Set_MSRbu(padapter, WIFI_FW_STATION_STATE);
		...
		/*	For the Win8 P2P connection, it will be hard to have a successful connection if this Wi-Fi doesn't connect to it. */
		{
			...
			for (pos = get_next(head); !rtw_end_of_queue_searchbu(head, pos); pos = get_next(pos)) {

				scanned = LIST_CONTAINOR(pos, struct wlan_network, list);

				if (_rtw_memcmpbu(&(scanned->network.Ssid), &(pnetwork->Ssid), sizeof(NDIS_802_11_SSID)) == _TRUE
				    && _rtw_memcmpbu(scanned->network.MacAddress, pnetwork->MacAddress, sizeof(NDIS_802_11_MAC_ADDRESS)) == _TRUE
				   ) {
					ie_offset = (scanned->network.Reserved[0] == BSS_TYPE_PROB_REQ ? 0 : 12);
					if (rtw_get_p2p_iebu(scanned->network.IEs + ie_offset, scanned->network.IELength - ie_offset, NULL, NULL))
						has_p2p_ie = _TRUE;
					break;
				}
			}
			...
			if (scanned == NULL || rtw_end_of_queue_searchbu(head, pos) || has_p2p_ie == _FALSE)
#endif /* CONFIG_P2P */
				/* To avoid connecting to AP fail during resume process, change retry count from 5 to 1 */
				issue_deauthbu_ex(padapter, pnetwork->MacAddress, WLAN_REASON_DEAUTH_LEAVING, 1, 100);
		}
#endif /* CONFIG_DEAUTH_BEFORE_CONNECT */

		/* here wait for receiving the beacon to start auth */
		/* and enable a timer */
		beacon_timeout = decide_wait_for_beacon_timeoutbu(pmlmeinfo->bcn_interval);
...
#ifdef CONFIG_RTW_80211R
		if (rtw_ft_roam(padapter)) {
			rtw_ft_start_clnt_joinbu(padapter);
		} else
#endif
		{
			rtw_sta_linking_test_set_startbu();
			pmlmeinfo->state = WIFI_FW_AUTH_NULL | WIFI_FW_STATION_STATE;
		}
	} else if (caps & cap_IBSS) { /* adhoc client */
		Set_MSRbu(padapter, WIFI_FW_ADHOC_STATE);
...
void start_clnt_authbu(_adapter *padapter)
{
	...
	if (rtw_sec_chk_auth_type(padapter, MLME_AUTHTYPE_SAE)) {
		...
		rtw_cfg80211_external_auth_requestbu(padapter, NULL);
		return;
	}
no_external_auth:
#endif /* CONFIG_IOCTL_CFG80211 */
...
void start_clnt_assocbu(_adapter *padapter)
{
	...
	if (rtw_ft_roam(padapter)
		...
		issue_reassocreqbu(padapter);
	else
#endif
		issue_assocreqbu(padapter);
...
unsigned int receive_disconnectbu(_adapter *padapter, unsigned char *MacAddr, unsigned short reason, u8 locally_generated)
{
	...
	if (!(_rtw_memcmpbu(MacAddr, get_my_bssidbu(&pmlmeinfo->network), ETH_ALEN)))
		return _SUCCESS;
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
			if (report_del_sta_eventbu(padapter, MacAddr, reason, _TRUE, locally_generated) != _FAIL)
				pmlmeinfo->state = WIFI_FW_NULL_STATE;
		} else if (pmlmeinfo->state & WIFI_FW_LINKING_STATE) {
			if (report_join_resbu(padapter, -2, reason) != _FAIL)
				pmlmeinfo->state = WIFI_FW_NULL_STATE;
		} else
...
static void rtw_hidden_ssid_bss_count(_adapter *adapter, WLAN_BSSID_EX *bss)
{
	...
	if (!hidden_ssid_apbu(bss))
		return;

	chset_idx = rtw_chset_search_chbu(chset, bss->Configuration.DSConfig);
	if (chset_idx < 0)
		return;
...
void report_survey_eventbu(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	_rtw_init_listheadbu(&pcmd_obj->list);
	...
	evt_hdr->seq = ATOMIC_INCbu_RETURN(&pmlmeext->event_seq);

	psurvey_evt = (struct survey_event *)(pevtcmd + sizeof(struct rtw_evt_header));

	if (collect_bss_infobu(padapter, precv_frame, (WLAN_BSSID_EX *)&psurvey_evt->bss) == _FAIL) {
		rtw_mfree((u8 *)pcmd_obj, sizeof(struct cmd_obj));
	...
	rtw_enqueue_cmdbu(pcmdpriv, pcmd_obj);
...
u32 report_join_resbu(_adapter *padapter, int aid_res, u16 status)
{
	...
	_rtw_memcpybu((unsigned char *)(&(pjoinbss_evt->network.network)), &(pmlmeinfo->network), sizeof(WLAN_BSSID_EX));
	pjoinbss_evt->network.join_res = pjoinbss_evt->network.aid = aid_res;

	RTW_INFO("report_join_resbu(%d, %u)\n", aid_res, status);


	rtw_joinbss_event_prehandlebu(padapter, (u8 *)&pjoinbss_evt->network, status);


	ret = rtw_enqueue_cmdbu(pcmdpriv, pcmd_obj);
...
u32 report_del_sta_eventbu(_adapter *padapter, unsigned char *MacAddr, unsigned short reason, bool enqueue, u8 locally_generated)
{
	...
	_rtw_memcpybu((unsigned char *)(&(pdel_sta_evt->macaddr)), MacAddr, ETH_ALEN);
	_rtw_memcpybu((unsigned char *)(pdel_sta_evt->rsvd), (unsigned char *)(&reason), 2);
	psta = rtw_get_stainfobu(&padapter->stapriv, MacAddr);
	if (psta)
	...
	if (!enqueue) {
		/* do directly */
		rtw_stadel_event_callbackbu(padapter, (u8 *)pdel_sta_evt);
		...
		res = rtw_enqueue_cmdbu(pcmdpriv, pcmd_obj);
	}
...
void report_add_sta_eventbu(_adapter *padapter, unsigned char *MacAddr)
{
	...
	_rtw_memcpybu((unsigned char *)(&(padd_sta_evt->macaddr)), MacAddr, ETH_ALEN);

	RTW_INFO("report_add_sta_eventbu: add STA\n");
...
void update_sta_infobu(_adapter *padapter, struct sta_info *psta)
{
	...
	if (pmlmepriv->htpriv.ht_option) {
		...
		if (support_short_GIbu(padapter, &(pmlmeinfo->HT_caps), CHANNEL_WIDTH_20))
			psta->htpriv.sgi_20m = _TRUE;

		if (support_short_GIbu(padapter, &(pmlmeinfo->HT_caps), CHANNEL_WIDTH_40))
		...
		_rtw_memcpybu(&psta->htpriv.ht_cap, &pmlmeinfo->HT_caps, sizeof(struct rtw_ieee80211_ht_cap));
		#ifdef CONFIG_BEAMFORMING
		psta->htpriv.beamform_cap = pmlmepriv->htpriv.beamform_cap;
...
#ifdef CONFIG_80211AC_VHT
	_rtw_memcpybu(&psta->vhtpriv, &pmlmepriv->vhtpriv, sizeof(struct vht_priv));
...
#endif /* CONFIG_80211AC_VHT */
	psta->cmn.ra_info.is_support_sgi = query_ra_short_GIbu(psta, rtw_get_tx_bw_modebu(padapter, psta));
	update_ldpc_stbc_capbu(psta);
...
static void rtw_mlmeext_disconnect(_adapter *padapter)
{
	...
	if (self_action == MLME_STA_DISCONNECTED)
		correct_TSFbu(padapter, self_action);
#endif
	rtw_hal_set_hwregbu(padapter, HW_VAR_MLME_DISCONNECT, 0);
	rtw_hal_set_hwregbu(padapter, HW_VAR_BSSID, null_addrbu);
	if (self_action == MLME_STA_DISCONNECTED)
		rtw_hal_rcr_set_chk_bssidbu(padapter, self_action);

	/* set MSR to no link state->infra. mode */
	Set_MSRbu(padapter, _HW_STATE_STATION_);
	...
	if (state_backup == WIFI_FW_STATION_STATE) {
		if (rtw_port_switch_chkbu(padapter) == _TRUE) {
			rtw_hal_set_hwregbu(padapter, HW_VAR_PORT_SWITCH, NULL);
#ifdef CONFIG_LPS
			{
				_adapter *port0_iface = dvobj_get_port0_adapterbu(adapter_to_dvobj(padapter));
				if (port0_iface)
	...
	if (EN_FCS(padapter))
		rtw_hal_set_hwregbu(padapter, HW_VAR_STOP_FCS_MODE, NULL);
#endif
	...
	if (!(MLME_IS_STA(padapter) && MLME_IS_OPCH_SW(padapter))) {
		...
		if (rtw_mi_get_ch_setting_unionbu_no_self(padapter, &ch, &bw, &offset) != 0)
			set_channel_bwmodebu(padapter, ch, offset, bw);
		rtw_mi_update_union_chan_infbu(padapter, ch, offset, bw);
		rtw_rfctl_update_op_mode(adapter_to_rfctl(padapter), BIT(padapter->iface_id), 0);
	}

	flush_all_cam_entrybu(padapter);
...
void mlmeext_joinbss_event_callbackbu(_adapter *padapter, int join_res)
{
	...
	if (join_res < 0) {
		...
		rtw_hal_set_hwregbu(padapter, HW_VAR_BSSID, null_addrbu);
		if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE)
			rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_STA_DISCONNECTED);

		rtw_btcoex_connect_notify(padapter, join_type);

		goto exit_mlmeext_joinbss_event_callbackbu;
	}
	...
	update_IOT_infobu(padapter);

	#ifdef CONFIG_RTS_FULL_BW
	rtw_set_rts_bwbu(padapter);
	#endif/*CONFIG_RTS_FULL_BW*/

	rtw_hal_set_hwregbu(padapter, HW_VAR_BASIC_RATE, cur_network->SupportedRates);

	/* BCN interval */
	rtw_hal_set_hwregbu(padapter, HW_VAR_BEACON_INTERVAL, (u8 *)(&pmlmeinfo->bcn_interval));

	/* udpate capability */
	update_capinfobu(padapter, pmlmeinfo->capability);

	/* WMM, Update EDCA param */
	WMMOnAssocRspbubu(padapter);
#ifdef CONFIG_80211N_HT
	/* HT */
	HTOnAssocRspbubu(padapter);
#endif /* CONFIG_80211N_HT */
#ifdef CONFIG_80211AC_VHT
	/* VHT */
	VHTOnAssocRspbububu(padapter);
#endif
	...
	if (psta) { /* only for infra. mode */
		...
		set_sta_ratebu(padapter, psta);

		rtw_sta_media_status_rptbu(padapter, psta, 1);

		/* wakeup macid after join bss successfully to ensure
			the subsequent data frames can be sent out normally */
		rtw_hal_macid_wakeupbu(padapter, psta->cmn.mac_id);

		rtw_xmitbu_queue_clear(psta);
	}

#ifndef CONFIG_IOCTL_CFG80211
	if (is_wep_enc(psecuritypriv->dot11PrivacyAlgrthm))
		rtw_sec_restore_wep_keybu(padapter);
#endif /* CONFIG_IOCTL_CFG80211 */
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_STA_CONNECTED);

		/* correcting TSF */
		correct_TSFbu(padapter, MLME_STA_CONNECTED);
...
exit_mlmeext_joinbss_event_callbackbu:

	rtw_join_done_chk_chbu(padapter, join_res);
#ifdef CONFIG_RTW_REPEATER_SON
	...
	if ((psecuritypriv->dot11PrivacyAlgrthm == _NO_PRIVACY_) &&
		...
		while ((rtw_end_of_queue_searchbu(phead, plist)) == _FALSE) {
			...
			pkt = rframe->pkt;
			rframe->pkt = NULL;
			rtw_xmitbu_posthandlebu(padapter, rframe, pkt);
		}
...
void mlmeext_sta_add_event_callbackbu(_adapter *padapter, struct sta_info *psta)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE) {
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) { /* adhoc master or sta_count>1 */
			...
			/* correcting TSF */
			correct_TSFbu(padapter, MLME_ADHOC_STARTED);
	...
	update_sta_infobu(padapter, psta);

	rtw_hal_update_sta_ra_infobu(padapter, psta);

	/* ToDo: HT for Ad-hoc */
	psta->wireless_mode = rtw_check_network_typebu(psta->bssrateset, psta->bssratelen, pmlmeext->cur_channel);
	rtw_hal_set_odm_varbu(padapter, HAL_ODM_STA_INFO, psta, _TRUE);

	/* rate radaptive */
	Update_RA_Entrybu(padapter, psta);
}

void mlmeext_sta_del_event_callbackbu(_adapter *padapter)
{
	if (is_client_associated_to_apbu(padapter) || is_IBSS_emptybu(padapter))
...
void _linked_info_dumpbubu(_adapter *padapter)
{
	if (padapter->bLinkInfoDump) {
		rtw_hal_get_def_varbu(padapter, HW_DEF_RA_INFO_DUMP, RTW_DBGDUMP);
		rtw_hal_set_odm_varbu(padapter, HAL_ODM_RX_INFO_DUMP, RTW_DBGDUMP, _FALSE);
	}
}
/********************************************************************
...
*********************************************************************/
void rtw_delba_checkbu(_adapter *padapter, struct sta_info *psta, u8 from_timer)
{
	int	i = 0;
	int ret = _SUCCESS;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	...
	if (pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_BROADCOM) {
		for (i = 0; i < TID_NUM ; i++) {
			if ((psta->recvreorder_ctrl[i].enable) && 
					...
					if (_TRUE == rtw_inc_and_chk_continual_no_rx_packetbu(psta, i)) {					
						/* send a DELBA frame to the peer STA with the Reason Code field set to TIMEOUT */
						if (!from_timer)
							ret = issue_del_babu_ex(padapter, psta->cmn.mac_addr, i, 39, 0, 3, 1);
						else
							issue_del_babu(padapter,  psta->cmn.mac_addr, i, 39, 0);
						psta->recvreorder_ctrl[i].enable = _FALSE;
						if (ret != _FAIL)
							psta->recvreorder_ctrl[i].ampdu_size = RX_AMPDU_SIZE_INVALID;
						rtw_reset_continual_no_rx_packetbu(psta, i);
					}				
			} else {
...
u8 chk_ap_is_alivebu(_adapter *padapter, struct sta_info *psta)
{
	u8 ret = _FALSE;
...
u8 chk_adhoc_peer_is_alivebu(struct sta_info *psta)
{
	u8 ret = _TRUE;
...
void linked_status_chkbu_tdls(_adapter *padapter)
{
...
#define ALIVE_MAX 5
	...
	_rtw_memsetbu(&txmgmt, 0x00, sizeof(struct tdls_txmgmt));
	_rtw_memsetbu(checkalive, 0x00, sizeof(checkalive));
	_rtw_memsetbu(teardown, 0x00, sizeof(teardown));
	...
	if ((padapter->tdlsinfo.link_established == _TRUE)) {
					...
					if (psta->alive_count >= ALIVE_MIN) {
						if (chk_tdls_peer_sta_is_alive(padapter, psta) == _FALSE) {
							if (psta->alive_count < ALIVE_MAX) {
								_rtw_memcpybu(checkalive[num_checkalive].addr, psta->cmn.mac_addr, ETH_ALEN);
								checkalive[num_checkalive].psta = psta;
								num_checkalive++;
							} else {
								_rtw_memcpybu(teardown[num_teardown].addr, psta->cmn.mac_addr, ETH_ALEN);
		...
		if (num_checkalive > 0) {
			for (i = 0; i < num_checkalive; i++) {
				_rtw_memcpybu(txmgmt.peer, checkalive[i].addr, ETH_ALEN);
		...
		if (num_teardown > 0) {
			for (i = 0; i < num_teardown; i++) {
				...
				_rtw_memcpybu(txmgmt.peer, teardown[i].addr, ETH_ALEN);
...
inline int rtw_get_rx_chk_limitbu(_adapter *adapter)
{
	return adapter->stapriv.rx_chk_limit;
}

inline void rtw_set_rx_chk_limitbu(_adapter *adapter, int limit)
{
...
void linked_status_chkbu(_adapter *padapter, u8 from_timer)
{
	...
	if (is_client_associated_to_apbu(padapter)) {
	...
	rtw_rson_scan_wk_cmd(padapter, RSON_SCAN_PROCESS);
		...
		if (rtw_chk_roam_flags(padapter, RTW_ROAM_ACTIVE)) {
			...
			if ((precvpriv->signal_strength_data.avg_val < pmlmepriv->roam_rssi_threshold)
				...
				rtw_drv_scan_by_selfbu(padapter, RTW_AUTO_SCAN_REASON_ROAM);
		 ...
		 * we may check chk_ap_is_alivebu fail, and may issue_probereqbu to wrong channel under sitesurvey
		...
		rx_chk_limit = rtw_get_rx_chk_limitbu(padapter);
		...
		if (!from_timer && pmlmepriv->bGetGateway == 1 && pmlmepriv->GetGatewayTryCnt < 3) {
			...
			if (rtw_gw_addr_query(padapter) == 0)
				...
				_rtw_memsetbu(pmlmepriv->gw_ip, 0, 4);
				_rtw_memsetbu(pmlmepriv->gw_mac_addr, 0, ETH_ALEN);
			}
		}
#endif
#ifdef CONFIG_P2P
...
#ifdef CONFIG_TDLS
#ifdef CONFIG_TDLS_CH_SW
		if (ATOMIC_READbu(&padapter->tdlsinfo.chsw_info.chsw_on) == _TRUE)
			return;
#endif /* CONFIG_TDLS_CH_SW */

#ifdef CONFIG_TDLS_AUTOCHECKALIVE
		linked_status_chkbu_tdls(padapter);
		...
		if (psta != NULL) {
			...
			rtw_delba_checkbu(padapter, psta, from_timer);
#endif
			if (chk_ap_is_alivebu(padapter, psta) == _FALSE)
				rx_chk = _FAIL;
			...
			if (!from_timer && pmlmeext->active_keep_alive_check && (rx_chk == _FAIL || tx_chk == _FAIL)
				...
				if (switch_channel_by_drv) {
					if (!rtw_mi_get_ch_setting_unionbu(padapter, &union_ch, &union_bw, &union_offset)
						|| pmlmeext->cur_channel != union_ch)
							goto bypass_active_keep_alive;

					/* switch to correct channel of current network  before issue keep-alive frames */
					if (rtw_get_oper_chbu(padapter) != pmlmeext->cur_channel) {
						backup_ch = rtw_get_oper_chbu(padapter);
						backup_bw = rtw_get_oper_bwbu(padapter);
						backup_offset = rtw_get_oper_chbuoffset(padapter);
						set_channel_bwmodebu(padapter, union_ch, union_offset, union_bw);
					}
				}

				if (rx_chk != _SUCCESS)
					issue_probereqbu_ex(padapter, &pmlmeinfo->network.Ssid, psta->cmn.mac_addr, 0, 0, 3, 1);

				if ((tx_chk != _SUCCESS && pmlmeinfo->link_count++ == link_count_limit) || rx_chk != _SUCCESS) {
					if (rtw_mi_check_fwstatebu(padapter, WIFI_UNDER_SURVEY))
						tx_chk = issue_nulldatabu(padapter, psta->cmn.mac_addr, 1, 3, 1);
					else
						tx_chk = issue_nulldatabu(padapter, psta->cmn.mac_addr, 0, 3, 1);
				...
				if (backup_ch > 0 && switch_channel_by_drv)
					set_channel_bwmodebu(padapter, backup_ch, backup_offset, backup_bw);
...
bypass_active_keep_alive:
...
#endif /* CONFIG_ACTIVE_KEEP_ALIVE_CHECK */
			{
				if (rx_chk != _SUCCESS) {
					if (pmlmeext->retry == 0) {
#ifdef DBG_EXPIRATION_CHK
						RTW_INFO("issue_probereqbu to trigger probersp, retry=%d\n", pmlmeext->retry);
#endif
						issue_probereqbu_ex(padapter, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
						issue_probereqbu_ex(padapter, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
						issue_probereqbu_ex(padapter, &pmlmeinfo->network.Ssid, pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
					}
				}
				...
				if (tx_chk != _SUCCESS && pmlmeinfo->link_count++ == link_count_limit
					...
					#ifdef DBG_EXPIRATION_CHK
					RTW_INFO("%s issue_nulldatabu(%d)\n", __FUNCTION__, from_timer ? 1 : 0);
					#endif
					if (from_timer || rtw_mi_check_fwstatebu(padapter, WIFI_UNDER_SURVEY))
						tx_chk = issue_nulldatabu(padapter, NULL, 1, 0, 0);
					else
						tx_chk = issue_nulldatabu(padapter, NULL, 0, 1, 1);
				}
			}
			...
			if (rx_chk == _FAIL) {
				...
				if (pmlmeext->retry > rx_chk_limit) {
					RTW_PRINT(FUNC_ADPT_FMT" disconnect or roaming\n",
						  FUNC_ADPT_ARG(padapter));
					receive_disconnectbu(padapter, pmlmeinfo->network.MacAddress
		...
		} /* end of if ((psta = rtw_get_stainfobu(pstapriv, passoc_res->network.MacAddress)) != NULL) */

	} else if (is_client_associated_to_ibssbu(padapter)) {
		_irqL irqL;
		_list *phead, *plist, dlist;

		_rtw_init_listheadbu(&dlist);
		...
		for (i = 0; i < NUM_STA; i++) {
			...
			while ((rtw_end_of_queue_searchbu(phead, plist)) == _FALSE) {
				...
				if (chk_adhoc_peer_is_alivebu(psta) || !psta->expire_to)
					psta->expire_to = pstapriv->adhoc_expire_to;
				else
					psta->expire_to--;
				...
				if (psta->expire_to <= 0) {
					...
					rtw_list_insert_tailbu(&psta->list, &dlist);
				}
			}
		}
		...
		while (rtw_end_of_queue_searchbu(&dlist, plist) == _FALSE) {
			psta = LIST_CONTAINOR(plist, struct sta_info, list);
			plist = get_next(plist);
			rtw_list_delete(&psta->list);
			RTW_INFO(FUNC_ADPT_FMT" ibss expire "MAC_FMT"\n"
				, FUNC_ADPT_ARG(padapter), MAC_ARG(psta->cmn.mac_addr));
			report_del_sta_eventbu(padapter, psta->cmn.mac_addr, WLAN_REASON_EXPIRATION_CHK, from_timer ? _TRUE : _FALSE, _FALSE);
		}
	}
...
void survey_timer_hdlbu(void *ctx)
{
	...
	if (mlmeext_scan_state(pmlmeext) > SCAN_DISABLE) {
		...
		rtw_enqueue_cmdbu(pcmdpriv, cmd);
	}
...
void rtw_tbtx_xmit_timer_hdl(void *ctx)
{
	...
	if (ATOMIC_READbu(&padapter->tbtx_remove_tx_pause) == _TRUE){
		ATOMIC_SETbu(&padapter->tbtx_tx_pause, _FALSE);
		rtw_tx_control_cmd(padapter);
	}else {
		rtw_issue_action_token_rel(padapter);
		ATOMIC_SETbu(&padapter->tbtx_tx_pause, _TRUE);
		rtw_tx_control_cmd(padapter);
		_set_timer(&pmlmeext->tbtx_xmit_timer, MAX_TXPAUSE_DURATION);
		ATOMIC_SETbu(&padapter->tbtx_remove_tx_pause, _TRUE);
	}
}
...
outof_loop:
	...
	ATOMIC_SETbu(&pstapriv->nr_token_keeper, nr_send);
...
void link_timer_hdlbu(void *ctx)
{
	...
	if (rtw_sta_linking_test_force_failbu())
		RTW_INFO("rtw_sta_linking_test_force_failbu\n");

	if (pmlmeext->join_abort && pmlmeinfo->state != WIFI_FW_NULL_STATE) {
		RTW_INFO(FUNC_ADPT_FMT" join abort\n", FUNC_ADPT_ARG(padapter));
		pmlmeinfo->state = WIFI_FW_NULL_STATE;
		report_join_resbu(padapter, -4, WLAN_STATUS_UNSPECIFIED_FAILURE);
		goto exit;
	}

	if (pmlmeinfo->state & WIFI_FW_AUTH_NULL) {
		RTW_INFO("link_timer_hdlbu:no beacon while connecting\n");
		pmlmeinfo->state = WIFI_FW_NULL_STATE;
		report_join_resbu(padapter, -3, WLAN_STATUS_UNSPECIFIED_FAILURE);
		...
		if (++pmlmeinfo->reauth_count > REAUTH_LIMIT) {
			...
			if (pmlmeinfo->auth_status) {
				report_join_resbu(padapter, -1, pmlmeinfo->auth_status);
		...
		RTW_INFO("link_timer_hdlbu: auth timeout and try again\n");
		pmlmeinfo->auth_seq = 1;
		issue_authbu(padapter, NULL, 0);
		set_link_timer(pmlmeext, REAUTH_TO);
...
#endif
			report_join_resbu(padapter, -2, WLAN_STATUS_UNSPECIFIED_FAILURE);
			return;
		}

#ifdef CONFIG_RTW_80211R
		if (rtw_ft_roam(padapter)) {
			RTW_INFO("link_timer_hdlbu: reassoc timeout and try again\n");
			issue_reassocreqbu(padapter);
		} else
#endif
		{
			RTW_INFO("link_timer_hdlbu: assoc timeout and try again\n");
			issue_assocreqbu(padapter);
		}
...
#ifdef CONFIG_IEEE80211W
void report_sta_timeout_eventbu(_adapter *padapter, u8 *MacAddr, unsigned short reason)
{
	struct cmd_obj *pcmd_obj;
	u8 *pevtcmd;
	u32 cmdsz;
	...
	RTW_INFO("report_del_sta_eventbu: delete STA, mac_id=%d\n", mac_id);
...
void clnt_sa_query_timeoutbu(_adapter *padapter)
{
	struct mlme_ext_priv *mlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info *mlmeinfo = &(mlmeext->mlmext_info);

	RTW_INFO(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(padapter));
	receive_disconnectbu(padapter, get_my_bssidbu(&(mlmeinfo->network)), WLAN_REASON_SA_QUERY_TIMEOUT, _FALSE);
}
...
void sa_query_timer_hdlbu(void *ctx)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE &&
		...
		clnt_sa_query_timeoutbu(padapter);
	else if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
		report_sta_timeout_eventbu(padapter, psta->cmn.mac_addr, WLAN_REASON_PREV_AUTH_NOT_VALID);
}
...
void rtw_start_auto_ap(_adapter *adapter)
{
	...
	rtw_set_802_11_infrastructure_modebu(adapter, Ndis802_11APMode, 0);

	rtw_setopmode_cmdbu(adapter, Ndis802_11APMode, RTW_CMDF_WAIT_ACK);
}
...
static int rtw_auto_ap_start_beacon(_adapter *adapter)
{
	...
	*(u16 *)ie |= cpu_to_le16(cap_ShortPremble);
	...
	/* SSID */
	ie = rtw_set_iebu(ie, _SSID_IE_, ssid_len, ssid, &sz);

	/* supported rates */
	wireless_mode = (WIRELESS_11BG_24N & padapter->registrypriv.wireless_mode);
	rtw_set_supported_ratebu(supportRate, wireless_mode);
	rateLen = rtw_get_rateset_lenbu(supportRate);
	if (rateLen > 8)
		ie = rtw_set_iebu(ie, _SUPPORTEDRATES_IE_, 8, supportRate, &sz);
	else
		ie = rtw_set_iebu(ie, _SUPPORTEDRATES_IE_, rateLen, supportRate, &sz);


	/* DS parameter set */
	if (rtw_mi_check_statusbu(adapter, MI_LINKED))
		oper_channel = rtw_mi_get_union_chan(adapter);
	else
		oper_channel = adapter_to_dvobj(adapter)->oper_channel;

	ie = rtw_set_iebu(ie, _DSSET_IE_, 1, &oper_channel, &sz);

	/* ext supported rates */
	if (rateLen > 8)
		ie = rtw_set_iebu(ie, _EXT_SUPPORTEDRATES_IE_, (rateLen - 8), (supportRate + 8), &sz);

	RTW_INFO("%s, start auto ap beacon sz=%d\n", __FUNCTION__, sz);

	/* lunch ap mode & start to issue beacon */
	if (rtw_check_beacon_databu(adapter, pbuf,  sz) == _SUCCESS) {
...
u8 tx_control_hdl(_adapter *adapter)
{
	...
	if(ATOMIC_READbu(&adapter->tbtx_tx_pause))
		val = 0xff;
	else
		val = 0x00;

	rtw_hal_set_hwregbu(adapter, HW_VAR_TXPAUSE, &val);
...
u8 stop_ap_hdlbu(_adapter *adapter)
{
	...
	rtw_set_802_11_infrastructure_modebu(adapter, Ndis802_11Infrastructure, RTW_CMDF_DIRECTLY);
	rtw_setopmode_cmdbu(adapter, Ndis802_11Infrastructure, RTW_CMDF_DIRECTLY);
...
u8 setopmode_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	rtw_hal_set_hwregbu(padapter, HW_VAR_PORT_SWITCH, (u8 *)(&type));
#endif

	rtw_hal_set_hwregbu(padapter, HW_VAR_SET_OPMODE, (u8 *)(&type));
...
u8 createbss_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	if ((parm->req_ch == 0 && pmlmeinfo->state == WIFI_FW_AP_STATE)
		...
		start_bss_networkbu(padapter, parm);
		goto exit;
	}
	...
	if (parm->adhoc) {
		...
		_rtw_memcpybu(pnetwork, pdev_network, FIELD_OFFSET(WLAN_BSSID_EX, IELength));
		...
		_rtw_memcpybu(pnetwork->IEs, pdev_network->IEs, pnetwork->IELength);
		start_create_ibssbu(padapter);
	} else {
		rtw_warn_on(1);
		ret = H2C_PARAMETERS_ERROR;
	}
...
ibss_post_hdl:
	rtw_create_ibss_post_hdlbu(padapter, ret);
...
u8 join_cmd_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
		...
		/* Set_MSRbu(padapter, _HW_STATE_NOLINK_); */
		Set_MSRbu(padapter, _HW_STATE_STATION_);
	...
	_rtw_memsetbu(pmlmeinfo->SupportedRates_infra_ap, 0, NDIS_802_11_LENGTH_RATES_EX);
#endif /* ROKU_PRIVATE */
	_rtw_memcpybu(pnetwork, pbuf, FIELD_OFFSET(WLAN_BSSID_EX, IELength));
	...
	_rtw_memcpybu(pnetwork->IEs, ((WLAN_BSSID_EX *)pbuf)->IEs, pnetwork->IELength);

	pmlmeinfo->bcn_interval = get_beacon_intervalbu(pnetwork);

	/* Check AP vendor to move rtw_joinbss_cmdbu() */
	/* pmlmeinfo->assoc_AP_vendor = check_assoc_APbu(pnetwork->IEs, pnetwork->IELength); */
	...
	for (i = _FIXED_IE_LENGTH_ ; i < pnetwork->IELength - 2 ;) {
		...
		case _VENDOR_SPECIFIC_IE_: /* Get WMM IE. */
			if (_rtw_memcmpbu(pIE->data, WMM_OUIbu, 4))
				WMM_param_handlerbu(padapter, pIE);
			break;
	...
	rtw_bss_get_chbwbu(pnetwork
		, &pmlmeext->cur_channel, &pmlmeext->cur_bwmode, &pmlmeext->cur_ch_offset, 1, 1);

	rtw_adjust_chbwbu(padapter, pmlmeext->cur_channel, &pmlmeext->cur_bwmode, &pmlmeext->cur_ch_offset);

#if 0
	if (padapter->registrypriv.wifi_spec) {
		/* for WiFi test, follow WMM test plan spec */
		acparm = 0x002F431C; /* VO */
		rtw_hal_set_hwregbu(padapter, HW_VAR_AC_PARAM_VO, (u8 *)(&acparm));
		acparm = 0x005E541C; /* VI */
		rtw_hal_set_hwregbu(padapter, HW_VAR_AC_PARAM_VI, (u8 *)(&acparm));
		acparm = 0x0000A525; /* BE */
		rtw_hal_set_hwregbu(padapter, HW_VAR_AC_PARAM_BE, (u8 *)(&acparm));
		acparm = 0x0000A549; /* BK */
		rtw_hal_set_hwregbu(padapter, HW_VAR_AC_PARAM_BK, (u8 *)(&acparm));
	...
	if (rtw_chk_start_clnt_joinbubu(padapter, &u_ch, &u_bw, &u_offset) == _FAIL) {
		report_join_resbu(padapter, (-4), WLAN_STATUS_UNSPECIFIED_FAILURE);
		return H2C_SUCCESS;
	}
	...
	/* initialgain = 0x1E; */
	/*rtw_hal_set_odm_varbu(padapter, HAL_ODM_INITIAL_GAIN, &initialgain, _FALSE);*/

	rtw_hal_set_hwregbu(padapter, HW_VAR_BSSID, pmlmeinfo->network.MacAddress);
	if (MLME_IS_STA(padapter))
		rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_STA_CONNECTING);
	else
		rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_ADHOC_STARTED);
	...
	set_channel_bwmodebu(padapter, u_ch, u_offset, u_bw);
	...
	if (rnetwork && (pmlmeinfo->state & (WIFI_FW_AUTH_NULL | WIFI_FW_AUTH_STATE))) {
		...
		update_networkbu(&(pmlmepriv->cur_network.network), &(rnetwork->network), padapter, _TRUE);

		/* update bcn keys */
		if (rtw_get_bcn_keysbu_from_bss(&rnetwork->network, &bcn_keys) == _TRUE) {
			_rtw_memcpybu(&pmlmepriv->cur_beacon_keys, &bcn_keys, sizeof(bcn_keys));
			...
			RTW_ERR("%s: get beacon keys failed\n", __func__);
			_rtw_memsetbu(&pmlmepriv->cur_beacon_keys, 0, sizeof(bcn_keys));
		}
		#ifdef CONFIG_BCN_CNT_CONFIRM_HDL
		pmlmepriv->new_beacon_cnts = 0;
		#endif
		...
		/* check the vendor of the assoc AP */
		pmlmeinfo->assoc_AP_vendor = check_assoc_APbu(rnetwork->network.IEs, rnetwork->network.IELength);
...
u8 disconnect_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
#if CONFIG_DFS
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
#endif
	...
	if (is_client_associated_to_apbu(padapter)
		...
		issue_deauthbu_ex(padapter, pnetwork->MacAddress, WLAN_REASON_DEAUTH_LEAVING, 1, 100);
		#else
		issue_deauthbu_ex(padapter, pnetwork->MacAddress, WLAN_REASON_DEAUTH_LEAVING, param->deauth_timeout_ms / 100, 100);
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE) || ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE)) {
		/* Stop BCN */
		val8 = 0;
		rtw_hal_set_hwregbu(padapter, HW_VAR_BCN_FUNC, (u8 *)(&val8));
	}
#endif

	rtw_sta_mstatus_reportbu(padapter);
	
	rtw_mlmeext_disconnect(padapter);

	rtw_free_uc_swdec_pending_queuebu(padapter);

	return	H2C_SUCCESS;
}

static const char *const _scan_state_strbu[] = {
	"SCAN_DISABLE",
	"SCAN_START",
	"SCAN_PS_ANNC_WAIT",
	"SCAN_ENTER",
...
const char *scan_state_strbu(u8 state)
{
	...
	return _scan_state_strbu[state];
}
...
u8 rtw_scan_sparsebu(_adapter *adapter, struct rtw_ieee80211_channel *ch, u8 ch_num)
{
...
#define SCAN_SPARSE_CH_NUM_INVALID 255
	...
	if (rtw_mi_busy_traffic_checkbu(adapter))
		busy_traffic = _TRUE;

	if (rtw_mi_check_miracast_enabledbu(adapter))
	...
	if (max_allow_ch != SCAN_SPARSE_CH_NUM_INVALID) {
		...
		for (i = 0; i < ch_num; i++) {
			if (ch[i].hw_value && (i % scan_division_num) == token
				...
				if (i != k)
					_rtw_memcpybu(&ch[k], &ch[i], sizeof(struct rtw_ieee80211_channel));
				k++;
			}
		}

		_rtw_memsetbu(&ch[k], 0, sizeof(struct rtw_ieee80211_channel));
...
#ifdef CONFIG_SCAN_BACKOP
u8 rtw_scan_backop_decisionbu(_adapter *adapter)
{
	struct mlme_ext_priv *mlmeext = &adapter->mlmeextpriv;
	struct mi_state mstate;
	u8 backop_flags = 0;

	rtw_mi_statusbu(adapter, &mstate);
...
u32 rtw_scan_timeout_decisionbu(_adapter *padapter)
{
	...
	#ifdef CONFIG_SCAN_BACKOP
	if (rtw_scan_backop_decisionbu(padapter))
		...
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num, bool no_sparse, int reason)
{
	...
	/* clear first */
	_rtw_memsetbu(out, 0, sizeof(struct rtw_ieee80211_channel) * out_num);

#ifdef CONFIG_RTW_ROAM_QUICKSCAN
	if ((reason == RTW_AUTO_SCAN_REASON_ROAM) && (pmlmeext->quickscan_next)) {
		pmlmeext->quickscan_next = _FALSE;
		_rtw_memcpybu(out, pmlmeext->roam_ch, sizeof(struct rtw_ieee80211_channel)*RTW_CHANNEL_SCAN_AMOUNT);
	...
	for (i = 0; i < in_num; i++) {
		...
		if (!in[i].hw_value || (in[i].flags & RTW_IEEE80211_CHAN_DISABLED))
			continue;
		if (rtw_mlme_band_checkbu(padapter, in[i].hw_value) == _FALSE)
			continue;

		set_idx = rtw_chset_search_chbu(rfctl->channel_set, in[i].hw_value);
		if (set_idx >= 0) {
			if (j >= out_num) {
			...
			_rtw_memcpybu(&out[j], &in[i], sizeof(struct rtw_ieee80211_channel));
	...
	if (j == 0) {
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			...
			if (rtw_mlme_band_checkbu(padapter, chan) == _TRUE) {
				if (rtw_mlme_ignore_chanbu(padapter, chan) == _TRUE)
					continue;
	...
	if (!no_sparse
		...
		/* scan_sparse */
		j = rtw_scan_sparsebu(padapter, out, j);
	}
...
static void sitesurvey_res_reset(_adapter *adapter, struct sitesurvey_parm *parm)
{
	...
	for (i = 0; i < RTW_SSID_SCAN_AMOUNT; i++) {
		if (parm->ssid[i].SsidLength) {
			_rtw_memcpybu(ss->ssid[i].Ssid, parm->ssid[i].Ssid, IW_ESSID_MAX_SIZE);
...
static u8 sitesurvey_pick_ch_behavior(_adapter *padapter, u8 *ch, RT_SCAN_TYPE *type)
{
	...
	if (pwdinfo->rx_invitereq_info.scan_op_ch_only || pwdinfo->p2p_info.scan_op_ch_only) {
		...
		*/
		...
		ch_set_idx = rtw_chset_search_chbu(rfctl->channel_set, scan_ch);
		if (ch_set_idx >= 0)
...
#endif /* CONFIG_P2P */
	{
		...
		#ifdef CONFIG_SCAN_BACKOP
		backop_flags = rtw_scan_backop_decisionbu(padapter);
		#endif
		...
		#endif
		{
			...
			if (ss->channel_idx != 0 && ss->force_ssid_scan == 0
				...
				ch_set_idx = rtw_chset_search_chbu(rfctl->channel_set, ss->ch[ss->channel_idx - 1].hw_value);
	...
	if (scan_ch != 0) {
...
#if defined(DBG_SCAN_SW_ANTDIV_BL)
		{
			...
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (rtw_linked_checkbu(dvobj->padapters[i]))
					is_linked = _TRUE;
			}
...
void site_surveybu(_adapter *padapter, u8 survey_channel, RT_SCAN_TYPE ScanType)
{
	...
	if (survey_channel != 0) {
		set_channel_bwmodebu(padapter, survey_channel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
		...
		if (ScanType == SCAN_PASSIVE && ss->force_ssid_scan)
...
#ifdef CONFIG_P2P
			#ifdef CONFIG_IOCTL_CFG80211
			if (rtw_cfg80211_is_p2p_scanbu(padapter))
			#else
...
#endif /* CONFIG_P2P */
			{
				if (pmlmeext->sitesurvey_res.scan_mode == SCAN_ACTIVE) {
					...
					if (padapter->registrypriv.wifi_spec)
						issue_probereqbu(padapter, NULL, NULL);
					else
						issue_probereqbu_ex(padapter, NULL, NULL, 0, 0, 0, 0);
					issue_probereqbu(padapter, NULL, NULL);
				}
		...
		if (ssid_scan) {
			...
			for (i = 0; i < RTW_SSID_SCAN_AMOUNT; i++) {
				if (pmlmeext->sitesurvey_res.ssid[i].SsidLength) {
					/* IOT issue, When wifi_spec is not set, send one probe req without WPS IE. */
					if (padapter->registrypriv.wifi_spec)
						issue_probereqbu(padapter, &(pmlmeext->sitesurvey_res.ssid[i]), NULL);
					else
						issue_probereqbu_ex(padapter, &(pmlmeext->sitesurvey_res.ssid[i]), NULL, 0, 0, 0, 0);
					issue_probereqbu(padapter, &(pmlmeext->sitesurvey_res.ssid[i]), NULL);
				}
			}
		}
...
void survey_done_set_ch_bwbu(_adapter *padapter)
{
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	u8 cur_channel = 0;
	u8 cur_bwmode;
	...
	if (rtw_mi_get_ch_setting_unionbu(padapter, &cur_channel, &cur_bwmode, &cur_ch_offset) != 0) {
		if (0)
...
exit:
#endif
	set_channel_bwmodebu(padapter, cur_channel, cur_ch_offset, cur_bwmode);
}

/**
 * rtw_ps_anncbu - check and doing ps announcement for all the adapters
 * @adapter: the requesting adapter
 * @ps: power saving or not
 *
 * Returns: 0: no ps announcement is doing. 1: ps announcement is doing
 */
u8 rtw_ps_anncbu(_adapter *adapter, bool ps)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	_adapter *iface;
	int i;
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (MLME_IS_STA(iface)) {
			if (is_client_associated_to_apbu(iface) == _TRUE) {
				/* TODO: TDLS peers */
				#ifdef CONFIG_MCC_MODE
				...
				if (MCC_EN(adapter) && rtw_hal_check_mcc_status(adapter, MCC_STATUS_NEED_MCC)) {
					...
					set_channel_bwmodebu(iface, ch, offset, bw);
				}
				#endif /* CONFIG_MCC_MODE */
				issue_nulldatabu(iface, NULL, ps, 3, 500);
				ps_anc = 1;
			}
		#ifdef CONFIG_RTW_MESH
...
void rtw_leave_opchbu(_adapter *adapter)
{
	...
	if (rfctl->offch_state == OFFCHS_NONE) {
		...
		/* clear HW TX queue */
		rtw_hal_set_hwregbu(adapter, HW_VAR_CHECK_TXBUF, 0);

		rtw_hal_macid_sleepbu_all_used(adapter);

		rtw_ps_anncbu(adapter, 1);
...
void rtw_back_opchbu(_adapter *adapter)
{
	...
	if (rfctl->offch_state != OFFCHS_NONE) {
		...
		rtw_hal_macid_wakeupbu_all_used(adapter);
		rtw_ps_anncbu(adapter, 0);

		rfctl->offch_state = OFFCHS_NONE;
		rtw_mi_os_xmit_schedulebu(adapter);
	}

	_exit_critical_mutex(&rfctl->offch_mutex, NULL);
}

void sitesurvey_set_igibu(_adapter *adapter)
{
	struct mlme_ext_priv *mlmeext = &adapter->mlmeextpriv;
	...
	case SCAN_ENTER:
		#ifdef CONFIG_P2P
		#ifdef CONFIG_IOCTL_CFG80211
		if (pwdinfo->driver_interface == DRIVER_CFG80211 && rtw_cfg80211_is_p2p_scanbu(adapter))
			igi = 0x30;
		else
		#endif /* CONFIG_IOCTL_CFG80211 */
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE))
		...
		/* record IGI status */
		ss->igi_scan = igi;
		rtw_hal_get_odm_varbu(adapter, HAL_ODM_INITIAL_GAIN, &ss->igi_before_scan, NULL);

		/* disable DIG and set IGI for scan */
		rtw_hal_set_odm_varbu(adapter, HAL_ODM_INITIAL_GAIN, &igi, _FALSE);
		break;
	case SCAN_COMPLETE:
	...
	case SCAN_BACKING_OP:
		...
		odm_write_digbu(adapter_to_phydm(adapter), ss->igi_before_scan);
		break;
	case SCAN_LEAVE_OP:
		/* write IGI for scan when DIG is not enabled */
		odm_write_digbu(adapter_to_phydm(adapter), ss->igi_scan);
		break;
...
void sitesurvey_set_msrbu(_adapter *adapter, bool enter)
{
	u8 network_type;
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	if (enter) {
		...
		rtw_hal_get_hwregbu(adapter, HW_VAR_MEDIA_STATUS, (u8 *)(&pmlmeinfo->hw_media_state));
#endif
	...
	Set_MSRbu(adapter, network_type);
}
...
#ifdef CONFIG_RTW_ROAM_QUICKSCAN
extern inline int is_same_essbu(WLAN_BSSID_EX *a, WLAN_BSSID_EX *b);
extern int rtw_is_desired_networkbu(_adapter *adapter, struct wlan_network *pnetwork);
void generate_quickss(_adapter *padapter)
{
	...
	_rtw_memsetbu(mark, 0, sizeof(u8)*166);
	...
	while (1) {
		...
		if (is_same_essbu(&pnetwork->network, &pmlmepriv->cur_network.network) &&
				rtw_is_desired_networkbu(padapter, pnetwork))
			mark[ds_cfg] = 1;
...
operation_by_state:
	...
	case SCAN_START:
		...
		*/
		...
		if (ss->rx_ampdu_accept != RX_AMPDU_ACCEPT_INVALID
			...
			rtw_rx_ampdu_applybu(padapter);

		/* clear HW TX queue before scan */
		rtw_hal_set_hwregbu(padapter, HW_VAR_CHECK_TXBUF, 0);

		rtw_hal_macid_sleepbu_all_used(padapter);

		/* power save state announcement */
		if (rtw_ps_anncbu(padapter, 1)) {
	...
	case SCAN_ENTER:
		...
		*/
		...
		rtw_phydm_abilitybu_backup(padapter);

		sitesurvey_set_igibu(padapter);

		/* config dynamic functions for off channel */
		rtw_phydm_func_for_offchannel(padapter);
		/* set MSR to no link state */
		sitesurvey_set_msrbu(padapter, _TRUE);
	...
	case SCAN_PROCESS: {
		...
		/* still SCAN_PROCESS state */
			...
			RTW_INFO(FUNC_ADPT_FMT" %s ch:%u (cnt:%u,idx:%d) at %dms, %c%c%c%c\n"
				, FUNC_ADPT_ARG(padapter)
				, mlmeext_scan_state_strbu(pmlmeext)
				, scan_ch
		...
		site_surveybu(padapter, scan_ch, scan_type);
	...
	case SCAN_BACKING_OP: {
		...
		if (need_ch_setting_union) {
			if (rtw_mi_get_ch_setting_unionbu(padapter, &back_ch, &back_bw, &back_ch_offset) == 0) {
				rtw_warn_on(1);
		...
		#endif /*DBG_SITESURVEY*/
		set_channel_bwmodebu(padapter, back_ch, back_ch_offset, back_bw);

		sitesurvey_set_msrbu(padapter, _FALSE);
		...
		if (mlmeext_chk_scan_backop_flags(pmlmeext, SS_BACKOP_PS_ANNC)) {
			sitesurvey_set_igibu(padapter);
			rtw_hal_macid_wakeupbu_all_used(padapter);
			rtw_ps_anncbu(padapter, 0);
		}
		...
		if (mlmeext_chk_scan_backop_flags(pmlmeext, SS_BACKOP_TX_RESUME))
			rtw_mi_os_xmit_schedulebu(padapter);
	...
	case SCAN_TO_P2P_LISTEN:
		...
		*/
		...
		rtw_phydm_abilitybu_restore(padapter);
	...
	case SCAN_COMPLETE:
		...
		/* switch channel */
		survey_done_set_ch_bwbu(padapter);
		...
		report_surveydone_eventbu(padapter, ss->acs);
#ifdef CONFIG_RTW_ACS
		if (IS_ACS_ENABLE(padapter))
...
#endif
		issue_action_BSSCoexistPacketbu(padapter);
		issue_action_BSSCoexistPacketbu(padapter);
		issue_action_BSSCoexistPacketbu(padapter);
...
u8 setauth_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
...
u8 setkey_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	unsigned char null_addrbu[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	u8 *addr;
	bool used = _FALSE;
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) || ((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE))
		cam_id = rtw_iface_bcmc_id_getbu(padapter);
	else
#endif
		cam_id = rtw_camid_allocbu(padapter, NULL, pparm->keyid, 1,
	...
	if (used == _TRUE && rtw_camid_is_gkbu(padapter, cam_id) == _FALSE) {
		s16 camid_clr;
		...
		rtw_camctl_set_flagsbu(padapter, SEC_STATUS_STA_PK_GK_CONFLICT_DIS_BMC_SEARCH);
		rtw_hal_set_hwregbu(padapter, HW_VAR_SEC_CFG, NULL);

		/* clear group key */
		while ((camid_clr = rtw_camid_searchbu(padapter, addr, -1, 1)) >= 0) {
			RTW_PRINT("clear group key for addr:"MAC_FMT", camid:%d\n", MAC_ARG(addr), camid_clr);
			clear_cam_entrybu(padapter, camid_clr);
			rtw_camid_freebu(padapter, camid_clr);
		}
	...
	RTW_PRINT("set group key camid:%d, addr:"MAC_FMT", kid:%d, type:%s\n"
		, cam_id, MAC_ARG(addr), pparm->keyid, security_type_strbu(pparm->algorithm));
	...
	if (amsdu_spp_enable(padapter, pparm->algorithm) == _SUCCESS)
		ctrl |= BIT(7);
	write_cambu(padapter, cam_id, ctrl, addr, pparm->key);
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) || ((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE)) {
		if (is_wep_enc(pparm->algorithm)) {
			...
			if ((org_cam_id != INVALID_SEC_MAC_CAM_ID) &&
				...
				clear_cam_entrybu(padapter, org_cam_id);
				rtw_camid_freebu(padapter, org_cam_id);
			}*/
		}
	}
#else
	if (cam_id >= 0 && cam_id <= 3)
		rtw_hal_set_hwregbu(padapter, HW_VAR_SEC_DK_CFG, (u8 *)_TRUE);
#endif
	...
	if (is_wep_enc(pparm->algorithm) && check_mlmeinfo_state(pmlmeext, WIFI_FW_STATION_STATE) &&
	    _rtw_camctl_chk_capbu(padapter, SEC_CAP_CHK_BMC)) {
		struct set_stakey_parm	sta_pparm;

		_rtw_memsetbu(&sta_pparm, 0, sizeof(struct set_stakey_parm));
		sta_pparm.algorithm = pparm->algorithm;
		sta_pparm.keyid = pparm->keyid;
		_rtw_memcpybu(sta_pparm.key, pparm->key, 16);
		_rtw_memcpybu(sta_pparm.addr, get_bssid(&padapter->mlmepriv), ETH_ALEN);
		set_stakey_hdlbu(padapter, (u8 *)&sta_pparm);
	}

enable_mc:
	/* allow multicast packets to driver */
	rtw_hal_set_hwregbu(padapter, HW_VAR_ON_RCR_AM, null_addrbu);
#ifdef CONFIG_LAYER2_ROAMING
...
void rtw_ap_wep_pk_settingbu(_adapter *adapter, struct sta_info *psta)
{
	...
	for (keyid = 0; keyid < 4; keyid++) {
		if ((psecuritypriv->key_mask & BIT(keyid)) && (keyid == psecuritypriv->dot11PrivacyKeyIndex)) {
			...
			sta_pparm.gk = 0;
			_rtw_memcpybu(sta_pparm.key, &(psecuritypriv->dot11DefKey[keyid].skey[0]), 16);
			_rtw_memcpybu(sta_pparm.addr, psta->cmn.mac_addr, ETH_ALEN);

			RTW_PRINT(FUNC_ADPT_FMT"set WEP - PK with "MAC_FMT" keyid:%u\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr), keyid);

			set_stakey_hdlbu(adapter, (u8 *)&sta_pparm);
		}
	}
}
...
u8 set_stakey_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	psta = rtw_get_stainfobu(pstapriv, pparm->addr);
	if (!psta) {
	...
	cam_id = rtw_camid_allocbu(padapter, psta, pparm->keyid, pparm->gk,
		!!(pparm->algorithm & _SEC_TYPE_256_), &used);
	if (cam_id < 0)
		goto exit;
	...
	if (!pparm->gk && used == _TRUE && rtw_camid_is_gkbu(padapter, cam_id) == _TRUE) {
		s16 camid_clr;
		...
		/* clear group key */
		while ((camid_clr = rtw_camid_searchbu(padapter, pparm->addr, -1, 1)) >= 0) {
...
write_to_cam:
	if (pparm->algorithm == _NO_PRIVACY_) {
		while ((cam_id = rtw_camid_searchbu(padapter, pparm->addr, -1, -1)) >= 0) {
			RTW_PRINT("clear key for addr:"MAC_FMT", camid:%d\n", MAC_ARG(pparm->addr), cam_id);
			clear_cam_entrybu(padapter, cam_id);
			rtw_camid_freebu(padapter, cam_id);
		}
	} else {
		RTW_PRINT("set %s key camid:%d, addr:"MAC_FMT", kid:%d, type:%s\n"
			, pparm->gk ? "group" : "pairwise"
			, cam_id, MAC_ARG(pparm->addr), pparm->keyid, security_type_strbu(pparm->algorithm));
		...
		if (amsdu_spp_enable(padapter, pparm->algorithm) == _SUCCESS)
			ctrl |= BIT(7);
		write_cambu(padapter, cam_id, ctrl, pparm->addr, pparm->key);
		if (!(pparm->gk))
			ATOMIC_INCbu(&psta->keytrack);	/*CVE-2020-24587*/
	}
	ret = H2C_SUCCESS_RSP;
...
u8 add_ba_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	struct sta_info *psta = rtw_get_stainfobu(&padapter->stapriv, pparm->addr);
	...
	if (((pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) && (pmlmeinfo->HT_enable)) ||
		...
		issue_addba_reqbu(padapter, pparm->addr, (u8)pparm->tid);
...
u8 add_ba_rsp_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	ret = issue_addba_rspbu_wait_ack(padapter, pparm->addr, pparm->tid, pparm->status, pparm->size, 3, 50);
...
#else
	rtw_set_bitbu(RTW_RECV_ACK_OR_TIMEOUT, &preorder_ctrl->rec_abba_rsp_ack);
	#ifdef DBG_RX_SEQ
...
u8 set_tx_beacon_cmdbu(_adapter *padapter, u8 flags)
{
	struct cmd_obj	*ph2c;
	...
	_rtw_memcpybu(&(ptxBeacon_parm->network), &(pmlmeinfo->network), sizeof(WLAN_BSSID_EX));
...
u8 mlme_evt_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (evt_hdr->seq != (ATOMIC_READbu(&pevt_priv->event_seq) & 0x7f)) {
	...
	ATOMIC_INCbu(&pevt_priv->event_seq);
...
u8 chk_bmc_sleepq_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	/* for BC/MC Frames */
	psta_bmc = rtw_get_bcmc_stainfobu(padapter);
	if (!psta_bmc)
		return H2C_SUCCESS;

	if ((rtw_tim_map_is_setbu(padapter, pstapriv->tim_bitmap, 0)) && (psta_bmc->sleepq_len > 0)) {
#ifndef CONFIG_PCI_HCI
		rtw_msleep_osbu(10);/* 10ms, ATIM(HIQ) Windows */
#endif
		...
		while ((rtw_end_of_queue_searchbu(xmitframe_phead, xmitframe_plist)) == _FALSE) {
			...
			if (xmitframe_hiq_filterbu(pxmitframe) == _TRUE)
				pxmitframe->attrib.qsel = QSLT_HIGH;/* HIQ */

#if 0
			_exit_critical_bh(&psta_bmc->sleep_q.lock, &irqL);
			if (rtw_hal_xmitbu(padapter, pxmitframe) == _TRUE)
				rtw_os_xmit_completebu(padapter, pxmitframe);
			_enter_critical_bh(&psta_bmc->sleep_q.lock, &irqL);
#endif
			rtw_hal_xmitbuframe_enqueuebu(padapter, pxmitframe);
		}
		...
		if (rtw_get_intf_type(padapter) != RTW_PCIE) {
			...
			rtw_chk_hi_queue_cmdbu(padapter);
		}
	}
#endif
...
u8 tx_beacon_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (send_beaconbu(padapter) == _FAIL) {
		RTW_INFO("issue_beaconbu, fail!\n");
		return H2C_PARAMETERS_ERROR;
	}

	/* tx bc/mc frames after update TIM */
	chk_bmc_sleepq_hdlbu(padapter, NULL);
#endif
#endif /* CONFIG_AP_MODE */
	return H2C_SUCCESS;
}
...
void change_band_update_iebu(_adapter *padapter, WLAN_BSSID_EX *pnetwork, u8 ch)
{
	...
	if (ch >= 36) {
		...
		rtw_remove_bcn_iebu(padapter, pnetwork, _ERPINFO_IE_);
		#ifdef CONFIG_80211AC_VHT
		...
		if ((pmlmepriv->htpriv.ht_option == _TRUE)
			...
			if (REGSTY_IS_11AC_AUTO(&padapter->registrypriv)
				...
				rtw_vht_ies_attachbu(padapter, pnetwork);
		}
		#endif
	} else {
		network_type = 0;
		...
		rtw_add_bcn_iebu(padapter, pnetwork, _ERPINFO_IE_, &erpinfo, 1);
		#ifdef CONFIG_80211AC_VHT
		rtw_vht_ies_detachbu(padapter, pnetwork);
		#endif
	}

	rtw_set_supported_ratebu(pnetwork->SupportedRates, network_type);

	UpdateBrateTblbu(padapter, pnetwork->SupportedRates);
	...
	rtw_add_bcn_iebu(padapter, pnetwork, _SUPPORTEDRATES_IE_, pnetwork->SupportedRates, rate_len);

	if (remainder_rate_len)
		rtw_add_bcn_iebu(padapter, pnetwork, _EXT_SUPPORTEDRATES_IE_, (pnetwork->SupportedRates + 8), remainder_rate_len);
	else
		rtw_remove_bcn_iebu(padapter, pnetwork, _EXT_SUPPORTEDRATES_IE_);

	pnetwork->Length = get_WLAN_BSSID_EX_sz(pnetwork);
}
#endif /* CONFIG_AP_MODE */

void rtw_join_done_chk_chbu(_adapter *adapter, int join_res)
{
#define DUMP_ADAPTERS_STATUS 0
	...
	if (DUMP_ADAPTERS_STATUS) {
		...
		dump_adapters_statusbu(RTW_DBGDUMP , dvobj);
	}

	ret = rtw_mi_get_ch_setting_unionbu(adapter, &u_ch, &u_bw, &u_offset);
	if (join_res >= 0 && ret <= 0) {
		join_res = -1;
		dump_adapters_statusbu(RTW_DBGDUMP , dvobj);
		rtw_warn_on(1);
	...
	if (join_res >= 0) {
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				bool is_grouped = rtw_is_chbw_groupedbu(u_ch, u_bw, u_offset
					, mlmeext->cur_channel, mlmeext->cur_bwmode, mlmeext->cur_ch_offset);
				...
				if (is_grouped == _FALSE) {
					...
					rtw_adjust_chbwbu(iface, mlmeext->cur_channel, &mlmeext->cur_bwmode, &mlmeext->cur_ch_offset);
					#ifdef CONFIG_RTW_MESH
					if (MLME_IS_MESH(iface))
						rtw_mesh_adjust_chbw(mlmeext->cur_channel, &mlmeext->cur_bwmode, &mlmeext->cur_ch_offset);
					#endif

					rtw_chset_sync_chbwbu(adapter_to_chset(adapter)
						, &mlmeext->cur_channel, &mlmeext->cur_bwmode, &mlmeext->cur_ch_offset
					...
					rtw_ap_update_bss_chbwbu(iface, &(mlmeext->mlmext_info.network)
						, mlmeext->cur_channel, mlmeext->cur_bwmode, mlmeext->cur_ch_offset);

					_rtw_memcpybu(&(mlme->cur_network.network), &(mlmeext->mlmext_info.network), sizeof(WLAN_BSSID_EX));

					rtw_start_bss_hdl_after_chbw_decidedbu(iface);

					{
						...
						rtw_cfg80211_ch_switch_notifybu(iface
							, mlmeext->cur_channel, mlmeext->cur_bwmode, mlmeext->cur_ch_offset
							, ht_option, 0);
						#endif
					}
	...
	if (rtw_mi_get_ch_setting_unionbu(adapter, &u_ch, &u_bw, &u_offset)) {
		RTW_INFO(FUNC_ADPT_FMT" union:%u,%u,%u\n", FUNC_ADPT_ARG(adapter), u_ch, u_bw, u_offset);
		set_channel_bwmodebu(adapter, u_ch, u_offset, u_bw);
	}

	rtw_mi_update_union_chan_infbu(adapter, u_ch, u_offset, u_bw);
...
int rtw_chk_start_clnt_joinbubu(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
#ifdef CONFIG_CONCURRENT_MODE
...
#ifdef CONFIG_CONCURRENT_MODE
	{
		...
		rtw_mi_statusbu_no_self(adapter, &mstate);
		...
		if (rtw_mi_get_ch_setting_unionbu_no_self(adapter, &u_ch, &u_bw, &u_offset) <= 0) {
			dump_adapters_statusbu(RTW_DBGDUMP , dvobj);
			rtw_warn_on(1);
		}
		...
		/* chbw_allow? */
		chbw_allow = rtw_is_chbw_groupedbu(pmlmeext->cur_channel, pmlmeext->cur_bwmode, pmlmeext->cur_ch_offset
						 , u_ch, u_bw, u_offset);
		...
		if (rtw_hal_set_mcc_setting_chk_start_clnt_joinbu(adapter, &u_ch, &u_bw, &u_offset, chbw_allow))
			goto exit;
#endif

		if (chbw_allow == _TRUE) {
			rtw_sync_chbwbu(&cur_ch, &cur_bw, &cur_ch_offset, &u_ch, &u_bw, &u_offset);
		...
		if (chbw_allow == _FALSE) {
			...
			for (i = 0; i < dvobj->iface_nums; i++) {
				...
				if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
					...
					#ifdef CONFIG_SPCT_CH_SWITCH
					if (1)
						rtw_ap_inform_ch_switchbu(iface, pmlmeext->cur_channel , pmlmeext->cur_ch_offset);
					else
					#endif
						rtw_sta_flushbu(iface, _FALSE);

					rtw_hal_set_hwregbu(iface, HW_VAR_CHECK_TXBUF, 0);
				...
				if (check_fwstate(mlme, WIFI_STATION_STATE)
					...
					rtw_disassoc_cmdbu(iface, 500, RTW_CMDF_DIRECTLY);
					rtw_indicate_disconnectbu(iface, 0, _FALSE);
					rtw_free_assoc_resourcesbu(iface, _TRUE);
				}
			}
		}
	...
	if (connect_allow == _TRUE) {
		...
		*offset = u_offset;
...
#if defined(CONFIG_IOCTL_CFG80211) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
		{
			...
			*/
			rtw_cfg80211_ch_switch_notifybu(adapter,
...
void rtw_set_external_auth_statusbu(_adapter *padapter,
	const void *data, int len)
{
	...
	_rtw_memcpybu(&params.bssid, (u8 *)data + 4, ETH_ALEN);
	_rtw_memcpybu(&params.ssid.ssid, (u8 *)data + 10, WLAN_SSID_MAXLEN);
	...
	_rtw_memcpybu(&params.pmkid, (u8 *)data + 64, PMKID_LEN);

	rtw_cfg80211_external_auth_statusbu(wiphy, dev, &params);
#endif /* CONFIG_IOCTL_CFG80211 */
}
...
u8 rtw_set_chbw_hdlbu(_adapter *padapter, u8 *pbuf)
{
	...
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	u8 ifbmp_s = rtw_mi_get_ld_sta_ifbmpbu(padapter);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	u8 u_ch, u_bw, u_offset;
	...
	if (ifbmp_s) {
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			/* updaet STA mode DSConfig , ap mode will update in rtw_change_bss_chbw_cmdbu */
	...
	set_channel_bwmodebu(padapter, set_ch_parm->ch, set_ch_parm->ch_offset, set_ch_parm->bw);

	rtw_mi_get_ch_setting_unionbu(padapter, &u_ch, &u_bw, &u_offset);
	rtw_mi_update_union_chan_infbu(padapter, u_ch, u_offset, u_bw);
...
#if CONFIG_TXPWR_LIMIT
	rtw_txpwr_init_regdbu(rfctl);
#endif

	rtw_rfctl_chplan_init(padapter);

	rtw_hal_set_odm_varbu(padapter, HAL_ODM_REGULATION, NULL, _TRUE);

#ifdef CONFIG_IOCTL_CFG80211
	rtw_regd_apply_flagsbu(adapter_to_wiphy(padapter));
#endif
...
u8 rtw_get_chplan_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (rfctl->country_ent) {
		_rtw_memcpybu(&resp->country_ent, rfctl->country_ent, sizeof(struct country_chplan));
		resp->has_country = 1;
	} else
		resp->has_country = 0;
	...
	_rtw_memcpybu(resp->chset, rfctl->channel_set, sizeof(RT_CHANNEL_INFO) * rfctl->max_chan_nums);
	*param->resp = resp;
...
u8 led_blink_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	BlinkHandlerbu((PLED_DATA)ledBlink_param->pLed);
#endif
...
void csa_timer_hdl(void *FunctionContext)
{
	...
	/* channel switch */
	if (rtw_set_csa_cmdbu(padapter) != _SUCCESS) {
			rfctl->csa_ch = 0;
...
u8 set_csa_hdlbu(_adapter *adapter, unsigned char *pbuf)
{
#if CONFIG_DFS
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);

	if (rfctl->csa_ch)
		rtw_dfs_ch_switch_hdlbu(adapter_to_dvobj(adapter));
#endif
	return	H2C_SUCCESS;
}
...
u8 tdls_hdlbu(_adapter *padapter, unsigned char *pbuf)
{
	...
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &pmlmeext->mlmext_info;
	struct sta_info *ap_sta = rtw_get_stainfobu(&padapter->stapriv, get_my_bssidbu(&(pmlmeinfo->network)));
	...
	if (!_rtw_memcmpbu(TDLSoption->addr, zaddr, ETH_ALEN)) {
		ptdls_sta = rtw_get_stainfobu(&(padapter->stapriv), TDLSoption->addr);
		if (ptdls_sta == NULL)
			return H2C_REJECTED;
	} else {
	...
	case TDLS_ESTABLISHED: {
		...
		rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_TDLS_LINKED);
		RTW_INFO("Created Direct Link with "MAC_FMT"\n", MAC_ARG(ptdls_sta->cmn.mac_addr));

		/* Set TDLS sta rate. */
		/* Update station supportRate */
		rtw_hal_update_sta_ra_infobu(padapter, ptdls_sta);
		...
		rtw_hal_update_sta_wsetbu(padapter, ptdls_sta);
		/* Sta mode */
		rtw_hal_set_odm_varbu(padapter, HAL_ODM_STA_INFO, ptdls_sta, _TRUE);

		set_sta_ratebu(padapter, ptdls_sta);
		rtw_sta_media_status_rptbu(padapter, ptdls_sta, 1);
		break;
	}
	case TDLS_ISSUE_PTI:
	...
	case TDLS_CH_SW_RESP:
		...
		_rtw_memcpybu(txmgmt.peer, ptdls_sta->cmn.mac_addr, ETH_ALEN);

		if (ap_sta)
			rtw_hal_macid_sleepbu(padapter, ap_sta->cmn.mac_id);
		issue_nulldatabu(padapter, NULL, 1, 3, 3);
		...
		if (padapter->mlmeextpriv.cur_channel == pchsw_info->off_ch_num) {
			rtw_tdls_cmdbu(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_END_TO_BASE_CHNL);
			break;
		}

		if (ret == _SUCCESS)
			rtw_tdls_cmdbu(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_TO_OFF_CHNL);
		else
	...
	case TDLS_CH_SW_PREPARE:
		...
		doiqk = _TRUE;
		rtw_hal_set_hwregbu(padapter, HW_VAR_DO_IQK, &doiqk);
		set_channel_bwmodebu(padapter, pchsw_info->off_ch_num, pchsw_info->ch_offset, (pchsw_info->ch_offset) ? CHANNEL_WIDTH_40 : CHANNEL_WIDTH_20);
		doiqk = _FALSE;
		rtw_hal_set_hwregbu(padapter, HW_VAR_DO_IQK, &doiqk);
		...
		rtw_tdls_cmdbu(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_START);
	...
	case TDLS_CH_SW_TO_OFF_CHNL:
		...
		if (rtw_tdls_do_ch_sw(padapter, ptdls_sta, TDLS_CH_SW_OFF_CHNL, pchsw_info->off_ch_num,
			...
			if (pchsw_info->ch_sw_state & TDLS_CH_SW_INITIATOR_STATE) {
				if (issue_nulldatabu_to_TDLS_peer_STA(ptdls_sta->padapter, ptdls_sta->cmn.mac_addr, 0, 1, 
					(padapter->registrypriv.wifi_spec == 0) ? 3 : 0) == _FAIL)
					rtw_tdls_cmdbu(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_TO_BASE_CHNL);
			}
		} else {
			if (!(pchsw_info->ch_sw_state & TDLS_CH_SW_INITIATOR_STATE))
...
#if 0
		_rtw_memsetbu(pHalData->tdls_ch_sw_iqk_info_base_chnl, 0x00, sizeof(pHalData->tdls_ch_sw_iqk_info_base_chnl));
		_rtw_memsetbu(pHalData->tdls_ch_sw_iqk_info_off_chnl, 0x00, sizeof(pHalData->tdls_ch_sw_iqk_info_off_chnl));
#endif
	...
	case TDLS_CH_SW_TO_BASE_CHNL:
		...
		if (rtw_tdls_do_ch_sw(padapter, ptdls_sta, TDLS_CH_SW_BASE_CHNL, pmlmeext->cur_channel,
			...
			if (ap_sta)
				rtw_hal_macid_wakeupbu(padapter, ap_sta->cmn.mac_id);
			issue_nulldatabu(padapter, NULL, 0, 3, 3);
...
#endif
	case TDLS_RS_RCR:
		rtw_hal_rcr_set_chk_bssidbu(padapter, MLME_TDLS_NOLINK);
		break;
	...
	case TDLS_TEARDOWN_STA_LOCALLY_POST:
#ifdef CONFIG_TDLS_CH_SW
		if (_rtw_memcmpbu(TDLSoption->addr, pchsw_info->addr, ETH_ALEN) == _TRUE) {
			pchsw_info->ch_sw_state &= ~(TDLS_CH_SW_INITIATOR_STATE |
						     TDLS_CH_SWITCH_ON_STATE |
						     TDLS_PEER_AT_OFF_STATE);
			rtw_tdls_set_ch_sw_oper_control(padapter, _FALSE);
			_rtw_memsetbu(pchsw_info->addr, 0x00, ETH_ALEN);
		}
#endif
		...
		if (ptdlsinfo->tdls_sctx != NULL)
			rtw_sctx_donebu(&(ptdlsinfo->tdls_sctx));
...
int rtw_sae_preprocessbu(_adapter *adapter, const u8 *buf, u32 len, u8 tx)
{
	...
	if (tx && (seq == 2) && (status == 0)) {
		...
		psta = rtw_get_stainfobu(pstapriv, GetAddr1Ptr(buf));
		if (psta) {
			_enter_critical_bh(&psta->lock, &irqL);
			if (psta->pauth_frame) {
			...
			if (psta->pauth_frame) {
				_rtw_memcpybu(psta->pauth_frame, buf, len);
				psta->auth_len = len;
			}
			_exit_critical_bh(&psta->lock, &irqL);
