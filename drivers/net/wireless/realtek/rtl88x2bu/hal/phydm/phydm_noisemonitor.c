...
void phydm_set_noise_data_sumbu(struct noise_level *noise_data, u8 max_rf_path)
{
	u8 i = 0;
...
s16 odm_inband_noise_monitorbu_n(struct dm_struct *dm, u8 is_pause_dig, u8 igi,
			       u32 max_time)
{
	u32 tmp4b;
	...
	func_start = odm_get_current_timebu(dm);
	...
	odm_memory_setbu(dm, &noise_data, 0, sizeof(struct noise_level));
	/* step 1. Disable DIG && Set initial gain. */

	if (is_pause_dig)
		odm_pause_digbu(dm, PHYDM_PAUSE, PHYDM_PAUSE_LEVEL_1, igi);

	/* step 3. Get noise power level */
	start = odm_get_current_timebu(dm);
	while (1) {
		/* Stop updating idle time pwer report (for driver read) */
		odm_set_bb_regbu(dm, REG_FPGA0_TX_GAIN_STAGE, BIT(25), 1);

		/* Read Noise Floor Report */
		tmp4b = odm_get_bb_regbu(dm, R_0x8f8, MASKDWORD);

		/* update idle time pwer report per 5us */
		odm_set_bb_regbu(dm, REG_FPGA0_TX_GAIN_STAGE, BIT(25), 0);
		...
		if (valid_done == max_rf_path ||
		    (odm_get_progressing_timebu(dm, start) > max_time)) {
			phydm_set_noise_data_sumbu(&noise_data, max_rf_path);
			break;
		}
	}
	reg_c50 = (u8)odm_get_bb_regbu(dm, REG_OFDM_0_XA_AGC_CORE1, MASKBYTE0);
	reg_c50 &= ~BIT(7);
	...
	if (max_rf_path == 2) {
		reg_c58 = (u8)odm_get_bb_regbu(dm, R_0xc58, MASKBYTE0);
		reg_c58 &= ~BIT(7);
	...
	if (is_pause_dig)
		odm_pause_digbu(dm, PHYDM_RESUME, PHYDM_PAUSE_LEVEL_1, igi);
	func_end = odm_get_progressing_timebu(dm, func_start);
...
s16 phydm_idle_noise_measure_acbu(struct dm_struct *dm, u8 pause_dig,
				u8 igi, u32 max_time)
{
	u32 tmp4b;
	u8 max_rf_path = 0, i = 0;
	...
	while (1) {
		/*Stop updating idle time pwer report (for driver read)*/
		odm_set_bb_regbu(dm, R_0x9e4, BIT(30), 0x1);

		/*Read Noise Floor Report*/
		tmp4b = odm_get_bb_regbu(dm, R_0xff0, MASKDWORD);

		/*update idle time pwer report per 5us*/
		odm_set_bb_regbu(dm, R_0x9e4, BIT(30), 0x0);
	...
	reg_c50 = (u8)odm_get_bb_regbu(dm, R_0xc50, MASKBYTE0);
	reg_c50 &= ~BIT(7);
	val_s8 = (s8)(-110 + reg_c50 + noise_data.sum[RF_PATH_A]);
	dm->noise_level.noise[RF_PATH_A] = val_s8;
	dm->noise_level.noise_all += dm->noise_level.noise[RF_PATH_A];
	...
	if (max_rf_path == 2) {
		reg_e50 = (u8)odm_get_bb_regbu(dm, R_0xe50, MASKBYTE0);
		reg_e50 &= ~BIT(7);
...
s16 odm_inband_noise_monitorbu_acbu(struct dm_struct *dm, u8 pause_dig, u8 igi,
				u32 max_time)
{
	...
	if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C)) {
		rpt = phydm_idle_noise_measure_acbu(dm, pause_dig, igi, max_time);
		return rpt;
	}
	...
	while (1) {
		/*Set IGI=0x1C */
		odm_write_digbu(dm, 0x1C);
		/*stop CK320&CK88 */
		odm_set_bb_regbu(dm, R_0x8b4, BIT(6), 1);
		/*Read path-A */
		/*set debug port*/
		odm_set_bb_regbu(dm, R_0x8fc, MASKDWORD, 0x200);
		/*read debug port*/
		value32 = odm_get_bb_regbu(dm, R_0xfa0, MASKDWORD);
		...
		if (!pd_flag || rxi_buf_anta != 0x200) {
			/*sign conversion*/
			rxi_buf_anta = odm_sign_conversionbu(rxi_buf_anta, 10);
			rxq_buf_anta = odm_sign_conversionbu(rxq_buf_anta, 10);

			val_s32 = rxi_buf_anta * rxi_buf_anta +
				  rxq_buf_anta * rxq_buf_anta;
			/*S(10,9)*S(10,9)=S(20,18)*/
			pwdb_A = odm_pwdb_conversionbu(val_s32, 20, 18);
		...
		/*Start CK320&CK88*/
		odm_set_bb_regbu(dm, R_0x8b4, BIT(6), 0);
		/*@BB Reset*/
		val_u8 = odm_read_1bytebu(dm, 0x02) & (~BIT(0));
		odm_write_1bytebu(dm, 0x02, val_u8);
		val_u8 = odm_read_1bytebu(dm, 0x02) | BIT(0);
		odm_write_1bytebu(dm, 0x02, val_u8);
		/*PMAC Reset*/
		val_u8 = odm_read_1bytebu(dm, 0xB03) & (~BIT(0));
		odm_write_1bytebu(dm, 0xB03, val_u8);
		val_u8 = odm_read_1bytebu(dm, 0xB03) | BIT(0);
		odm_write_1bytebu(dm, 0xB03, val_u8);
		/*@CCK Reset*/
		if (odm_read_1bytebu(dm, 0x80B) & BIT(4)) {
			val_u8 = odm_read_1bytebu(dm, 0x80B) & (~BIT(4));
			odm_write_1bytebu(dm, 0x80B, val_u8);
			val_u8 = odm_read_1bytebu(dm, 0x80B) | BIT(4);
			odm_write_1bytebu(dm, 0x80B, val_u8);
		}
...
s16 odm_inband_noise_monitorbu(void *dm_void, u8 pause_dig, u8 igi,
			     u32 max_time)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES)
		val = odm_inband_noise_monitorbu_acbu(dm, pause_dig, igi, max_time);
	#endif

	#if (ODM_IC_11N_SERIES_SUPPORT)
	if (dm->support_ic_type & ODM_IC_11N_SERIES)
		val = odm_inband_noise_monitorbu_n(dm, pause_dig, igi, max_time);
	#endif
