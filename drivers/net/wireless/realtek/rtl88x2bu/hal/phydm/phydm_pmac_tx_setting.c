				  ...
				  struct phydm_pmac_info *tx_info)
{
	...
	/* if CCK block on? */
	if (!odm_get_bb_regbu(dm, R_0x1c3c, BIT(1)))
		odm_set_bb_regbu(dm, R_0x1c3c, BIT(1), 0x1);
	if (dm->support_ic_type & ODM_RTL8723F) {
		odm_set_bb_regbu(dm, R_0x2a08, BIT(21)|BIT(20), rate);
		odm_set_bb_regbu(dm, R_0x2a04, BIT(5), 0x0); /* turn on scrambler*/
	} else {
	/* Turn Off All Test mode */
	odm_set_bb_regbu(dm, R_0x1ca4, 0x7, 0x0);

	odm_set_bb_regbu(dm, R_0x1a00, 0x3000, rate);
	odm_set_bb_regbu(dm, R_0x1a00, 0x3, 0x2); /* transmit mode */
	odm_set_bb_regbu(dm, R_0x1a00, BIT(3), 0x1); /* turn on scrambler*/

	/* Fix rate selection issue */
	odm_set_bb_regbu(dm, R_0x1a70, BIT(14), 0x1);
	/* set RX weighting for path I & Q to 0 */
	odm_set_bb_regbu(dm, R_0x1a14, 0x300, 0x3);
	/* set loopback mode */
	odm_set_bb_regbu(dm, R_0x1c3c, BIT(4), 0x1);
	}
...
void phydm_stop_cck_cont_tx_jgr3(void *dm_void)
{
	...
	if (dm->support_ic_type & ODM_RTL8723F) {
		...
		odm_set_bb_regbu(dm, R_0x1a00, 0x3, 0x0); /* normal mode */
		odm_set_bb_regbu(dm, R_0x1a00, BIT(3), 0x1); /* turn on scrambler*/

		/* back to default */
		odm_set_bb_regbu(dm, R_0x1a70, BIT(14), 0x0);
		odm_set_bb_regbu(dm, R_0x1a14, 0x300, 0x0);
		odm_set_bb_regbu(dm, R_0x1c3c, BIT(4), 0x0);
	}
	/* BB Reset */
	odm_set_bb_regbu(dm, R_0x1d0c, BIT(16), 0x0);
	odm_set_bb_regbu(dm, R_0x1d0c, BIT(16), 0x1);
}
...
void phydm_start_ofdm_cont_tx_jgr3(void *dm_void)
{
	...
	/* 1. if OFDM block on */
	if (!odm_get_bb_regbu(dm, R_0x1c3c, BIT(0)))
		odm_set_bb_regbu(dm, R_0x1c3c, BIT(0), 0x1);
	if (!(dm->support_ic_type & ODM_RTL8723F)) {

		/* 2. set CCK test mode off, set to CCK normal mode */
		odm_set_bb_regbu(dm, R_0x1a00, 0x3, 0x0);

		/* 3. turn on scramble setting */
		odm_set_bb_regbu(dm, R_0x1a00, BIT(3), 0x1);
	}
	/* 4. Turn On Continue Tx and turn off the other test modes. */
	odm_set_bb_regbu(dm, R_0x1ca4, 0x7, 0x1);
...
void phydm_stop_pmac_tx_jgr3(void *dm_void, struct phydm_pmac_info *tx_info)
{
	...
	odm_set_bb_regbu(dm, R_0x1e70, 0xf, 0x2); /* TX Stop */
	if (dm->support_ic_type & ODM_RTL8723F) {
		if (tx_info->mode == CONT_TX) {
			if (pmac_tx->is_cck_rate) {
				/* TX Stop */
				odm_set_bb_regbu(dm, R_0x2a00, BIT(0), 0x1);
				/* Clear BB cont tx */
				odm_set_bb_regbu(dm, R_0x2a00, BIT(28), 0x0);
				/* Clear PMAC cont tx */
				odm_set_bb_regbu(dm, R_0x2a08, BIT(17), 0x0);
				/* Clear TX Stop */
				odm_set_bb_regbu(dm, R_0x2a00, BIT(0), 0x0);
			...
			if (pmac_tx->is_cck_rate) {
				/* packet_count = 0x1 */
				odm_set_bb_regbu(dm, R_0x2a04, 0x03ff0000, 0x1);
				/* @Disable pmac tx_en*/
				odm_set_bb_regbu(dm, R_0x2a08, BIT(31), 0x0);
				/* @Enable pmac tx_en*/
				odm_set_bb_regbu(dm, R_0x2a08, BIT(31), 0x1);
				phydm_stop_cck_cont_tx_jgr3(dm);
			}
		} 
	}else {
				   ...
				   struct phydm_pmac_info *tx_info)
{
	...
	odm_set_bb_regbu(dm, R_0xa58, 0x003f8000, tx_info->tx_rate);

	/*0x900[1] ndp_sound */
	odm_set_bb_regbu(dm, R_0x900, BIT(1), tx_info->ndp_sound);
	...
	tmp = (tx_info->tx_sc) | ((tx_info->bw) << 4) |
		((tx_info->m_stbc) << 6);
	odm_set_bb_regbu(dm, R_0x900, 0xff000000, tmp);

	if (tx_info->tx_sc == 1) /*upper*/
		odm_set_bb_regbu(dm, R_0x1ae0, 0x7000, 0x5);
	else if (tx_info->tx_sc == 2) /*lower*/
		odm_set_bb_regbu(dm, R_0x1ae0, 0x7000, 0x6);
	else /* duplicate*/
		odm_set_bb_regbu(dm, R_0x1ae0, 0x7000, 0x0);

	if (pmac_tx->is_ht_rate) {
		odm_set_bb_regbu(dm, R_0x900, BIT(0), 0x1);
		odm_set_bb_regbu(dm, R_0x900, BIT(2), 0x0);
	} else if (pmac_tx->is_vht_rate) {
		odm_set_bb_regbu(dm, R_0x900, BIT(0), 0x0);
		odm_set_bb_regbu(dm, R_0x900, BIT(2), 0x1);
	} else {
		odm_set_bb_regbu(dm, R_0x900, BIT(0), 0x0);
		odm_set_bb_regbu(dm, R_0x900, BIT(2), 0x0);
	}
	...
	/* for TX interval */
	odm_set_bb_regbu(dm, R_0x9b8, MASKHWORD, tx_info->packet_period);
}
...
void phydm_set_sig_jgr3(void *dm_void, struct phydm_pmac_info *tx_info)
{
	...
	odm_set_bb_regbu(dm, R_0x1eb4, 0xfffff, tx_info->packet_count);

	/* L-SIG */
	tmp = BYTE_2_DWORD(0, tx_info->lsig[2], tx_info->lsig[1],
			   tx_info->lsig[0]);
	odm_set_bb_regbu(dm, R_0x908, 0xffffff, tmp);
	if (pmac_tx->is_ht_rate) {
	/* HT SIG */
		tmp = BYTE_2_DWORD(0, tx_info->ht_sig[2], tx_info->ht_sig[1],
				   tx_info->ht_sig[0]);
		odm_set_bb_regbu(dm, R_0x90c, 0xffffff, tmp);
		tmp = BYTE_2_DWORD(0, tx_info->ht_sig[5], tx_info->ht_sig[4],
				   tx_info->ht_sig[3]);
		odm_set_bb_regbu(dm, R_0x910, 0xffffff, tmp);
	} else if (pmac_tx->is_vht_rate) {
	/* VHT SIG A/B/serv_field/delimiter */
		...
		tmp = BYTE_2_DWORD(tx_info->vht_sig_b[3], tx_info->vht_sig_b[2],
				   tx_info->vht_sig_b[1],
				   tx_info->vht_sig_b[0]);
		odm_set_bb_regbu(dm, R_0x914, 0x1fffffff, tmp);
		odm_set_bb_regbu(dm, R_0x938, 0xff00, tx_info->vht_sig_b_crc);

		tmp = BYTE_2_DWORD(tx_info->vht_delimiter[3],
				   tx_info->vht_delimiter[2],
				   tx_info->vht_delimiter[1],
				   tx_info->vht_delimiter[0]);
		odm_set_bb_regbu(dm, R_0x940, MASKDWORD, tmp);
	}
}
				     ...
				     struct phydm_pmac_info *tx_info)
{
	...
	if (dm->support_ic_type & ODM_RTL8723F) {
		#if (RTL8723F_SUPPORT)
		odm_set_bb_regbu(dm, R_0x2a04, 0x03ff0000, tx_info->packet_count);
		odm_set_bb_regbu(dm, R_0x2a08, BIT(22), tx_info->service_field_bit2);
		odm_set_bb_regbu(dm, R_0x2a08, BIT(21) | BIT(20), rate);
		odm_set_bb_regbu(dm, R_0x2a08, 0x1ffff, tx_info->packet_length);
		/* turn on scrambler */
		odm_set_bb_regbu(dm, R_0x2a04, BIT(5), 0x0);

		if (tx_info->is_short_preamble)
			odm_set_bb_regbu(dm, R_0x2a08, BIT(19), 0x1);
		else
			odm_set_bb_regbu(dm, R_0x2a08, BIT(19), 0x0);
		#endif
	} else {
		tmp = tx_info->packet_count | (tx_info->sfd << 16);
		odm_set_bb_regbu(dm, R_0x1e64, MASKDWORD, tmp);
		tmp = tx_info->signal_field | (tx_info->service_field << 8) |
	      	(tx_info->length << 16);
		odm_set_bb_regbu(dm, R_0x1e68, MASKDWORD, tmp);
		tmp = BYTE_2_DWORD(0, 0, tx_info->crc16[1], tx_info->crc16[0]);
		odm_set_bb_regbu(dm, R_0x1e6c, MASKLWORD, tmp);

		if (tx_info->is_short_preamble)
			odm_set_bb_regbu(dm, R_0x1e6c, BIT(16), 0x0);
		else
			odm_set_bb_regbu(dm, R_0x1e6c, BIT(16), 0x1);
	}
}
...
void phydm_set_pmac_txon_jgr3(void *dm_void, struct phydm_pmac_info *tx_info)
{
	...
	odm_set_bb_regbu(dm, R_0x1d08, BIT(0), 0x1); /*Turn on PMAC */

	if (dm->support_ic_type & ODM_RTL8723F) {
		if (pmac_tx->is_cck_rate) {
			if (tx_info->mode == CONT_TX) {
				/* BB and PMAC cont tx */
				odm_set_bb_regbu(dm, R_0x2a08, BIT(17), 0x1);
				odm_set_bb_regbu(dm, R_0x2a00, BIT(28), 0x1);
			}
			/* TX CCK ON */
			odm_set_bb_regbu(dm, R_0x2a08, BIT(31), 0x0);
			odm_set_bb_regbu(dm, R_0x2a08, BIT(31), 0x1);
		} else {
			odm_set_bb_regbu(dm, R_0x1e70, 0xf, 0x0); /*TX Ofdm OFF */
			odm_set_bb_regbu(dm, R_0x1e70, 0xf, 0x4); /*TX Ofdm ON */
		}
	} else {
		/*mac scramble seed setting, only in 8198F */
		#if (RTL8198F_SUPPORT)
			if (dm->support_ic_type & ODM_RTL8198F)
				if (!odm_get_bb_regbu(dm, R_0x1d10, BIT(16)))
					odm_set_bb_regbu(dm, R_0x1d10, BIT(16), 0x1);
		#endif
 
		if (pmac_tx->is_cck_rate){
			odm_set_bb_regbu(dm, R_0x1e70, 0xf, 0x8); /*TX CCK ON */
			odm_set_bb_regbu(dm, R_0x1a84, BIT(31), 0x0);
		} else {
			odm_set_bb_regbu(dm, R_0x1e70, 0xf, 0x4); /*TX Ofdm ON */
		}
	}
			    ...
			    enum rf_path mpt_rf_path)
{
	...
	pmac_tx->is_cck_rate = phydm_is_cck_ratebu(dm, tx_info->tx_rate);
	pmac_tx->is_ofdm_rate = phydm_is_ofdm_ratebu(dm, tx_info->tx_rate);
	pmac_tx->is_ht_rate = phydm_is_ht_ratebu(dm, tx_info->tx_rate);
	pmac_tx->is_vht_rate = phydm_is_vht_ratebu(dm, tx_info->tx_rate);
...
void phydm_set_tmac_tx_jgr3(void *dm_void)
{
	...
	/* Turn on TMAC */
	if (odm_get_bb_regbu(dm, R_0x1d08, BIT(0)))
		odm_set_bb_regbu(dm, R_0x1d08, BIT(0), 0x0);

	/* mac scramble seed setting, only in 8198F */
	#if (RTL8198F_SUPPORT)
		if (dm->support_ic_type & ODM_RTL8198F)
			if (odm_get_bb_regbu(dm, R_0x1d10, BIT(16)))
				odm_set_bb_regbu(dm, R_0x1d10, BIT(16), 0x0);
	#endif

	/* Turn on TMAC CCK */
	if (!(dm->support_ic_type & ODM_RTL8723F)) {
		if (!odm_get_bb_regbu(dm, R_0x1a84, BIT(31)))
			odm_set_bb_regbu(dm, R_0x1a84, BIT(31), 0x1);
	}
}
#endif
		       ...
		       char *output, u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		phydm_print_ratebu_2_buff(dm, tx_info.tx_rate, dbg_buf,
					PHYDM_SNPRINT_SIZE);
		PDM_SNPF(out_len, used, output + used, out_len - used,
		...
		while (1) {
			if (phydm_is_cck_ratebu(dm, tx_info.tx_rate))
				tx_cnt = odm_get_bb_regbu(dm, R_0x2de4,
							MASKLWORD);
			else
				tx_cnt = odm_get_bb_regbu(dm, R_0x2de0,
							MASKLWORD);
		...
		phydm_stop_ic_trxbu(dm, PHYDM_REVERT);
	}
	*_used = used;
	*_out_len = out_len;
}
