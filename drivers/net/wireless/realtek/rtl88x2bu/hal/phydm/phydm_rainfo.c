		     ...
		     char *output, u32 *_out_len)
{
	...
	odm_fill_h2c_cmdbu(dm, phydm_h2c_id, H2C_MAX_LENGTH, h2c_parameter);
...
void phydm_fw_fix_ratebu(void *dm_void, u8 en, u8 macid, u8 bw, u8 rate)
{
	...
	if (dm->support_ic_type & PHYDM_IC_8051_SERIES) {
		...
		odm_set_mac_regbu(dm, R_0x4a0, MASKDWORD, reg_u32_tmp);
		...
		if (dm->support_ic_type & ODM_RTL8814B)
			odm_set_mac_regbu(dm, R_0x448, MASKDWORD, reg_u32_tmp);
		else
			odm_set_mac_regbu(dm, R_0x450, MASKDWORD, reg_u32_tmp);
	}
	if (en == 1) {
		PHYDM_DBG(dm, ODM_COMP_API,
			  "FW fix TX rate[id =%d], %dM, Rate(%d)=", macid,
			  (20 << bw), rate);
		phydm_print_ratebu(dm, rate, ODM_COMP_API);
	} else {
		    ...
		    u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		phydm_fw_fix_ratebu(dm, (u8)var[1], macid, bw, rate);
		...
		if (tx_cls_en) {
			odm_set_bb_regbu(dm, R_0x8f8, BIT(16), 1);
			...
			odm_set_bb_regbu(dm, R_0x8f8, BIT(16), 0);
		}
				      ...
				      struct ra_mask_rpt_trig *trig_rpt)
{
	...
	phydm_fw_trace_en_h2cbu(dm, true, 1, 2, trig_rpt->macid);
...
void phydm_ra_mask_report_c2h_result(void *dm_void, struct ra_mask_rpt *rpt)
{
	...
	odm_move_memorybu(dm, &rpt->ra_mask_buf[0], &ra_tab->ra_mask_buf[0], 8);
}
...
void odm_c2h_ra_para_report_handlerbu(void *dm_void, u8 *cmd_buf, u8 cmd_len)
{
	...
	if (mode == RADBG_DEBUG_MONITOR1) {
		...
		if (dm->support_ic_type & PHYDM_IC_3081_SERIES) {
			...
			odm_move_memorybu(dm, &ra_tab->ra_mask_buf[0], &cmd_buf[4], 8);
...
void phydm_ra_dynamic_retry_countbu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	if (dm->pre_b_noisy != dm->noisy_decision) {
		if (dm->noisy_decision) {
			...
			odm_set_mac_regbu(dm, R_0x430, MASKDWORD, 0x0);
			odm_set_mac_regbu(dm, R_0x434, MASKDWORD, 0x04030201);
		} else {
			PHYDM_DBG(dm, DBG_DYN_ARFR, "Clean Env. RA fallback\n");
			odm_set_mac_regbu(dm, R_0x430, MASKDWORD, 0x01000000);
			odm_set_mac_regbu(dm, R_0x434, MASKDWORD, 0x06050402);
		}
...
void phydm_print_ratebu(void *dm_void, u8 rate, u32 dbg_component)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 rate_idx = rate & 0x7f; /*remove bit7 SGI*/
	boolean vht_en = phydm_is_vht_ratebu(dm, rate_idx);
	u8 b_sgi = (rate & 0x80) >> 7;
	u8 rate_ss = phydm_rate_to_num_ssbu(dm, rate_idx);
	u8 rate_digit = phydm_rate_2_rate_digitbu(dm, rate_idx);
...
void odm_ra_post_action_on_assocbu(void *dm_void)
{
}
...
void phydm_rate_adaptive_mask_initbu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
u8 phydm_get_tx_stream_numbu(void *dm_void, enum rf_type type)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 tx_num = 1;
...
u64 phydm_get_bb_mod_ra_maskbu(void *dm_void, u8 sta_idx)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	if (is_sta_active(sta)) {
		...
		tx_stream_num = phydm_get_tx_stream_numbu(dm, sta->mimo_type);
...
u8 phydm_get_rate_idbu(void *dm_void, u8 sta_idx)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct cmn_sta_info *sta = dm->phydm_sta_info[sta_idx];
	struct ra_sta_info *ra = NULL;
...
void phydm_ra_h2cbu(void *dm_void, u8 sta_idx, u8 dis_ra, u8 dis_pt,
		  u8 no_update_bw, u8 init_ra_lv, u64 ra_mask)
{
	...
	odm_fill_h2c_cmdbu(dm, PHYDM_H2C_RA_MASK, H2C_MAX_LENGTH, h2c_val);
	...
	if (dm->support_ic_type & (PHYDM_IC_ABOVE_3SS)) {
		...
		odm_fill_h2c_cmdbu(dm, PHYDM_RA_MASK_ABOVE_3SS,
				 H2C_MAX_LENGTH, h2c_val);
	}
	#endif
}

void phydm_ra_registedbu(void *dm_void, u8 sta_idx,
		       ...
		       u8 rssi_from_assoc)
{
	...
	if (dm->support_ic_type == ODM_RTL8188E)
		ra->rate_id = phydm_get_rate_idbu_88e(dm, sta_idx);
	else
	#endif
	{
		ra->rate_id = phydm_get_rate_idbu(dm, sta_idx);
	}

	ra_mask = phydm_get_bb_mod_ra_maskbu(dm, sta_idx);
	...
	if (ra_t->record_ra_infobu)
		ra_t->record_ra_infobu(dm, sta_idx, sta, ra_mask);
	...
	{
		/*@FW RA*/
		phydm_ra_h2cbu(dm, sta_idx, ra->disable_ra, ra->disable_pt, 0,
			     init_ra_lv, ra_mask);
	}
}

void phydm_ra_offlinebu(void *dm_void, u8 sta_idx)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct ra_table *ra_t = &dm->dm_ra_table;
	struct cmn_sta_info *sta = dm->phydm_sta_info[sta_idx];
	...
	odm_memory_setbu(dm, &ra->rate_id, 0, sizeof(struct ra_sta_info));
	ra->disable_ra = 1;
	ra->disable_pt = 1;

	if (ra_t->record_ra_infobu)
		ra_t->record_ra_infobu(dm, sta->mac_id, sta, 0);

	if (dm->support_ic_type != ODM_RTL8188E)
		phydm_ra_h2cbu(dm, sta->mac_id, 1, 1, 0, 0, 0);
}
...
void phydm_ra_mask_watchdogbu(void *dm_void)
{
	...
	for (sta_idx = 0; sta_idx < ODM_ASSOCIATE_ENTRY_NUM; sta_idx++) {
		...
		rssi_lv_new = phydm_rssi_lv_decbu(dm, (u32)rssi, ra->rssi_level);
		...
		if (ra->rssi_level != rssi_lv_new ||
			...
			{
				/*@FW RA*/
				phydm_ra_h2cbu(dm, sta_idx, ra->disable_ra,
...
u8 phydm_rftype2rateid_2g_n20bu(void *dm_void, u8 rf_type)
{
	u8 rate_id_idx = 0;
...
u8 phydm_rftype2rateid_2g_n40bu(void *dm_void, u8 rf_type)
{
	u8 rate_id_idx = 0;
...
u8 phydm_rftype2rateid_5g_nbu(void *dm_void, u8 rf_type)
{
	u8 rate_id_idx = 0;
...
u8 phydm_rftype2rateid_ac80bu(void *dm_void, u8 rf_type)
{
	u8 rate_id_idx = 0;
...
u8 phydm_rftype2rateid_ac40bu(void *dm_void, u8 rf_type)
{
	u8 rate_id_idx = 0;
...
u8 phydm_rate_id_mappingbu(void *dm_void, u32 wireless_mode, u8 rf_type, u8 bw)
{
	...
	case PHYDM_WIRELESS_MODE_N_24G:
		if (bw == CHANNEL_WIDTH_40)
			rate_id_idx = phydm_rftype2rateid_2g_n40bu(dm, rf_type);
		else
			rate_id_idx = phydm_rftype2rateid_2g_n20bu(dm, rf_type);
		break;

	case PHYDM_WIRELESS_MODE_N_5G:
		rate_id_idx = phydm_rftype2rateid_5g_nbu(dm, rf_type);
		break;
	...
	case PHYDM_WIRELESS_MODE_AC_ONLY:
		rate_id_idx = phydm_rftype2rateid_ac80bu(dm, rf_type);
		break;
...
/*@Becareful to set "Lowest rate" while using PHYDM_ARFR4_AC_3SS in 2.4G/5G*/
		if (bw >= CHANNEL_WIDTH_80)
			...
			rate_id_idx = phydm_rftype2rateid_ac40bu(dm, rf_type);
		break;
...
enum phydm_qam_order phydm_get_ofdm_qam_orderbu(void *dm_void, u8 rate_idx)
{
	u8 tmp_idx = rate_idx;
...
#if (DM_ODM_SUPPORT_TYPE == ODM_CE)
u8 phydm_rate2ssbu(void *dm_void, u8 rate_idx)
{
	u8 ret = 0xff;
	u8 i, j;
	u8 search_idx;
...
u8 phydm_rate2plcpbu(void *dm_void, u8 rate_idx)
{
	u8 rate2ss = 0;
	u8 ltftime = 0;
	u8 plcptime = 0xff;
	...
	if (rate_idx < ODM_RATE6M) {
		...
		rate2ss = phydm_rate2ssbu(dm_void, rate_idx);
		if (rate2ss != 0xff)
			ltftime = (rate2ss + 1) * 4;
		else
			return 0xff;
...
u8 phydm_get_plcpbu(void *dm_void, u16 macid)
{
	...
	sta = dm->phydm_sta_info[macid];
	ra = &sta->ra_info;
	plcp_time = phydm_rate2plcpbu(dm, ra->curr_tx_rate);
	return plcp_time;
}
#endif

void phydm_ra_common_info_updatebu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct ra_table *ra_tab = &dm->dm_ra_table;
	struct cmn_sta_info *sta = NULL;
	...
	for (macid = 0; macid < ODM_ASSOCIATE_ENTRY_NUM; macid++) {
		...
		rate_order_tmp = phydm_rate_order_computebu(dm, rate_idx);
...
void phydm_rrsr_set_registerbu(void *dm_void, u32 rrsr_val)
{
	...
	odm_set_mac_regbu(dm, R_0x440, 0xfffff, rrsr_val);
}

void phydm_masked_rrsr_set_registerbu(void *dm_void, u32 rrsr_val)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct ra_table *ra_tab = &dm->dm_ra_table;
...
void phydm_rrsr_maskbu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	if (!dm->is_linked) {
		phydm_masked_rrsr_set_registerbu(dm, ra->rrsr_val_init);
		return;
	}
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		rate_order = phydm_rate_order_computebu(dm, tx_rate_idx);
	...
	rate_order = phydm_rate_order_computebu(dm, sta->ra_info.curr_tx_rate);
#endif
	if (rate_order_min == 0) {
		rrsr_mask = 0x1f;
	} else {
		rrsr_mask_ofdm = (u32)phydm_gen_bitmaskbu(rate_order_min);
		rrsr_mask = (rrsr_mask_ofdm << 4) | 0xf;
	}

	/*ra->rrsr_val_init = 0x15d;*/

	phydm_masked_rrsr_set_registerbu(dm, ra->rrsr_val_init & rrsr_mask);
...
void phydm_ra_info_watchdogbu(void *dm_void)
{
	...
	phydm_ra_common_info_updatebu(dm);
	phydm_ra_dynamic_retry_countbu(dm);
	phydm_rrsr_maskbu(dm);
	phydm_ra_mask_watchdogbu(dm);
...
void phydm_arfr_table_init(void *dm_void)
{
	...
	if (dm->support_ic_type & PHYDM_IC_RATEID_IDX_TYPE2) {
		...
		odm_set_mac_regbu(dm, R_0x494, MASKDWORD, 0xfe01f015);
		odm_set_mac_regbu(dm, R_0x498, MASKDWORD, 0x40000000);

		/*ARFR table5(2.4g ac 1ss) for rate_id = 18*/
		odm_set_mac_regbu(dm, R_0x4a4, MASKDWORD, 0x3ff015);
		odm_set_mac_regbu(dm, R_0x4a8, MASKDWORD, 0x40000000);
	}
}
...
void phydm_ra_info_initbu(void *dm_void)
{
	...
	ra_tab->ra_th_ofst = 0;
	ra_tab->ra_ofst_direc = 0;
	ra_tab->rrsr_val_init = odm_get_mac_regbu(dm, R_0x440, MASKDWORD);
	...
	if (dm->support_ic_type == ODM_RTL8822B) {
		...
		ret_value = odm_get_mac_regbu(dm, R_0x4c8, MASKBYTE2);
		odm_set_mac_regbu(dm, R_0x4cc, MASKBYTE3, (ret_value - 1));
	}
#endif
	...
	phydm_rate_adaptive_mask_initbu(dm);
}
	...
	void *dm_void)
{
	...
	if (cur_stage != stage) {
		if (cur_stage == 1) {
			...
			phydm_set_hw_reg_handler_interfacebu(dm, HW_VAR_BASIC_RATE, (u8 *)&os_rate_set);
		} else if (cur_stage == 3 && (stage == 1 || stage == 2))
			phydm_set_hw_reg_handler_interfacebu(dm, HW_VAR_BASIC_RATE, (u8 *)(&mgnt_info->mBrates));
	}
	...
	u8 init_rate_id)
{
	...
	if (dm->rf_type == RF_2T2R || dm->rf_type == RF_2T3R || dm->rf_type == RF_2T4R) {
		if ((dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8192E)) &&
			...
			odm_set_mac_regbu(dm, R_0x4a4, MASKDWORD, 0xfc1ffff); /*N-2SS, ARFR5, rate_id = 0xe*/
			odm_set_mac_regbu(dm, R_0x4a8, MASKDWORD, 0x0); /*N-2SS, ARFR5, rate_id = 0xe*/
		} else if ((dm->support_ic_type & (ODM_RTL8812)) &&
			   (wireless_mode & (ODM_WM_AC_5G | ODM_WM_AC_24G | ODM_WM_AC_ONLY))) {
			PHYDM_DBG(dm, DBG_RA,
				  "[ON ASSOC] set AC-2SS ARFR0 table\n");
			odm_set_mac_regbu(dm, R_0x444, MASKDWORD, 0x0fff); /*@AC-2SS, ARFR0, rate_id = 0x9*/
			odm_set_mac_regbu(dm, R_0x448, MASKDWORD, 0xff01f000); /*@AC-2SS, ARFR0, rate_id = 0x9*/
		}
	}
}
