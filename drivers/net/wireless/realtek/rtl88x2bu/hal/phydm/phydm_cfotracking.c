...
s32 phydm_get_cfo_hzbu(void *dm_void, u32 val, u8 bit_num, u8 frac_num)
{
	s32 val_s = 0;

	val_s = phydm_cnvrt_2_signbu(val, bit_num);
...
void phydm_get_cfo_infobu_acbu(void *dm_void, struct phydm_cfo_rpt *cfo)
{
	...
	val[0] = odm_read_4bytebu(dm, R_0xd0c);
	val_1[0] = odm_read_4bytebu(dm, R_0xd10);
	val_2[0] = odm_get_bb_regbu(dm, R_0xd14, 0x1fff0000);

	#if (defined(PHYDM_COMPILE_ABOVE_2SS))
	val[1] = odm_read_4bytebu(dm, R_0xd4c);
	val_1[1] = odm_read_4bytebu(dm, R_0xd50);
	val_2[1] = odm_get_bb_regbu(dm, R_0xd54, 0x1fff0000);
	#endif

	#if (defined(PHYDM_COMPILE_ABOVE_3SS))
	val[2] = odm_read_4bytebu(dm, R_0xd8c);
	val_1[2] = odm_read_4bytebu(dm, R_0xd90);
	val_2[2] = odm_get_bb_regbu(dm, R_0xd94, 0x1fff0000);
	#endif

	#if (defined(PHYDM_COMPILE_ABOVE_4SS))
	val[3] = odm_read_4bytebu(dm, R_0xdcc);
	val_1[3] = odm_read_4bytebu(dm, R_0xdd0);
	val_2[3] = odm_get_bb_regbu(dm, R_0xdd4, 0x1fff0000);
	#endif

	for (i = 0; i < dm->num_rf_path; i++) {
		val_tmp = val[i] & 0xfff;	/*@ Short CFO, S(12,11)*/
		cfo->cfo_rpt_s[i] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);

		val_tmp = val[i] >> 16;		/*@ Long CFO, S(13,12)*/
		cfo->cfo_rpt_l[i] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);

		val_tmp = val_1[i] & 0x7ff;	/*@ SCFO, S(11,10)*/
		cfo->cfo_rpt_sec[i] = phydm_get_cfo_hzbu(dm, val_tmp, 11, 10);

		val_tmp = val_1[i] >> 16;	/*@ Acq CFO, S(13,12)*/
		cfo->cfo_rpt_acq[i] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);

		val_tmp = val_2[i];		/*@ End CFO, S(13,12)*/
		cfo->cfo_rpt_end[i] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
	}
}
#endif

#if (ODM_IC_11N_SERIES_SUPPORT)
void phydm_get_cfo_infobu_n(void *dm_void, struct phydm_cfo_rpt *cfo)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 val[5] = {0};
	u32 val_tmp = 0;
	...
	val[0] = odm_read_4bytebu(dm, R_0xdac); /*@ Short CFO*/
	val[1] = odm_read_4bytebu(dm, R_0xdb0); /*@ Long CFO*/
	val[2] = odm_read_4bytebu(dm, R_0xdb8); /*@ Sec CFO*/
	val[3] = odm_read_4bytebu(dm, R_0xde0); /*@ Acq CFO*/
	val[4] = odm_read_4bytebu(dm, R_0xdbc); /*@ End CFO*/

	/*@[path-A]*/
	if (dm->support_ic_type & (ODM_RTL8721D | ODM_RTL8710C)) {
		val_tmp = (val[0] & 0x0fff0000) >> 16; /*@ Short CFO, S(12,11)*/
		cfo->cfo_rpt_s[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
		val_tmp = (val[1] & 0x0fff0000) >> 16;	/*@ Long CFO, S(12,11)*/
		cfo->cfo_rpt_l[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
		val_tmp = (val[2] & 0x0fff0000) >> 16;	/*@ Sec CFO, S(12,11)*/
		cfo->cfo_rpt_sec[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
		val_tmp = (val[3] & 0x0fff0000) >> 16;	/*@ Acq CFO, S(12,11)*/
		cfo->cfo_rpt_acq[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
		val_tmp = (val[4] & 0x0fff0000) >> 16;	/*@ Acq CFO, S(12,11)*/
		cfo->cfo_rpt_end[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
	} else {
		val_tmp = (val[0] & 0x0fff0000) >> 16; /*@ Short CFO, S(12,11)*/
		cfo->cfo_rpt_s[0] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
		val_tmp = (val[1] & 0x1fff0000) >> 16;	/*@ Long CFO, S(13,12)*/
		cfo->cfo_rpt_l[0] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
		val_tmp = (val[2] & 0x7ff0000) >> 16;	/*@ Sec CFO, S(11,10)*/
		cfo->cfo_rpt_sec[0] = phydm_get_cfo_hzbu(dm, val_tmp, 11, 10);
		val_tmp = (val[3] & 0x1fff0000) >> 16;	/*@ Acq CFO, S(13,12)*/
		cfo->cfo_rpt_acq[0] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
		val_tmp = (val[4] & 0x1fff0000) >> 16;	/*@ Acq CFO, S(13,12)*/
		cfo->cfo_rpt_end[0] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
	}

	#if (defined(PHYDM_COMPILE_ABOVE_2SS))
	/*@[path-B]*/
	val_tmp = val[0] & 0xfff;		/*@ Short CFO, S(12,11)*/
	cfo->cfo_rpt_s[1] = phydm_get_cfo_hzbu(dm, val_tmp, 12, 11);
	val_tmp = val[1] & 0x1fff;		/*@ Long CFO, S(13,12)*/
	cfo->cfo_rpt_l[1] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
	val_tmp = val[2] & 0x7ff;		/*@ Sec CFO, S(11,10)*/
	cfo->cfo_rpt_sec[1] = phydm_get_cfo_hzbu(dm, val_tmp, 11, 10);
	val_tmp = val[3] & 0x1fff;		/*@ Acq CFO, S(13,12)*/
	cfo->cfo_rpt_acq[1] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
	val_tmp = val[4] & 0x1fff;		/*@ Acq CFO, S(13,12)*/
	cfo->cfo_rpt_end[1] = phydm_get_cfo_hzbu(dm, val_tmp, 13, 12);
	#endif
}
...
void phydm_set_atc_status(void *dm_void, boolean atc_status)
{
	...
	odm_set_bb_regbu(dm, reg_tmp, mask_tmp, atc_status);
...
phydm_get_atc_status(void *dm_void)
{
	...
	atc_status = (boolean)odm_get_bb_regbu(dm, reg_tmp, mask_tmp);
...
void phydm_get_cfo_infobu(void *dm_void, struct phydm_cfo_rpt *cfo)
{
	...
	#if (ODM_IC_11N_SERIES_SUPPORT)
	case PHYDM_IC_N:
		phydm_get_cfo_infobu_n(dm, cfo);
		break;
	#endif
	#if (ODM_IC_11AC_SERIES_SUPPORT)
	case PHYDM_IC_AC:
		phydm_get_cfo_infobu_acbu(dm, cfo);
		break;
	#endif
	default:
		break;
...
boolean
phydm_set_crystal_capbu_regbu(void *dm_void, u8 crystal_cap)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct phydm_cfo_track_struct *cfo_track = &dm->dm_cfo_track;
	u32 reg_val = 0;
	...
	if (dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8188F)) {
		...
		odm_set_mac_regbu(dm, R_0x24, 0x7ff800, reg_val);
		#endif
	}
	#if (RTL8812A_SUPPORT)
	else if (dm->support_ic_type & ODM_RTL8812) {
		/* write 0x2C[30:25] = 0x2C[24:19] = crystal_cap */
		odm_set_mac_regbu(dm, R_0x2c, 0x7FF80000, reg_val);
	}
	#endif
		...
		odm_set_mac_regbu(dm, R_0x2c, 0x00FFF000, reg_val);
	}
	#endif
	#if (RTL8814A_SUPPORT)
	else if (dm->support_ic_type & ODM_RTL8814A) {
		/* write 0x2C[26:21] = 0x2C[20:15] = crystal_cap */
		odm_set_mac_regbu(dm, R_0x2c, 0x07FF8000, reg_val);
	}
	#endif
		...
		odm_set_mac_regbu(dm, R_0x24, 0x7e000000, crystal_cap);
		odm_set_mac_regbu(dm, R_0x28, 0x7e, crystal_cap);
	}
	#endif
	#if (RTL8710B_SUPPORT)
		...
		phydm_set_crystalcapbu(dm, (u8)(reg_val & 0x7f));
	}
	#endif
	#if (RTL8721D_SUPPORT)
		...
		odm_set_mac_regbu(dm, R_0x103c, 0x00FFFC00, reg_val);
	}
	#endif
		...
		odm_set_mac_regbu(dm, R_0x1040, 0x00FFFC00, reg_val);
	} else {
		return false;
	}
#endif
...
void phydm_set_crystal_capbu(void *dm_void, u8 crystal_cap)
{
	...
	if (phydm_set_crystal_capbu_regbu(dm, crystal_cap))
...
void phydm_cfo_trackingbu_resetbu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct phydm_cfo_track_struct *cfo_track = &dm->dm_cfo_track;
	...
	if (cfo_track->crystal_cap > cfo_track->def_x_cap) {
		phydm_set_crystal_capbu(dm, cfo_track->crystal_cap - 1);
	...
	if (dm->support_ic_type & ODM_RTL8814B) {
		...
		odm_set_bb_regbu(dm, R_0xc2c, BIT29, 0x0);
	}
#endif
#endif
}
...
void phydm_cfo_trackingbu_initbu(void *dm_void)
{
	...
	if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C))
		odm_set_mac_regbu(dm, R_0x10, 0x40, 0x1);
#endif
}
...
void phydm_cfo_trackingbu(void *dm_void)
{
	...
	if (!dm->is_linked || !dm->is_one_entry_only) {
		phydm_cfo_trackingbu_resetbu(dm);
		...
		if (dm->bt_info_table.is_bt_enabled) {
			cfo_track->is_adjust = false;
			phydm_set_crystal_capbu(dm, cfo_track->def_x_cap);
		...
		if (cfo_track->is_adjust) {
			...
			phydm_set_crystal_capbu(dm, (u8)crystal_cap);
		}
			      ...
			      char *output, u32 *_out_len)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		if (var1[0] == 1) {
			...
			phydm_set_crystal_capbu(dm, (u8)var1[1]);
			PDM_SNPF(out_len, used, output + used, out_len - used,
				 "Set X_cap=0x%x\n", cfo_track->crystal_cap);
		} else if (var1[0] == 100) {
			PDM_SNPF(out_len, used, output + used, out_len - used,
