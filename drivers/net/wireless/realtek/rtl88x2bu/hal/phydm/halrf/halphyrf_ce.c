...
void odm_get_tracking_tablebu(void *dm_void, u8 thermal_value, u8 delta)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_rf_calibration_struct *cali_info = &dm->rf_calibrate_info;
	struct _hal_rf_ *rf = &dm->rf_table;
...
void odm_pwrtrk_methodbu(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u8 p, idxforchnl = 0;
	...
	if (dm->support_ic_type &
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		       "***Enter PwrTrk MIX_MODE***\n");
		p = (u8)odm_get_bb_regbu(dm, R_0x948, 0x00000080);
		(*c.odm_tx_pwr_track_set_pwr)(dm, MIX_MODE, p, 0);
		/*if open ant_div 0x948=140,do 2 path pwr_track*/
		if (odm_get_bb_regbu(dm, R_0x948, 0x00000040))
...
void odm_txpowertracking_callback_thermal_meterbu(struct dm_struct *dm)
...
#endif
{
	...
	thermal_value =
		(u8)odm_get_rf_regbu(dm, RF_PATH_A, c.thermal_reg_addr, 0xfc00);
	...
	if (delta > 0 && cali_info->txpowertrack_control) {
		...
		odm_get_tracking_tablebu(dm, thermal_value, delta);
...
void odm_iq_calibratebu(struct dm_struct *dm)
{
