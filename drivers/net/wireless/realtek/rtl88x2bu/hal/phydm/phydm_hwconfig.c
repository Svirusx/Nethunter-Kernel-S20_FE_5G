...
enum hal_status
odm_config_rf_with_header_filebu(struct dm_struct *dm,
 ...
 /*8814B need review, when phydm has related files*/
	...
	if (config_type == CONFIG_RF_RADIO) {
		if (dm->fw_offload_ability & PHYDM_PHY_PARAM_OFFLOAD) {
			result = phydm_set_reg_by_fwbu(dm,
...
enum hal_status
odm_config_rf_with_tx_pwr_track_header_filebu(struct dm_struct *dm)
{
	PHYDM_DBG(dm, ODM_COMP_INIT, "===>%s (%s)\n", __func__,
		  (dm->is_mp_chip) ? "MPChip" : "TestChip");
	PHYDM_DBG(dm, ODM_COMP_INIT,
...
#if RTL8188E_SUPPORT
	if (dm->support_ic_type == ODM_RTL8188E) {
		if (odm_get_mac_regbu(dm, R_0xf0, 0xF000) >= 8) { /*@if 0xF0[15:12] >= 8, SMIC*/
			if (dm->support_interface == ODM_ITRF_PCIE)
...
enum hal_status
odm_config_bb_with_header_filebu(struct dm_struct *dm,
...
/*@jj add 20170822*/
	...
	if (dm->support_ic_type == ODM_RTL8195B) {
		if (config_type == CONFIG_BB_PHY_REG) {
			...
			if (dm->package_type == 1)
				odm_set_bb_regbu(dm, R_0xaa8, 0x1f0000, 0x10);
			else
				odm_set_bb_regbu(dm, R_0xaa8, 0x1f0000, 0x12);
		}
	}
#endif
#if (RTL8198F_SUPPORT == 1)
...
enum hal_status
odm_config_mac_with_header_filebu(struct dm_struct *dm)
{
	enum hal_status result = HAL_STATUS_SUCCESS;
...
u32 odm_get_hw_img_versionbu(struct dm_struct *dm)
{
	u32 version = 0;
...
u32 query_phydm_trx_capabilitybu(struct dm_struct *dm)
{
	u32 value32 = 0xFFFFFFFF;

#if (RTL8821C_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8821C)
		value32 = query_phydm_trx_capabilitybu_8821c(dm);
#endif
#if (RTL8195B_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8195B)
		value32 = query_phydm_trx_capabilitybu_8195b(dm);
#endif
	return value32;
}

u32 query_phydm_stbc_capabilitybu(struct dm_struct *dm)
{
	u32 value32 = 0xFFFFFFFF;

#if (RTL8821C_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8821C)
		value32 = query_phydm_stbc_capabilitybu_8821c(dm);
#endif
#if (RTL8195B_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8195B)
		value32 = query_phydm_stbc_capabilitybu_8195b(dm);
#endif

	return value32;
}

u32 query_phydm_ldpc_capabilitybu(struct dm_struct *dm)
{
	u32 value32 = 0xFFFFFFFF;

#if (RTL8821C_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8821C)
		value32 = query_phydm_ldpc_capabilitybu_8821c(dm);
#endif
#if (RTL8195B_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8195B)
		value32 = query_phydm_ldpc_capabilitybu_8195b(dm);
#endif
	return value32;
}

u32 query_phydm_txbf_parametersbu(struct dm_struct *dm)
{
	u32 value32 = 0xFFFFFFFF;

#if (RTL8821C_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8821C)
		value32 = query_phydm_txbf_parametersbu_8821c(dm);
#endif
#if (RTL8195B_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8195B)
		value32 = query_phydm_txbf_parametersbu_8195b(dm);
#endif
	return value32;
}

u32 query_phydm_txbf_capabilitybu(struct dm_struct *dm)
{
	u32 value32 = 0xFFFFFFFF;

#if (RTL8821C_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8821C)
		value32 = query_phydm_txbf_capabilitybu_8821c(dm);
#endif
#if (RTL8195B_SUPPORT == 1)
	if (dm->support_ic_type == ODM_RTL8195B)
		value32 = query_phydm_txbf_capabilitybu_8195b(dm);
#endif
	return value32;
}
