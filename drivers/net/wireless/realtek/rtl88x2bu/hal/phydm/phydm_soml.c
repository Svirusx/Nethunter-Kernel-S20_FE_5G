...
void phydm_dynamicsoftmletting(void *dm_void)
{
	...
	if (!*dm->mp_mode) {
		if (dm->support_ic_type & ODM_RTL8822B) {
			...
			ret_val = odm_get_bb_regbu(dm, R_0xf8c, MASKBYTE0);
			PHYDM_DBG(dm, ODM_COMP_API,
...
#if 0
			/*odm_set_bb_regbu(dm, R_0x19a8, MASKDWORD, 0xc10a0000);*/
#endif
...
void phydm_soml_on_off(void *dm_void, u8 swch)
{
	...
	if (swch == SOML_ON) {
		...
		if (dm->support_ic_type & (ODM_RTL8197F | ODM_RTL8192F))
			odm_set_bb_regbu(dm, R_0x998, BIT(6), swch);
#if (RTL8822B_SUPPORT == 1)
...
void phydm_soml_cfo_process(void *dm_void, s32 *diff_a, s32 *diff_b)
{
	...
	value32 = odm_get_bb_regbu(dm, R_0xd10, MASKDWORD);
	value32_1 = odm_get_bb_regbu(dm, R_0xd14, MASKDWORD);
	value32_2 = odm_get_bb_regbu(dm, R_0xd50, MASKDWORD);
	value32_3 = odm_get_bb_regbu(dm, R_0xd54, MASKDWORD);
...
void phydm_adsl_init_state(void *dm_void)
{
	...
	odm_move_memorybu(dm, soml_tab->ht_byte, ht_reset,
			HT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->ht_byte_on, ht_reset,
			HT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->ht_byte_off, ht_reset,
			HT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->vht_byte, vht_reset,
			VHT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->vht_byte_on, vht_reset,
			VHT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->vht_byte_off, vht_reset,
			VHT_RATE_IDX * size);
	if (dm->support_ic_type == ODM_RTL8822B) {
		soml_tab->cfo_cnt++;
	...
	odm_set_timerbu(dm, &soml_tab->phydm_adaptive_soml_timer,
...
void phydm_adsl_odd_state(void *dm_void)
{
	...
	soml_tab->soml_state_cnt++;
	odm_move_memorybu(dm, soml_tab->pre_ht_cnt, soml_tab->ht_cnt,
			HT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->pre_vht_cnt, soml_tab->vht_cnt,
			VHT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->pre_ht_byte, soml_tab->ht_byte,
			HT_RATE_IDX * size);
	odm_move_memorybu(dm, soml_tab->pre_vht_byte, soml_tab->vht_byte,
			VHT_RATE_IDX * size);
...
void phydm_adaptive_soml_timers_usb(void *dm_void, u8 state)
{
	...
	if (state == INIT_SOML_TIMMER) {
		...
		odm_cancel_timerbu(dm, &soml_tab->phydm_adaptive_soml_timer);
	} else if (state == RELEASE_SOML_TIMMER) {
		odm_release_timerbu(dm, &soml_tab->phydm_adaptive_soml_timer);
	}
}
...
void phydm_adaptive_soml_timers(void *dm_void, u8 state)
{
	...
	{
	if (state == INIT_SOML_TIMMER) {
		odm_initialize_timerbu(dm, &soml_tab->phydm_adaptive_soml_timer,
...
void phydm_adaptive_soml_init(void *dm_void)
{
	...
	if (dm->support_ic_type & (ODM_RTL8197F | ODM_RTL8192F))
		odm_set_bb_regbu(dm, 0x988, BIT(25), 1);
}
...
void phydm_init_soft_ml_settingbu(void *dm_void)
{
	...
	if (!*dm->mp_mode) {
		if (dm->support_ic_type & ODM_RTL8822B) {
#if 0
			/*odm_set_bb_regbu(dm, R_0x19a8, MASKDWORD, 0xd10a0000);*/
#endif
			phydm_somlrxhp_setting(dm, true);
			dm->bsomlenabled = true;
		}
	...
	if (!*dm->mp_mode) {
		if (dm->support_ic_type & ODM_RTL8821C)
			odm_set_bb_regbu(dm, R_0x19a8, soml_mask, 0xd);
	}
#endif
