...
enum odm_board_type boardTypebu(u8 InterfaceSel)
{
...
#endif
	/* RTW_INFO("===> boardTypebu(): (pHalData->InterfaceSel, pDM_Odm->BoardType) = (%d, %d)\n", InterfaceSel, board); */

	return board;
}

void rtw_hal_update_iqk_fw_offload_capbu(_adapter *adapter)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	struct dm_struct *p_dm_odm = adapter_to_phydm(adapter);

	if (hal->RegIQKFWOffload) {
		rtw_sctx_initbu(&hal->iqk_sctx, 0);
		phydm_fwoffload_ability_initbu(p_dm_odm, PHYDM_RF_IQK_OFFLOAD);
	} else
		phydm_fwoffload_ability_clearbu(p_dm_odm, PHYDM_RF_IQK_OFFLOAD);
	...
	if (rtw_mi_check_statusbu(adapter, MI_LINKED)) {
		...
		#endif
		halrf_iqk_triggerbu(p_dm_odm, _FALSE);
	}
}
...
void rtw_phydm_iqk_trigger(_adapter *adapter)
{
	...
	halrf_cmn_info_setbu(p_dm_odm, HALRF_CMNINFO_RFK_FORBIDDEN, rfk_forbidden);
#if (RTL8822C_SUPPORT == 1) || (RTL8814B_SUPPORT == 1) || (RTL8723F_SUPPORT == 1)
	/* halrf_cmn_info_setbu(p_dm_odm, HALRF_CMNINFO_IQK_SEGMENT, segment); to do */
	halrf_rf_k_connect_triggerbu(p_dm_odm, _TRUE, SEGMENT_FREE);
#else
	/*segment = _rtw_phydm_iqk_segment_chk(adapter);*/
	halrf_cmn_info_setbu(p_dm_odm, HALRF_CMNINFO_IQK_SEGMENT, segment);
	halrf_segment_iqk_triggerbu(p_dm_odm, clear, segment);
#endif
}
#endif
...
void rtw_phydm_iqk_trigger_dbgbu(_adapter *adapter, bool recovery, bool clear, bool segment)
{
	struct dm_struct *p_dm_odm = adapter_to_phydm(adapter);
		...
		halrf_segment_iqk_triggerbu(p_dm_odm, clear, segment);
#else
		halrf_iqk_triggerbu(p_dm_odm, recovery);
#endif
}
void rtw_phydm_lck_triggerbu(_adapter *adapter)
{
	struct dm_struct *p_dm_odm = adapter_to_phydm(adapter);

	halrf_lck_triggerbu(p_dm_odm);
}
...
void rtw_hal_iqk_test(_adapter *adapter, bool recovery, bool clear, bool segment)
{
	...
	rtw_phydm_abilitybu_backup(adapter);
	rtw_phydm_func_disable_all(adapter);

	halrf_cmn_info_setbu(p_dm_odm, HALRF_CMNINFO_ABILITY, HAL_RF_IQK);

	rtw_phydm_iqk_trigger_dbgbu(adapter, recovery, clear, segment);
	rtw_phydm_abilitybu_restore(adapter);
...
void rtw_hal_lck_test(_adapter *adapter)
{
	...
	halrf_cmn_info_setbu(p_dm_odm, HALRF_CMNINFO_ABILITY, HAL_RF_LCK);

	rtw_phydm_lck_triggerbu(adapter);
...
void rtw_hal_update_param_init_fw_offload_cap(_adapter *adapter)
{
	...
	if (adapter->registrypriv.fw_param_init)
		phydm_fwoffload_ability_initbu(p_dm_odm, PHYDM_PHY_PARAM_OFFLOAD);
	else
		phydm_fwoffload_ability_clearbu(p_dm_odm, PHYDM_PHY_PARAM_OFFLOAD);

	RTW_INFO("Init-Parameter FW offload:%s\n", adapter->registrypriv.fw_param_init ? "enable" : "disable");
}
#endif

void record_ra_infobu(void *p_dm_void, u8 macid, struct cmn_sta_info *p_sta, u64 ra_mask)
{
	...
	if (p_sta) {
		rtw_macid_ctl_set_bwbu(macid_ctl, macid, p_sta->ra_info.ra_bw_mode);
		rtw_macid_ctl_set_vht_enbu(macid_ctl, macid, p_sta->ra_info.is_vht_enable);
		rtw_macid_ctl_set_rate_bmp0bu(macid_ctl, macid, ra_mask);
		rtw_macid_ctl_set_rate_bmp1bu(macid_ctl, macid, ra_mask >> 32);
...
void rtw_phydm_ops_func_initbu(struct dm_struct *p_phydm)
{
	struct ra_table *p_ra_t = &p_phydm->dm_ra_table;

	p_ra_t->record_ra_infobu = record_ra_infobu;
	#ifdef CONFIG_SUPPORT_DYNAMIC_TXPWR
	p_phydm->fill_desc_dyntxpwr = rtw_phydm_fill_desc_dpt;
	#endif
}
void rtw_phydm_priv_initbu(_adapter *adapter)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	struct dm_struct *phydm = &(hal->odmpriv);

	phydm->adapter = adapter;
	odm_cmn_info_initbu(phydm, ODM_CMNINFO_PLATFORM, ODM_CE);
}

void Init_ODM_ComInfobu(_adapter *adapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	rtw_odm_init_ic_typebu(adapter);

	if (rtw_get_intf_type(adapter) == RTW_GSPI)
		odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_INTERFACE, ODM_ITRF_SDIO);
	else
		odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_INTERFACE, rtw_get_intf_type(adapter));

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_MP_TEST_CHIP, IS_NORMAL_CHIP(pHalData->version_id));

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_PATCH_ID, pHalData->CustomerID);

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_BWIFI_TEST, adapter->registrypriv.wifi_spec);

#ifdef CONFIG_ADVANCE_OTA
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_ADVANCE_OTA, adapter->registrypriv.adv_ota);
#endif
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_RF_TYPE, pHalData->rf_type);

	{
		...
		if (pHalData->ExternalLNA_2G != 0) {
			...
			odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_EXT_LNA, 1);
		}
		if (pHalData->external_lna_5g != 0) {
			odm_board_type |= ODM_BOARD_EXT_LNA_5G;
			odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_5G_EXT_LNA, 1);
		}
		if (pHalData->ExternalPA_2G != 0) {
			odm_board_type |= ODM_BOARD_EXT_PA;
			odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_EXT_PA, 1);
		}
		if (pHalData->external_pa_5g != 0) {
			odm_board_type |= ODM_BOARD_EXT_PA_5G;
			odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_5G_EXT_PA, 1);
		}
		if (pHalData->EEPROMBluetoothCoexist)
			odm_board_type |= ODM_BOARD_BT;

		odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_BOARD_TYPE, odm_board_type);
		/* 1 ============== End of BoardType ============== */
	}

	rtw_hal_set_odm_varbu(adapter, HAL_ODM_REGULATION, NULL, _TRUE);

#ifdef CONFIG_DFS_MASTER
	rtw_odm_update_dfs_region(dvobj);
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_DFS_MASTER_ENABLE, &(adapter_to_rfctl(adapter)->radar_detect_enabled));
#endif

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_GPA, pHalData->TypeGPA);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_APA, pHalData->TypeAPA);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_GLNA, pHalData->TypeGLNA);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_ALNA, pHalData->TypeALNA);

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_RFE_TYPE, pHalData->rfe_type);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_X_CAP_SETTING, pHalData->crystal_cap);

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_EXT_TRSW, 0);

	/*Add by YuChen for kfree init*/
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_REGRFKFREEENABLE, adapter->registrypriv.RegPwrTrimEnable);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_RFKFREEENABLE, pHalData->RfKFreeEnable);

	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_RF_ANTENNA_TYPE, pHalData->TRxAntDivType);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_BE_FIX_TX_ANT, pHalData->b_fix_tx_ant);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_WITH_EXT_ANTENNA_SWITCH, pHalData->with_extenal_ant_switch);

	/* (8822B) efuse 0x3D7 & 0x3D8 for TX PA bias */
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_EFUSE0X3D7, pHalData->efuse0x3d7);
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_EFUSE0X3D8, pHalData->efuse0x3d8);

	/* waiting for PhyDMV034 support*/
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_MANUAL_SUPPORTABILITY, &(adapter->registrypriv.phydm_ability)); 
	/*Add by YuChen for adaptivity init*/
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_ADAPTIVITY, &(adapter->registrypriv.adaptivity_en));
	phydm_adaptivity_info_initbu(pDM_Odm, PHYDM_ADAPINFO_CARRIER_SENSE_ENABLE, (adapter->registrypriv.adaptivity_mode != 0) ? TRUE : FALSE);
	phydm_adaptivity_info_initbu(pDM_Odm, PHYDM_ADAPINFO_TH_L2H_INI, adapter->registrypriv.adaptivity_th_l2h_ini);
	phydm_adaptivity_info_initbu(pDM_Odm, PHYDM_ADAPINFO_TH_EDCCA_HL_DIFF, adapter->registrypriv.adaptivity_th_edcca_hl_diff);

	/*halrf info init*/
	halrf_cmn_info_initbu(pDM_Odm, HALRF_CMNINFO_EEPROM_THERMAL_VALUE, pHalData->eeprom_thermal_meter);
	halrf_cmn_info_initbu(pDM_Odm, HALRF_CMNINFO_PWT_TYPE, 0);
	halrf_cmn_info_initbu(pDM_Odm, HALRF_CMNINFO_MP_POWER_TRACKING_TYPE, pHalData->txpwr_pg_mode);

	if (rtw_odm_adaptivity_neededbu(adapter) == _TRUE)
		rtw_odm_adaptivity_config_msgbu(RTW_DBGDUMP, adapter);

#ifdef CONFIG_IQK_PA_OFF
	odm_cmn_info_initbu(pDM_Odm, ODM_CMNINFO_IQKPAOFF, 1);
#endif
	rtw_hal_update_iqk_fw_offload_capbu(adapter);
	#ifdef CONFIG_FW_OFFLOAD_PARAM_INIT
	...
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_ANT_DIV, &(pHalData->AntDivCfg));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_MP_MODE, &(adapter->registrypriv.mp_mode));

	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BB_OPERATION_MODE, &(pHalData->phydm_op_mode));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_TX_UNI, &(dvobj->traffic_stat.tx_bytes));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_RX_UNI, &(dvobj->traffic_stat.rx_bytes));

	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BAND, &(pHalData->current_band_type));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_FORCED_RATE, &(pHalData->ForcedDataRate));

	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_SEC_CHNL_OFFSET, &(pHalData->nCur40MhzPrimeSC));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_SEC_MODE, &(adapter->securitypriv.dot11PrivacyAlgrthm));
	...
	if ((adapter->registrypriv.rtw_nb_config == RTW_NB_CONFIG_WIDTH_10)
		...
		odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BW, &(adapter->registrypriv.rtw_nb_config));
	}
	else
#endif
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BW, &(pHalData->current_channel_bw));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_CHNL, &(pHalData->current_channel));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_NET_CLOSED, &(adapter->net_closed));

	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_SCAN, &(pHalData->bScanInProcess));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_POWER_SAVING, &(pwrctl->bpower_saving));
	/*Add by Yuchen for phydm beamforming*/
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_TX_TP, &(dvobj->traffic_stat.cur_tx_tp));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_RX_TP, &(dvobj->traffic_stat.cur_rx_tp));
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_ANT_TEST, &(pHalData->antenna_test));
#ifdef CONFIG_RTL8723B
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_IS1ANTENNA, &pHalData->EEPROMBluetoothAntNum);
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_RFDEFAULTPATH, &pHalData->ant_path);
#endif /*CONFIG_RTL8723B*/
#ifdef CONFIG_USB_HCI
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_HUBUSBMODE, &(dvobj->usb_speed));
#endif

#ifdef CONFIG_DYNAMIC_SOML
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_ADAPTIVE_SOML, &(adapter->registrypriv.dyn_soml_en));
#endif
#ifdef CONFIG_RTW_PATH_DIV
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_PATH_DIV, &(adapter->registrypriv.path_div));
#endif
	odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_FCS_MODE, &(pHalData->multi_ch_switch_mode));

	/*halrf info hook*/
	/* waiting for PhyDMV034 support*/
	halrf_cmn_info_hookbu(pDM_Odm, HALRF_CMNINFO_MANUAL_RF_SUPPORTABILITY, &(adapter->registrypriv.halrf_ability));
#ifdef CONFIG_MP_INCLUDED
	halrf_cmn_info_hookbu(pDM_Odm, HALRF_CMNINFO_CON_TX, &(adapter->mppriv.mpt_ctx.is_start_cont_tx));
	halrf_cmn_info_hookbu(pDM_Odm, HALRF_CMNINFO_SINGLE_TONE, &(adapter->mppriv.mpt_ctx.is_single_tone));
	halrf_cmn_info_hookbu(pDM_Odm, HALRF_CMNINFO_CARRIER_SUPPRESSION, &(adapter->mppriv.mpt_ctx.is_carrier_suppression));
	halrf_cmn_info_hookbu(pDM_Odm, HALRF_CMNINFO_MP_RATE_INDEX, &(adapter->mppriv.mpt_ctx.mpt_rate_index));
#endif/*CONFIG_MP_INCLUDED*/
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++)
		phydm_cmn_sta_info_hookbu(pDM_Odm, i, NULL);

	rtw_phydm_ops_func_initbu(pDM_Odm);
	phydm_dm_early_initbu(pDM_Odm);
	/* TODO */
	/* odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BT_OPERATION, _FALSE); */
	/* odm_cmn_info_hookbu(pDM_Odm, ODM_CMNINFO_BT_DISABLE_EDCA, _FALSE); */
}
...
void rtw_hal_turbo_edcabu(_adapter *adapter)
{
	...
	if (rtw_mi_check_statusbu(adapter, MI_ASSOC))
		is_linked = _TRUE;
...
#define TXOP_MCS6M			0x0061
{
			...
			for (mac_id = 0; mac_id < macid_ctl->num; mac_id++) {
				if (rtw_macid_is_usedbu(macid_ctl, mac_id))  {
			...
			if ( edca_param != hal_data->ac_param_be) {
				...
				rtw_hal_set_hwregbu(adapter, HW_VAR_AC_PARAM_BE, (u8 *)(&edca_param));
...
s8 rtw_dm_get_min_rssibu(_adapter *adapter)
{
...
s8 rtw_phydm_get_min_rssibu(_adapter *adapter)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	s8 rssi_min = 0;

	rssi_min = phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_RSSI_MIN);
	return rssi_min;
}

u8 rtw_phydm_get_cur_igibu(_adapter *adapter)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	u8 cur_igi = 0;

	cur_igi = phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CURR_IGI);
	return cur_igi;
}
...
bool rtw_phydm_get_edcca_flag(_adapter *adapter)
{
	...
	cur_edcca_flag = phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_EDCCA_FLAG);
	return cur_edcca_flag;
}

u32 rtw_phydm_get_phy_cntbu(_adapter *adapter, enum phy_cnt cnt)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);

	if (cnt == FA_OFDM)
		return  phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_FA_OFDM);
	else if (cnt == FA_CCK)
		return  phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_FA_CCK);
	else if (cnt == FA_TOTAL)
		return  phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_FA_TOTAL);
	else if (cnt == CCA_OFDM)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CCA_OFDM);
	else if (cnt == CCA_CCK)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CCA_CCK);
	else if (cnt == CCA_ALL)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CCA_ALL);
	else if (cnt == CRC32_OK_VHT)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_OK_VHT);
	else if (cnt == CRC32_OK_HT)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_OK_HT);
	else if (cnt == CRC32_OK_LEGACY)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_OK_LEGACY);
	else if (cnt == CRC32_OK_CCK)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_OK_CCK);
	else if (cnt == CRC32_ERROR_VHT)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_ERROR_VHT);
	else if (cnt == CRC32_ERROR_HT)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_ERROR_HT);
	else if (cnt == CRC32_ERROR_LEGACY)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_ERROR_LEGACY);
	else if (cnt == CRC32_ERROR_CCK)
		return	phydm_cmn_info_querybu(phydm, (enum phydm_info_query) PHYDM_INFO_CRC32_ERROR_CCK);
	else
		return 0;
}

u8 rtw_phydm_is_iqk_in_progressbu(_adapter *adapter)
{
	u8 rts = _FALSE;
	struct dm_struct *podmpriv = adapter_to_phydm(adapter);

	odm_acquire_spin_lockbu(podmpriv, RT_IQK_SPINLOCK);
	if (podmpriv->rf_calibrate_info.is_iqk_in_progress == _TRUE) {
		RTW_ERR("IQK InProgress\n");
		rts = _TRUE;
	}
	odm_release_spin_lockbu(podmpriv, RT_IQK_SPINLOCK);
	...
	BOOLEAN					bSet)
{
	...
	case HAL_ODM_STA_INFO: {
		...
		if (bSet) {
			...
			phydm_cmn_sta_info_hookbu(podmpriv, psta->cmn.mac_id, &(psta->cmn));
		} else {
			RTW_INFO("### Clean STA_(%d) info ###\n", psta->cmn.mac_id);
			/* _enter_critical_bh(&pHalData->odm_stainfo_lock, &irqL); */
			psta->cmn.dm_ctrl = 0;
			phydm_cmn_sta_info_hookbu(podmpriv, psta->cmn.mac_id, NULL);
	...
	case HAL_ODM_P2P_STATE:
		odm_cmn_info_updatebu(podmpriv, ODM_CMNINFO_WIFI_DIRECT, bSet);
		break;
	case HAL_ODM_WIFI_DISPLAY_STATE:
		odm_cmn_info_updatebu(podmpriv, ODM_CMNINFO_WIFI_DISPLAY, bSet);
		break;
	case HAL_ODM_REGULATION:
		/* used to auto enable/disable adaptivity by SD7 */
		phydm_adaptivity_info_updatebu(podmpriv, PHYDM_ADAPINFO_DOMAIN_CODE_2G, 0);
		phydm_adaptivity_info_updatebu(podmpriv, PHYDM_ADAPINFO_DOMAIN_CODE_5G, 0);
		break;
	case HAL_ODM_INITIAL_GAIN: {
		u8 rx_gain = *((u8 *)(pValue1));
		/*printk("rx_gain:%x\n",rx_gain);*/
		if (rx_gain == 0xff) {/*restore rx gain*/
			/*odm_write_digbu(podmpriv,pDigTable->backup_ig_value);*/
			odm_pause_digbu(podmpriv, PHYDM_RESUME, PHYDM_PAUSE_LEVEL_0, rx_gain);
		} else {
			/*pDigTable->backup_ig_value = pDigTable->cur_ig_value;*/
			/*odm_write_digbu(podmpriv,rx_gain);*/
			odm_pause_digbu(podmpriv, PHYDM_PAUSE, PHYDM_PAUSE_LEVEL_0, rx_gain);
		}
	}
	break;
	case HAL_ODM_RX_INFO_DUMP: {
		...
		sel = pValue1;
		cur_igi = rtw_phydm_get_cur_igibu(Adapter);
		rssi_min = rtw_phydm_get_min_rssibu(Adapter);

		_RTW_PRINT_SEL(sel, "============ Rx Info dump ===================\n");
		_RTW_PRINT_SEL(sel, "is_linked = %d, rssi_min = %d(%%)(%d(%%)), current_igi = 0x%x\n"
			, podmpriv->is_linked, rssi_min, rtw_dm_get_min_rssibu(Adapter), cur_igi);
		_RTW_PRINT_SEL(sel, "cnt_cck_fail = %d, cnt_ofdm_fail = %d, Total False Alarm = %d\n",
			rtw_phydm_get_phy_cntbu(Adapter, FA_CCK),
			rtw_phydm_get_phy_cntbu(Adapter, FA_OFDM),
			rtw_phydm_get_phy_cntbu(Adapter, FA_TOTAL));
	...
	case HAL_ODM_RX_Dframe_INFO: {
		...
		rtw_dump_rx_dframe_infobu(Adapter, sel);
#endif
	}
		break;
...
#endif
	case HAL_ODM_INITIAL_GAIN:
		*((u8 *)pValue1) = rtw_phydm_get_cur_igibu(Adapter);
		break;
	default:
		break;
	}
...
void rtw_phydm_wd_lps_lclk_hdl(_adapter *adapter)
{
	...
	odm_cmn_info_updatebu(&pHalData->odmpriv, ODM_CMNINFO_LINK, is_linked);

	phydm_watchdogbu_lpsbu_32kbu(&pHalData->odmpriv);
...
void rtw_phydm_watchdogbubu_in_lps_lclk(_adapter *adapter)
{
	struct mlme_priv	*pmlmepriv = &adapter->mlmepriv;
	struct sta_priv *pstapriv = &adapter->stapriv;
	u8 cur_igi = 0;
	...
	cur_igi = rtw_phydm_get_cur_igibu(adapter);
	min_rssi = rtw_dm_get_min_rssibu(adapter);
...
void dump_sta_trafficbu(void *sel, _adapter *adapter, struct sta_info *psta)
{
	...
	if (is_client_associated_to_apbu(psta->padapter))
	...
	curr_sgi = rtw_get_current_tx_sgibu(adapter, psta);
	RTW_PRINT_SEL(sel, "tx_rate : %s(%s)  rx_rate : %s, rx_rate_bmc : %s, rssi : %d %%\n"
		, HDATA_RATE(rtw_get_current_tx_ratebu(adapter, psta)), (curr_sgi) ? "S" : "L"
...
void dump_sta_infobu(void *sel, struct sta_info *psta)
{
	struct ra_sta_info *ra_info;
	...
	curr_tx_sgi = rtw_get_current_tx_sgibu(psta->padapter, psta);
	curr_tx_rate = rtw_get_current_tx_ratebu(psta->padapter, psta);
...
void rtw_phydm_ra_registedbubu(_adapter *adapter, struct sta_info *psta)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(dvobj);
	...
	phydm_ra_registedbu(&hal_data->odmpriv, psta->cmn.mac_id, psta->cmn.rssi_stat.rssi);
	dump_sta_infobu(RTW_DBGDUMP, psta);
}
...
static void init_phydm_info(_adapter *adapter)
{
	...
	odm_cmn_info_initbu(phydm, ODM_CMNINFO_FW_VER, hal_data->firmware_version);
	odm_cmn_info_initbu(phydm, ODM_CMNINFO_FW_SUB_VER, hal_data->firmware_sub_version);
}
...
*/
static u8 rtw_phydm_config_trx_pathbu(_adapter *adapter)
{
	u8 rst = _SUCCESS;
...
#elif defined(CONFIG_RTL8812A) || defined(CONFIG_RTL8192E)
{
	...
	if (hal_data->txpath_num_nss[0] == 2)
		phydm_tx_2pathbu(adapter_to_phydm(adapter));
}
#endif
...
void rtw_phydm_initbu(_adapter *adapter)
{
	PHAL_DATA_TYPE	hal_data = GET_HAL_DATA(adapter);
	struct dm_struct	*phydm = &(hal_data->odmpriv);

	rtw_phydm_config_trx_pathbu(adapter);
	init_phydm_info(adapter);
	hal_data->phydm_init_result = odm_dm_initbu(phydm);

#ifdef CONFIG_CUSTOMER01_SMART_ANTENNA
	phydm_pathb_q_matrix_rotatebu_enbu(phydm);
#endif
}

bool rtw_phydm_set_crystal_capbu(_adapter *adapter, u8 crystal_cap)
{
	PHAL_DATA_TYPE	hal_data = GET_HAL_DATA(adapter);
	struct dm_struct	*phydm = &(hal_data->odmpriv);

	return phydm_set_crystal_capbu_regbu(phydm, crystal_cap);
}
...
void rtw_phydm_lps_pg_hdl(_adapter *adapter, struct sta_info *sta, bool in_lpspg)
{
	...
	if (in_lpspg) {
		...
		/*rate_id = phydm_get_rate_from_rssi_lvbu(phydm, sta->cmn.mac_id);*/
	} else {
...
static u8 _rtw_phydm_rfk_condition_check(_adapter *adapter, u8 is_scaning, u8 ifs_linked)
{
	...
	if (ifs_linked) {
		if (is_scaning) {
			...
			rfk_allowed = rtw_mi_stayin_union_ch_chkbu(adapter) ? _TRUE : _FALSE;
...
void rtw_dyn_soml_config(_adapter *adapter)
{
	...
	if (adapter->registrypriv.dyn_soml_en == 1) {
		...
		rtw_hal_set_hwregbu(adapter , HW_VAR_SET_SOML_PARAM , NULL);
...
void rtw_phydm_set_rrsrbu(_adapter *adapter, u32 rrsr_value, bool write_rrsr)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	...
	odm_cmn_info_updatebu(phydm, ODM_CMNINFO_RRSR_VAL, temp_rrsr);
	if(write_rrsr)
		phydm_rrsr_set_registerbu(phydm, temp_rrsr);
}
void rtw_phydm_dyn_rrsr_en(_adapter *adapter, bool en_rrsr)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);

	phydm_rrsr_enbu(phydm, en_rrsr);
}
void rtw_phydm_read_efusebu(_adapter *adapter)
{
	PHAL_DATA_TYPE hal_data = GET_HAL_DATA(adapter);
	struct dm_struct *phydm = &(hal_data->odmpriv);

	/*PHYDM API - thermal trim*/
	phydm_get_thermal_trim_offsetbu(phydm);
	/*PHYDM API - power trim*/
	phydm_get_power_trim_offsetbu(phydm);
}
...
void rtw_phydm_pwr_tracking_directly(_adapter *adapter)
{
	...
	halrf_cmn_info_setbu(&hal_data->odmpriv, HALRF_CMNINFO_RFK_FORBIDDEN, rfk_forbidden);

	odm_txpowertracking_direct_cebu(&hal_data->odmpriv);
}
#endif

void rtw_phydm_watchdogbubu(_adapter *adapter, bool in_lps)
{
	u8	bLinked = _FALSE;
	...
	if (rtw_mi_check_statusbu(adapter, MI_ASSOC)) {
		bLinked = _TRUE;
		if (rtw_mi_check_statusbu(adapter, MI_STA_LINKED))
		bsta_state = _TRUE;
	}

	odm_cmn_info_updatebu(&pHalData->odmpriv, ODM_CMNINFO_LINK, bLinked);
	odm_cmn_info_updatebu(&pHalData->odmpriv, ODM_CMNINFO_STATION_STATE, bsta_state);

	#ifdef CONFIG_BT_COEXIST
	bBtDisabled = rtw_btcoex_IsBtDisabled(adapter);
	odm_cmn_info_updatebu(&pHalData->odmpriv, ODM_CMNINFO_BT_ENABLED,
				(bBtDisabled == _TRUE) ? _FALSE : _TRUE);
	#else
	odm_cmn_info_updatebu(&pHalData->odmpriv, ODM_CMNINFO_BT_ENABLED, _FALSE);
	#endif /* CONFIG_BT_COEXIST */

	rfk_forbidden = (_rtw_phydm_rfk_condition_check(adapter, pHalData->bScanInProcess, bLinked) == _TRUE) ? _FALSE : _TRUE;
	halrf_cmn_info_setbu(&pHalData->odmpriv, HALRF_CMNINFO_RFK_FORBIDDEN, rfk_forbidden);

	#if ((RTL8822B_SUPPORT == 1) || (RTL8821C_SUPPORT == 1) || (RTL8814B_SUPPORT == 1) || (RTL8822C_SUPPORT == 1))
	segment_iqk = _rtw_phydm_iqk_segment_chk(adapter, bLinked);
	halrf_cmn_info_setbu(&pHalData->odmpriv, HALRF_CMNINFO_IQK_SEGMENT, segment_iqk);
	#endif
	...
	if (bLinked == _FALSE) {
		...
		halrf_cmn_info_setbu(&pHalData->odmpriv, HALRF_CMNINFO_RATE_INDEX, tx_unlinked_low_rate);
	}

	/*if (!rtw_mi_stayin_union_band_chkbu(adapter)) {
		#ifdef DBG_PHYDM_STATE_CHK
	...
	if (in_lps)
		phydm_watchdogbu_lpsbu(&pHalData->odmpriv);
	else
		phydm_watchdogbu(&pHalData->odmpriv);
