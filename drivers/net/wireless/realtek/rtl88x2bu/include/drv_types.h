...
struct rx_logs {
	...
	u32 core_rx_post_portctrlbu_err;
...
{
	ATOMIC_SETbu(&dvobj->bSurpriseRemoved, _TRUE);
}
static inline void dev_clr_surprise_removed(struct dvobj_priv *dvobj)
{
	ATOMIC_SETbu(&dvobj->bSurpriseRemoved, _FALSE);
}
static inline void dev_set_drv_stopped(struct dvobj_priv *dvobj)
{
	ATOMIC_SETbu(&dvobj->bDriverStopped, _TRUE);
}
static inline void dev_clr_drv_stopped(struct dvobj_priv *dvobj)
{
	ATOMIC_SETbu(&dvobj->bDriverStopped, _FALSE);
}
#define dev_is_surprise_removed(dvobj)	(ATOMIC_READbu(&dvobj->bSurpriseRemoved) == _TRUE)
#define dev_is_drv_stopped(dvobj)		(ATOMIC_READbu(&dvobj->bDriverStopped) == _TRUE)
...
/* #define RTW_DISABLE_FUNC(padapter, func) (ATOMIC_ADDbu(&adapter_to_dvobj(padapter)->disable_func, (func))) */
/* #define RTW_ENABLE_FUNC(padapter, func) (ATOMIC_SUBbu(&adapter_to_dvobj(padapter)->disable_func, (func))) */
__inline static void RTW_DISABLE_FUNC(_adapter *padapter, int func_bit)
{
	int	df = ATOMIC_READbu(&adapter_to_dvobj(padapter)->disable_func);
	df |= func_bit;
	ATOMIC_SETbu(&adapter_to_dvobj(padapter)->disable_func, df);
}
...
#define RTW_IS_FUNC_DISABLED(padapter, func_bit) (ATOMIC_READbu(&adapter_to_dvobj(padapter)->disable_func) & (func_bit))
