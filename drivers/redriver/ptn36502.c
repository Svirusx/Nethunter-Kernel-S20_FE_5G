...
static int ptn36502_set_gpios(struct device *dev)
{
	...
	if (gpio_is_valid(redrv_data->redriver_en)) {
		ret = gpio_request(redrv_data->redriver_en, "ptn36502_redriver_en");
		pr_info("%s: ptn36502_redriver_en ret gpio_request (%d)\n", __func__, ret);
		ret = gpio_direction_output(redrv_data->redriver_en, 1);
		pr_info("%s: ptn36502_redriver_en ret gpio_direction_output (%d)\n", __func__, ret);
	} else
		pr_info("%s: ptn36502_redriver_en gpio is invalid\n", __func__);
