...
static int sx9360_check_hallic_state(char *file_path, char hall_ic_status[], int size)
{
	...
	if (IS_ERR(filep)) {
		iRet = PTR_ERR(filep);
		pr_err("[SX9360_SUB3]: %s - file open fail %d\n", __func__, iRet);
		set_fs(old_fs);
		return iRet;
	}
	...
	if (iRet <= 0) {
		pr_err("[SX9360_SUB3]: %s - file read fail %d\n", __func__, iRet);
...
static int sx9360_i2c_write(struct sx9360_p *data, u8 reg_addr, u8 buf)
{
	...
	if (ret < 0)
		pr_err("[SX9360_SUB3]: %s - i2c write error %d\n", __func__, ret);
...
static int sx9360_i2c_read(struct sx9360_p *data, u8 reg_addr, u8 *buf)
{
	...
	if (ret < 0)
		pr_err("[SX9360_SUB3]: %s - i2c read error %d\n", __func__, ret);
...
static void sx9360_initialize_register(struct sx9360_p *data)
{
	...
	for (idx = 0; idx < (unsigned int)(sizeof(setup_reg) >> 1); idx++) {
		...
		pr_info("[SX9360_SUB3]: %s - Write Reg: 0x%x Value: 0x%x\n",
			__func__, setup_reg[idx].reg, setup_reg[idx].val);

		sx9360_i2c_read(data, setup_reg[idx].reg, &val);
		pr_info("[SX9360_SUB3]: %s - Read Reg: 0x%x Value: 0x%x\n\n",
	...
	pr_info("[SX9360_SUB3]: %s - detect threshold: %u\n", __func__, data->detect_threshold);
...
static void sx9360_initialize_chip(struct sx9360_p *data)
{
	...
	if (cnt >= 10)
		pr_err("[SX9360_SUB3]: %s - s/w reset fail(%d)\n", __func__, cnt);
...
static void sx9360_send_event(struct sx9360_p *data, u8 state)
{
	if (data->skip_data == true) {
		pr_info("[SX9360_SUB3]: %s - skip grip event\n", __func__);
		return;
	}

	if (state == ACTIVE) {
		data->state = ACTIVE;
		pr_info("[SX9360_SUB3]: %s - touched\n", __func__);
	} else {
		data->state = IDLE;
		pr_info("[SX9360_SUB3]: %s - released\n", __func__);
	}
...
static void sx9360_display_data_reg(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_SUB3]: ############# %d reference #############\n", 0);
	for (reg = SX9360_REGUSEMSBPHR; reg <= SX9360_REGOFFSETLSBPHR; reg++)
	{
		sx9360_i2c_read(data, reg, &val);
		pr_info("[SX9360_SUB3]: %s - Register(0x%2x) data(0x%2x)\n",
			__func__, reg, val);
	}
	pr_info("[SX9360_SUB3]: ############# %d Main #############\n", 0);
...
static void sx9360_get_data(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_SUB3]: %s retry : %d, CONVSTAT : %u\n", __func__, retry, convstat);
	...
	pr_info("[SX9360_SUB3]: %s - capMain: %ld, useful: %ld, avg: %d, diff: %d, Offset: %u\n",
...
static int sx9360_set_mode(struct sx9360_p *data, unsigned char mode)
{
	...
	pr_info("[SX9360_SUB3]: %s - change the mode : %u\n", __func__, mode);
...
static void sx9360_check_status(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_SUB3]: %s - (status: 0x%x)\n", __func__, status);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%2x,%2x", &regist, &val) != 2) {
		pr_err("[SX9360_SUB3]: %s - The number of data are wrong\n",
			__func__);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - kstrtou8 failed.(%d)\n", __func__, ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		pr_err("[SX9360_SUB3]: %s - unknown value %d\n", __func__, onoff);
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9360_SUB3]: %s - new_value = %u\n", __func__, enable);
...
static void sx9360_process_interrupt(struct sx9360_p *data)
{
	...
	pr_info("[SX9360_SUB3]: %s - status %d\n", __func__, status);
...
static void sx9360_irq_work_func(struct work_struct *work)
{
	...
	if (sx9360_get_nirq_state(data) == 0)
		...
		pr_err("[SX9360_SUB3]: %s - nirq read high %d\n",
...
static int sx9360_setup_pin(struct sx9360_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - gpio %d request failed (%d)\n",
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - failed to set gpio %d as input (%d)\n",
			__func__, data->gpio_nirq, ret);
...
static int sx9360_read_setupreg(struct device_node *dnode, char *str, u32 *val)
{
	...
	if (!ret)
		*val = temp_val;
		...
		pr_err("[SX9360_SUB3]: %s - %s: property read err 0x%2x (%d)\n",
...
static int sx9360_parse_dt(struct sx9360_p *data, struct device *dev)
{
	...
	data->gpio_nirq = of_get_named_gpio_flags(dNode,
		"sx9360_sub3,nirq-gpio", 0, &flags);
	if (data->gpio_nirq < 0) {
		pr_err("[SX9360_SUB3]: %s - get gpio_nirq error\n", __func__);
		return -ENODEV;
	}
...
static int sx9360_check_chip_id(struct sx9360_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: whoami[0x%x] read failed %d\n", value, ret);
		return ret;
	}
	if(value != WHO_AM_I)
	{
		pr_err("[SX9360_SUB3]: invalid whoami(%x)\n", value);
		return -1;
	}
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("[SX9360_SUB3]: %s - Probe Start!\n", __func__);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[SX9360_SUB3]: %s - i2c_check_functionality error\n",
			__func__);
		goto exit;
	}
	...
	if (data == NULL) {
		pr_err("[SX9360_SUB3]: %s - kzalloc error\n", __func__);
		ret = -ENOMEM;
		goto exit_kzalloc;
	}
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - of_node error\n", __func__);
		ret = -ENODEV;
		goto exit_of_node;
	}
	...
	if (ret) {
		pr_err("[SX9360_SUB3]: %s - could not setup pin\n", __func__);
		goto exit_setup_pin;
	}
	...
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - chip id check failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}

	ret = sx9360_i2c_write(data, SX9360_SOFTRESET_REG, SX9360_SOFTRESET);
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - chip reset failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}
	...
	ret = request_threaded_irq(data->irq, NULL, sx9360_interrupt_thread,
			...
			"sx9360_irq", data);
	if (ret < 0) {
		pr_err("[SX9360_SUB3]: %s - failed to set request_threaded_irq %d"
	...
	if (ret) {
		pr_err("[SX9360_SUB3] %s - could not register sensor(%d).\n",
			__func__, ret);
	...
	pr_info("[SX9360_SUB3]: %s - Probe done!\n", __func__);
...
exit_kzalloc:
exit:
	pr_err("[SX9360_SUB3]: %s - Probe fail!\n", __func__);
	return ret;
}
