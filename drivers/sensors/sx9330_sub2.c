...
#ifdef TAG
#undef TAG
#define TAG "[GRIP_SUB2]"
#endif
#define VENDOR_NAME              "SEMTECH"
#define MODEL_NAME               "SX9330_SUB2"
...
static int sx9330_check_hallic_state(char *file_path, char hall_ic_status[], int size)
{
	int iRet = 0;
	mm_segment_t old_fs;
	struct file *filep;
	...
	if (IS_ERR(filep)) {
		iRet = PTR_ERR(filep);
		pr_err("[SX9330_SUB2]: %s - file open fail %d\n", __func__, iRet);
		set_fs(old_fs);
		return iRet;
	}
	...
	if (iRet <= 0) {
		pr_err("[SX9330_SUB2]: %s - file read fail %d\n", __func__, iRet);
...
static int sx9330_i2c_write_16bit(struct sx9330_p *data, u16 reg_addr, u32 buf)
{
	...
	if (ret < 0)
		pr_err("[SX9330_SUB2]: %s - i2c write error %d\n", __func__, ret);
...
static int sx9330_i2c_read_16bit(struct sx9330_p *data, u16 reg_addr, u32 *data32)
{
	...
	if (ret < 0)
		pr_err("[SX9330_SUB2]: %s - i2c read error %d\n", __func__, ret);
...
static void sx9330_initialize_chip(struct sx9330_p *data)
{
	...
	if (cnt >= 10)
		pr_err("[SX9330_SUB2]: %s - s/w reset fail(%d)\n", __func__, cnt);
...
static int sx9330_set_offset_calibration(struct sx9330_p *data)
{
	...
	while (retries < 50) {
		...
		if (!(status & MSK_TOPSTAT0_CMDBUSY)) {
			pr_info("[SX9330_SUB2]: %s - TOPSTAT0 status: 0x%x, retries: %d\n", __func__, status, retries);
			break;
		}
	...
	while (retries < 50) {
		...
		if (!(status & MSK_STAT1_CONVSTAT)) {
			pr_info("[SX9330_SUB2]: %s - STAT1 status: 0x%x, retries: %d\n", __func__, status, retries);
			break;
		}
...
static void sx9330_send_event(struct sx9330_p *data, u8 state)
{
	if (data->skip_data == true) {
		pr_info("[SX9330_SUB2]: %s - skip grip event\n", __func__);
		return;
	}

	if (state == ACTIVE) {
		data->state = ACTIVE;
		pr_info("[SX9330_SUB2]: %s - touched\n", __func__);
	} else {
		data->state = IDLE;
		pr_info("[SX9330_SUB2]: %s - released\n", __func__);
	}
...
static void sx9330_get_data(struct sx9330_p *data)
{
	...
	pr_info("[SX9330_SUB2]: %s - CapsMain: %ld, useful: %ld, avg: %ld, diff: %ld, Offset: %u\n",
...
static int sx9330_set_mode(struct sx9330_p *data, unsigned char mode)
{
	...
	while (retries < 50) {
		ret = sx9330_i2c_read_16bit(data, SX9330_TOPSTAT0_REG, &status);
		if (ret < 0) {
			pr_info("[SX9330_SUB2]: %s - change the mode : %u FAILED reading TOPSTAT0!!\n", __func__, mode);
			return ret;
		}
		// If this is 0, we can issue the next command
		if (!(status & MSK_TOPSTAT0_CMDBUSY))
	...
	while (retries < 50) {
		ret = sx9330_i2c_read_16bit(data, SX9330_STAT1_REG, &status);
		if (ret < 0) {
			pr_info("[SX9330_SUB2]: %s - change the mode : %u FAILED reading STAT1!!\n", __func__, mode);
			return ret;	    
		}
	...
	pr_info("[SX9330_SUB2]: %s - change the mode : %u\n", __func__, mode);
...
static void sx9330_check_status(struct sx9330_p *data, int enable)
{
	...
	status = status >> 24;
	pr_info("[SX9330_SUB2]: %s - (status: 0x%x)\n", __func__, status);
...
static void sx9330_set_enable(struct sx9330_p *data, int enable)
{
	...
	if (enable) {
		...
		pr_info("[SX9330_SUB2]: %s - enable(status : 0x%x)\n",
			__func__, status);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 10, &val)) {
		pr_err("[SX9330_SUB2]: %s - Invalid Argument\n", __func__);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%6x,%10x", &regist, &val) != 2) {
		pr_err("[SX9330_SUB2]: %s - The number of data are wrong\n",
			__func__);
		return -EINVAL;
	}

	sx9330_i2c_write_16bit(data, (u16)regist, (u32)val);
	pr_info("[SX9330_SUB2]: %s - Register(0x%2x) data(0x%4x)\n",
		__func__, regist, val);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: %s - kstrtou8 failed.(%d)\n", __func__, ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		pr_err("[SX9330_SUB2]: %s - unknown value %d\n", __func__, onoff);
	}

	mutex_unlock(&data->read_mutex);

	pr_info("[SX9330_SUB2]: %s - %d\n", __func__, onoff);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9330_SUB2]: %s - new_value = %u old_value = %d\n",
...
static void sx9330_touch_process(struct sx9330_p *data)
{
	...
	if (data->state == IDLE) {
		if (status & (CSX_STATUS_REG << MAIN_SENSOR))
			...
			pr_info("[SX9330_SUB2]: %s - %x already released.\n",
					__func__, status);
	} else { // User released
		if (!(status & (CSX_STATUS_REG << MAIN_SENSOR)))
			sx9330_send_event(data, IDLE);
		else
			pr_info("[SX9330_SUB2]: %s - %x still touched\n",
					__func__, status);
	}
}
...
static void sx9330_process_interrupt(struct sx9330_p *data)
{
	...
	if (status & IRQ_PROCESS_CONDITION) {
		...
		pr_info("[SX9330_SUB2]: %s interrupt generated but skip - status : %d\n",
			__func__, status);
	}
}
...
static void sx9330_init_work_func(struct work_struct *work)
{
	...
	while(retry++ < 10) {
		...
		if (data->capMain == 0 && data->avg == 0 && data->diff == 0
			...
			pr_info("[SX9330_SUB2]: Defence code for grip sensor - retry: %d\n", retry);
...
static void sx9330_irq_work_func(struct work_struct *work)
{
	...
	if (sx9330_get_nirq_state(data) == 0)
		...
		pr_err("[SX9330_SUB2]: %s - nirq read high %d\n",
...
#ifdef CONFIG_SENSORS_HALL
	sx9330_check_hallic_state(HALLIC_PATH, data->hall_ic, sizeof(data->hall_ic)-1);
	data->hall_ic[sizeof(data->hall_ic)-1] = '\0';
	if (strcmp(data->hall_ic, "CLOSE") == 0) {
		if (data->hall_ic_status == HALL_OPEN) {
			pr_info("[SX9330_SUB2]: %s - hall IC is closed\n", __func__);
			sx9330_set_offset_calibration(data);
...
#endif
#ifdef CONFIG_CERTIFY_HALL
	sx9330_check_hallic_state(HALLIC_CERT_PATH, data->hall_ic_cert, sizeof(data->hall_ic_cert)-1);
	data->hall_ic_cert[sizeof(data->hall_ic_cert)-1] = '\0';
	if (strcmp(data->hall_ic_cert, "CLOSE") == 0) {
		if (data->hall_ic_cert_status == HALL_OPEN) {
			pr_info("[SX9330_SUB2]: %s - cert hall IC is closed\n", __func__);
			sx9330_set_offset_calibration(data);
...
#endif
#ifdef CONFIG_FOLDER_HALL
	sx9330_check_hallic_state(HALLIC_FOLD_PATH, data->hall_ic_fold, sizeof(data->hall_ic_fold)-1);
	data->hall_ic_fold[sizeof(data->hall_ic_fold)-1] = '\0';
	if (strcmp(data->hall_ic_fold, "0") == 0) {
		if (data->hall_ic_fold_status == HALL_OPEN) {
			pr_info("[SX9330_SUB2]: %s - fold hall IC is closed\n", __func__);
			sx9330_set_offset_calibration(data);
...
static int sx9330_setup_pin(struct sx9330_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: %s - gpio %d request failed (%d)\n",
	...
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: %s - failed to set gpio %d as input (%d)\n",
			__func__, data->gpio_nirq, ret);
...
static int sx9330_read_setupreg(struct device_node *dnode, char *str, u32 *val)
{
	...
	if (!ret)
		*val = temp_val;
		...
		pr_err("[SX9330_SUB2]: %s - %s: property read err 0x%08x (%d)\n",
...
static int sx9330_parse_dt(struct sx9330_p *data, struct device *dev)
{
	...
	data->gpio_nirq = of_get_named_gpio_flags(node,
		"sx9330_sub2,nirq-gpio", 0, &flags);
	if (data->gpio_nirq < 0) {
		pr_err("[SX9330_SUB2]: %s - get gpio_nirq error\n", __func__);
		return -ENODEV;
	}
		...
		unsigned long action, void *data)
{
	...
	struct sx9330_p *pdata =
		container_of(nb, struct sx9330_p, ccic_nb);
	...
	if (pdata->init_done == ON) {
		...
		case USB_STATUS_NOTIFY_DETACH:
			pr_info("[SX9330_SUB2]: %s accept attach = %d\n",
				__func__, usb_status.drp);
			sx9330_set_offset_calibration(pdata);
			break;
		default:
			pr_info("[SX9330_SUB2]: %s skip attach = %d\n",
				__func__, usb_status.drp);
			break;
		}
	}
...
#endif
#ifdef CONFIG_VBUS_NOTIFIER
static int sx9330_vbus_handle_notification(struct notifier_block *nb,
		unsigned long action, void *data)
{
	vbus_status_t vbus_type = *(vbus_status_t *) data;
	struct sx9330_p *pdata =
		container_of(nb, struct sx9330_p, vbus_nb);
	static int pre_attach;
...
static int sx9330_check_chip_id(struct sx9330_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: whoami[0x%x] read failed %d\n", value, ret);
		return ret;
	}
	...
	case 0x34:
		return 0;
	default:
		pr_err("[SX9330_SUB2]: invalid whoami(%x)\n", value);
		return -1;
	}
}
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("[SX9330_SUB2]: %s - Probe Start!\n", __func__);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[SX9330_SUB2]: %s - i2c_check_functionality error\n",
			__func__);
		goto exit;
	}
	...
	if (data == NULL) {
		pr_err("[SX9330_SUB2]: %s - kzalloc error\n", __func__);
		ret = -ENOMEM;
		goto exit_kzalloc;
	}
	...
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: %s - of_node error\n", __func__);
		ret = -ENODEV;
		goto exit_of_node;
	}
	...
	if (ret) {
		pr_err("[SX9330_SUB2]: %s - could not setup pin\n", __func__);
		goto exit_setup_pin;
	}
	...
	ret = request_threaded_irq(data->irq, NULL, sx9330_interrupt_thread,
			...
			"sx9330_irq", data);
	if (ret < 0) {
		pr_err("[SX9330_SUB2]: %s - failed to set request_threaded_irq %d"
	...
	if (ret) {
		pr_err("[SX9330_SUB2] %s - cound not register sensor(%d).\n",
			__func__, ret);
...
#if defined(CONFIG_CCIC_NOTIFIER) && defined(CONFIG_USB_TYPEC_MANAGER_NOTIFIER)
	pr_info("[SX9330_SUB2]: %s - register ccic notifier\n", __func__);
	manager_notifier_register(&data->ccic_nb,
					sx9330_ccic_handle_notification,
					MANAGER_NOTIFY_CCIC_USB);
#endif
#ifdef CONFIG_VBUS_NOTIFIER
	pr_info("[SX9330_SUB2]: %s - register vbus notifier\n", __func__);
	vbus_notifier_register(&data->vbus_nb, sx9330_vbus_handle_notification,
					VBUS_NOTIFY_DEV_CHARGER);
#endif
	pr_info("[SX9330_SUB2]: %s - Probe done!\n", __func__);
...
exit_kzalloc:
exit:
	pr_err("[SX9330_SUB2]: %s - Probe fail!\n", __func__);
	return ret;
}
