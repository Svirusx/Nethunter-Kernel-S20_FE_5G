...
#define SUB_PREDEFINE_FILE_PATH      "/efs/FactoryApp/predefine_sub"
#define HH_SUB_VERSION_FILE_PATH      "/efs/FactoryApp/hh_sub_version"

struct {
	int version;
	int octa_id;
	int data[EFS_SAVE_NUMS];
} hidden_table_sub[ID_INDEX_NUMS] = {
	{190212, ID_UTYPE,
	{1997, -683, 1958, -1604, 520, 3628, 789, 1100, 800, 550, 9055} },
	{190212, ID_BLACK,
	{1997, -683, 1958, -1604, 520, 3628, 789, 1100, 800, 550, 9055} },
};
...
static int need_update_coef_efs(void)
{
	...
	type_filp = filp_open(HH_SUB_VERSION_FILE_PATH, O_RDONLY, 0440);
	...
	if (current_coef_version < hidden_table_sub[ID_INDEX_NUMS - 1].version) {
		pr_err("[FACTORY] %s : small:%d:%d", __func__,
			current_coef_version,
			hidden_table_sub[ID_INDEX_NUMS - 1].version);
		return true;
	}
...
static int check_crc_table(int index)
{
	int i, sum = 0;

	for (i = 0; i < SUM_CRC; i++)
		sum += hidden_table_sub[index].data[i];

	return (sum == hidden_table_sub[index].data[SUM_CRC]) ? true : false;
}

static int make_coef_efs(int index)
{
	struct file *type_filp = NULL;
	mm_segment_t old_fs;
	int ret = 0;
	...
	snprintf(predefine_value_path, PATH_LEN,
		"%s%d", SUB_PREDEFINE_FILE_PATH, hidden_table_sub[index].octa_id);

	pr_info("[FACTORY] %s: path:%s\n", __func__,
		predefine_value_path);

	sprintf(write_buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		hidden_table_sub[index].data[0], hidden_table_sub[index].data[1],
		hidden_table_sub[index].data[2], hidden_table_sub[index].data[3],
		hidden_table_sub[index].data[4], hidden_table_sub[index].data[5],
		hidden_table_sub[index].data[6], hidden_table_sub[index].data[7],
		hidden_table_sub[index].data[8], hidden_table_sub[index].data[9],
		hidden_table_sub[index].data[10]);
...
static int update_coef_version(void)
{
	struct file *type_filp = NULL;
	mm_segment_t old_fs;
	...
	sprintf(version, "%d.",	hidden_table_sub[ID_INDEX_NUMS - 1].version);

	for (i = 0 ; i < ID_INDEX_NUMS ; i++) {
		if (check_crc_table(i)) {
			tmd490x_itoa(tmp, hidden_table_sub[i].octa_id);
	...
	type_filp = filp_open(HH_SUB_VERSION_FILE_PATH,
			O_TRUNC | O_RDWR | O_CREAT, 0660);
	if (IS_ERR(type_filp)) {
		set_fs(old_fs);
		ret = PTR_ERR(type_filp);
...
void hidden_hole_sub_init_work(void)
{
	...
	if (win_type >= 0)
		snprintf(predefine_value_path, PATH_LEN,
			"%s%d", SUB_PREDEFINE_FILE_PATH, win_type);
	else {
	...
	adsp_unicast(msg_buf, sizeof(msg_buf),
		MSG_LIGHT_SUB, 0, MSG_TYPE_SET_HIDDEN_HOLE_DATA);
#endif
exit:
	kfree(read_buf);
	kfree(predefine_value_path);
...
static int tmd490x_hh_sub_check_crc(void)
{
	struct file *hole_filp = NULL;
	int msg_buf[EFS_SAVE_NUMS];
	mm_segment_t old_fs;
	...
	hole_filp = filp_open(HH_SUB_VERSION_FILE_PATH, O_RDONLY, 0440);
	if (IS_ERR(hole_filp)) {
...
static ssize_t tmd490x_hh_sub_ver_store(struct device *dev,
...
static ssize_t tmd490x_hh_sub_ver_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	ver_filp = filp_open(HH_SUB_VERSION_FILE_PATH, O_RDONLY, 0440);
	if (IS_ERR(ver_filp)) {
	...
	sprintf(table_version, "%d.", hidden_table_sub[ID_INDEX_NUMS - 1].version);
...
static ssize_t tmd490x_hh_sub_write_all_data_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct file *type_filp = NULL;
	int msg_buf[EFS_SAVE_NUMS];
	...
	snprintf(predefine_value_path, PATH_LEN,
		"%s%d", SUB_PREDEFINE_FILE_PATH, octa_id);
...
static ssize_t tmd490x_hh_sub_write_all_data_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = 0;
	int i;
...
static ssize_t tmd490x_hh_sub_is_exist_efs_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct file *hole_filp = NULL;
	mm_segment_t old_fs;
...
static ssize_t tmd490x_hh_sub_ext_prox_th_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int threshold = 0;
	int win_type;
	...
	hidden_table_sub[win_type].data[IRIS_PROX_THD] = threshold;

	return size;
}

static ssize_t tmd490x_hh_sub_ext_prox_th_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int win_type;
	...
	pr_info("[FACTORY] %s: win_type:%d, thd:%d\n", __func__, win_type,
		hidden_table_sub[win_type].data[IRIS_PROX_THD]);

	return snprintf(buf, PAGE_SIZE, "%d\n",
		hidden_table_sub[win_type].data[IRIS_PROX_THD]);
}

static DEVICE_ATTR(hh_sub_ver, 0664, tmd490x_hh_sub_ver_show, tmd490x_hh_sub_ver_store);
static DEVICE_ATTR(hh_sub_write_all_data, 0664,
	tmd490x_hh_sub_write_all_data_show, tmd490x_hh_sub_write_all_data_store);
static DEVICE_ATTR(hh_sub_is_exist_efs, 0444, tmd490x_hh_sub_is_exist_efs_show, NULL);
static DEVICE_ATTR(hh_sub_ext_prox_th, 0664,
	tmd490x_hh_sub_ext_prox_th_show, tmd490x_hh_sub_ext_prox_th_store);

static struct device_attribute *hh_hole_sub_attrs[] = {
	&dev_attr_hh_sub_ver,
	&dev_attr_hh_sub_write_all_data,
	&dev_attr_hh_sub_is_exist_efs,
	&dev_attr_hh_sub_ext_prox_th,
	NULL,
};

static int __init hh_hole_sub_factory_init(void)
{
	adsp_factory_register(MSG_HH_HOLE_SUB, hh_hole_sub_attrs);
...
static void __exit hh_hole_sub_factory_exit(void)
{
	adsp_factory_unregister(MSG_HH_HOLE_SUB);

	pr_info("[FACTORY] %s\n", __func__);
}
module_init(hh_hole_sub_factory_init);
module_exit(hh_hole_sub_factory_exit);
