...
{
	gpio_direction_output(info->dtdata->gpio_en, enable);
	msleep(150);
	return 1;
}
	...
	for (i = 0; i < sz; i += info->event_size) {
		...
		if (type == MIP4_EVENT_INPUT_TYPE_SCREEN) {
			...
			if ((state == MMS_TS_COORDINATE_ACTION_RELEASE) ||
				...
				input_report_abs(info->input_dev, ABS_MT_CUSTOM, info->coord[id].palm);
				if (info->finger_state[id] == 0) {
...
int mms_parse_dt(struct device *dev, struct mms_ts_info *info)
{
	...
	info->dtdata->gpio_en= of_get_named_gpio(np, "melfas,gpio-en", 0);
	gpio_request(info->dtdata->gpio_en, "gpio-en");
...
void mms_config_input(struct mms_ts_info *info)
{
	...
	snprintf(info->phys, sizeof(info->phys), "%s/input0", dev_name(&info->client->dev));
	input_dev->name = "sec_touchscreen";
	input_dev->phys = info->phys;
	input_dev->id.bustype = BUS_I2C;
	input_dev->dev.parent = &info->client->dev;
	...
	input_set_events_per_packet(info->input_dev, 200);
		...
		unsigned long cmd, void *data)
{
	...
	if (info->power_status == STATE_POWER_OFF) {
		input_err(true, &info->client->dev, "%s tsp power off\n", __func__);
		return 0;
	}
...
int mms_lowpower_mode(struct mms_ts_info *info, u8 on)
{
	...
	if (on == TO_LOWPOWER_MODE)
		info->power_status = STATE_LPM;
	else
		info->power_status = STATE_POWER_ON;

	input_info(true, &info->client->dev, "%s: %s mode_flag:%x prox_power:%ld ed:%d pocket:%d\n",
