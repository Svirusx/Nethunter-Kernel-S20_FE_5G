...
static bool is_unknown_pad(struct mfc_charger_data *charger)
{
	...
	if (charger->tx_id_cnt > 3 && charger->tx_id == TX_ID_UNKNOWN)
		return true;
	return false;
}
#endif
...
static void mfc_uno_on(struct mfc_charger_data *charger, bool on)
{
	...
	if (charger->wc_tx_enable && on) { /* UNO ON */
		...
		if (delayed_work_pending(&charger->wpc_tx_op_freq_work)) {
			__pm_relax(&charger->wpc_tx_opfq_lock);
...
static void mfc_set_tx_power(struct mfc_charger_data *charger, bool on)
{
	...
	if (on) {
		...
		cancel_delayed_work(&charger->wpc_tx_op_freq_work);
		__pm_stay_awake(&charger->wpc_tx_opfq_lock);
		queue_delayed_work(charger->wqueue,
		...
		__pm_relax(&charger->wpc_rx_connection_lock);
...
static void mfc_tx_phm_work(struct work_struct *work)
{
	...
	/* noirq resume of devices complete after 1000 msecs */
	while (charger->is_suspend) {
		msleep(100);
		cnt++;
		if (cnt >= 10)
			break;
	...
	if (!(data & MFC_STAT_H_TX_CON_DISCON_MASK)) {
		if (!charger->wc_rx_connected) {
			...
			charger->wc_rx_connected = false;
			__pm_stay_awake(&charger->wpc_rx_connection_lock);
			queue_delayed_work(charger->wqueue,
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (val->intval == POWER_SUPPLY_STATUS_FULL) {
			...
			__pm_stay_awake(&charger->wpc_cs100_lock);
			if (charger->tx_id)
				queue_delayed_work(charger->wqueue,
...
static void mfc_wpc_det_work(struct work_struct *work)
{
	...
	__pm_relax(&charger->wpc_vrect_check_lock);
...
static void mfc_wpc_isr_work(struct work_struct *work)
{
	...
	if (cmd_data == WPC_TX_COM_AFC_SET) {
		...
		if (charger->req_tx_id || !charger->req_afc_tx) {
			...
			charger->is_abnormal_pad = true;
			if (charger->req_tx_id && charger->tx_id_cnt <= 1)
				mfc_send_command(charger, MFC_AFC_CONF_5V_TX);
			__pm_relax(&charger->wpc_rx_wake_lock);
			return;
		}
...
static void mfc_wpc_tx_id_work(struct work_struct *work)
{
	...
	if (!charger->tx_id && (!charger->is_abnormal_pad || charger->tx_id_cnt<=1)) {
...
static irqreturn_t mfc_wpc_irq_thread(int irq, void *irq_data)
{
	...
	if (wc_w_state_irq == 1 &&
		...
		charger->initial_vrect = false;
		__pm_stay_awake(&charger->wpc_vrect_check_lock);
		...
		mfc_charger_platform_data_t *pdata)
{
	...
	if (!np) {
		...
		pdata->keep_tx_vout = of_property_read_bool(np, "battery,keep_tx_vout");
						...
						const struct i2c_device_id *id)
{
	...
	wakeup_source_init(&charger->wpc_wake_lock, "wpc_wakelock");
	wakeup_source_init(&charger->wpc_rx_wake_lock, "wpc_rx_wakelock");
	wakeup_source_init(&charger->wpc_tx_wake_lock, "wpc_tx_wakelock");
	wakeup_source_init(&charger->wpc_update_lock, "wpc_update_lock");
	wakeup_source_init(&charger->wpc_opfq_lock, "wpc_opfq_lock");
	wakeup_source_init(&charger->wpc_tx_opfq_lock, "wpc_tx_opfq_lock");
	wakeup_source_init(&charger->wpc_afc_vout_lock, "wpc_afc_vout_lock");
	wakeup_source_init(&charger->wpc_vout_mode_lock, "wpc_vout_mode_lock");
	wakeup_source_init(&charger->wpc_rx_connection_lock, "wpc_rx_connection_lock");
	wakeup_source_init(&charger->wpc_rx_det_lock, "wpc_rx_det_lock");
	wakeup_source_init(&charger->wpc_tx_phm_lock, "wpc_tx_phm_lock");
	wakeup_source_init(&charger->wpc_vrect_check_lock, "wpc_vrect_check_lock");
	wakeup_source_init(&charger->wpc_tx_id_lock, "wpc_tx_id_lock");
	wakeup_source_init(&charger->wpc_cs100_lock, "wpc_cs100_lock");
...
err_irq_wpc_det:
	wakeup_source_trash(&charger->wpc_wake_lock);
	wakeup_source_trash(&charger->wpc_rx_wake_lock);
	wakeup_source_trash(&charger->wpc_tx_wake_lock);
	wakeup_source_trash(&charger->wpc_update_lock);
	wakeup_source_trash(&charger->wpc_opfq_lock);
	wakeup_source_trash(&charger->wpc_tx_opfq_lock);	
	wakeup_source_trash(&charger->wpc_afc_vout_lock);
	wakeup_source_trash(&charger->wpc_vout_mode_lock);
	wakeup_source_trash(&charger->wpc_rx_connection_lock);
	wakeup_source_trash(&charger->wpc_rx_det_lock);
	wakeup_source_trash(&charger->wpc_tx_phm_lock);
	wakeup_source_trash(&charger->wpc_vrect_check_lock);
	wakeup_source_trash(&charger->wpc_tx_id_lock);
err_pdata_free:
...
static void mfc_charger_shutdown(struct i2c_client *client)
{
	...
	if (gpio_get_value(charger->pdata->wpc_det)) {
		...
		if (charger->is_afc_tx && !charger->pdata->keep_tx_vout)
			mfc_send_command(charger, MFC_AFC_CONF_5V_TX);
	}
}
