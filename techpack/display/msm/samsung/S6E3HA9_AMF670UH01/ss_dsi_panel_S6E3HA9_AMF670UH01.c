...
static struct dsi_panel_cmd_set *ss_acl_on(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*level_key = LEVEL1_KEY;
	...
	if (vdd->br_info.common_br.bl_level > 25500) { /* HBM */
		pcmds->cmds[0].msg.tx_buf[3] = 0x80;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].msg.tx_buf[4] = 0x65;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].msg.tx_buf[6] = 0x26;	/* 8% */
	} else { /* Normal */
		pcmds->cmds[0].msg.tx_buf[3] = 0x40;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].msg.tx_buf[4] = 0xFF;	/* use hbm acl statr step 60% */
		pcmds->cmds[0].msg.tx_buf[6] = 0x50;	/* 15% */ 
	}
...
static struct dsi_panel_cmd_set *ss_elvss(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	elvss_cmds->cmds[0].msg.tx_buf[57] = vdd->br_info.common_br.elvss_value[0];
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.get_gamma_V_size = get_gamma_V_size_S6E3HA9_AMF670UH01;
