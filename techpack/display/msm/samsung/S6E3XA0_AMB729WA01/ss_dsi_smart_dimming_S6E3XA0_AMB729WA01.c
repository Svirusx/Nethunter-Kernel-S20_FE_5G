...
#include "ss_dsi_smart_dimming_S6E3XA0_AMB729WA01.h"
...
static void TP_gamma_voltage_calc(struct SMART_DIM *pSmart)
{
	...
	for (j = 0; j < RGB_MAX; j++) {
		...
		idx = MTP_OFFSET + center_gamma[VT][j];
	...
	for (j = 0; j < RGB_MAX; j++) {
		...
		idx = MTP_OFFSET + center_gamma[V0][j];
...
// 1. V255 : ((VREG1 - V255) * denominator_TP / (VREG1 - VREF)) - numerator_TP
static unsigned long long v255_TP_gamma_code_calc(int VREG1,  int VREF, int GRAY, int fraction[])
{
	unsigned long long val1, val2, val3;

	val1 = VREG1 - GRAY;
	val2 = val1 * fraction[1];
	do_div(val2, (VREG1 - VREF));
...
static void TP_gamma_code_calc(struct SMART_DIM *pSmart, int *M_GRAY, int *str)
{
	...
	for (j = 0; j < RGB_MAX; j++) {
		...
		v255_tmp[j] = (val >> 8) & 0x03;
		str[cnt++] = val & 0xFF;
	}
	str[0] = (v255_tmp[R] << 4) | (v255_tmp[G] << 2) | v255_tmp[B];
...
static void mtp_offset_substraction(struct SMART_DIM *pSmart, int *str)
{
	...
	/* V255 : str[0] ~ str[3] */
	v255_tmp[R] = (str[0] & 0x30) >> 4;
	v255_tmp[G] = (str[0] & 0x0C) >> 2;
	v255_tmp[B] = str[0] & 0x03;

	level_V255[R] = str[1] | (v255_tmp[R] << 8);
	level_V255[G] = str[2] | (v255_tmp[G] << 8);
	level_V255[B] = str[3] | (v255_tmp[B] << 8);

	level_V255[R] -= pSmart->MTP[V255][R];
	level_V255[G] -= pSmart->MTP[V255][G];
	level_V255[B] -= pSmart->MTP[V255][B];

	v255_tmp[R] = (level_V255[R]  >> 8) & 0x03;
	v255_tmp[G] = (level_V255[G]  >> 8) & 0x03;
	v255_tmp[B] = (level_V255[B]  >> 8) & 0x03;
...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	case 'A':
		memcpy(base_luminance, base_luminance_winner_revA, sizeof(base_luminance_winner_revA));
		break;
	default:
		memcpy(base_luminance, base_luminance_winner_revA, sizeof(base_luminance_winner_revA));
		break;
...
static int get_gradation_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
	case 'A':
		ret = gradation_offset_winner_revA[table_index][index];
		break;
	default:
		ret = gradation_offset_winner_revA[table_index][index];
		break;
...
static int get_rgb_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
	case 'A':
		ret = rgb_offset_winner_revA[table_index][index];
		break;
	default:
		ret = rgb_offset_winner_revA[table_index][index];
		break;
...
static void Color_shift_compensation(struct SMART_DIM *pSmart, int *gamma_setting, int table_index)
{
	...
	v255_tmp[R] = (gamma_setting[0] & 0x30) >> 4;
	v255_tmp[G] = (gamma_setting[0] & 0x0C) >> 2;
	v255_tmp[B] = gamma_setting[0] & 0x03;

	level_V255[R] = gamma_setting[1] | (v255_tmp[R] << 8);
	level_V255[G] = gamma_setting[2] | (v255_tmp[G] << 8);
	level_V255[B] = gamma_setting[3] | (v255_tmp[B] << 8);
	...
	gamma_setting[0] = v255_tmp[R] << 4 | v255_tmp[G] << 2 | v255_tmp[B];
...
static void set_max_lux_table(int ldi_revision)
{
	...
	v255_tmp[R] = ((center_gamma[V255][R] >> 8)  & 0x03) << 4;
	v255_tmp[G] = ((center_gamma[V255][G] >> 8)  & 0x03) << 2;
	v255_tmp[B] = (center_gamma[V255][B] >> 8)  & 0x03;

	cnt = 0;
	max_lux_table[cnt++] = v255_tmp[R] | v255_tmp[G] | v255_tmp[B];
...
static void mtp_sorting(struct SMART_DIM *psmart)
{
	...
	/* V255 */
	cnt = 0;
	v255_tmp[R] = (pfrom[cnt] & 0x30) >> 4;
	v255_tmp[G] = (pfrom[cnt] & 0x0C) >> 2;
	v255_tmp[B] = pfrom[cnt] & 0x03;
	cnt++;
...
static void hbm_interpolation_init(struct SMART_DIM *pSmart)
{
	...
	v255_tmp[R] = (hbm_payload[0] & 0x30) >> 4;
	v255_tmp[G] = (hbm_payload[0] & 0x0C) >> 2;
	v255_tmp[B] = hbm_payload[0] & 0x03;

	hbm_gamma[R] = (hbm_payload[R + 1] & 0xFF) | (v255_tmp[R] << 8);
	hbm_gamma[G] = (hbm_payload[G + 1] & 0xFF) | (v255_tmp[G] << 8);
	hbm_gamma[B] = (hbm_payload[B + 1] & 0xFF) | (v255_tmp[B] << 8);
	...
	v255_tmp[R] = (max_lux_table[0] & 0x30) >> 4;
	v255_tmp[G] = (max_lux_table[0] & 0x0C) >> 2;
	v255_tmp[B] = max_lux_table[0] & 0x03;

	max_gamma[R] = (max_lux_table[R + 1] & 0xFF) | (v255_tmp[R] << 8);
	max_gamma[G] = (max_lux_table[G + 1] & 0xFF) | (v255_tmp[G] << 8);
	max_gamma[B] = (max_lux_table[B + 1] & 0xFF) | (v255_tmp[B] << 8);
	...
	for (loop = 0; loop < hbm_steps; loop++) {
		...
		/* V255 R,G,B */
		v255_tmp[R] = (hbm_interpolation_gamma[loop][R] >> 8 ) & 0x03;
		v255_tmp[G] = (hbm_interpolation_gamma[loop][G] >> 8 ) & 0x03;
		v255_tmp[B] = (hbm_interpolation_gamma[loop][B] >> 8 ) & 0x03;

		pSmart->hbm_interpolation_table[loop].gamma_setting[0] =
			(v255_tmp[R] << 4) | (v255_tmp[G] << 2) | v255_tmp[B];
...
struct smartdim_conf *smart_get_conf_S6E3XA0_AMB729WA01(void)
{
...
struct smartdim_conf *smart_get_conf_S6E3XA0_AMB729WA01_hmt(void)
{
