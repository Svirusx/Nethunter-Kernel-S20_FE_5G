 ...
 */
void make_self_display_img_cmds_XA1(struct samsung_display_driver_data *vdd,
...
void make_mass_self_display_img_cmds_XA1(struct samsung_display_driver_data *vdd,
		enum dsi_cmd_set_type cmd, u32 op)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
			 ...
			 size_t count, loff_t *ppos)
{
	...
	LCD_INFO("%s Header Buffer = %c%c\n", vdd->self_disp.dev.name, op_buf[0], op_buf[1]);
	...
	case FLAG_SELF_MOVE:
		...
		break;
	case FLAG_SELF_MASK:
		make_mass_self_display_img_cmds_XA1(vdd, TX_SELF_MASK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_ICON:
		make_mass_self_display_img_cmds_XA1(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_GRID:
		...
		break;
	case FLAG_SELF_ACLK:
		make_mass_self_display_img_cmds_XA1(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_mass_self_display_img_cmds_XA1(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
