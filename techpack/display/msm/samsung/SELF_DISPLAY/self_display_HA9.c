 ...
 */
void make_self_dispaly_img_cmds_HA9(struct samsung_display_driver_data *vdd,
			 ...
			 size_t count, loff_t *ppos)
{
	...
	case FLAG_SELF_MOVE:
		...
		break;
	case FLAG_SELF_MASK:
		make_self_dispaly_img_cmds_HA9(vdd, TX_SELF_MASK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_ICON:
		make_self_dispaly_img_cmds_HA9(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_GRID:
		...
		break;
	case FLAG_SELF_ACLK:
		make_self_dispaly_img_cmds_HA9(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_self_dispaly_img_cmds_HA9(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
...
int self_display_init_HA9(struct samsung_display_driver_data *vdd)
{
	int ret = 0;
