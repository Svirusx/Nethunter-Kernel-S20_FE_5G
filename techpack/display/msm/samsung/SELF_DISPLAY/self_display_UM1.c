		...
		enum dsi_cmd_set_type cmd, u32 op)
{
	...
	/* msg.tx_buf size */
	/*UUM1 flash done problem so limit modified to 65 */
	...
	for (i = 0; i < pcmds->count; i++) {
		tcmds[i].msg.type = MIPI_DSI_DCS_LONG_WRITE;
...
static int self_display_debug(struct samsung_display_driver_data *vdd)
{
	...
	if (ss_get_cmds(vdd, RX_SELF_DISP_DEBUG)->count) {
		...
		vdd->self_disp.debug.SM_SUM_O = ((buf[4] & 0xFF) << 24);
		vdd->self_disp.debug.SM_SUM_O |= ((buf[3] & 0xFF) << 16);
		vdd->self_disp.debug.SM_SUM_O |= ((buf[2] & 0xFF) << 8);
		vdd->self_disp.debug.SM_SUM_O |= (buf[1] & 0xFF);
			 ...
			 size_t count, loff_t *ppos)
{
	...
	switch (op) {
	case FLAG_SELF_MASK:
		make_self_dispaly_img_cmds_UM1(vdd, TX_SELF_MASK_IMAGE, op);
