...
static int self_time_set(struct samsung_display_driver_data *vdd, int from_self_move)
{
	...
	/* BIT7: SELF_IP_HOP_SS_EN, BIT6: SELF_IP_HOP_MSS_EN */
	cmd_pload[7] |=  (BIT(7) | BIT(6));
...
static void self_icon_img_write(struct samsung_display_driver_data *vdd)
{
	...
	/* delay 1 frame, add dummy 1ms */
	usleep_range(ONE_FRAME_US + 1000, ONE_FRAME_US + 1000);
...
static void self_mask_img_write(struct samsung_display_driver_data *vdd)
{
	...
	LCD_INFO("tx self mask ++ (cur_rr: %d)\n", vdd->vrr.cur_refresh_rate);
	ss_send_cmd(vdd, TX_LEVEL1_KEY_ENABLE);
			 ...
			 size_t count, loff_t *ppos)
{
	...
	case FLAG_SELF_MOVE:
		...
		break;
	case FLAG_SELF_MASK:
		make_mass_self_display_img_cmds_HAC(vdd, TX_SELF_MASK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_ICON:
		make_mass_self_display_img_cmds_HAC(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_GRID:
		...
		break;
	case FLAG_SELF_ACLK:
		make_mass_self_display_img_cmds_HAC(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_mass_self_display_img_cmds_HAC(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
