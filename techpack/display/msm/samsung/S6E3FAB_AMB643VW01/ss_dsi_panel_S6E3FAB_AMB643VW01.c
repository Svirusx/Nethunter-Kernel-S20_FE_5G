...
*/
#include "ss_dsi_panel_S6E3FAB_AMB643VW01.h"
#include "ss_dsi_mdnie_S6E3FAB_AMB643VW01.h"
#include "ss_dsi_interpolation_S6E3FAB_AMB643VW01.h"
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	LCD_INFO("HBM : cd_idx [%d], force normal transtion (temp)\n", vdd->br_info.common_br.cd_idx);
	//pcmds->cmds[0].msg.tx_buf[1] = vdd->finger_mask_updated? 0xE0 : 0xE8;	/* HBM Smooth transition : 0xE8 */
	pcmds->cmds[0].msg.tx_buf[1] = 0xE0;	/* HBM: 0xE0=normal, 0xE8=smooth transition */
	...
	LCD_INFO("cd_idx: %d, cd_level: %d, WRDISBV: %x %x, elvss: %x\n",
			vdd->br_info.common_br.cd_idx,
...
#if 1
	LCD_INFO("tmp: skip set lpm brightness\n");
	return;
#endif
...
#if 1
	LCD_INFO("tmp: skip update lpm cmd\n");
	return;
#endif
...
static int ss_vrr_init(struct vrr_info *vrr)
{
	...
	/* Bootloader: FHD@60hz Normal mode */
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3FAB_AMB643VW01_FHD";
