...
*/
#include "ss_dsi_panel_EA8076A_AMS646UJ10.h"
#include "ss_dsi_mdnie_EA8076A_AMS646UJ10.h"
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	/* ELVSS Read Value */
	pcmds->cmds[1].msg.tx_buf[8] = vdd->br_info.common_br.elvss_value[0];
	pcmds->cmds[1].msg.tx_buf[9] = vdd->br_info.common_br.elvss_value[1];
	/* ELVSS TSET */
	...
	pcmds->cmds[1].msg.tx_buf[6] = 0x96;
...
static int ss_dyn_mipi_post(struct samsung_display_driver_data *vdd)
{
	...
	if (pos_ffc == -EINVAL) {
		LCD_ERR("fail to find FFC(E9h) offset in set\n");
		return -EINVAL;
	}
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_EA8076A_AMS646UJ10_FHD";
