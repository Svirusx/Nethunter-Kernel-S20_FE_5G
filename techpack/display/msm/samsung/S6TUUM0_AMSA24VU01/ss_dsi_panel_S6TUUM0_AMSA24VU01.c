...
*/
#include "ss_dsi_panel_S6TUUM0_AMSA24VU01.h"
#include "ss_dsi_mdnie_S6TUUM0_AMSA24VU01.h"
...
static struct dsi_panel_cmd_set * ss_brightness_gamma_mode2_normal(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if(vdd->br_info.common_br.cd_idx <= MAX_BL_PF_LEVEL){
		...
		pcmds->cmds[3].msg.tx_buf[9] = get_bit(vdd->br_info.common_br.cd_level, 0, 8); /* DBV [7:0] */
		pcmds->cmds[3].msg.tx_buf[10] = get_bit(vdd->br_info.common_br.cd_level, 8, 3); /* DBV [15:8] */
	}
	else{
		LCD_INFO("HBM : cd_idx [%d] \n", vdd->br_info.common_br.cd_idx);
		pcmds->cmds[1].msg.tx_buf[9] = 0xE0;	/* HBM Smooth transition : 0xE8 */
		pcmds->cmds[3].msg.tx_buf[9] = get_bit(vdd->br_info.common_br.cd_level, 0, 8);
		pcmds->cmds[3].msg.tx_buf[10] = get_bit(vdd->br_info.common_br.cd_level, 8, 3);
	}
...
static struct dsi_panel_cmd_set * mdss_brightness_hmt(struct samsung_display_driver_data *vdd, int *level_key)
{
    ...
    pcmds->cmds[2].msg.tx_buf[1] = get_bit(vdd->br_info.hmt_stat.candela_level_hmt, 8, 2);
    pcmds->cmds[2].msg.tx_buf[2] = get_bit(vdd->br_info.hmt_stat.candela_level_hmt, 0, 8);
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->mipi_header_modi = true; //for gts7p
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6TUUM0_AMSA24VU01_WQXGA";
