...
*/
#include "ss_dsi_panel_S6E3FA7_AMB575WD01.h"
#include "ss_dsi_mdnie_S6E3FA7_AMB575WD01.h"
#include "ss_dsi_interpolation_S6E3FA7_AMB575WD01.h"
...
static struct dsi_panel_cmd_set *ss_hbm_etc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	/*	0xB5 24th : ELVSS_CAL_OFFSET
	*/
	elvss_24th_val = vdd->br.elvss_value2;
	hbm_etc_cmds->cmds[1].msg.tx_buf[23] = elvss_24th_val;

	/*	0xB4 : ACL CONTROL */
	if (vdd->gradual_acl_val) {
		hbm_etc_cmds->cmds[2].msg.tx_buf[1] = 0x50;	/* 32Frame */
		hbm_etc_cmds->cmds[2].msg.tx_buf[13] = 0x50;

		hbm_etc_cmds->cmds[2].msg.tx_buf[5] = 0x29; /* 8% */

		hbm_etc_cmds->cmds[3].msg.tx_buf[1] = 0x2;	/* ACL ON */
	} else {
		hbm_etc_cmds->cmds[2].msg.tx_buf[1] = 0x40;	/* 16Frame */
		hbm_etc_cmds->cmds[2].msg.tx_buf[13] = 0x40;

		hbm_etc_cmds->cmds[2].msg.tx_buf[5] = 0x00; /* 0% */

		hbm_etc_cmds->cmds[3].msg.tx_buf[1] = 0x0;	/* ACL OFF */
	}
...
#if 0
	LCD_DEBUG("0xB5 3th: 0x%x 0xB5 elvss_23th_val(elvss val) 0x%x, acl=%x, per=%x, vint=%x\n",
			hbm_elvss,
			vdd->br.elvss_value2,
...
static int ss_irc_read(struct samsung_display_driver_data *vdd)
{
	...
	/* Read mtp (B5h 1th,2th) for elvss*/
	...
	if (!SS_IS_CMDS_NULL(set))
		memcpy(&set->cmds->msg.tx_buf[1], vdd->br.irc_otp, NUM_IRC_OTP);

	set = ss_get_cmds(vdd, TX_HBM_IRC);
	if (!SS_IS_CMDS_NULL(set))
		memcpy(&set->cmds->msg.tx_buf[1], vdd->br.irc_otp, NUM_IRC_OTP);

	set = ss_get_cmds(vdd, TX_HMT_GAMMA);
	if (!SS_IS_CMDS_NULL(set))
		memcpy(&set->cmds[3].msg.tx_buf[1], vdd->br.irc_otp, NUM_IRC_OTP);
...
static struct dsi_panel_cmd_set *ss_elvss_hmt(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	if (IS_ERR_OR_NULL(vdd) || SS_IS_CMDS_NULL(elvss_cmds)) {
...
static int ss_self_display_data_init(struct samsung_display_driver_data *vdd)
{
	...
	if (panel_type == PANEL_TYPE_PHOLE) {
		if (panel_color == PANEL_COLOR_UTYPE) {
...
static void poc_comp(struct samsung_display_driver_data *vdd)
{
	...
	LCD_DEBUG("cd_idx (%d) val (%02x)\n", cd_idx, poc_comp_table[cd_idx]);

	poc_comp_cmds->cmds[4].msg.tx_buf[1] = poc_comp_table[cd_idx];
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3FA7_AMB575WD01;

	/* GAMMA FLASH */
	vdd->panel_func.gen_hbm_interpolation_gamma = gen_hbm_interpolation_gamma_S6E3FA7_AMB575WD01;
	vdd->panel_func.gen_hbm_interpolation_irc = gen_hbm_interpolation_irc_S6E3FA7_AMB575WD01;
	vdd->panel_func.gen_normal_interpolation_irc = gen_normal_interpolation_irc_S6E3FA7_AMB575WD01;
	vdd->panel_func.samsung_flash_gamma_support = flash_gamma_support_S6E3FA7_AMB575WD01;
	vdd->panel_func.samsung_interpolation_init = init_interpolation_S6E3FA7_AMB575WD01;
	...
	vdd->panel_func.samsung_smart_get_conf_hmt = smart_get_conf_S6E3FA7_AMB575WD01_hmt;
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3FA7_AMB575WD01_FHD";
