...
int cam_custom_csid_reserve(void *hw_priv, void *reserve_args,
	uint32_t arg_size)
{
	int rc = 0;
	...
	if (rc < 0) {
		CAM_ERR(CAM_SBI, "Failed in reserving the cid : %d", rc);
		return rc;
	}

	rc = cam_ife_csid_path_reserve(csid_hw, reserv);
	if (rc < 0) {
		CAM_ERR(CAM_SBI, "Failed in csid path reserve: %d", rc);
		return rc;
	}
...
static int cam_custom_csid_probe(struct platform_device *pdev)
{
	...
	CAM_DBG(CAM_SBI, "CSID:%d probe successful for dev %s",
